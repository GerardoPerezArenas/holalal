        ...
        <script type="text/javascript">
            ...
            function pulsarModifVisitaPros(){                
                if(tabVisitas_pros.selectedIndex != -1) {
                   ...
                   */     opcion = 'modificar';
                   // }
                    if(navigator.appName.indexOf("Internet Explorer")!=-1){
                        result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarNuevaVisitaProspector&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&preparador=<%=prospector!=null?prospector.getJusProspectoresCod():""  %>&idVisModificar='+listaVisitas_pros[tabVisitas_pros.selectedIndex][0]+'&control='+control.getTime(),390,820,'no','no');
                    }else{
                        result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarNuevaVisitaProspector&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&preparador=<%=prospector!=null?prospector.getJusProspectoresCod():"" %>&idVisModificar='+listaVisitas_pros[tabVisitas_pros.selectedIndex][0]+'&control='+control.getTime(),410,850,'no','no');
                    }
                    if (result != undefined){
           ...
           <%-- function pulsarEliminarSegPreparador(){
                    if(tabVisitas_pros.selectedIndex != -1) {
                        ...
                        if (resultado == 1){
                            ...
                            parametros = 'tarea=preparar&modulo=MELANBIDE35&operacion=eliminarSegPreparador&tipo=0&numero=<%=numExpediente%>&idSegPrep='+listaVisitas_pros[tabVisitas_pros.selectedIndex][0]+'&control='+control.getTime();
                            try{
           ...
           // var camposErrores = new Array();
                ...
                for(var i = 1;i< result.length; i++){
                    ...
                    listaVisitas_prosTabla[i-1] = [fila[19] != undefined && fila[19].length > 0 ? '<img src="<%=request.getContextPath()%>/images/extension/melanbide35/alert.png"/>' : '', fila[1], fila[2], fila[3], fila[4], fila[5], fila[6], fila[7], fila[8], fila[9], fila[10], fila[11], fila[12], fila[13], fila[14], fila[15], fila[16], fila[17], fila[18]];
                ...
                tabVisitas_pros.addColumna('80','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.visita.tabla.col15")%>');
        ...
        <form>
            ...
            <div style="width: 95%; padding: 10px; text-align: left;">
                ...
                <div class="lineaFormulario">
                    <div>
                        <%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.justificacion.visita.prospector").toUpperCase()%>:<span 
                ...
                <div class="botonera">
                    <input type="button" id="btnNuevoVisitaPros" name="btnNuevoVisitaPros" class="botonGeneral" value="<%=meLanbide35I18n.getMensaje(idiomaUsuario, "btn.general.nuevo")%>" onclick="pulsarNuevoVisitaPros();">
