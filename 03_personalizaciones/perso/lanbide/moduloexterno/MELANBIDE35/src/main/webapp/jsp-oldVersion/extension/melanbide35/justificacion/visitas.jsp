           ...
           /* window.onbeforeunload  = function () {
            ...
            function pulsarNuevaVisitaPros(){
                    ...
                    if(navigator.appName.indexOf("Internet Explorer")!=-1){
                        result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarNuevaVisitaProspector&tipo=0&numero=<%=numExpediente%>&opcion=nuevo&prospector=<%=prospector != null ? prospector.getJusProspectoresCod() : ""%>&control='+control.getTime(),390,820,'no','no');
                    }else{
                        result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarNuevaVisitaProspector&tipo=0&numero=<%=numExpediente%>&opcion=nuevo&prospector=<%=prospector != null ? prospector.getJusProspectoresCod() : ""%>&control='+control.getTime(),410,850,'no','no');
                    }
                    if (result != undefined){
                        if(result[0] == '0'){
