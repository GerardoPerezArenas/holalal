...
<script type="text/javascript">
    ...
    function pulsarAltaUbicacionCE(){
        ...
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE32&operacion=cargarNuevaUbicacionCE&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),180,820,'no','no');
        }else{
            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE32&operacion=cargarNuevaUbicacionCE&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),200,850,'no','no');
        }
        if (result != undefined){
            if(result[0] == '0'){
    ...
    function pulsarModificarUbicacionCE(){
        if(tabUbicacionesCE.selectedIndex != -1) {
            ...
            var result = null;
            if(navigator.appName.indexOf("Internet Explorer")!=-1){
                result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE32&operacion=cargarModifUbicacionCE&tipo=0&numero=<%=numExpediente%>&idUbic='+listaUbicacionesCE[tabUbicacionesCE.selectedIndex][0]+'&control='+control.getTime(),180,820,'no','no');
            }else{
                result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE32&operacion=cargarModifUbicacionCE&tipo=0&numero=<%=numExpediente%>&idUbic='+listaUbicacionesCE[tabUbicacionesCE.selectedIndex][0]+'&control='+control.getTime(),200,850,'no','no');
            }
            if (result != undefined){
                if(result[0] == '0'){
                    recargarTablaUbicacionesCE(result);
    ...
    function pulsarAltaTrayectoriaCE(){
        ...
        var result = null;
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE32&operacion=cargarNuevaTrayectoriaCE&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),180,750,'no','no');
        }else{
            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE32&operacion=cargarNuevaTrayectoriaCE&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),200,780,'no','no');
        }
        if (result != undefined){
            if(result[0] == '0'){
    ...
    function pulsarModificarTrayectoriaCE(){
        if(tabTrayectoriaCE.selectedIndex != -1) {
            ...
            var result = null;
            if(navigator.appName.indexOf("Internet Explorer")!=-1){
                result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE32&operacion=cargarModifTrayectoriaCE&tipo=0&numero=<%=numExpediente%>&idTray='+listaTrayectoriaCE[tabTrayectoriaCE.selectedIndex][0]+'&control='+control.getTime(),180,750,'no','no');
            }else{
                result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE32&operacion=cargarModifTrayectoriaCE&tipo=0&numero=<%=numExpediente%>&idTray='+listaTrayectoriaCE[tabTrayectoriaCE.selectedIndex][0]+'&control='+control.getTime(),200,780,'no','no');
            }
            if (result != undefined){
                if(result[0] == '0'){
                    recargarTablaTrayectoriaCE(result);
    ...
    function recargarTablaUbicacionesCE(result){
        ...
        for(var i = 1;i< result.length; i++){
            ...
            listaUbicacionesCETabla[i-1] = [fila[1], fila[2], fila[3], fila[4], fila[5], fila[7], fila[6]];
        }
        tabUbicacionesCE = new Tabla(document.getElementById('listaUbicacionesCE'), 874);
    ...
    function recargarTablaTrayectoriaCE(result){
        ...
        tabTrayectoriaCE = new Tabla(document.getElementById('listaTrayectoriaCE'), 874);
        ...
        <div style="clear: both;">
            <fieldset style="width: 97.5%;">
                ...
                <div id="divGeneral" onscroll="deshabilitarRadios();" style="overflow-y: auto; overflow-x: hidden; height: 440px;">
                    ...
                    <div id="listaUbicacionesCE" class="tablaPersCE" style="padding: 5px; width:900px; height: 190px; text-align: center; overflow-x: auto; overflow-y: auto;margin:0px;margin-top:0px;" align="center"></div>
                    <div class="botonera">
                    ...
                    <div id="listaTrayectoriaCE" style="padding: 5px; width:900px; height: 190px; text-align: center; overflow-x: auto; overflow-y: auto;margin:0px;margin-top:0px;" align="center"></div>
                    <div class="botonera">
...
<script type="text/javascript">
    ...
    %>
        listaUbicacionesCE[<%=i%>] = ['<%=voU.getOriCeUbicCod()%>','<%=voU.getEspecial()%>','<%=voU.getProvincia()%>','<%=voU.getAmbito()%>','<%=voU.getMunicipio()%>','<%=voU.getDireccion()%>','<%=voU.getValidacion()%>','<%=voU.getAdjudicado()%>'];
        listaUbicacionesCETabla[<%=i%>] = ['<%=voU.getEspecial()%>','<%=voU.getProvincia()%>','<%=voU.getAmbito()%>','<%=voU.getMunicipio()%>','<%=voU.getDireccion()%>','<%=voU.getValidacion()%>','<%=voU.getAdjudicado()%>'];
    <%
            }// for
        }// if
    %>
    ...
    if(navigator.appName.indexOf("Internet Explorer")!=-1){
        try{
            var div = document.getElementById('listaUbicacionesCE');
            div.children[0].children[0].children[0].children[0].children[0].children[0].children[6].style.backgroundColor = 'FF9966';
            div.children[0].children[0].children[0].children[0].children[0].children[0].children[6].style.width = '100%';
            div.children[0].children[1].children[0].children[0].children[0].children[6].style.width = '100%';
        }
        catch(err){
