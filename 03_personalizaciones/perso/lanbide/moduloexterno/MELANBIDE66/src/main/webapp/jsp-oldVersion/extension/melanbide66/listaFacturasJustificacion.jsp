...
<script type="text/javascript">   
    ...
    var COD_SUBV15 = '<%=ConstantesMeLanbide66.getPropVal_CS_SUBV15(codOrganizacion,ConstantesMeLanbide66.getCOD_PROCEDIMIENTO())%>';
    ...
    function procesarRespuestaGuardarImportePago(result){
        ...
        if(result){
            var datos = JSON.parse(result);
            if(datos.tabla.error=="0"){
                // Reflejamos los camibios: tipoCalculo=reintegro -> actualizamos IMPREINTEGRO y NOJUST2P ó tipoCalculo=pago -> actualizamos IMPSEGUNDOPAGO
                if($('#capaDatosSuplementarios_DEF')){
                    var importe = eval("datos.tabla."+COD_IMPREINTEGRO);
                    if(datos.tabla.tipoCalculo=="pago"){
                        importe = eval("datos.tabla."+COD_SEGPAGO);
                        eval("$('#"+COD_SEGPAGO+"').val(datos.tabla."+COD_SEGPAGO+")");
                        eval("$('#"+COD_IMPREINTEGRO+"').val('')");
                        ...
                        if(facturas.length>0){
                            ...
                            for(var contador=0; contador<facturas.length; contador++){
                                ...
                                listaTablaFacturas[contador] = [
                                    fact.codEstado,fact.fechaStr,fact.identFactura,fact.descConcepto,fact.descDesgloseCpto,fact.importeStr,fact.descEntregaFact,fact.descEntregaJustif
                                ];
    ...
    function procesarRespuestaRecargarFechasCalculadas(result){
        ...
        if(result){
            ...
            } else mostrarMensajeError(datos.error);
        } else mostrarMensajeError();
    }
    ...
    function procesarRespuestaCopiarTextoAPortapapeles(result){
        ...
        if(result){
            ...
            datos = datos.tabla;
            if(datos.error=="0"){
                mostrarVentana(datos.texto,'<%=meLanbide66I18n.getMensaje(idiomaUsuario,"label.etiqFactASubsanar")%>');
            } else mostrarMensajeError(datos.error);
        } else mostrarMensajeError();
    }
    ...
    function inicializarDesplegables(){
        ...
        %>
                ...
                if(valActualFechaSubs=="null") valActualFechaSubs='';
        <% } %>
    ...
    function inicializarListas(){
        ...
        %>
                    listaTablaFacturas[contador] = [
                        '<%=factura.getCodEstado()%>','<%=factura.getFechaStr()%>','<%=factura.getIdentFactura()%>','<%=factura.getDescConcepto()%>',
                        '<%=factura.getDescDesgloseCpto()%>','<%=factura.getImporteStr()%>','<%=factura.getDescEntregaFact()%>','<%=factura.getDescEntregaJustif()%>'
                    ];
                    listaFacturas[contador++] = [
                        '<%=factura.getCodIdent()%>','<%=factura.getNumExpediente()%>','<%=factura.getCodEstado()%>','<%=factura.getFecha()%>','<%=factura.getIdentFactura()%>',
    ...
    */
    ...
    function realizarCalculo(){
        ...
        var subv15 = eval("datosCS."+COD_SUBV15);
        var causa = eval("datosCS."+COD_CAUSAREINT);
        var campos = [pago1,subv15];
        
        if(validarCSparaCalculo(campos)){
            if(pago1==undefined) pago1 = 0;
            else pago1 = eval(pago1);
            if(subv15==undefined) subv15 = 0;
            else subv15 = eval(subv15);
            ...
            if(valSubs=="X" && (valFec==undefined || valFec=="" || valFec==null)) {
                ...
                if(pago2 > 0 && pago2 != (subv15 - pago1)){
                    cantmin = subv15 - justificado;
                } else {
                    cantmin = 0;
                }
        ...
        <div style="clear: both;">
            <div id="divGeneral" style="overflow-y: auto; overflow-x: hidden; padding: 20px;">
                ...
                <div class="lineaFormulario" style="width: 100%">
                    ...
                    <%--Importe--%>
                    <div class="etiqueta" style="float: left;margin-left: 30px; margin-right: -20px" >
