...
<%@page import="es.altia.flexia.integracion.moduloexterno.melanbide47.vo.procesos.FilaAuditoriaProcesosVO" %>
...
<script type="text/javascript">
    ...
    function ejecutarResolucionProvisionalHoras(){
        ...
        var ano = solicitarAno();
        if(ano != null && ano != undefined && ano != ''){
            parametros = 'tarea=preparar&modulo=MELANBIDE47&operacion=adjudicaOrientacion&tipo=0&codProcedimiento=<%=codProcedimiento%>&ano='+ano+'&control='+control.getTime();
            try{
                ...
                if(codigoOperacion == "0"){
                    jsp_alerta("A",'<%=meLanbide47I18n.getMensaje(idiomaUsuario,"msg.procesos.procesoOk")%>'+'. \n'+estado);
    ...
    function generarDocumentacionResolucionHoras(){
        ...
        if(ano != null && ano != undefined && ano != ''){
            ...
            var parametros = "";
            var control = new Date();
            parametros = '?tarea=preparar&modulo=MELANBIDE47&operacion=crearDocumentacionResolucionHoras&tipo=0&ano='+ano+'&codProcedimiento=<%=codProcedimiento%>&control='+control.getTime();
            inicio = false;
    ...
    function consolidarHoras(){
        ...
        var parametros = "";
        var control = new Date();
        parametros = 'tarea=preparar&modulo=MELANBIDE47&operacion=consolidaHoras&tipo=0&codProcedimiento=<%=codProcedimiento%>&control='+control.getTime();
        try{
            ajax.open("POST",url,false);
            ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=ISO-8859-1");
            ajax.setRequestHeader("Accept", "text/xml, application/xml, text/plain");     
    ...
    function deshacerConsolidacionHoras(){
        ...
        var ano = solicitarAno();
        if(ano != null && ano != undefined && ano != ''){
            parametros = 'tarea=preparar&modulo=MELANBIDE47&operacion=deshacerConsolidacionHoras&tipo=0&ano='+ano+'&codProcedimiento=<%=codProcedimiento%>&control='+control.getTime();
            try{
                ajax.open("POST",url,false);
                ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=ISO-8859-1");
                ajax.setRequestHeader("Accept", "text/xml, application/xml, text/plain");     
    ...
    function inicializar(){
        ...
        if(navigator.appName=="Microsoft Internet Explorer"){
            if(document.getElementById('fieldsetOtros') != undefined){
    ...
    function solicitarAno(){
        ...
        var result = null;
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            result = lanzarPopUpModal('<%=request.getContextPath()%>/jsp/extension/melanbide47/procesos/solicitarAno.jsp?idioma=<%=idiomaUsuario%>&control='+control.getTime(),120,200,'no','no');
        }else{
            result = lanzarPopUpModal('<%=request.getContextPath()%>/jsp/extension/melanbide47/procesos/solicitarAno.jsp?idioma=<%=idiomaUsuario%>&control='+control.getTime(),120,250,'no','no');
        }
        //return '2014';
        return result;
    }
    ...
    function filtrarAuditorias()
    {
        if(validarDatosFiltro()){
            ...
            parametros = 'tarea=preparar&modulo=MELANBIDE47&operacion=filtrarAuditoriaProcesos&tipo=0&numero=<%=numExpediente%>&nomApellidos='+nomApellidos
    ...
    function cargarComboProcesos(){
            ...
            var parametros = "";
            var control = new Date();
            parametros = 'tarea=preparar&modulo=MELANBIDE47&operacion=cargarComboProcesos&tipo=0&numero=<%=numExpediente%>&control='+control.getTime();
            try{
                ajax.open("POST",url,false);
                ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=ISO-8859-1");
                ajax.setRequestHeader("Accept", "text/xml, application/xml, text/plain");     
    ...
    function comprobarFecha(inputFecha) {
        if (Trim(inputFecha.value)!='') {
            if (!ValidarFechaConFormato(document.forms[0],inputFecha)){
                jsp_alerta("A","<%=meLanbide47I18n.getMensaje(idiomaUsuario, "msg.procesos.fechaNoVal")%>");
                return false;
            }
        }
        return true;
    ...
    function validarDatosFiltro(){
        ...
        if(txtDesde != null && txtDesde != ''){
            if(!ValidarFechaConFormato(document.forms[0],document.getElementById('fechaDesde'))){
                jsp_alerta("A","<%=meLanbide47I18n.getMensaje(idiomaUsuario, "msg.procesos.fechaDesdeIncorrecto")%>");
                return false;
            }else{
                hayFechaDesde = true;
            }
        ...
        if(txtHasta != null && txtHasta != ''){
            if(!ValidarFechaConFormato(document.forms[0],document.getElementById('fechaHasta'))){
                jsp_alerta("A","<%=meLanbide47I18n.getMensaje(idiomaUsuario, "msg.procesos.fechaHastaIncorrecto")%>");
                return false;
            }else{
                hayFechaHasta = true;
            }
        ...
        if(hayFechaDesde && hayFechaHasta){
            ...
            /*if(n2 <= 0){
                ...
                if(mensajeError == "")
                    mensajeError = "<%=meLanbide47I18n.getMensaje(idiomaUsuario, "fechaNacNoVal")%>"+" '"+"<%=meLanbide47I18n.getMensaje(idiomaUsuario, "legend.personaContratada")%>"+"'";
                correcto = false;
            }*/
            if(result < 0){
                jsp_alerta("A","<%=meLanbide47I18n.getMensaje(idiomaUsuario, "msg.procesos.fechaDesdeAnteriorHasta")%>");
                correcto = false;
            }
        }
        return correcto;
...
<body class="bandaBody" onload="javascript:{if (window.top.principal.frames[0]&&window.top.principal.frames['menu'].Go)
    ...
    <form id="formProcesos">
        <div class="tab-page" id="tabPage323" style="height:500px; width: 100%;">
            <table width="100%" style="height: 500px;" cellpadding="0px" cellspacing="0px" border="0px">
                <tr>
                    <td>
                            ...
                            <tr>
                                <td>
                                    <table class="bordeInteriorPantalla" cellspacing="1px">
                                        <tr>
                                            <td style="width:100%;  height:30px; padding-left: 15px" class="txttitblanco"><%=meLanbide47I18n.getMensaje(idiomaUsuario, "label.procesos.tit_procesos")%></td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            <td class="contenidoPantalla" valign="top" style="padding-top: 5px; padding-bottom: 10px;">
                                                <div id="contenidoProc" class="cuadroFondoBlanco" style="width:970px; height: 500px; overflow-x: auto; overflow-y: auto; padding: 10px;margin:0px;margin-top:0px;" align="center">
                                                    <fieldset id="fieldsetResolucionHoras" style="width: 47%; float: left; padding-left: 10px; padding-right: 10px;">
                                                        <legend class="legendAzul"><%=meLanbide47I18n.getMensaje(idiomaUsuario, "legend.procesos.orientacion")%></legend>
                                                        <div class="lineaFormularioFont">
                                                            <div style="width: 300px; float: left; text-align: left;">
                                                                <%=meLanbide47I18n.getMensaje(idiomaUsuario, "label.procesos.RPH")%>
                                                            </div>
                                                            <div style="width: 100px; float: left;">
                                                                <div style="float: left;">
                                                                    <input type="button" id="btnRPH" name="btnRPH" class="botonGeneral" value="<%=meLanbide47I18n.getMensaje(idiomaUsuario, "btn.procesos.ejecutar")%>" onclick="ejecutarResolucionProvisionalHoras();">
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="lineaFormularioFont">
                                                            <div style="width: 300px; float: left; text-align: left;">
                                                                <%=meLanbide47I18n.getMensaje(idiomaUsuario, "label.procesos.DRH")%>
                                                            </div>
                                                            <div style="width: 100px; float: left;">
                                                                <div style="float: left;">
                                                                    <input type="button" id="btnDRH" name="btnDRH" class="botonGeneral" value="<%=meLanbide47I18n.getMensaje(idiomaUsuario, "btn.procesos.generar")%>" onclick="generarDocumentacionResolucionHoras();">
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="lineaFormularioFont">
                                                            <div style="width: 300px; float: left; text-align: left;">
                                                                <%=meLanbide47I18n.getMensaje(idiomaUsuario, "label.procesos.CH")%>
                                                            </div>
                                                            <div style="width: 100px; float: left;">
                                                                <div style="float: left;">
                                                                    <input type="button" id="btnCH" name="btnCH" class="botonGeneral" value="<%=meLanbide47I18n.getMensaje(idiomaUsuario, "btn.procesos.ejecutar")%>" onclick="consolidarHoras();">
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="lineaFormularioFont">
                                                            <div style="width: 300px; float: left; text-align: left;">
                                                                <%=meLanbide47I18n.getMensaje(idiomaUsuario, "label.procesos.DCH")%>
                                                            </div>
                                                            <div style="width: 100px; float: left;">
                                                                <div style="float: left;">
                                                                    <input type="button" id="btnDCH" name="btnDCH" class="botonGeneral" value="<%=meLanbide47I18n.getMensaje(idiomaUsuario, "btn.procesos.ejecutar")%>" onclick="deshacerConsolidacionHoras();">
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </fieldset>
<!--                     //                                <fieldset id="fieldsetOtros" style="width: 97%; float: right; padding-left: 10px; padding-right: 10px;">
                                                        <legend class="legendAzul"><%=meLanbide47I18n.getMensaje(idiomaUsuario, "legend.procesos.otros")%></legend>
                                                        <div class="lineaFormularioFont">
                                                            <div style="width: 300px; float: left; text-align: left;">
                                                                <%=meLanbide47I18n.getMensaje(idiomaUsuario, "label.procesos.LAC")%>
                                                            </div>
                                                            <div style="width: 100px; float: left;">
                                                                <div style="float: left;">
                                                                    <input type="button" id="btnLAC" name="btnLAC" class="botonGeneral" value="<%=meLanbide47I18n.getMensaje(idiomaUsuario, "btn.procesos.ejecutar")%>" onclick="ejecutarListadoAltaCentros();">
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </fieldset>-->
                                                    <div style="clear: both; float: left; width: 100%; padding-top: 50px;">
                                                        <fieldset style="width: 99%; float: left; padding-top: 10px;">
                                                            <legend class="legendAzul"><%=meLanbide47I18n.getMensaje(idiomaUsuario, "legend.procesos.auditoriaProcesos")%></legend>
                                                            <div style="clear: both; float: left; width: 100%;">
                                                                <table style="width: 100%; font-size: 12px;">
                                                                    <tr>
                                                                        <td>
                                                                            <b><u><%=meLanbide47I18n.getMensaje(idiomaUsuario,"label.procesos.criteriosBusq")%></u></b>
                                                                        </td>
                                                                    </tr>
                                                                    <tr>
                                                                        <td>
                                                                            <%=meLanbide47I18n.getMensaje(idiomaUsuario,"label.procesos.nomApellidos")%>
                                                                            <input id="nomApellidos" name="nomApellidos" type="text" class="inputTexto" size="52" maxlength="40" />
                                                                            <%=meLanbide47I18n.getMensaje(idiomaUsuario,"label.procesos.fDesde")%>
                                                                            ...
                                                                            <A href="javascript:calClick(event);" onclick="mostrarCalFechaDesde(event);return false;" style="text-decoration:none; padding-right: 10px;" >
                                                                                <IMG style="border: 0" height="17" id="calFechaDesde" name="calFechaDesde" alt="<%=meLanbide47I18n.getMensaje(idiomaUsuario,"label.procesos.fDesde")%>" src="<c:url value='/images/calendario/icono.gif'/>">
                                                                            </A>
                                                                            <%=meLanbide47I18n.getMensaje(idiomaUsuario,"label.procesos.fHasta")%>
                                                                            ...
                                                                            <A href="javascript:calClick(event);" onclick="mostrarCalFechaHasta(event);return false;" style="text-decoration:none; padding-right: 10px;" >
                                                                                <IMG style="border: 0" height="17" id="calFechaHasta" name="calFechaHasta" alt="<%=meLanbide47I18n.getMensaje(idiomaUsuario,"label.fNac")%>" src="<c:url value='/images/calendario/icono.gif'/>">
                                                                            </A>
                                                                        </td>
                                                                    </tr>
                                                                    <tr>
                                                                        <td>
                                                                            <%=meLanbide47I18n.getMensaje(idiomaUsuario,"label.procesos.proceso")%>
                                                                            ...
                                                                            <a id="anchorProceso" name="anchorProceso" href=""><img src="<%=request.getContextPath()%>/images/listas/botondesplegable.gif" id="botonProceso" name="botonProceso" border="0" width="14" height="14" style="cursor:hand;" alt="<%=meLanbide47I18n.getMensaje(idiomaUsuario, "altDesplegable")%>" title="<%=meLanbide47I18n.getMensaje(idiomaUsuario, "altDesplegable")%>"></a>
                                                                        </td>
                                                                    </tr>
                                                                    <tr>
                                                                        <td>
                                                                            <div class="botonera">
                                                                                <input type="button" id="btnBuscar" name="btnBuscar" class="botonGeneral" value="<%=meLanbide47I18n.getMensaje(idiomaUsuario, "btn.procesos.buscar")%>" onclick="buscar();">
                                                                                <input type="button" id="btnLimpiar" name="btnLimpiar" class="botonGeneral" value="<%=meLanbide47I18n.getMensaje(idiomaUsuario, "btn.procesos.limpiar")%>" onclick="limpiarFiltro();">
                                                                            </div>
                                                                        </td>
                                                                    </tr>
                                                                </table>
...
<script type="text/javascript">
   ...
   // JAVASCRIPT DE LA TABLA AUDITORIA
    ...
    var tabAud = new Tabla(document.getElementById('tablaAuditoria'), 930);
    //TOTAL ANCHO: 928
    tabAud.addColumna('245','left','<%= meLanbide47I18n.getMensaje(idiomaUsuario, "label.procesos.auditoria.nombre")%>');
    tabAud.addColumna('250','left','<%= meLanbide47I18n.getMensaje(idiomaUsuario, "label.procesos.auditoria.proceso")%>');
    tabAud.addColumna('165','center','<%= meLanbide47I18n.getMensaje(idiomaUsuario, "label.procesos.auditoria.fecha")%>');
    tabAud.addColumna('244','center','<%= meLanbide47I18n.getMensaje(idiomaUsuario, "label.procesos.auditoria.resultado")%>');
