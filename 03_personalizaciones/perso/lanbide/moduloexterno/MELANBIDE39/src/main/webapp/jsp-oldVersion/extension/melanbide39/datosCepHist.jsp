    ...
    function mostrarImpConCalculado(event){
        ...
        if(document.forms[0].modoConsulta.value == "si"){
            if(minimis != undefined && minimis != ''){
                resultado = jsp_alerta('A','<%=meLanbide39I18n.getMensaje(idiomaUsuario, "msg.datosCpe.impConCalculadoEs")%>: <br/>'+document.getElementById('concedidoReal').value+' '+'<%=meLanbide39I18n.getMensaje(idiomaUsuario, "msg.euros")%>'+'<br/>'+'<%=meLanbide39I18n.getMensaje(idiomaUsuario, "msg.avisoMinimis")%>'+' '+document.getElementById('minimisConce').value);
            }else{
                resultado = jsp_alerta('A','<%=meLanbide39I18n.getMensaje(idiomaUsuario, "msg.datosCpe.impConCalculadoEs")%>: <br/>'+document.getElementById('concedidoReal').value+' '+'<%=meLanbide39I18n.getMensaje(idiomaUsuario, "msg.euros")%>');
            }
        }else{
            if(minimis != undefined && minimis != ''){
                resultado = jsp_alerta('','<%=meLanbide39I18n.getMensaje(idiomaUsuario, "msg.datosCpe.impConCalculadoEs")%>: <br/>'+document.getElementById('concedidoReal').value+' '+'<%=meLanbide39I18n.getMensaje(idiomaUsuario, "msg.euros")%>'+'<br/>'+'<%=meLanbide39I18n.getMensaje(idiomaUsuario, "msg.avisoMinimis")%>'+' '+document.getElementById('minimisConce').value+'<br/><br/>'+'<%=meLanbide39I18n.getMensaje(idiomaUsuario, "msg.datosCpe.preguntaAceptarImporte")%>');
            }else{
                resultado = jsp_alerta('','<%=meLanbide39I18n.getMensaje(idiomaUsuario, "msg.datosCpe.impConCalculadoEs")%>: <br/>'+document.getElementById('concedidoReal').value+' '+'<%=meLanbide39I18n.getMensaje(idiomaUsuario, "msg.euros")%>'+'<br/><br/>'+'<%=meLanbide39I18n.getMensaje(idiomaUsuario, "msg.datosCpe.preguntaAceptarImporte")%>');
            }
        }
        if (resultado == 1){
        ...
        try{
            if(estanCargadosCamposSuplementarios()){
                ...
                if(importes){
                    ...
                    try{
                        document.getElementById('<%=ConfigurationParameter.getParameter(ConstantesMeLanbide39.CAMPO_SUPL_IMP_JUS, ConstantesMeLanbide39.FICHERO_PROPIEDADES)%>').value = importes[4];
                    }catch(err){
                    }
                    try{
...
<body>
    <div class="tab-page" id="tabPage391" style="height:480px; width: 100%; ">
        <h2 class="tab" id="pestana391" style="font-size:1em;"><%=meLanbide39I18n.getMensaje(idiomaUsuario,"label.datosCpe.tituloPestanaHist")%></h2>
