...
public class MELANBIDE43  extends ModuloIntegracionExterno {
    //Logger
    private static Logger log = Logger.getLogger(MELANBIDE43.class);
   ...
   public String generarMisGestInicio (int codOrganizacion,int codTramite,int ocurrenciaTramite,String numExpediente) throws Exception
   {
       String mensaje = "";
       try{
        if(log.isDebugEnabled()) log.info("generarMisGestionesInicio() : BEGIN");
        ...
        if(log.isDebugEnabled()) log.info("generarMisGestionesInicio() : END");
       }
       catch(Exception x)
       {
   ...
   public String comprobarExpGenerado (int codOrganizacion,int codTramite,int ocurrenciaTramite,String numExpediente) throws Exception
   {
       ...
       try{
        ...
        if (!meLanbide43Manager.getInstance().comprobarExpGenerado(codOrganizacion, codTramite, ocurrenciaTramite, numExpediente, con)){
            log.info("comprobarExpGenerado: INICIO llamada a generarMisGestiones exp " + numExpediente);
            codOperacion = MeLanbide43Manager.getInstance().generarMisGestiones(codOrganizacion, codTramite, ocurrenciaTramite, numExpediente, "I",getAdaptSQLBD(String.valueOf(codOrganizacion)));            
            log.info("comprobarExpGenerado: FIN llamada a generarMisGestiones exp " + numExpediente + " - codOperacion: "+ codOperacion);
                    ...
                    if(misGestiones.size() > 0){
                        ...
                        if(pepe[1].equals("CEI")){
                            ...
                            if(codTramite == tramSeg){//JUSTIFICACION CONTABLE PARA SEGUNDO PAGO
                                log.info("tramite 23");
                                lan6Tramite.setInstanciaId(ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_PAGO2_LAN68_CEI, ConstantesMeLanbide43.FICHERO_ADAPTADORES));// Apertura del trÃ¡mite para el segundo pago
                            }
                            if(codTramite == tramTer){//JUSTIFICACION CONTABLE PARA TERCER PAGO
                                log.info("tramite 54");
                                lan6Tramite.setInstanciaId(ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_PAGO3_LAN68_CEI, ConstantesMeLanbide43.FICHERO_ADAPTADORES));// Apertura del trÃ¡mite para el tercer pago
                            }
                        ...
                        //Llamada método       
                        log.error("llamada al metodo actualizarTramites: " + numExpediente);
                        String res = servicios.actualizarTramites(tramites);
                        log.info("resutlado de actualizarTRamites : " + res);
                    }
                }
            }
       ...
       try
       {
            ...
            lan6Expediente.setEjercicio(pepe[0]);
             ...
             if(oids != null && oids.size() > 0)
                lan6Tramite.setId(Lan6Constantes.TRAMITE_SUBSANACION_DOC+"_"+fecha);
             else
                 lan6Tramite.setId(Lan6Constantes.TRAMITE_SUBSANACION_DOC);
             lan6Tramite.setDescripcion(Lan6Constantes.TRAMITE_SUBSANACION_DOC_DESC);
  ...
  public String actualizaInteresados (int codOrganizacion, int codTramite, int ocurrenciaTramite, String numExpediente) throws Exception
  {
      ...
      try
      {
        ...
        String[] dato = numExpediente.split("/");
        ...
        if (esDesarrolloCero > 0){
                ...
                idProcedimiento = MeLanbide43Manager.getInstance().convierteProcedimiento(dato[1]);
        }
        //DESARROLLO 0_______________________________________________________________________________________________________________________DESARROLLO 0
        ...
        String fechaIni = MeLanbide43Manager.getInstance().fechaInicioExpediente(numExpediente, adaptador);                
        date = dateFormat.parse(fechaIni);
        String fecha = MeLanbide43Manager.getInstance().verificarFecha(dato[1], adaptador);
        Date fec = null;
        if(fecha != ""){
            fec = dateFormat.parse(fecha);
            if(date.after(fec)){ 
                ...
                lan6Expediente.setEjercicio(dato[0]);
                ...
                if(date.after(fec)){                     
                    if("IN".equals(parametros.getOrigenLlamada())) {
                        ...
                        gestionarEspera(codOrganizacion, codTramite, ocurrenciaTramite, numExpediente, 
                                Lan6Constantes.TRAMITE_ESPERA_DESISTIMIENTO, Lan6Constantes.TRAMITE_DESISTIMIENTO_DESC, true, "cerrarEsperaDesistimiento");
                        //abrirEsperaRecurso
                        gestionarEspera(codOrganizacion, codTramite, ocurrenciaTramite, numExpediente, 
                                Lan6Constantes.TRAMITE_ESPERA_RECURSO_ALZADA, Lan6Constantes.TRAMITE_RECURSO_ALZADA_DOC_DESC, false, "abrirEsperaRecurso");
                        //abrirEsperaRenuncia
                        gestionarEspera(codOrganizacion, codTramite, ocurrenciaTramite, numExpediente, 
                                Lan6Constantes.TRAMITE_ESPERA_RESIGNACION, Lan6Constantes.TRAMITE_RENUNCIA_DESC, false, "abrirEsperaRenuncia");
                    }
                }
            }
            ...
            if(fecha != ""){
                ...
                if(date.after(fec)){                     
                    if("IN".equals(parametros.getOrigenLlamada())) {
                        ...
                        gestionarEspera(codOrganizacion, codTramite, ocurrenciaTramite, numExpediente, 
                                Lan6Constantes.TRAMITE_ESPERA_RESIGNACION, Lan6Constantes.TRAMITE_RENUNCIA_DESC, true, "cerrarEsperaRenuncia");
                        //cerrarEsperaRecurso
                        gestionarEspera(codOrganizacion, codTramite, ocurrenciaTramite, numExpediente, 
                                Lan6Constantes.TRAMITE_ESPERA_RECURSO_ALZADA, Lan6Constantes.TRAMITE_RECURSO_ALZADA_DOC_DESC, true, "cerrarEsperaRecurso");
                    }
                }
            }
