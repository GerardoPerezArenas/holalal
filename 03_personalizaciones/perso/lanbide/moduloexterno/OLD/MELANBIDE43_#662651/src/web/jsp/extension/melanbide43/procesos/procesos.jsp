...
<%@page import="es.altia.flexia.integracion.moduloexterno.melanbide43.i18n.MeLanbide37I18n" %>
...
<script type="text/javascript">
    ...
    function ejecutarListadoCP(){        
        ...
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE43&operacion=cargarCriteriosFiltroListadoCP&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),330,840,'no','no');
        }else{
            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE43&operacion=cargarCriteriosFiltroListadoCP&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),400,870,'no','no');
        }
        if (result != undefined){
            if(result[0] == '0'){
                ...
                var parametros = "";
                var control = new Date();
                parametros = '?tarea=preparar&modulo=MELANBIDE43&operacion=generarListadoCP&tipo=0&tipoacred='+tipoacred
    ...
    function ejecutarListadoComprobacionCP(){
        ...
        var result = null;
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE43&operacion=cargarCriteriosFiltroListadoCompCP&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),330,840,'no','no');
        }else{
            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE43&operacion=cargarCriteriosFiltroListadoCompCP&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),400,870,'no','no');
        }
        if (result != undefined){
            ...
            var parametros = "";
            var control = new Date();
            parametros = '?tarea=preparar&modulo=MELANBIDE43&operacion=generarListadoComprobacionCP&tipo=0'
                +'&codigoCP='+codigoCP
                +'&fecSoliDesde='+fecdesde+'&fecSoliHasta='+fechasta+'&control='+control.getTime();
            window.open(url+parametros, "_blank");
    ...
    function ejecutarListadoComprobacionAPA(){
        ...
        if (result != undefined){
            ...
            var parametros = "";
            var control = new Date();
            parametros = '?tarea=preparar&modulo=MELANBIDE43&operacion=generarListadoComprobacionAPA&tipo=0'
                +'&codigoCP='+codigoCP
                +'&fecSoliDesde='+fecdesde+'&fecSoliHasta='+fechasta+'&control='+control.getTime();
            window.open(url+parametros, "_blank");
    ...
    function solicitarTipoAcreditacion(){
        ...
        var result = null;
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            result = lanzarPopUpModal('<%=request.getContextPath()%>/jsp/extension/melanbide43/procesos/solicitarTipoAcreditacion.jsp?idioma=<%=idiomaUsuario%>&control='+control.getTime(),120,200,'no','no');
        }else{
            result = lanzarPopUpModal('<%=request.getContextPath()%>/jsp/extension/melanbide43/procesos/solicitarTipoAcreditacion.jsp?idioma=<%=idiomaUsuario%>&control='+control.getTime(),120,250,'no','no');
        }
        return result;
    }
</script>
                    ...
                    <td class="contenidoPantalla" valign="top" style="padding-top: 5px; padding-bottom: 10px;">
                        <div id="contenidoProc" class="cuadroFondoBlanco" style="width:970px; height: 550px; overflow-x: auto; overflow-y: auto; padding: 10px;margin:0px;margin-top:0px;" align="center">
                            <fieldset style="width: 47%; float: left; padding-left: 10px; padding-right: 10px;">
                                <legend class="legendAzul"><%=meLanbide37I18n.getMensaje(idiomaUsuario, "proc.legend.listados")%></legend>
