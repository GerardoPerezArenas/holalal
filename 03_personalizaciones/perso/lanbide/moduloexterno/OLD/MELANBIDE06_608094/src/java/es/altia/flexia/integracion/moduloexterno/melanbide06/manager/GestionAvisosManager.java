...
public class GestionAvisosManager {
    ...
    //Logger
    private static Logger log = Logger.getLogger(GestionAvisosManager.class);
...
public List<SelectItem> getListaEventos(AdaptadorSQLBD adaptador) throws Exception {
    ...
    public GestionAvisosVO recuperarConfiguracionEmail(AdaptadorSQLBD adaptador, String codEvento, String codAsunto) {     
        if(log.isDebugEnabled()) log.debug("recuperarConfiguracionEmail() : BEGIN");
        Connection con = null;
        GestionAvisosVO gestionAvisosVO = new GestionAvisosVO();
        try{
            con = adaptador.getConnection();
            GestionAvisosDAO gestionAvisosDao = GestionAvisosDAO.getInstance();
            if(log.isDebugEnabled()) log.debug("Recuperamos la configuración del email");
            gestionAvisosVO = gestionAvisosDao.getConfiguracionEmail(con, codEvento, codAsunto);
        }catch(Exception ex){
        ...
        if(log.isDebugEnabled()) log.debug("recuperarConfiguracionEmail() : END");
        
        return gestionAvisosVO;
    }
    
    public String buscarEventoExtracto(AdaptadorSQLBD adaptador, String extracto) {     
        if(log.isDebugEnabled()) log.debug("buscarEventoExtracto() : BEGIN");
        Connection con = null;
        String resultado = null;
        try{
            con = adaptador.getConnection();
            GestionAvisosDAO gestionAvisosDao = GestionAvisosDAO.getInstance();
            if(log.isDebugEnabled()) log.debug("Recuperamos el evento");
        ...
        if(log.isDebugEnabled()) log.debug("buscarEventoExtracto() : END");
