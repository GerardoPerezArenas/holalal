...
<script type="text/javascript">
    ...
    function recargarTablaProspectoresJustificacionEca(result, actualizarOtrasPestanas){
        ...
        tabProspectores_jus.addColumna('150','left','<%=meLanbide44I18n.getMensaje(idiomaUsuario, "label.justificacion.prospectores.tabla.col2")%>');
        ...
        tabProspectores_jus.addColumna('130','right','<%=meLanbide44I18n.getMensaje(idiomaUsuario, "label.justificacion.prospectores.tabla.col11")%>');
    ...
    <%--function mostrarBotonCargarFicheroProspectores(){
        ...
        function cargarTodasVisitasJus(){        
            ...
            if(navigator.appName.indexOf("Internet Explorer")!=-1){
                result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE44&operacion=cargarVisitas&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&control='+control.getTime(),430,820,'no','no');
            }else{
                result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE44&operacion=cargarVisitas&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&control='+control.getTime(),450,850,'no','no');
            }
        ...
        function cargarVisitasProspector(idProspector){
            ...
            if(navigator.appName.indexOf("Internet Explorer")!=-1){
                result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE44&operacion=cargarVisitas&tipo=0&numero=<%=numExpediente%>&idProspector='+idProspector+'&opcion='+opcion+'&control='+control.getTime(),430,820,'no','no');
            }else{
                result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE44&operacion=cargarVisitas&tipo=0&numero=<%=numExpediente%>&idProspector='+idProspector+'&opcion='+opcion+'&control='+control.getTime(),450,850,'no','no');
            }
            if(result != undefined && result == true){
                var lista = getListaProspectoresJustificacionEca();
                recargarTablaProspectoresJustificacionEca(lista, true)
        ...
        <div style="clear: both;">
            ...
            <div style="width: 78%; text-align: right; height: 110px; float: right; padding-top: 10px;">
                <fieldset id="opcionescargavis" style="display: none; width: 100%; height: 100px; padding-top: 24px;">
                    ...
                    <div class="botonera">
                        <input type="button" id="btnCargarFicheroVisitasJus" name="btnCargarFicheroVisitasJus" class="botonMasLargo" value="<%=meLanbide44I18n.getMensaje(idiomaUsuario,"btn.general.cargar")%>" onclick="cargarFicheroVisitasProsEca();">
