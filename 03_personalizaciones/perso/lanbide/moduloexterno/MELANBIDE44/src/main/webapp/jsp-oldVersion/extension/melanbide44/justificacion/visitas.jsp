        ...
        <script type="text/javascript">
           /* window.onbeforeunload  = function () {
            ...
            function pulsarNuevaVisitaPros(){
                    ...
                    if(navigator.appName.indexOf("Internet Explorer")!=-1){
                        result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE44&operacion=cargarNuevaVisitaProspector&tipo=0&numero=<%=numExpediente%>&opcion=nuevo&prospector=<%=prospector != null ? prospector.getJusProspectoresCod() : ""%>&control='+control.getTime(),390,820,'no','no');
                    }else{
                        result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE44&operacion=cargarNuevaVisitaProspector&tipo=0&numero=<%=numExpediente%>&opcion=nuevo&prospector=<%=prospector != null ? prospector.getJusProspectoresCod() : ""%>&control='+control.getTime(),410,850,'no','no');
                    }
                    if (result != undefined){
                        if(result[0] == '0'){
            ...
            function pulsarEliminarVisitaPros(){
                    if(tabVisitas_pros.selectedIndex != -1) {
                        ...
                        if (resultado == 1){
                            ...
                            parametros = 'tarea=preparar&modulo=MELANBIDE44&operacion=eliminarVisitaProspectorJustificacion&tipo=0&numero=<%=numExpediente%>&idVis='+listaVisitas_pros[tabVisitas_pros.selectedIndex][0]+'&filtrar=<%=prospector != null ? "1" : "0"%>&control='+control.getTime();
                            try{
           ...
           // var camposErrores = new Array();
                ...
                for(var i = 1;i< result.length; i++){
                    ...
                    listaVisitas_prosTabla[i-1] = [fila[19] != undefined && fila[19].length > 0 ? '<img src="<%=request.getContextPath()%>/images/extension/melanbide44/alert.png"/>' : '', fila[1], fila[2], fila[3], fila[4], fila[5], fila[6], fila[7], fila[8], fila[9], fila[10], fila[11], fila[12], fila[13], fila[14], 
                        <%
                        if(prospector == null)
                        {
                        %>
                ...
                tabVisitas_pros.addColumna('80','right','<%=meLanbide44I18n.getMensaje(idiomaUsuario, "label.justificacion.visita.tabla.col15")%>');
                tabVisitas_pros.addColumna('100','left','<%=meLanbide44I18n.getMensaje(idiomaUsuario, "label.justificacion.visita.tabla.col16")%>');
                tabVisitas_pros.addColumna('100','left','<%=meLanbide44I18n.getMensaje(idiomaUsuario, "label.justificacion.visita.tabla.col17")%>');
                tabVisitas_pros.addColumna('100','left','<%=meLanbide44I18n.getMensaje(idiomaUsuario, "label.justificacion.visita.tabla.col18")%>');
        ...
        <form>
            ...
            <div style="width: 95%; padding: 10px; text-align: left;">
                ...
                <div class="lineaFormulario">
                    <div>
                        <%=meLanbide44I18n.getMensaje(idiomaUsuario,"label.justificacion.visita.prospector").toUpperCase()%>:
                    ...
                    <div class="botonera">
                        <input type="button" id="btnNuevoVisitaPros" name="btnNuevoVisitaPros" class="botonGeneral" value="<%=meLanbide44I18n.getMensaje(idiomaUsuario, "btn.general.nuevo")%>" onclick="pulsarNuevaVisitaPros();">
                        ...
                        <input type="button" id="btnConsultarVisitaPros" name="btnConsultarVisitaPros" class="botonGeneral" value="<%=meLanbide44I18n.getMensaje(idiomaUsuario, "btn.general.consultar")%>" onclick="pulsarModifVisitaPros();">
