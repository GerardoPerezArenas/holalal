...
<script type="text/javascript">
    ...
    function crearTablaEncargado(){
        TablaAnexoC = new Tabla(document.getElementById('tablaAnexoC'), 880);
        TablaAnexoC.addColumna('25','center',"<%=meLanbide65I18n.getMensaje(idiomaUsuario,"etiq.TablaAnexoCId")%>");
        TablaAnexoC.addColumna('50','center',"<%=meLanbide65I18n.getMensaje(idiomaUsuario,"etiq.TablaAnexoCNumLinea")%>");
        TablaAnexoC.addColumna('250','center',"<%=meLanbide65I18n.getMensaje(idiomaUsuario,"etiq.TablaAnexoCNombreC")%>");
        TablaAnexoC.addColumna('50','center',"<%=meLanbide65I18n.getMensaje(idiomaUsuario,"etiq.TablaAnexoCSexo")%>");
        TablaAnexoC.addColumna('70','center',"<%=meLanbide65I18n.getMensaje(idiomaUsuario,"etiq.TablaAnexoCDNI")%>");
        TablaAnexoC.addColumna('95','center',"<%=meLanbide65I18n.getMensaje(idiomaUsuario,"etiq.TablaAnexoCFechaAlta")%>");
        TablaAnexoC.addColumna('95','center',"<%=meLanbide65I18n.getMensaje(idiomaUsuario,"etiq.TablaAnexoCJornadaLaboralCompletaA")%>");
        TablaAnexoC.addColumna('95','center',"<%=meLanbide65I18n.getMensaje(idiomaUsuario,"etiq.TablaAnexoCJornadaLaboralParcialPorcjB")%>");
        TablaAnexoC.addColumna('95','center',"<%=meLanbide65I18n.getMensaje(idiomaUsuario,"etiq.TablaAnexoCJornadaLaboralParcialPagC")%>");
    ...
    function recargarTablaEncargado(result) {
            ...
            for (var i = 1; i < result.length; i++) {
                var sexo;
                fila = result[i];
                listaEncargados[i - 1] = fila;
                if(fila[4]=="1") sexo="Varón";
                else if(fila[4]=="2") sexo="Mujer";
                else sexo=fila[4];
                listaEncargTabla[i - 1] = [fila[0], fila[1], fila[2] +", "+fila[3],
 ...
 <div class="tab-page" id="tabPage653" style="height:520px; width: 100%;">
    ...
    <form>
        ...
        <div> 
            <div>
                ...
                <div id="divGeneralAnexoC"  style="overflow-y: auto; overflow-x: hidden; height: 290px;">     
                    <div id="tablaAnexoC" style="padding: 5px; width:990px; height: 280px; text-align: center; overflow-x: auto; overflow-y: auto;margin:0px;margin-top:0px;" align="center"></div>
                </div>
                <br/><br>
     ...
     <script  type="text/javascript">
        ...
        %>
            listaEncargados[<%=indice%>] = ['<%=objectVO.getIdentificador()%>','<%=objectVO.getNumLinea()%>','<%=objectVO.getApellidos()%>','<%=objectVO.getNombre()%>' ,
            ...
            listaEncargTabla[<%=indice%>] = ['<%=objectVO.getIdentificador()%>','<%=objectVO.getNumLinea()%>','<%=objectVO.getApellidos()%>'+', '+ '<%=objectVO.getNombre()%>' ,
