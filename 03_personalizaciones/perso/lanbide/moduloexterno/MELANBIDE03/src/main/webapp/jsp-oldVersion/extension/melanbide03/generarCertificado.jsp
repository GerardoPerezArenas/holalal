...
<script type="text/javascript">
    ...
    function ejecutarListadoCP(){        
        ...
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE03&operacion=cargarCriteriosFiltroListadoCP_APA&tipo=0&numExp=<%=numExp%>&control='+control.getTime(),330,840,'no','no');
        }else{
            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE03&operacion=cargarCriteriosFiltroListadoCP_APA&tipo=0&numExp=<%=numExp%>&control='+control.getTime(),400,870,'no','no');
        }
        if (result != undefined){
            if(result[0] == '0'){
