...
<html>
    ...
    <body>
            <div class="tab-page" id="tabPage501" style="height:420px; width: 91%;">
            ...
            <script type="text/javascript">
                ...
                function pulsarAltaServicio(){
                    ...
                    var result = null;
                    if(navigator.appName.indexOf("Internet Explorer")!=-1){
                        result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarNuevoServicio&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),320,780,'no','no');
                    }else{
                        result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarNuevoServicio&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),320,780,'no','no');
                    }
                    if (result != undefined){
                ...
                function pulsarModificarServicio(){
                    if(tabServicios.selectedIndex != -1) {
                        ...
                        var result = null;
                        if(navigator.appName.indexOf("Internet Explorer")!=-1){
                            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarModifServicio&tipo=0&numero=<%=numExpediente%>&id='+listaServicios[tabServicios.selectedIndex][0]+'&control='+control.getTime(),320,780,'no','no');
                        }else{
                            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarModifServicio&tipo=0&numero=<%=numExpediente%>&id='+listaServicios[tabServicios.selectedIndex][0]+'&control='+control.getTime(),320,780,'no','no');
                        }
                        if (result != undefined){
