    ...
    <body>
            ...
            <script type="text/javascript">
                ...
                function pulsarAltaDisponibilidad(){
                    ...
                    var result = null;
                    if(navigator.appName.indexOf("Internet Explorer")!=-1){
                        result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarNuevaEspecialdad&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),500,850,'no','no');
                    }else{
                        result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarNuevaEspecialdad&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),500,850,'no','no');
                    }
                    if (result != undefined){
                ...
                function pulsarModificarDisponibilidad(){
                    if(tabDisponibilidad.selectedIndex != -1) {
                        ...
                        var result = null;
                        if(navigator.appName.indexOf("Internet Explorer")!=-1){
                            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarModifDisponibilidad&tipo=0&numero=<%=numExpediente%>&id='+listaDisponibilidad[tabDisponibilidad.selectedIndex][0]+'&idespsol='+listaDisponibilidad[tabDisponibilidad.selectedIndex][1]+'&control='+control.getTime(),500,850,'no','no');
                        }else{
                            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarModifDisponibilidad&tipo=0&numero=<%=numExpediente%>&id='+listaDisponibilidad[tabDisponibilidad.selectedIndex][0]+'&idespsol='+listaDisponibilidad[tabDisponibilidad.selectedIndex][1]+'&control='+control.getTime(),500,850,'no','no');
                        }
                        if (result != undefined){
