...
<%@page import="es.altia.flexia.integracion.moduloexterno.melanbide60.vo.FilaPersonaContratadaVO" %>
...
<html>
    <head> 
        <%
            ...
            tituloPagina = meLanbide60I18n.getMensaje(idiomaUsuario,"label.justif.titulo.datosJustif");
        ...
        <script type="text/javascript">
            ...
            function inicio(){
                ...
                reemplazarPuntosCme(document.getElementById('txtTotalConcedido'));
            ...
            function verDocumento(documento){
                ...
                parametros = '?tarea=preparar&modulo=MELANBIDE60&operacion=descargarDocumentoOferta&tipo=0&idOferta=<%=ofertaConsulta != null && ofertaConsulta.getIdOferta() != null ? ofertaConsulta.getIdOferta() : ""%>&idPuesto=<%=ofertaConsulta != null && ofertaConsulta.getCodPuesto() != null ? ofertaConsulta.getCodPuesto() : ""%>&numero=<%=numExpediente%>&documento='+documento+'&control='+control.getTime();
                inicio = false;
                window.open(url+parametros, "_blank");
            }
    
            function dblClckTablaPersonasJustifCme(rowID,tableName){
                pulsarConsultarPersonaContratada();
            }
            
            function pulsarConsultarPersonaContratada(){
                var fila;
                if(tabPersonasJustifCme.selectedIndex != -1) {
                    fila = tabPersonasJustifCme.selectedIndex;
                    var control = new Date();
                    var result = null;
                    var opcion = '<%=consulta == true ? "consultar" : "modificar"%>';
                    if(navigator.appName.indexOf("Internet Explorer")!=-1){
                        result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE60&operacion=cargarDatosPersonaContratada&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&idJustif='+listaPersonasJustifCme[fila][1]+'&idOferta='+listaPersonasJustifCme[fila][0]+'&idPuesto=<%=puestoConsulta.getCodPuesto()%>&control='+control.getTime(),380,980,'yes','no');
                    }else{
                        result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE60&operacion=cargarDatosPersonaContratada&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&idJustif='+listaPersonasJustifCme[fila][1]+'&idOferta='+listaPersonasJustifCme[fila][0]+'&idPuesto=<%=puestoConsulta.getCodPuesto()%>&control='+control.getTime(),440,980,'yes','no');
                    }
                    if (result != undefined){
                        if(result[0] == '0'){
                            recargarTablaContratacionesCme(result);
                        }
                    }
                }else{
            ...
            function recargarTablaContratacionesCme(result){
                var fila;
                tabPersonasJustifCme = new Array();
                listaPersonasJustifCme = new Array();
                listaPersonasJustifCmeTabla = new Array();
                listaPersonasJustifCmeTabla_titulos = new Array();
                listaPersonasJustifCmeTabla_estilos = new Array();
                totalJustificado = 0;
                fila = result[1];
                
                //this.parent.recargarCalculosCme(fila);
                window.returnValue = fila;
                //this.parent.parent.opener.recargarCalculosCme(fila);
                //this.parent.parent.opener.actualizarPestanaJustificacion();
                var justifAct;
                for(var i = 2;i< result.length; i++){
                    fila = result[i];
                    listaPersonasJustifCme[i-2] = fila;
                    listaPersonasJustifCmeTabla[i-2] = [fila[2], fila[3], fila[4], fila[5], fila[6]];
                    listaPersonasJustifCmeTabla_titulos[i-2] = [fila[2], fila[3], fila[4], fila[5], fila[6]];
                    try{
                        if(fila[6] != null){
                            justifAct = fila[6];
                            justifAct = reemplazarTextoCme(justifAct, /,/g, '.');
                            if(!isNaN(justifAct)){
                ...
                tabPersonasJustifCme = new FixedColumnTable(document.getElementById('justificacionesCme'), 850, 876, 'justificacionesCme');
                tabPersonasJustifCme.addColumna('80','left',"<%= meLanbide60I18n.getMensaje(idiomaUsuario,"label.nuevaJustificacion.tabla.personas.col1")%>");
                tabPersonasJustifCme.addColumna('490','left',"<%= meLanbide60I18n.getMensaje(idiomaUsuario,"label.nuevaJustificacion.tabla.personas.col2")%>");    
                tabPersonasJustifCme.addColumna('90','left',"<%= meLanbide60I18n.getMensaje(idiomaUsuario,"label.nuevaJustificacion.tabla.personas.col3")%>");   
                tabPersonasJustifCme.addColumna('90','left',"<%= meLanbide60I18n.getMensaje(idiomaUsuario,"label.nuevaJustificacion.tabla.personas.col4")%>");     
                tabPersonasJustifCme.addColumna('97','right',"<%= meLanbide60I18n.getMensaje(idiomaUsuario,"label.nuevaJustificacion.tabla.personas.col5")%>");     

                tabPersonasJustifCme.numColumnasFijas = 0;

                tabPersonasJustifCme.displayCabecera=true;

                for(var cont = 0; cont < listaPersonasJustifCmeTabla.length; cont++){
                    tabPersonasJustifCme.addFilaConFormato(listaPersonasJustifCmeTabla[cont], listaPersonasJustifCmeTabla_titulos[cont], listaPersonasJustifCmeTabla_estilos[cont])
                }

                tabPersonasJustifCme.displayCabecera=true;

                tabPersonasJustifCme.height = '246';

                tabPersonasJustifCme.altoCabecera = 50;

                tabPersonasJustifCme.scrollWidth = 850;

                tabPersonasJustifCme.dblClkFunction = 'dblClckTablaPersonasJustifCme';

                tabPersonasJustifCme.displayTabla();

                tabPersonasJustifCme.pack();
                ...
                if(!isNaN(totalJustificado)){
                    ...
                    reemplazarPuntosCme(document.getElementById('txtTotalJustificado'));
                }
            }
        </script>
    </head>
        ...
        <div id="divCuerpo" style="overflow-y: auto; padding: 10px;">
            <form  id="formNuevaOferta">
                ...
                <fieldset  style="width: 100%;">
                    ...
                    <fieldset  style="width: 100%;">
                        <legend class="legendAzul"><%=meLanbide60I18n.getMensaje(idiomaUsuario,"legend.justificacion.personasContratadas")%></legend>
                        <div id="justificacionesCme" style="padding: 5px; width:876px; height: 269px; text-align: center; margin:0px;margin-top:0px;" align="center"></div>
                        <div class="lineaFormulario">
                            <div style="float: right; padding-right: 50px;">
                                <div style="float: left; width: 125px;">
                                    <%=meLanbide60I18n.getMensaje(idiomaUsuario,"label.justificacion.personasContratadas.totalJustificado")%>
                                </div>
                                <div style="width: auto; float: left; margin-left: 10px;">
                        ...
                        <div class="lineaFormulario">
                            <div style="float: right; padding-right: 50px;">
                                <div style="float: left; width: 125px;">
                                    <%=meLanbide60I18n.getMensaje(idiomaUsuario,"label.justificacion.personasContratadas.totalConcedido")%>
                                </div>
                                <div style="width: auto; float: left; margin-left: 10px;">
                        ...
                        <div class="botonera" id="botoneraJustificaciones">
                            <input type="button" id="btnModifJustif" name="btnModifJustif" class="botonGeneral" value="<%=meLanbide60I18n.getMensaje(idiomaUsuario, "btn.modificar")%>" onclick="pulsarConsultarPersonaContratada();">
                            <input type="button" id="btnConsultarJustif" name="btnConsultarJustif" class="botonGeneral" value="<%=meLanbide60I18n.getMensaje(idiomaUsuario, "btn.consultar")%>" onclick="pulsarConsultarPersonaContratada();">
                        </div>
                    </fieldset>
                    <div class="botonera">
                        <input type="button" id="btnCerrar" name="btnCerrar" class="botonGeneral" value="<%=meLanbide60I18n.getMensaje(idiomaUsuario,"btn.cerrar")%>" onclick="cerrarVentana();">
                    </div>
                </form>
        </div>
        <script type="text/javascript">            
            var tabPersonasJustifCme;
            var listaPersonasJustifCme = new Array();
            var listaPersonasJustifCmeTabla = new Array();
            var listaPersonasJustifCmeTabla_titulos = new Array();
            var listaPersonasJustifCmeTabla_estilos = new Array();
            ...
            %>
                listaPersonasJustifCme[<%=i%>] = ['<%=contrat.getIdOferta()%>', '<%=contrat.getIdJustificacion()%>','<%=contrat.getNif()%>', '<%=contrat.getNomApel()%>','<%=contrat.getFeDesde()%>', '<%=contrat.getFeHasta()%>', '<%=contrat.getImpJustif()%>'];
                listaPersonasJustifCmeTabla[<%=i%>] = ['<%=contrat.getNif()%>', '<%=contrat.getNomApel()%>','<%=contrat.getFeDesde()%>', '<%=contrat.getFeHasta()%>', '<%=contrat.getImpJustif()%>'];
                listaPersonasJustifCmeTabla_titulos[<%=i%>] = ['<%=contrat.getNif()%>', '<%=contrat.getNomApel()%>','<%=contrat.getFeDesde()%>', '<%=contrat.getFeHasta()%>', '<%=contrat.getImpJustif()%>'];
                
                totalJustificado += parseFloat('<%=contrat.getImpJustif() != null && !contrat.getImpJustif().equals("") && !contrat.getImpJustif().equals("-") ? contrat.getImpJustif() : "0"%>');
            <%
                    }// for
                }// if
            %>
