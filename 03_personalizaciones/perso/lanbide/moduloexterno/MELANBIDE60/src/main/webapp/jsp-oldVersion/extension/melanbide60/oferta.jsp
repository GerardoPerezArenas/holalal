...
<script type="text/javascript">   
    function pulsarAltaOfertaCme(){
        ...
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            nodos = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE60&operacion=cargarNuevaOferta&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),560,980,'no','no');
        }else{
            nodos = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE60&operacion=cargarNuevaOferta&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),620,980,'no','no');
        }
        if (nodos != undefined){
    ...
    function pulsarModificarOfertaCme(){
        ...
        if(tabOfertasCme.selectedIndex != -1) {
            ...
            if(navigator.appName.indexOf("Internet Explorer")!=-1){
                nodos = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE60&operacion=cargarModificarOferta&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&idOferta='+listaOfertasCme[fila][0]+'&idPuesto='+listaOfertasCme[fila][1]+'&control='+control.getTime(),560,980,'yes','no');
            }else{
                nodos = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE60&operacion=cargarModificarOferta&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&idOferta='+listaOfertasCme[fila][0]+'&idPuesto='+listaOfertasCme[fila][1]+'&control='+control.getTime(),620,980,'yes','no');
            }
            if (nodos != undefined){
                result = extraerListadoOfertasCme(nodos);
                if(result != undefined){
