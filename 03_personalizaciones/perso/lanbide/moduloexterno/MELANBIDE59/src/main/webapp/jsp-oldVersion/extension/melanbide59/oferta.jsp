...
<script type="text/javascript">   
    function pulsarAltaOfertaCpe(){
        ...
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            nodos = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE59&operacion=cargarNuevaOferta&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),560,980,'no','no');
        }else{
            nodos = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE59&operacion=cargarNuevaOferta&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),620,980,'no','no');
        }
        if (nodos != undefined){
    ...
    function pulsarModificarOfertaCpe(){
        ...
        if(tabOfertasCpe.selectedIndex != -1) {
            ...
            if(navigator.appName.indexOf("Internet Explorer")!=-1){
                nodos = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE59&operacion=cargarModificarOferta&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&idOferta='+listaOfertasCpe[fila][0]+'&idPuesto='+listaOfertasCpe[fila][1]+'&control='+control.getTime(),560,980,'yes','no');
            }else{
                nodos = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE59&operacion=cargarModificarOferta&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&idOferta='+listaOfertasCpe[fila][0]+'&idPuesto='+listaOfertasCpe[fila][1]+'&control='+control.getTime(),620,980,'yes','no');
            }
            if (nodos != undefined){
                result = extraerListadoOfertasCpe(nodos);
                if(result != undefined){
    ...
    function recargarTablaOfertasCpe(result){
        ...
        tabOfertasCpe.addColumna('140','left',"<%= meLanbide59I18n.getMensaje(idiomaUsuario,"label.solicitud.tabla.ofertas.col8")%>");     
