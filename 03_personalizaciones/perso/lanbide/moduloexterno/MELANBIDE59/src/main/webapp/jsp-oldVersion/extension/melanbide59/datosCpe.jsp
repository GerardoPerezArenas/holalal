        ...
        var rsultado;
            ...
            if(document.forms[0].modoConsulta.value == "si"){
                if(minimis != undefined && minimis != ''){
                    resultado = jsp_alerta('A','<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.datosCpe.impConCalculadoEs")%>: <br/>'+document.getElementById('concedidoReal').value+' '+'<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.euros")%>'+'<br/>'+'<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.avisoMinimis")%>'+' '+document.getElementById('minimisConce').value+'<br/>'+'<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.avisoImporteRenunciado")%>'+' '+impRen);
                }else{
                    resultado = jsp_alerta('A','<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.datosCpe.impConCalculadoEs")%>: <br/>'+document.getElementById('concedidoReal').value+' '+'<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.euros")%>'+'<br/>'+'<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.avisoImporteRenunciado")%>'+' '+impRen);
                }
            }else{
                if(minimis != undefined && minimis != ''){
                    resultado = jsp_alerta('','<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.datosCpe.impConCalculadoEs")%>: <br/>'+document.getElementById('concedidoReal').value+' '+'<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.euros")%>'+'<br/>'+'<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.avisoMinimis")%>'+' '+document.getElementById('minimisConce').value+'<br/>'+'<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.avisoImporteRenunciado")%>'+' '+impRen+'<br/><br/>'+'<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.datosCpe.preguntaAceptarImporte")%>');
                }else{
                    resultado = jsp_alerta('','<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.datosCpe.impConCalculadoEs")%>: <br/>'+document.getElementById('concedidoReal').value+' '+'<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.euros")%>'+'<br/>'+'<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.avisoImporteRenunciado")%>'+' '+impRen+'<br/><br/>'+'<%=meLanbide59I18n.getMensaje(idiomaUsuario, "msg.datosCpe.preguntaAceptarImporte")%>');
                }
            }
            if (resultado == 1){
    ...
    function actualizarPestanaDatosSuplementarios(){
     ...
     function consultarImportesResolucion(res){
        try{
            if(res != undefined && res != ''){
                var control = new Date();
                if(navigator.appName.indexOf("Internet Explorer")!=-1){
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE59&operacion=cargarImportesResolucion&res='+res+'&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),380,395,'yes','no');
                }else{
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE59&operacion=cargarImportesResolucion&res='+res+'&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),400,440,'yes','no');
                }
            }else{
     ...
     function consultarHistoricos(res){
        try{
            var control = new Date();
            if(navigator.appName.indexOf("Internet Explorer")!=-1){
                lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE59&operacion=cargarHistorico&tipo=0&tram='+res+'&numero=<%=numExpediente%>&control='+control.getTime(),380,950,'yes','no');
            }else{            
                lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE59&operacion=cargarHistorico&tipo=0&tram='+res+'&numero=<%=numExpediente%>&control='+control.getTime(),400,950,'yes','no');
            }
        }catch(err){
                ...
                <div style="clear: both; width: 100%; text-align: left; padding-left: 12px;" class="textoAzul">
                    <div class="lineaFormulario">
                        ...
                        <div style="width: 259px; float: left;">
                            ...
                            <a id="anchorGestorTramitador" name="anchorGestorTramitador" href=""><img src="<%=request.getContextPath()%>/images/listas/botondesplegable.gif" id="botonGestorTramitador" name="botonGestorTramitador" border="0" width="14" height="14" style="cursor:hand;" alt="<%=meLanbide59I18n.getMensaje(idiomaUsuario, "altDesplegable")%>" title="<%=meLanbide59I18n.getMensaje(idiomaUsuario, "altDesplegable")%>"></a>
                        </div>
