...
<%@ page import="es.altia.flexia.integracion.moduloexterno.melanbide29.vo.FilaContratoRenovacionPlantillaVO"%>
	...
	var nombreModuloLicitacion = "<%=nombreModulo%>";
        ...
        function pulsarAltaContrato(){
            ...
            if(navigator.appName.indexOf("Internet Explorer")!=-1){
                result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE29&operacion=cargarNuevoContrato&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),600,980,'no','no');
            }else{
                result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE29&operacion=cargarNuevoContrato&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),642,990,'no','no');
            }
            if (result != undefined){
                if(result[0] == '0'){
        ...
        function pulsarEliminarContrato(){
            ...
            if(tabContratos.selectedIndex != -1) {
                var resultado = jsp_alerta('', '<%=meLanbide29I18n.getMensaje(idiomaUsuario, "msg.preguntaEliminar")%>');
                if (resultado == 1){
                    <%
                        mensajeProgreso = meLanbide29I18n.getMensaje(idiomaUsuario, "msg.procesoEliminarContrato");
                    %>
                    ...
                    var parametros = "";
                    var control = new Date();
                    parametros = 'tarea=preparar&modulo=MELANBIDE29&operacion=eliminarContrato&tipo=0&numero=<%=numExpediente%>&idCon='+listaContratos[tabContratos.selectedIndex][0]+'&control='+control.getTime();
                    try{
        ...
        function recargarTabla(result){
            ...
            tabContratos.addColumna('100','left',"<%= meLanbide29I18n.getMensaje(idiomaUsuario,"contratos.tabla.col1")%>");
            tabContratos.addColumna('195','left',"<%= meLanbide29I18n.getMensaje(idiomaUsuario,"contratos.tabla.col2")%>");                                                            
            tabContratos.addColumna('100','left',"<%= meLanbide29I18n.getMensaje(idiomaUsuario,"contratos.tabla.col3")%>");
            tabContratos.addColumna('195','left',"<%= meLanbide29I18n.getMensaje(idiomaUsuario,"contratos.tabla.col4")%>");
            tabContratos.addColumna('100','left',"<%= meLanbide29I18n.getMensaje(idiomaUsuario,"contratos.tabla.col5")%>");
            tabContratos.addColumna('178','left',"<%= meLanbide29I18n.getMensaje(idiomaUsuario,"contratos.tabla.col6")%>");
        ...
        function pulsarModificarContrato(event){
            ...
            if(fila >= 0 && fila < listaContratos.length) {
                ...
                if(navigator.appName.indexOf("Internet Explorer")!=-1){
                    result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE29&operacion=cargarModificarContrato&tipo=0&numero=<%=numExpediente%>&idCon='+listaContratos[fila][0]+'&cerrado='+cerrado+'&control='+control.getTime(),520,980,'no','no');
                }else{
                    result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE29&operacion=cargarModificarContrato&tipo=0&numero=<%=numExpediente%>&idCon='+listaContratos[fila][0]+'&cerrado='+cerrado+'&control='+control.getTime(),570,990,'no','no');
                }
                if (result != undefined){
                    if(result[0] == '0'){
                        recargarTabla(result);
...
<body>
    ...
    <div class="tab-page" id="tabPage291" style="height:520px; width: 100%;">
            ...
            <div style="clear: both;">
                <div>
                    <fieldset style="width: 97.5%; padding-top: 10px; margin-top: 10px;">
                        <legend class="legendAzul"><%=meLanbide29I18n.getMensaje(idiomaUsuario, "label.listadoContratos")%></legend>
                        <div id="contratos" style="width:910px; height: 450px; text-align: center; overflow-x: auto; overflow-y: auto;margin:0px;margin-top:0px;" align="center"></div>
                        <div style="width: 100%; text-align: center;">
                            <input type="button" id="btnNuevoContrato" name="btnNuevoContrato" class="botonGeneral" value="<%=meLanbide29I18n.getMensaje(idiomaUsuario, "btn.alta")%>" onclick="pulsarAltaContrato();">
                            <input type="button" id="btnModificarContrato" name="btnModificarContrato" class="botonGeneral" value="<%=meLanbide29I18n.getMensaje(idiomaUsuario, "btn.modificar")%>" onclick="pulsarModificarContrato();">
                            <input type="button" id="btnEliminarContrato" name="btnEliminarContrato" class="botonGeneral" value="<%=meLanbide29I18n.getMensaje(idiomaUsuario, "btn.eliminar")%>" onclick="pulsarEliminarContrato();">
                        </div>
                    </fieldset>
                </div>
            </div>
