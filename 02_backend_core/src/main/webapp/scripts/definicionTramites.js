...
function crearListasTablaEntrada() {
    ...
    for (i=0; i < listaTablaEntrada.length; i++) {
        ...
        if (listaTablaEntradaOriginal[i][0]!="" && listaTablaEntradaOriginal[i][0].trim()!=""){
            listaCodTramitesTabla +=listaTablaEntradaOriginal[i][0]+separador;
        } else {
            listaCodTramitesTabla +="_"+separador;
        }
        if (listaTablaEntradaOriginal[i][2]!="" && listaTablaEntradaOriginal[i][2].trim()!=""){
            listaTramitesTabla +=listaTablaEntradaOriginal[i][2]+separador;
        } else {
            listaTramitesTabla +="_"+separador;
        }
        listaTiposTabla +=listaTablaEntrada[i][0]+separador;
        if (listaTablaEntradaOriginal[i][4]!="" && listaTablaEntradaOriginal[i][4].trim()!=""){
            listaEstadosTabla +=listaTablaEntradaOriginal[i][4]+separador;
        } else {
            listaEstadosTabla +="_"+separador;
        }
        ...
        if (listaTablaEntradaOriginal[i].length>=7 && listaTablaEntradaOriginal[i][6]!="" && listaTablaEntradaOriginal[i][6].trim()!=""){
            listaCodDocumentos +=listaTablaEntradaOriginal[i][6]+separador;
        } else {
            listaCodDocumentos +="_"+separador;
        }

        listaCodCondicion +=listaTablaEntradaOriginal[i][3]+separador;
        listaExpresionesTabla +=listaTablaEntrada[i][1]+separador;
...
function crearListasDoc() {
    ...
    for (i=0; i < listaDoc.length; i++) {
        listaCodigosDoc +=listaDoc[i][0]+separador;
        listaNombresDoc +=listaDoc[i][1]+separador;
        listaVisibleDoc +="S"+separador;
        listaPlantillaDoc +=listaDocOriginal[i][3]+separador;
        listaCodPlantilla +=listaDocOriginal[i][4]+separador;
        if (  (!(listaDocOriginal[i][6])) || (listaDocOriginal[i][6]=='') ) {
            listaFirmaPlantilla +='N'+separador;
        } else {
            listaFirmaPlantilla +=listaDocOriginal[i][6]+separador;
        }
        listaActivos += listaDocOriginal[i][7]+separador;
    }
...
function crearListasEnlaces() {
    ...
    for (i=0; i < listaEnlacesOriginal.length; i++) {
        listaCodigoEnlaces +=listaEnlacesOriginal[i][0]+separador;
        listaDescripcionEnlaces +=listaEnlacesOriginal[i][1]+separador;
        listaUrlEnlaces +=listaEnlacesOriginal[i][2]+separador;
        listaEstadoEnlaces +=listaEnlacesOriginal[i][3]+separador;
    }
...
function crearListasCampos() {
    ...
    for (i=0; i < listaCampos.length; i++) {
        listaCodCampos += listaCamposOriginal[i][0]+separador;
        listaDescCampos += listaCamposOriginal[i][1]+separador;
        listaCodPlantilla += listaCamposOriginal[i][2]+separador;
        listaCodTipoDato += listaCamposOriginal[i][3]+separador;
        listaTamano += listaCamposOriginal[i][4]+separador;
        if(listaCamposOriginal[i][5] == "") {
            listaMascara += " "+separador;
        } else {
            listaMascara += listaCamposOriginal[i][5]+separador;
        }
        listaObligatorio += listaCamposOriginal[i][6]+separador;
        listaOrden += listaCamposOriginal[i][7]+separador;
        listaRotulo += listaCamposOriginal[i][10]+separador;
        listaVisible += listaCamposOriginal[i][11]+separador;
        listaActivo += listaCamposOriginal[i][12]+separador;
        listaOcultos += listaCamposOriginal[i][13]+separador;
        listaBloqueados += listaCamposOriginal[i][14]+separador;
        //PlazoFecha y CheckPlazoFecha
        if(listaCamposOriginal[i][15] == "") {
             listaPlazoFecha += " "+separador;
        } else {
            listaPlazoFecha += listaCamposOriginal[i][15]+separador;
        }
        if(listaCamposOriginal[i][16] == "") {
             listaCheckPlazoFecha += " "+separador;
        } else {
            listaCheckPlazoFecha += listaCamposOriginal[i][16]+separador;
        }	
        if(listaCamposOriginal[i][17] == "") {
            listaValidacion += " "+separador;
        } else {
            listaValidacion += listaCamposOriginal[i][17]+separador;
        }
        if(listaCamposOriginal[i][18] == "") {
            listaOperacion += " "+separador;
        } else {
            listaOperacion += listaCamposOriginal[i][18]+separador;
        }
        if(listaCamposOriginal[i][19] == ""){
            listaCodAgrupacion += " "+separador;
        }else
        {
            listaCodAgrupacion += listaCamposOriginal[i][19]+separador;
        }
        if(listaCamposOriginal[i][20] === ""){
            listaPosicionXCampo += null + separador;
        }else{
            listaPosicionXCampo += listaCamposOriginal[i][20]+separador;
        }//if(listaCamposOriginal[i][20] == "")
        if(listaCamposOriginal[i][21] === ""){
            listaPosicionYCampo += null + separador;
        }else{
            listaPosicionYCampo += listaCamposOriginal[i][21]+separador;
        }//if(listaCamposOriginal[i][21] == "")
    } 
...
function crearListasAgrupaciones(){
    ...
    for (i=0; i < listaAgrupacionesOriginal.length; i++) {
        listaCodAgrupaciones += listaAgrupacionesOriginal[i][0] +separador;
        listaDescAgrupaciones += listaAgrupacionesOriginal[i][1] +separador;
        listaOrdenAgrupaciones +=listaAgrupacionesOriginal[i][2] +separador;
        listaAgrupacionesActivas += listaAgrupacionesOriginal[i][3] +separador;
...
function mostrarListaUnidadInicio(){
    ...
    condiciones[0]='UOR_NO_VIS'+separador;
    condiciones[1]='0';
    condiciones[2]='UOR_ESTADO'+separador;
    condiciones[3]='A';
