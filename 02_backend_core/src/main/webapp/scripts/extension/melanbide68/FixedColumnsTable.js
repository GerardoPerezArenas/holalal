...
function FixedColumnTable(par,anchoTabla,pWidth,nomT){
  ...
  this.addColumna = function(ancho, align, nombre, titulo, tipoDato){
    ...
    this.displayTabla = function(){
            ...
            var str = '';
            str += '<table border="0" id="tbl_'+this.nombreTabla+'" cellpadding="0" cellspacing="0" class="main" height="'+this.height+'" width="'+this.anchoTabla+'" onmousewheel="forwardEvent(event, \''+this.nombreTabla+'\');"  onselectstart="return false;" onclick="fctb_selectRow(event, \''+this.nombreTabla+'\');" '+(this.dblClkFunction != '' ? 'ondblclick="fctb_selectRow(event, \''+this.nombreTabla+'\'); '+this.dblClkFunction+'();"' : 'fctb_selectRow(event, \''+this.nombreTabla+'\');')+'>';
                        ...
                        str += '<table border="0" cellpadding="0" cellspacing="0" width="auto" class="root">';
                            ...
                            for(var i = 0; i < this.numColumnasFijas; i++){
                                ...
                                if(i == this.numColumnasFijas - 1){
                                    str += '<td  id="cabecera'+i+'" class="inner frozencol toprow xCabecera" width="'+this.columnas[i][0]+'" style="height: '+this.altoCabecera+'px; white-space: normal; border-right: 0px; border-left: 1px solid #ffffff;" title="'+this.columnas[i][5]+'">';
                                }else{
                                    str += '<td id="cabecera'+i+'" class="inner frozencol toprow xCabecera" width="'+this.columnas[i][0]+'" style="height: '+this.altoCabecera+'px; white-space: normal; border-right: 0px; border-left: 1px solid #ffffff;" title="'+this.columnas[i][5]+'">';
                                }
                                ...
                                for(var j = 0; j < this.numColumnasFijas; j++){
                                    ...
                                    if(i == 0){
                                        if(j == this.numColumnasFijas-1){
                                            str += '<td  id="fila'+j+'"  class="inner toprow rightcol" width="'+this.columnas[j][0]+'" title="'+titleAct+'" align="'+this.columnas[j][2]+'" style="border-right: 0px solid #666;'+estiloAct+'">';
                                        }else if(j == 0){
                                            str += '<td  id="fila'+j+'" class="inner frozencol toprow" width="'+(parseFloat(this.columnas[j][0])+2)+'" title="'+titleAct+'" align="'+this.columnas[j][2]+'" style="border-left: 0px solid #666;'+estiloAct+'">';
                                        }
                                        else{
                                            str += '<td  id="fila'+j+'" class="inner frozencol toprow" width="'+(parseFloat(this.columnas[j][0])+1)+'" title="'+titleAct+'" align="'+this.columnas[j][2]+'" style="border-left: 0px solid #666;'+estiloAct+'">';
                                        }
                                    }else if(i == this.lineas.length - 1){
                                        if(j == this.numColumnasFijas-1){
                                            str += '<td  id="fila'+j+'" class="inner bottomcol rightcol" width="'+this.columnas[j][0]+'" title="'+titleAct+'" align="'+this.columnas[j][2]+'" style="border-right: 0px solid #666;'+estiloAct+'">';
                                        }else if(j == 0){
                                            str += '<td  id="fila'+j+'" class="inner frozencol bottomcol" width="'+(parseFloat(this.columnas[j][0])+2)+'" title="'+titleAct+'" align="'+this.columnas[j][2]+'" style="border-left: 0px solid #666;'+estiloAct+'">';
                                        }else{
                                            str += '<td  id="fila'+j+'" class="inner frozencol bottomcol" width="'+(parseFloat(this.columnas[j][0])+1)+'" title="'+titleAct+'" align="'+this.columnas[j][2]+'" style="border-left: 0px solid #666;'+estiloAct+'">';
                                        }
                                    }else{
                                        if(j == this.numColumnasFijas-1){
                                            str += '<td id="fila'+j+'" class="inner rightcol" width="'+this.columnas[j][0]+'" title="'+titleAct+'" align="'+this.columnas[j][2]+'" style="border-right: 0px solid #666;'+estiloAct+'">';
                                        }else if(i == 0){
                                            str += '<td id="fila'+j+'" class="inner frozencol" width="'+(parseFloat(this.columnas[j][0])+2)+'" title="'+titleAct+'" align="'+this.columnas[j][2]+'" style="border-left: 0px solid #666;'+estiloAct+'">';
                                        }else{
                                            str += '<td id="fila'+j+'" class="inner frozencol" width="'+(parseFloat(this.columnas[j][0])+1)+'" title="'+titleAct+'" align="'+this.columnas[j][2]+'" style="border-left: 0px solid #666;'+estiloAct+'">';
                                        }
                                    }
                                            str += this.lineas[i][j];
                                        str += '</td>';
            ...
            str += '<tr class="horizontal-scroll">';
                str += '<td colspan="2" width="'+anchoTotal+'" valign="top" height="22">';
                if(navigator.appName.indexOf("Internet Explorer") != -1){
                    str += '<div id="hScroll_'+this.nombreTabla+'" class="horizontal-scroll" style="display: inline-flex; position: relative; width: '+anchoTabla+'; margin-top: -10px;" onscroll="reposHorizontal(this, \''+this.nombreTabla+'\');">';
                }else{
                    str += '<div id="hScroll_'+this.nombreTabla+'" class="horizontal-scroll" onscroll="reposHorizontal(this, \''+this.nombreTabla+'\');">';
                }
                        str += '<div style="width: '+this.scrollWidth+'px;">';
