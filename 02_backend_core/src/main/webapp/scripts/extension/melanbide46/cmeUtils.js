...
function validarNumericoCme(numero, longTotal){
    try{
...
function validarNumericoDecimalCme(numero, longTotal, longParteDecimal){
    try{
...
function reemplazarPuntosCme(campo){
    try{
...
function reemplazarComasCme(campo){
    try{
        var valor = campo.value;
        if(valor != null && valor != ''){
            valor = valor.replace(/\,/g,'.');
...
/*function comprobarCaracteresEspecialesCme(texto){
    //var iChars = "!@#$%^&*()+=-[]\\';,./{}|\":<>?";
    var iChars = "&'<>|^/\\%?∆¨??^~?¢≥";
...
function comprobarCaracteresEspecialesCme(valor){
    if(Trim(valor) != ''){
        valor = normalizarTextoCme(valor);
...
function barraProgresoCme(valor, idBarra) {
    if(valor=='on'){
...
function validarFechaCme(form, inputFecha)
{
    if (Trim(inputFecha.value)!='') {
        if (!ValidarFechaConFormatoCme(inputFecha.value,'dd/mm/yyyy')){
            return false;
        }
    }
    return true;
...
function ValidarFechaConFormatoCme(fecha, formato) {
...
function reemplazarTextoCme(text, regExp, replacement){
    try{
        if(text != null && text != ''){
...
function getListAsTextCme(lista){
    var retStr = '';
    var act = '';
    try{
...
function comprobarFechaCme(inputFecha, msgError) {
  ...
  if (Trim(inputFecha.value)!='') {
    ...
    // No consultando. Unico formato posible: dd/mm/yy o dd/mm/yyyy
      var D = ValidarFechaConFormatoCme(inputFecha.value,formato);
      if (!D[0]){
...
function ajustarDecimalesCme(num, numDecimales){
    try{
        var txt = ''+num;
        if(txt.indexOf(".") >= 0){
            txt = txt + '001';
...
function normalizarTextoCme(text){
