 ...
 */
 ...
 function comboSexo() {
     ...
     var listaSexoJSON = JSON.parse($("#listaSexo").val());
     var variableComboSexo = new Combo("Sexo");
 //    variableComboSexo.change = hola;
     var lcodSexoGA = new Array();
     var ldescSexoGA = new Array();

     if (listaSexoJSON != null && listaSexoJSON.length > 0)
     {
         listaSexoJSON.forEach(function (sexo, index) {
             lcodSexoGA.push(sexo.codigo);
             ldescSexoGA.push(traducirDesplegable(sexo.descripcion));
         });
         variableComboSexo.addItems(lcodSexoGA, ldescSexoGA);
     }
 }

 function comboTipoDisc() {

     var listaTipoDiscJSON = JSON.parse($("#listaTipoDisc").val());
     var variableComboTipoDisc = new Combo("TipoDisc");
 //    variableComboSexo.change = hola;
     var lcodTipoDiscGA = new Array();
     var ldescTipoDiscGA = new Array();

     if (listaTipoDiscJSON != null && listaTipoDiscJSON.length > 0)
     {
         listaTipoDiscJSON.forEach(function (tipoDisc, index) {
             lcodTipoDiscGA.push(tipoDisc.codigo);
             ldescTipoDiscGA.push(traducirDesplegable(tipoDisc.descripcion));
         });
         variableComboTipoDisc.addItems(lcodTipoDiscGA, ldescTipoDiscGA);
     }
 }

  function comboColectivo() {

      var listaColectivoJSON = JSON.parse($("#listaColectivo").val());
      var variableComboColectivo = new Combo("Colectivo");
  //    variableComboSexo.change = hola;
      var lcodColectivoGA = new Array();
      var ldescColectivoGA = new Array();

      if (listaColectivoJSON != null && listaColectivoJSON.length > 0)
      {
          listaColectivoJSON.forEach(function (colectivo, index) {
              lcodColectivoGA.push(colectivo.codigo);
              ldescColectivoGA.push(traducirDesplegable(colectivo.descripcion));
          });
          variableComboColectivo.addItems(lcodColectivoGA, ldescColectivoGA);
      }
  }

    function comboTipoContrato() {

        var listaTipoContratoJSON = JSON.parse($("#listaTipoContrato").val());
        var variableTipoContratoColectivo = new Combo("TipoContrato");
    //    variableComboSexo.change = hola;
        var lcodTipoContratoGA = new Array();
        var ldescTipoContratoGA = new Array();

        if (listaTipoContratoJSON != null && listaTipoContratoJSON.length > 0)
        {
            listaTipoContratoJSON.forEach(function (tipoContrato, index) {
                lcodTipoContratoGA.push(tipoContrato.codigo);
                ldescTipoContratoGA.push(traducirDesplegable(tipoContrato.descripcion));
            });
            variableTipoContratoColectivo.addItems(lcodTipoContratoGA, ldescTipoContratoGA);
        }
    }

    function comboTipoEdadSexo() {

        var listaTipoEdadSexoJSON = JSON.parse($("#listaTipoEdadSexo").val());
        var variableComboTipoEdadSexo = new Combo("TipoEdadSexo");
    //    variableComboSexo.change = hola;
        var lcodTipoEdadSexoGA = new Array();
        var ldescTipoEdadSexoGA = new Array();

        if (listaTipoEdadSexoJSON != null && listaTipoEdadSexoJSON.length > 0)
        {
            listaTipoEdadSexoJSON.forEach(function (tipoEdadSexo, index) {
                lcodTipoEdadSexoGA.push(tipoEdadSexo.codigo);
                ldescTipoEdadSexoGA.push(traducirDesplegable(tipoEdadSexo.descripcion));
            });
            variableComboTipoEdadSexo.addItems(lcodTipoEdadSexoGA, ldescTipoEdadSexoGA);
        }
    }

    function comboCnae() {

        var listaCnaeJSON = JSON.parse($("#listaCnae").val());
        var variableComboCnaeSexo = new Combo("Cnae");
    //    variableComboSexo.change = hola;
        var lcodCnaeGA = new Array();
        var ldescCnaeGA = new Array();

        if (listaCnaeJSON != null && listaCnaeJSON.length > 0)
        {
            listaCnaeJSON.forEach(function (cnae, index) {
                lcodCnaeGA.push(cnae.codigo);
                ldescCnaeGA.push(traducirDesplegable(cnae.descripcion));
            });
            variableComboCnaeSexo.addItems(lcodCnaeGA, ldescCnaeGA);
        }
    }

    function traducirDesplegable(texto) {
        if (idiomaUsuario == 4) {
...
function pulsarAlta() {
    ...
    var dataParameter = $.extend({}, parametrosLLamadaM35);
