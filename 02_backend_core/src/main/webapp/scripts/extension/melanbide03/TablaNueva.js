...
function GrupoColumna(tabla,nombre,padre){
    ...
    this.nombre = nombre;
    this.items = new Array();
    this.addColumna=function(ancho, align, nombre, tipoDato, oculto){
        ...
        for(var i=0; i<this.items.length; i++){
          if(!this.items[i].nombre){
            if(this.items[i][0]>0){
                ...
                if(texto=='') texto = '&nbsp;';
                str += '<th style="width:' + anchoCol + (this.items[i][4]?"; display:none;":"")+ '"';
...
function Tabla(paginar,buscar,anterior,siguiente,mosFilasPag,msgNoResultBusq,mosPagDePags,noRegDisp,filtrDeTotal,primero,ultimo,par,anchoTabla){
...
function TB_addColumna(ancho, align, nombre,tipoDato,oculto){
	this.estructuraColumnas.addColumna(ancho,align,nombre,tipoDato, oculto);
}
    ...
    for(var i=0; i < this.lineas.length; i++){
      ...
      for(var j=0; j < lin.length; j++){	  
        if(this.columnas[j][0]>0) {
            ...
            str += '<td style="align:' + this.columnas[j][2]+';width:'+anchoCol + ';'+(this.columnas[j][4]?"display:none;":"")+'" onclick="selectRow(' + i + ',' + 
...
function selectRow(rowID,tab){
    habilitarBotonLargo(cmdVerDoc1);
...
function enlacesPaginacion(litMosPagDePags,litAnterior,litSiguiente,numPaginaActual,numeroPaginas,nombreFuncionCarga) {
    var htmlString = " ";
