...
function ValidarFechaConFormatoIker(fecha, formato) {
    if (formato == null)
        formato = "dd/mm/yyyy";
    if (formato == "mm/yyyy")
        fecha = "01/" + fecha;
...
function comprobarCaracteresEspecialesIker(valor) {
    if (Trim(valor) != '') {
        valor = normalizarTextoIker(valor);
        var patron = /^[a-zA-Z0-9,.ºª_@#!()=¿?¡*\[\]-]*$/;
...
function normalizarTextoIker(text) {
