...
function tb_deleteElementLpeel(lista,index){
...
function GrupoColumnaLpeel(tabla,nombre,padre){
    this.nivel = 1;
    ...
    this.addGrupoColumna=function(nombre){
        var grupo =  new GrupoColumnaLpeel(this.tabla,nombre,this);
...
function TablaLpeel(paginar,buscar,anterior,siguiente,mosFilasPag,msgNoResultBusq,mosPagDePags,noRegDisp,filtrDeTotal,primero,ultimo,par,anchoTabla,iDisplayLength){
  ...
  this.selectedItems = new ListaLpeel();
  this.estructuraColumnas = new GrupoColumnaLpeel(this);
  ...
  this.widthTabla = TB_widthTablaLpeel;
  this.addGrupoColumna = TB_addGrupoColumnaLpeel;
  this.addColumna = TB_addColumnaLpeel;
  this.addLinea = TB_addLineaLpeel;
  this.getLinea = TB_getLineaLpeel;
  this.setLinea = TB_setLineaLpeel;
  this.removeLinea = TB_removeLineaLpeel;
  this.selectLinea = TB_selectLineaLpeel;
  this.restoreIndex = TB_restoreIndexLpeel;
  this.activaRow = TB_activaRowLpeel;
  this.desactivaRow = TB_desactivaRowLpeel;
  this.clearTabla = TB_clearTablaLpeel;
  this.readOnly = false;
  ...
  this.displayTabla = TB_displayTablaLpeel;
  this.displayTablaConContenido = TB_displayTablaConContenidoLpeel;
  this.displayDatos = function(datos){};
  this.setLineas = TB_setLineasLpeel;
}

function TB_widthTablaLpeel(){
...
function TB_addGrupoColumnaLpeel(nombre){
    return this.estructuraColumnas.addGrupoColumna(nombre);
}

function TB_addColumnaLpeel(ancho, align, nombre,tipoDato){
	this.estructuraColumnas.addColumna(ancho,align,nombre,tipoDato);
}

function TB_addLineaLpeel(datos){
...
function TB_getLineaLpeel(rowID){
  if (TB_getLineaLpeel.arguments.length == 0) rowID = this.selectedIndex;
  if (rowID < 0 || rowID >= this.lineas.length) return false;
  return this.lineas[rowID];
}

function TB_setLineaLpeel(datos,rowID){
  if (datos.length != this.columnas.length) return false;
  if (TB_setLineaLpeel.arguments.length == 1) rowID = this.selectedIndex;
  if (rowID < 0 || rowID >= this.lineas.length) return false;
...
function TB_removeLineaLpeel(rowID){
  if (TB_removeLineaLpeel.arguments.length == 0) rowID = this.selectedIndex;
...
function TB_clearTablaLpeel(){
  this.lineas = new Array();
  this.displayTabla();
  return true;
}

function TB_displayTablaLpeel(){
    this.restoreIndex();
    TB_NumLpeel += 1;
    ...
    for(var i=0; i < this.lineas.length; i++){
      ...
      for(var j=0; j < lin.length; j++){	  
        if(this.columnas[j][0]>0) {
            ...
            str += '<td style="text-align:' + this.columnas[j][2]+';width:'+anchoCol + ';" onclick="selectRowLpeel(' + i + ',' + 
                    this.id + ');" ondblclick="callFromTableToLpeel(' + i + ',\'' + this.id + '\');">'+
                    (lin[j]==''?'&nbsp;':lin[j])+'</td>';
        }
      }
      str += '</tr>';
...
function TB_displayTablaConContenidoLpeel(){
    this.restoreIndex();
    TB_NumLpeel += 1;
    var ancho=this.widthTabla();
    ...
    for(var i=0; i < this.lineas.length; i++){
      ...
      for(var j=0; j < lin.length; j++){	  
        if(this.columnas[j][0]>0) {
            var anchoCol = (this.columnas[j][0]).indexOf('%')<0?(this.columnas[j][0]/ancho)*100+'%':this.columnas[j][0];
            str += '<td style="text-align:' + this.columnas[j][2]+';width:'+anchoCol + ';" ondblclick="callFromTableToLpeel(' + i + ',\'' + this.id + '\');">'+
                    (lin[j]==''?'&nbsp;':lin[j])+'</td>';
        }
      }
      str += '</tr>';
...
function TB_selectLineaLpeel(rowID) {
...
function TB_activaRowLpeel(rowID){
    if(this.readOnly) return;
    if(rowID < 0 || rowID>=this.lineas.length) 
        return;
...
function TB_desactivaRowLpeel(rowID){
    if(this.readOnly) return;
    if(rowID < 0 || rowID>=this.lineas.length) 
        return;
...
function selectRowLpeel(rowID,tab){
    if(tab.tabla){
        tab.tabla.selectLinea(rowID);
        callClickLpeel(rowID,tab.tabla);
...
function TB_setLineasLpeel(datos){
    this.lineas = new Array();
...
function pushEnterTableLpeel(tableName,listName){
    if(tableName.multiple){
        if((tableName.focusedIndex>-1)&&(tableName.focusedIndex < listName.length)){
            ...
            desactiveFocusTableLpeel(tableName, tableName.focusedIndex);

            tableName.focusedIndex++;
            if(tableName.focusedIndex == listName.length) {
                tableName.focusedIndex=0;
            }
            rellenarDatosMultipleLpeel(tableName, tableName.focusedIndex); //No es general.
        }
    }else{
        callFromTableToLpeel(tableName.focusedIndex, tableName.id); //No es general.
    }
}

function upDownTableLpeel(tableObject,listName,tecla){
    ...
    if(tableObject.multiple){
        desactiveFocusTableLpeel(tableObject, tableObject.focusedIndex);
    }else{
    ...
    if(!tableObject.multiple) 
        selectRowLpeel(document.getElementById(tableObject.id).tBodies[0].rows[posicion].getAttribute('indice'),document.getElementById(tableObject.id));
}

function callClickLpeel(rowID,tableObject){
    window.focus();

    if(tableObject.multiple){
        desactiveFocusTableLpeel(tableObject, tableObject.focusedIndex);
        if(!event.ctrlKey) desactiveSelectedRowsLpeel(tableObject, rowID);
        if(event.shiftKey) activeSelectedRowsLpeel(tableObject, rowID);
        tableObject.focusedIndex = rowID;
        rellenarDatosMultipleLpeel(tableObject, rowID);// No es general
    }else{
        rellenarDatosLpeel(tableObject, rowID); //No es general.
    }
}

function desactiveFocusTableLpeel(tableName, rowID){
    if(rowID > -1){
...
function activeSelectedRowsLpeel(tableObject, rowID){
    ...
    tableObject.selectedItems = new ListaLpeel();
...
function desactiveSelectedRowsLpeel(tableObject, rowID){
    var items = tableObject.selectedItems.items;
    for(var i=0; i < items.length; i++){
        if(items[i]!=rowID){
            tableObject.desactivaRow(items[i]);
...
function callFromTableToLpeel(rowID,tableName){}	//Se ejecuta en el evento onDbClick.

function rellenarDatosLpeel(tableName,rowID){}	//Se ejecuta al cambiar la selección en una Tabla Simple.
function rellenarDatosMultipleLpeel(tableName,rowID){}	//Se ejecuta al cambiar la selección en una Tabla Múltiple.
// FIN - Funciones a redefinir en la jsp.

function enlacesPaginacionLpeel(litMosPagDePags,litAnterior,litSiguiente,numPaginaActual,numeroPaginas,nombreFuncionCarga, litMosResultados, numResultados) {
    var htmlString = " ";
