...
function tb_deleteElementOri(lista,index){
...
function GrupoColumnaOri(tabla,nombre,padre){
    this.nivel = 1;
    ...
    this.addGrupoColumna=function(nombre){
        var grupo =  new GrupoColumnaOri(this.tabla,nombre,this);
...
function TablaOri(paginar,buscar,anterior,siguiente,mosFilasPag,msgNoResultBusq,mosPagDePags,noRegDisp,filtrDeTotal,primero,ultimo,par,anchoTabla,iDisplayLength){
  ...
  this.selectedItems = new ListaOri();
  this.estructuraColumnas = new GrupoColumnaOri(this);
  ...
  this.widthTabla = TB_widthTablaOri;
  this.addGrupoColumna = TB_addGrupoColumnaOri;
  this.addColumna = TB_addColumnaOri;
  this.addLinea = TB_addLineaOri;
  this.getLinea = TB_getLineaOri;
  this.setLinea = TB_setLineaOri;
  this.removeLinea = TB_removeLineaOri;
  this.selectLinea = TB_selectLineaOri;
  this.restoreIndex = TB_restoreIndexOri;
  this.activaRow = TB_activaRowOri;
  this.desactivaRow = TB_desactivaRowOri;
  this.clearTabla = TB_clearTablaOri;
  this.readOnly = false;
  ...
  this.displayTabla = TB_displayTablaOri;
  this.displayTablaConContenido = TB_displayTablaConContenidoOri;
  this.displayDatos = function(datos){};
  this.setLineas = TB_setLineasOri;
}

function TB_widthTablaOri(){
...
function TB_addGrupoColumnaOri(nombre){
    return this.estructuraColumnas.addGrupoColumna(nombre);
}

function TB_addColumnaOri(ancho, align, nombre,tipoDato){
	this.estructuraColumnas.addColumna(ancho,align,nombre,tipoDato);
}

function TB_addLineaOri(datos){
...
function TB_getLineaOri(rowID){
  if (TB_getLineaOri.arguments.length == 0) rowID = this.selectedIndex;
  if (rowID < 0 || rowID >= this.lineas.length) return false;
  return this.lineas[rowID];
}

function TB_setLineaOri(datos,rowID){
  if (datos.length != this.columnas.length) return false;
  if (TB_setLineaOri.arguments.length == 1) rowID = this.selectedIndex;
  if (rowID < 0 || rowID >= this.lineas.length) return false;
...
function TB_removeLineaOri(rowID){
  if (TB_removeLineaOri.arguments.length == 0) rowID = this.selectedIndex;
...
function TB_clearTablaOri(){
  this.lineas = new Array();
  this.displayTabla();
  return true;
}

function TB_displayTablaOri(){
    this.restoreIndex();
    TB_NumOri += 1;
    ...
    for(var i=0; i < this.lineas.length; i++){
      ...
      for(var j=0; j < lin.length; j++){	  
        if(this.columnas[j][0]>0) {
            ...
            str += '<td style="text-align:' + this.columnas[j][2]+';width:'+anchoCol + ';" onclick="selectRowOri(' + i + ',' + 
                    this.id + ');" ondblclick="callFromTableToOri(' + i + ',\'' + this.id + '\');">'+
                    (lin[j]==''?'&nbsp;':lin[j])+'</td>';
        }
      }
      str += '</tr>';
...
function TB_displayTablaConContenidoOri(){
    this.restoreIndex();
    TB_NumOri += 1;
    var ancho=this.widthTabla();
    ...
    for(var i=0; i < this.lineas.length; i++){
      ...
      for(var j=0; j < lin.length; j++){	  
        if(this.columnas[j][0]>0) {
            var anchoCol = (this.columnas[j][0]).indexOf('%')<0?(this.columnas[j][0]/ancho)*100+'%':this.columnas[j][0];
            str += '<td style="text-align:' + this.columnas[j][2]+';width:'+anchoCol + ';" ondblclick="callFromTableToOri(' + i + ',\'' + this.id + '\');">'+
                    (lin[j]==''?'&nbsp;':lin[j])+'</td>';
        }
      }
      str += '</tr>';
...
function TB_selectLineaOri(rowID) {
...
function TB_activaRowOri(rowID){
    if(this.readOnly) return;
    if(rowID < 0 || rowID>=this.lineas.length) 
        return;
...
function TB_desactivaRowOri(rowID){
    if(this.readOnly) return;
    if(rowID < 0 || rowID>=this.lineas.length) 
        return;
...
function selectRowOri(rowID,tab){
    if(tab.tabla){
        tab.tabla.selectLinea(rowID);
        callClickOri(rowID,tab.tabla);
...
function TB_setLineasOri(datos){
    this.lineas = new Array();
...
function pushEnterTableOri(tableName,listName){
    if(tableName.multiple){
        if((tableName.focusedIndex>-1)&&(tableName.focusedIndex < listName.length)){
            ...
            desactiveFocusTableOri(tableName, tableName.focusedIndex);

            tableName.focusedIndex++;
            if(tableName.focusedIndex == listName.length) {
                tableName.focusedIndex=0;
            }
            rellenarDatosMultipleOri(tableName, tableName.focusedIndex); //No es general.
        }
    }else{
        callFromTableToOri(tableName.focusedIndex, tableName.id); //No es general.
    }
}

function upDownTableOri(tableObject,listName,tecla){
    ...
    if(tableObject.multiple){
        desactiveFocusTableOri(tableObject, tableObject.focusedIndex);
    }else{
    ...
    if(!tableObject.multiple) 
        selectRowOri(document.getElementById(tableObject.id).tBodies[0].rows[posicion].getAttribute('indice'),document.getElementById(tableObject.id));
}

function callClickOri(rowID,tableObject){
    window.focus();

    if(tableObject.multiple){
        desactiveFocusTableOri(tableObject, tableObject.focusedIndex);
        if(!event.ctrlKey) desactiveSelectedRowsOri(tableObject, rowID);
        if(event.shiftKey) activeSelectedRowsOri(tableObject, rowID);
        tableObject.focusedIndex = rowID;
        rellenarDatosMultipleOri(tableObject, rowID);// No es general
    }else{
        rellenarDatosOri(tableObject, rowID); //No es general.
    }
}

function desactiveFocusTableOri(tableName, rowID){
    if(rowID > -1){
...
function activeSelectedRowsOri(tableObject, rowID){
    ...
    tableObject.selectedItems = new ListaOri();
...
function desactiveSelectedRowsOri(tableObject, rowID){
    var items = tableObject.selectedItems.items;
    for(var i=0; i < items.length; i++){
        if(items[i]!=rowID){
            tableObject.desactivaRow(items[i]);
...
function callFromTableToOri(rowID,tableName){}	//Se ejecuta en el evento onDbClick.

function rellenarDatosOri(tableName,rowID){}	//Se ejecuta al cambiar la selección en una Tabla Simple.
function rellenarDatosMultipleOri(tableName,rowID){}	//Se ejecuta al cambiar la selección en una Tabla Múltiple.
// FIN - Funciones a redefinir en la jsp.

function enlacesPaginacionOri(litMosPagDePags,litAnterior,litSiguiente,numPaginaActual,numeroPaginas,nombreFuncionCarga, litMosResultados, numResultados) {
    var htmlString = " ";
