...
function validarCif(campo){ 
    ...
    var letraSociedadNumerica = "KLMNPQRSW";
    var valor = campo.value;
    var let = valor.charAt(0); 
    var ultimaLetra= valor.substr(valor.length-1);
    ...
    if (letraSociedadNumerica.indexOf(let) >= 0) {                   
                    // Obtiene el c�digo ASCII asociado, al sumar 64 al c�digo de
                    // control.
                    if (control == 0)
                        control = 10;
                    control = control + 64;
                    ...
                    // El �ltimo d�gito tiene que coincidir con el d�gito de control
                    // obtenido
                    //resultado = (ultimaLetra.equals(new String(ascii)));
                    var ascii = String.fromCharCode(control);
                    if (ultimaLetra!=ascii) return false;
    }else    
