...
function limpiarFiltros(){
    ...
    document.getElementById("codAsunto").style.pointerEvents = 'auto';
    document.getElementById("anchorAsunto").style.pointerEvents = 'auto';
    filaIDseleccionada = null;
}
...
function cargarTablaGestionAvisos(){
    ...
    var urlBaseLlamada = APP_CONTEXT_PATH + "/gestionAvisos/GestionAvisosAction.do";
     ...
     success: function (response) {   
        ...
        tablaGestionAvisos.addColumna('150','left',$('#gestionAvisos_descriptor_evento').val());
        tablaGestionAvisos.addColumna('90','left',$('#gestionAvisos_descriptor_mailUOR').val());
        tablaGestionAvisos.addColumna('100','left',$('#gestionAvisos_descriptor_asunto').val());
        tablaGestionAvisos.addColumna('300','left',$('#gestionAvisos_descriptor_contenido').val());
        ...
        datosGA.forEach(function(datoGA) {       
            datos[contador] = [datoGA.codProcedimiento,datoGA.evento,datoGA.email,datoGA.asunto,datoGA.contenido];         
            contador++;
        });
...
function pulsarEtiquetasGA(){
    pleaseWait('on'); 
    lanzarPopUpModal(APP_CONTEXT_PATH+'/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE06&operacion=cargarEtiquetas&tipo=0', 460, 850, 'no', 'no', function (result) {
    pleaseWait('off');
...
function cargarMetadatosGA(datosGA){  
    ...
    document.getElementById("desAsunto").value = datosGA.asunto;
    document.getElementById("desContenido").value = datosGA.contenido;
