...
function verificarPantallaExterna(i){
    ...
    if(ajax!=null){
        ...
        // Se solicita ahora los par�metros de configuraci�n de la ventana
...
// Visualizar un documento asociado al expediente de la pesta�a 'Otros Documentos'
...
function actualizaEliminarDoc(){ 
  ...
  function interesado_Obligatorio(){ 
   ...
   function activarFormulario() {
     ...
     function valoresInicialesTercero() {
        ...
        for (i=0; i < terceros.length; i++) {
            listaCodTercero += terceros[i][0]+separador;
            listaVersionTercero += terceros[i][1]+separador;
            listaRol += terceros[i][7]+ separador;
            listaCodDomicilios += terceros[i][4]+separador;
        }
      ...
      function calcularAltoAgrupacion(codAgrupacion){
       ...
       if (listaCampos.length>0 && maxY > 0){
        ...
        try{
            ...
            // Puede dar error establecer el tama�o de la agrupaci�n por defecto, se captura el error pero no se hace nada
        }
   ...
   /**
     * Funci�n utilizada cuando se ha indicado que un tipo de documento es un CIF, pero se va a valir
     * si se trata de un NIF. No vac�a el campo de formulario con el documento como hace la funci�n
...
function procesarRespuestaListadoDocumentosExternos(ajaxResult){    
    try{     
        ...
        tabOtrosDocumentos.lineas=listaOtrosDocumentos;
        tabOtrosDocumentos.displayTabla();
        // Se pone a true esta variable, para que al seleccionar la pesta�a "Otros documentos",
...
function procesarRespuestaOtrosDatosExpediente(ajaxResult){    
    ...
    if(resultado!=null){
        ...
        if(resultado.status!=0){
            ...
            // Se actualiza la secci�n de Enlaces, si los hubiese
            if(resultado.enlaces!=null && resultado.enlaces!=undefined){
                ...
                if(listaEnlaces.length>0){
                    ...
                    // Se genera el contenido en la funci�n enlaces() y se muestra en la capa "capaEnlaces"
                    domlay('capaEnlaces',1,0,0,enlaces());
                }
            }
            
            
            // Se actualiza la secci�n de asientos de registro, si los hubiese
            if(resultado.asientos!=null && resultado.asientos!=undefined){    
                
                // Si se ha producido alg�n al recuperar asientos de registro externos, se muestran los errores al usuario
                if(resultado.errores!=null && resultado.errores!=undefined){
                    mostrarMensajesErrorAsientosRegistroExternos(resultado.errores);
                }
                
                // Se obtienen los asientos de registro recuperados, que est�n asociados al expediente
            ...
            if(resultado.documentos!=null && resultado.documentos!=undefined){                               
                ...
                if(estadoExpediente==1 || estadoExpediente==9) { 
                    // Si el expediente est� anulado o finalizado, se habilita el bot�n [Ver] 
                    ...
                    // Se deshabilitan los checkbox de cada documento de inicio, porque est� habilitado y el usuario
            ...
            // Se pone a true esta variable, para indicar que la pesta�a "Otros Datos" 
...
function getHTMLCampoTexto(campo,mostrarDescTramite){    
    try{
        ...
        // Si se est�n consultando, entonces se deshabilita el campo de texto
        ...
        if(campo.descripcionTramite==undefined || campo.descripcionTramite==''){
           var idTabla = "T" + campo.codTramite + campo.ocurrencia + campo.codCampo;
            ...
            if(mostrarDescTramite)
                inputForm +="<div class='deTramite'> Tr�mite " + campo.descripcionTramite + "</div>";
            inputForm +="<table id='" + idTabla + "' name='" + idTabla + "' width='100%' border='0px' cellspacing='2px' cellpadding='1px'>";       
       ...
       return inputForm;
        ...
        alert("Error al obtener el HTML de un campo suplementario de texto con c�digo " + codCampo + ": " + Err.description);
    }
}
...
/**
 * Devuelve el c�digo HTML para un campo suplementario de tipo num�rico
...
function getHTMLCampoNumerico(campo,mostrarDescTramite){
    try{                
        ...
        if(campo.soloLectura=="true") modoLectura="readonly";
        if(campo.bloqueado=="SI") modoLectura="readonly";
        // Si se est�n consultando, entonces se deshabilita el campo num�rico
        if(gConsultando) disabled='disabled';            
...
/**
 * Devuelve el c�digo HTML para un campo suplementario de tipo fichero
 * @param: Objeto que contiene la estructura del campo a crear, as� como su valor
 */
function getHTMLCampoFichero(campo,mostrarDescTramite){
    try{                
        ...
        if(campo.soloLectura=="true") modoLectura="readonly";
        if(campo.bloqueado=="SI") modoLectura="readonly";
        // Si se est�n consultando, entonces se deshabilita el campo fichero
        ...
        if(campo.codTramite==undefined || campo.codTramite==''){
            // Si el campo no es de tr�mite, se a�ade el bot�n eliminar. Pero hay que comprobar si se muestra o no deshabilitado
        ...
        if(campo.descripcionTramite==undefined || campo.descripcionTramite==''){
            ...
            salida +="<div id='campo_" + campo.codCampo + "' style='width:" + campo.tamanoVista + "%'>";
            if(mostrarDescTramite)
                salida +="<div class='deTramite'> Tr�mite " + campo.descripcionTramite + "</div>";
            salida +="<table id='prueba' name='prueba' width='100%' border='0px' cellspacing='2px' cellpadding='1px'>";        
        ...
        alert("Error al obtener el HTML de un campo suplementario de fichero con c�digo " + codCampo + ": " + Err.description);
    }
}
...
function getHTMLCampoTextoLargo(campo,mostrarDescTramite){    
    try{ 
        ...
        if(campo.soloLectura=="true") modoLectura="readonly";
        if(campo.bloqueado=="SI") modoLectura="readonly";      
        // Si se est�n consultando, entonces se deshabilita el campo de texto largo
        if(gConsultando) disabled='disabled';            
...
/**
 * Devuelve el c�digo HTML para un campo suplementario de tipo fecha
 * @param: Objeto que contiene la estructura del campo a crear, as� como su valor
 */
function getHTMLCampoFecha(campo,mostrarDescTramite){        
    try{ 
       ...
       if(campo.plazoFecha!=undefined && campo.checkPlazoFecha!=undefined && campo.plazoFecha!='' && campo.checkPlazoFecha!=''){                      
           if(valorPlazoActivo=="0"){ // Est� desactivada la alarma, se da la posibilidad de activarla
                                      // Si est� desactivada la alarma, habr� que activarla               
           ...
           if(valorPlazoActivo=="1"){ // Est� activada la alarma, se da la posibilidad de desactivarla               
...
/**
 * Devuelve el c�digo HTML para un campo suplementario de tipo fecha calculado
 * @param: Objeto que contiene la estructura del campo a crear, as� como su valor
 */
function getHTMLCampoFechaCalculado(campo,mostrarDescTramite){        
    try{ 
        ...
        alert("Error al obtener el HTML de un campo suplementario de fecha calculado con c�digo " + codCampo + ": " + Err.description);
    }
} 
...
function getHTMLCampoDesplegable(campo,mostrarDescTramite){
    try{ 
       ...
       var inputDescCombo   = "&nbsp;<input type='text' id='desc" + nombreCampo + "' name='desc" + nombreCampo + "' " + disabled + " class='" + estiloCSS + "' style='width:70%' readonly='true'>";
        ...
        alert("Error al obtener el HTML de un campo suplementario desplegable con c�digo " + codCampo + ": " + Err.description);
    }
}


/**
 * Devuelve el c�digo HTML para un campo suplementario de tipo num�rico calculado
 * @param: Objeto que contiene la estructura del campo a crear, as� como su valor
 */
function getHTMLCampoNumericoCalculado(campo,mostrarDescTramite){    
   try{          
        ...
        modoLectura="readonly";        
        // Si se est�n consultando, entonces se deshabilita el campo num�rico calculado
        if(gConsultando) disabled='disabled';            
       ...
       return inputForm;
        ...
        alert("Error al obtener el HTML de un campo suplementario num�rico calculado con c�digo " + codCampo + ": " + Err.description);
    }
}
...
/**
 * Devuelve el c�digo HTML para un campo suplementario desplegable exerno
 * @param: Objeto que contiene la estructura del campo a crear, as� como su valor
 */
function getHTMLCampoDesplegableExterno(campo,mostrarDescTramite){    
 ...
 /**
  * Crea la tabla que contendr� las agrupaciones con sus campos suplementarios correspondientes
  */ 
 ...
 function procesarRespuestaListadoCamposSuplementarios(ajaxResult){              
    try{     
        ...
        // Se vac�a el contenido del DIV en el que se mostrar�n los datos suplementarios
        ...
        // suplementarios en formato JSON. De modo al grabar, la estructura podr� ser recuperada desde el Action y 
        // parseada a la colecci�n de objetos EstructuraCampo, para proceder a grabar los valores de los campos
        ...
        // Se recorre cada agrupaci�n y se recupera la estructura de campos de cada de ellas.
        // Por cada campo, se recupera adem�s su valor
            ...
            for(var j=0;estructura!=undefined && j<estructura.length;j++){                            
                ...
                if(codAgrupacion=='DVT'){
                    ...
                    if(codTramiteActual!=codTramiteTramiteAnterior){
                        // Si la agrupaci�n es de tr�mite y el campo pertenece a un tr�mite distinto del anterior, se a�ade una 
                        // fila para indicar el nombre del tr�mite                        
        ...
        if(contadorCampos>0){
            ...
            // Si hay cambios calculados, se llama a la funci�n recargarCamposCalculados para mostrar su valor
        ...
        // Se pone a true la variable, para que al hacer click sobre la pesta�a "Datos Suplementarios", no se 
        // vuelva a recargar la pesta�a
 ...
 function mostrarDIVNoHayCamposSuplementarios(){     
  ...
  /**
     * Funci�n que es llamada para cambiar el color de fondo de la pesta�a de los datos suplementarios.
     * Para ello tiene en cuenta el que el valor de la variable global contadorValoresCamposSuplementariosExpediente, sea
     * mayor que cero, que es el n�mero de campos suplementarios a mostrar en la pesta�a "Datos Suplementarios"
     **/
 ...
 /**
  * Eliminar del par�metro cadena las comillas simples que hay al principio y al final del string
  * @param cadena
 ...
 function generarCombos(listaCombos){
    ...
    try{
        ...
        for(i=0;listaCombos!=undefined && i<listaCombos.length;i++){
            ...
            // Se obtiene la lista con los c�digos del desplegable
            ...
            if(gConsultando){                
                // Si se est� consulntando
               combo.deactivate();
            }         
            
            // Se a�ade el combo en el array de combos
         ...
         alert(" Error en funci�n generarCombos(): " + Err.description);
     }
 }
...
function getNombreCompletoInteresado(nombre,apellido1,apellido2,masInteresados){
    ...
    try{
        ...
        alert("Error al generar el nombre completo del interesado de una anotaci�n de registro: " + nombreCompleto);
    }        
...
// Muestra un mensaje de error. Se le pasa el c�digo del campo de formulario y el tama�o m�ximo para su contenido
...
/**
 * FUNCIONES USADAS POR LOS CAMPOS SUPLEMENTARIOS NUM�RICOS CALCULADOS
 */
...
function marcarExpedienteNotifTelematica(){
    //comprobarmos  si se marca el check de notificaci�n telem�tica en la pesta�a de Notificaciones
