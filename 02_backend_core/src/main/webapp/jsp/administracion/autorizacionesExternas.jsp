        ...
        j=0;
         <%for (int i = 0; i < listaUsuarios.size(); i++) {
            ...
            for (i=0; i < listaEntidades.length; i++) {
                var caja = "boxE" + i;
                if(eval("document.forms[0]." + caja + ".checked") == true) {
                    lOrganizaciones +=listaEntidadesOriginal[i][2]+'<%=separador%>';
                    lEntidades +=listaEntidadesOriginal[i][4]+'<%=separador%>';
                    lBD += listaEntidadesOriginal[i][6]+'<%=separador%>';
                }
            }
            for (i=0; i < listaAplicaciones.length; i++) {
                var caja = "boxA" + i;
                lUsuarios +=listaAplicacionesOriginal[i][2]+'<%=separador%>';
                if(eval("document.forms[0]." + caja + ".checked") == true) {
                    lAplicaciones +=listaAplicacionesOriginal[i][1]+'<%=separador%>';
                }
            }
            for (i=0; i < listaEnt.length; i++) {
                var caja = "boxEU" + i;
                lUsuar += listaUsuariosOriginal[lineaUsuario][0]+'<%=separador%>';
                lAplic += listaAplicacionesOriginal[lineaAplicacion][1]+'<%=separador%>';
                if(eval("document.forms[0]." + caja + ".checked") == true) {
                    lOrg +=listaEntOriginal[i][2]+'<%=separador%>';
                    lEnt +=listaEntOriginal[i][4]+'<%=separador%>';
                }
            }
