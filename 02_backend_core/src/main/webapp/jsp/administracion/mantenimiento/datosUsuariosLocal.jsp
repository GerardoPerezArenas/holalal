...
<html:html>
  ...
  <% UsuariosGruposForm uForm = new UsuariosGruposForm();
            ...
            Vector lUsuarGrupos = (Vector) uVO.getListaUsuariosTodos();
            if (lUsuarGrupos != null) {
                for (int k = 0; k < lUsuarGrupos.size(); k++) {
                    GeneralValueObject g = new GeneralValueObject();
                    g = (GeneralValueObject) lUsuarGrupos.elementAt(k);
  %>
  ...
  %>      
      ...
      var checkEntreg1 = "<input type='checkbox' class='check' name='box" + cont + "' value='SI' ";
      if(aut == "si") {
          checkEntreg1 += "checked ";
      }
      checkEntreg1 += " disabled=true>";
   ...
   */
   function getXMLHttpRequest(){
    ...
    function verificarAsignacionPermisosCorrecta(){
        ...
        var parametros = "opcion=verificarAsignacionPermisosCorrectaAdmLocal&listaUnidades=" + escape(listaUnidades);        
            ...
            if (ajax.readyState==4 && ajax.status==200){                
                ...
                var codigoOperacion = null;
                    ...
                    for(i=0;i<errores.length;i++){
                        ...
                        if(status!="" && status=="4"){
                            // SE HA ASIGNADO PERMISO SOBRE UNA OFICINA DE REGISTRO QUE NO DEPENDE DE NINGUNA UNIDAD DE TIPO REGISTRO
                            mensaje = mensaje + "<p style=\"font-size:14px;\">"
                            mensaje = mensaje + "No se puede asignar permiso sobre la unidad " +  "(" + codVisibleUorPadre + ") " + descUorPadre + " " + " ya que no depende de ninguna unidad de tipo registro";
                            mensaje = mensaje + "</p>";
                            mensaje = mensaje + "<p style=\"font-size:11px;;margin-left:9px;\">";                            
            ...
            if( validarObligatorios('<%=descriptor.getDescripcion("msjObligTodos")%>')) {
                if (validarNifObligatorio()&&validarEmails()) {
                    ...
                    if(yaExiste == 0 ) {                                    
                        var exito = verificarAsignacionPermisosCorrecta();
                        if(exito){
                            document.forms[0].opcion.value="modificarUsuarioLocal";
                            document.forms[0].target="oculto";
                            document.forms[0].action="<%=request.getContextPath()%>/administracion/UsuariosGrupos.do";
                            document.forms[0].submit();
                        }                
    ...
    function crearListaDirectivasChecked() {
        ...
        for(var i=0; i<listaCodDirectivas.length; i++) {
            if (document.getElementById('checkDirectiva' + listaCodDirectivas[i]).checked) {
                listaDirectivasTxt += listaCodDirectivas[i] + '<%=separador%>';
            }
        }
...
function verGrupos(rowID) {
	...
	'width=850,height=450,status='+ '<%=statusBar%>',function(datos){
                        if(datos!=undefined){
...
function pulsarEliminarGrupo() {
    ...
    function pulsarAltaGrupo() {
        ...
        datosAEnviar[6] = "";
        datosAEnviar[7] = "";
        var source = "<%=request.getContextPath()%>/administracion/UsuariosGrupos.do?opcion=verGruposAdmLocal&primero="+crearListaOrganizacionesUsuarios('si');
        abrirXanelaAuxiliar("<%=request.getContextPath()%>/jsp/administracion/mainVentana.jsp?source="+source,datosAEnviar,
	'width=785,height=450,status='+ '<%=statusBar%>',function(datos){
                        if(datos!=undefined){
                            if(navigator.appName.indexOf("Internet Explorer")!=-1){
                            if(datos[4].length <= 3) {
                                altaGrupo(datos);
    ...
    function crearListasGrupos() {
        ...
        for (i=0; i < listaGrupos.length; i++) {
            lOrganizaciones +=listaGruposOriginal[i][0]+'<%=separador%>';
            lEntidades +=listaGruposOriginal[i][1]+'<%=separador%>';
            lAplicaciones +=listaGruposOriginal[i][2]+'<%=separador%>';
            lGrupos +=listaGruposOriginal[i][3]+'<%=separador%>';
        }
    ...
    function altaUnidOrganica(datos) { 
        if(!existeUOR(datos)) {
            if (datos[10] && datos[4] != -1 && datos[11]){ // Añadimos la uor seleccionada y sus unidades dependientes (datos[11] indica si la uor seleccionada tiene o no descendencia)
    ...
    function eliminarUnidadesOrganicasDescendientes(indice){      
        ...
        var tieneDescendencia = uorTieneDescendencia(codUorSeleccionada);
        eliminarUnidadOrganicaSeleccionada(tabUnidOrganicas.selectedIndex);
    ...
    function pulsarAltaUnidOrganica() { 
        ...
        var source = "<%=request.getContextPath()%>/administracion/UsuariosGrupos.do?opcion=verUnidOrganicasAdmLocal&primero="+crearListaOrganizacionesUsuarios('si');
        abrirXanelaAuxiliar("<%=request.getContextPath()%>/jsp/administracion/mainVentana.jsp?source="+source,datosAEnviar,
	'width=785,height=450,status='+ '<%=statusBar%>',function(datos){
                        if(datos!=undefined){
                            if (datos[4] == -1) {
        ...
        function crearListasUnidOrganicas() {
            ...
            for (i=0; i < listaUnidOrganicas.length; i++) {
                lOrganizaciones +=listaUnidOrganicasOriginal[i][0]+'<%=separador%>';
                lEntidades +=listaUnidOrganicasOriginal[i][1]+'<%=separador%>';
                lUnidOrganicas +=listaUnidOrganicasOriginal[i][2]+'<%=separador%>';
                if (listaUnidOrganicasOriginal[i][3]=="") lCargos +=null+'<%=separador%>';
                else lCargos +=listaUnidOrganicasOriginal[i][3]+'<%=separador%>';
            }
            var listas = new Array();
...
<html:form action="/administracion/UsuariosGrupos.do" target="_self"  enctype="multipart/form-data">
    ...
    <div class="contenidoPantalla">
        <div class="tab-pane" id="tab-pane-1">
                ...
                <TABLE id ="tablaDatosGral" class="contenidoPestanha" cellpadding="4" cellspacing="5">
                    ...
                    <tr>
                        ...
                        <td  class="columnP">
                            ...
                            <html:text styleId="obligatorio" styleClass="inputTextoObligatorioSinMayusculas" property="descIdioma" size="62px" readonly="true"/>
            ...
            <div class="tab-page" id="tabPage3">
                ...
                <TABLE id ="tablaDatosGral" class="contenidoPestanha" style="margin-bottom: 5px" align="center">
                    <tr>
                        <td>
                            <table align="center">
                    <tr>
    ...
    <div class="capaFooter">
        <div class="botoneraPrincipal"> 
            <input type="button" class="botonGeneral" value="<%=descriptor.getDescripcion("msjRecuperarUsuario")%>" name="cmdRecuperar"  id="cmdRecuperar" onclick="pulsarRecuperar();" >
                    ...
                    tabGrupos.addColumna('225','left','<%= descriptor.getDescripcion("etiq_grup")%>');
                        tabGrupos.addColumna('225','left','<%= descriptor.getDescripcion("gEtiq_Organizacion")%>');
                            tabGrupos.addColumna('225','left','<%= descriptor.getDescripcion("gEtiq_Entidad")%>');
                                tabGrupos.addColumna('200','left','<%= descriptor.getDescripcion("gEtiq_Aplicacion")%>');
                                    ...
                                    tabUnidOrganicas.addColumna('225','left','<%= descriptor.getDescripcion("gEtiq_unidOrg")%>');
                                        tabUnidOrganicas.addColumna('225','left','<%= descriptor.getDescripcion("gEtiq_Organizacion")%>');
                                            tabUnidOrganicas.addColumna('225','left','<%= descriptor.getDescripcion("gEtiq_Entidad")%>');
                                                tabUnidOrganicas.addColumna('200','left','<%= descriptor.getDescripcion("gEtiq_Cargo")%>');
                                                ...
                                                if (teclaAuxiliar == 9){
                                                    ...
                                                    if (comboIdioma.base.style.visibility == "visible") comboIdioma.ocultar();
                                                }
