...
<%
  ...
  boolean textoLargo=false;	
  for(int i=0; i<valores.size(); i++){	
      	
      if(valores.elementAt(i).length()>50){	
          textoLargo=true;	
          break;	
      }	
  }	
...
<input type="text" id="desc<%=name%>"  name="desc<%=name%>" <%=disabled%> class="<%= texto %>"  style="width:<%=tamanoVista%>%" readonly="true"  >
...
<input type="hidden" id="<%= id %>" name="cod<%=name%>"
       class="<%= texto %>" disabled style="width: 10%" maxlength="8" >
<input type="hidden" id="<%= id %>" name="desc<%=name%>" disabled class="<%= texto %>"  style="width:<%=tamanoVista%>%" readonly="true"  >


 <input type="text" id="texto1" name="texto1"
       class="<%= texto %>" disabled style="width: 10%" maxlength="8" value="<%=valor%>" readonly="true" >
 <input type="text" id="texto2" name="texto2" disabled class="<%= texto %>"  style="width:<%=tamanoVista%>%" readonly="true" value="<%=valorC%>" >
 <A href="" id="anchor<%=name%>" name="anchor<%=name%>" style="text-decoration:none;" >
    <span class="fa fa-chevron-circle-down" aria-hidden="true"  id="boton<%=name%>" name="boton<%=name%>" style="cursor:hand;"></span>
</A>
<%}%>
