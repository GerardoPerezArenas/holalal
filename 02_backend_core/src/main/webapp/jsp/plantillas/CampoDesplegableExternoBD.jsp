...
<jsp:useBean id="CAMPO_EXTERNO" scope="request" class="es.altia.agora.business.sge.DesplegableExternoValueObject"/>
...
<%
  ...
  //System.out.println("numero de codigos:" + codigos.size());
  for(int i=0;i<codigos.size();i++)
  {
      ...
      if(v.equals(vaux)){
          valorC=((String)valores.elementAt(i)).replace("'","");
...
<input type="text" id="<%= id %>" name="cod<%=name%>" onkeyup=" return xAMayusculas(this);"
       class="<%= texto %>" <%=disabled%> size="10" maxlength="8" >
<input type="text" id="<%= id %>" name="desc<%=name%>" <%=disabled%> class="<%= texto %>"  style="width: 610px" readonly="true"  >
...
<input type="hidden" id="<%= id %>" name="cod<%=name%>"
       class="<%= texto %>" disabled size="10" maxlength="8" >
<input type="hidden" id="<%= id %>" name="desc<%=name%>" disabled class="<%= texto %>"  style="width: 610px" readonly="true"  >
