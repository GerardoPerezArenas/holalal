...
<% TramitacionExpedientesForm tEF = new TramitacionExpedientesForm();
  ...
  %>
	...
	datosOriginal[cont] = ["<%=(String)g.getAtributo("codTercero")%>",
						   ...
						   "<%=(String)g.getAtributo("ejercicio")%>","<%=(String)g.getAtributo("numeroRelacion")%>"];
	cont++;
<% } %>
...
function crearListaCodigos() {
  ...
  for (i=0; i < datos.length; i++) {
    ...
    if(eval("document.forms[0]." + caja + ".checked") == true) {
      listaCodInteresados += datosOriginal[i][0]+'<%=separador%>';
    }
  }
...
function crearListaVersiones() {
  ...
  for (i=0; i < datos.length; i++) {
    var caja = "box" + i;
    if(eval("document.forms[0]." + caja + ".checked") == true) {
      listaVersInteresados += datosOriginal[i][1]+'<%=separador%>';
    }
  }
...
function actualizarTabla() {
  if(datosOriginal.length == 0) {
	...
	document.forms[0].numeroRelacion.value = numRel;
  } else {
    ...
    document.forms[0].numeroRelacion.value = datosOriginal[0][10];
  }
  document.forms[0].opcion.value="actualizarTablaInteresados";
  document.forms[0].target="oculto";
  document.forms[0].action="<c:url value='/sge/TramitacionRelacionExpedientes.do'/>";//+numeroRelacion;// es el numero de relacion
...
<input type="Hidden" name="numeroRelacion" >
...
function checkKeysLocal(evento,tecla) {
    ...
    if (teclaAuxiliar==9)  comboRol.ocultar();
