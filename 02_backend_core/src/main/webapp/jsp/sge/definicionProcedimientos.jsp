...
<% if("deTramite".equals(deTramite)){ %>    
	...
	var automatico = '<bean:write name="DefinicionProcedimientosForm" property="automatico"/>';
...
function crearListas() {
  ...
  for (i=0; i < listaUnidadesInicio.length; i++) {
    listaCodUnidadesInicio += listaUnidadesInicioOriginal[i][0]+'<%=separador%>';
    listaCodVisibleUnidadesInicio += listaUnidadesInicio[i][0]+'<%=separador%>';
    listaDescUnidadesInicio += listaUnidadesInicioOriginal[i][1]+'<%=separador%>';
  }
...
function recuperaDatos(datos,lista,lista1,lista2,lista4,lista5,lista6,lista7,lista8,lista9,lista10,lista11,lista12,
   ...
   document.forms[0].plazo.value = datos[12];
    ...
    if(datos[30]!=undefined && datos[30]=="1"){
        // Se comprueba si el procedimiento esta o no restringido
        document.forms[0].automatico.checked = true;
        document.forms[0].automatico.value='1';
    }else{
        document.forms[0].automatico.checked = false;
        document.forms[0].automatico.value='0';
    }
...
function desactivarFormulario() {
  ...
  document.forms[0].automatico.disabled = true;
}
...
function crearListasDoc() {
  ...
  for (i=0; i < listaDoc.length; i++) {
	listaNombresDoc +=listaDoc[i][0]+'<%=separador%>';
    listaCondicionDoc +=listaDoc[i][1]+'<%=separador%>';
	listaCodigosDoc += listaDocOriginal[i][0]+'<%=separador%>';
  }
...
function crearListasCampos() {
  ...
  for (i=0; i < listaCampos.length; i++) {
    ...
    listaActivos += listaCamposOriginal[i][11]+'<%=separador%>';
    listaOcultos += listaCamposOriginal[i][12]+'<%=separador%>';
    listaBloqueados += listaCamposOriginal[i][13]+'<%=separador%>';
    if(listaCamposOriginal[i][14] == "") {
      listaPlazoFecha += " "+'<%=separador%>';
    } else {
      listaPlazoFecha += listaCamposOriginal[i][14]+'<%=separador%>';
    }
    if(listaCamposOriginal[i][15] == "") {
      listaCheckPlazoFecha += " "+'<%=separador%>';
    } else {
      listaCheckPlazoFecha += listaCamposOriginal[i][15]+'<%=separador%>';
    }
    if(listaCamposOriginal[i][16] == ""){
        listaValidacion += " " + '<%=separador%>';
    } else{
        listaValidacion += listaCamposOriginal[i][16]+'<%=separador%>';
    }
    if(listaCamposOriginal[i][17] == ""){
        listaOperacion += " " + '<%=separador%>';
    } else{
        listaOperacion += listaCamposOriginal[i][17]+'<%=separador%>';
    }
    if(listaCamposOriginal[i][18] == ""){
        listaAgrupacionesCampos += " " + '<%=separador%>';
    }else{
        listaAgrupacionesCampos += listaCamposOriginal[i][18]+'<%=separador%>';
    }
    listaPosicionXCampo += listaCamposOriginal[i][19]+'<%=separador%>';
    listaPosicionYCampo += listaCamposOriginal[i][20]+'<%=separador%>';
  }
...
function crearListasEnlaces() {
  ...
  for (i=0; i < listaEnlaces.length; i++) {
    listaCodigosEnl +=listaEnlacesOriginal[i][0]+'<%=separador%>';
    listaDescripcionEnl +=listaEnlaces[i][0]+'<%=separador%>';
    listaURLEnl +=listaEnlaces[i][1]+'<%=separador%>';
    listaEstadoEnl +=listaEnlaces[i][2]+'<%=separador%>';
  }
...
function crearListasRoles() {
  ...
  for (i=0; i < listaRoles.length; i++) {
    listaCodigosRol +=listaRoles[i][0]+'<%=separador%>';
    listaDescripcionRol +=listaRoles[i][1]+'<%=separador%>';
    if(listaRoles.length == 1) {
      listaPorDefecto += '1'+'<%=separador%>';
    } else {
      listaPorDefecto += listaRolesOriginal[i][2]+'<%=separador%>';
    }
    listaConsultaWebRol +=listaRolesOriginal[i][3]+'<%=separador%>';
  }
...
function validarProcAutomatico(){
                ...
                <tr style="margin-top:2px;">
                    ...
                    <TR>
                        ...
                        <TD class="columnP">
							...
							<html:checkbox property="automatico" style="margin-left: 88px" onclick="javascript:validarProcAutomatico()" />&nbsp;&nbsp;<%=descriptor.getDescripcion("etiqExpInicioAutomatico")%>
                        </TD>
                    </TR>
                    <tr>
