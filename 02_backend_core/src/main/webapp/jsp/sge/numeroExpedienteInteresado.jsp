            ...
            if(numeroInteresados==0)
            {
                ...
                if(datos!=undefined){
                    if(datos[0] ==1) {
                        ...
                        /*
                        #898170: no se permite añadir los interesados del asiento al expediente
                        if(datos[1] =="si") {
    ...
    <div class="contenidoPantalla" valign="top" style="height: 100%; padding: 50px 5px 40px 5px">
        ...
        <script type="text/javascript">
            ...
            function cargarTabla(){
                ...
                var contador = 0;
                <%
                    if(request.getAttribute("relacionExpInt")!=null){
                        ArrayList<GeneralValueObject> relacionExpInt = (ArrayList<GeneralValueObject>) request.getAttribute("relacionExpInt");
                        if(relacionExpInt.size()>0){
                            for(int i=0; i<relacionExpInt.size(); i++){
                                GeneralValueObject gvoExp = relacionExpInt.get(i);
                                String estado = ((Integer) gvoExp.getAtributo("estado")).toString();
                                String fecha = DateOperations.toString((java.util.Date) gvoExp.getAtributo("fechaInicio"),"dd/MM/yyyy");
                                if(estado.equals("0")) estado = descriptor.getDescripcion("gbPendiente");
                                else if(estado.equals("9")) estado = descriptor.getDescripcion("gbFinalizado");
                %>
                            listaExpedientes[contador++] = [
                                '<%=fecha%>','<%=(String) gvoExp.getAtributo("numExpediente")%>',
                                '<%=estado%>','<%=StringEscapeUtils.escapeJavaScript((String) gvoExp.getAtributo("asunto"))%>'  
                            ];
                <%  } } } %>
