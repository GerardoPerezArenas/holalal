   ...
   function acreditarModulo(){
    ...
    <div class="tab-page" id="tabPage401" style="height:520px; width: 100%;">
        ...
        <div style="clear: both;">
			...
			<div id="divGeneral" style="height: 380px; padding: 20px;">
   ...
   var contador = 0;
    ...
    comboListaCertificados.change = cargarDatosCertificado;
     ...
     /* Creamos la tabla */
        ...
        if(document.all){           
            tablaExpedientesCEPAP = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.all.tablaExpedientesCEPAP,910);
        }else{
            
            tablaExpedientesCEPAP = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.getElementById("tablaExpedientesCEPAP"),910);
        }//if(document.all)
