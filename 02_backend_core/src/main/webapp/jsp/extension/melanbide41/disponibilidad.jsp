    ...
    <body>
        ...
        <script type="text/javascript">
            ...
            function pulsarAltaDisponibilidad() {
                ...
                var result = null;
                lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE41&operacion=cargarNuevaEspecialdad&tipo=0&numero=<%=numExpediente%>&control=' + control.getTime(), 600, 850, 'no', 'no', function (result) {
                    if (result != undefined) {
                        if (result[0] == '0') {
            ...
            function pulsarModificarDisponibilidad() {
                if (tabDisponibilidad.selectedIndex != -1) {
                    var control = new Date();
                    var result = null;
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE41&operacion=cargarModifDisponibilidad&tipo=0&numero=<%=numExpediente%>&id=' + listaDisponibilidad[tabDisponibilidad.selectedIndex][0] + '&idespsol=' + listaDisponibilidad[tabDisponibilidad.selectedIndex][1] + '&control=' + control.getTime(), 600, 850, 'no', 'no', function (result) {
                        if (result != undefined) {
                            if (result[0] == '0') {
                                recargarTablaDisponibilidad(result);
                            }
