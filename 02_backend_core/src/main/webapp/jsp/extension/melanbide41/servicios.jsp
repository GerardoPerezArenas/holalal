    ...
    <body>
        ...
        <script type="text/javascript">
            ...
            function pulsarAltaServicio() {
                ...
                lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE41&operacion=cargarNuevoServicio&tipo=0&numero=<%=numExpediente%>&control=' + control.getTime(), 440, 850, 'no', 'no', function (result) {
                    if (result != undefined) {
                        if (result[0] == '0') {
            ...
            function pulsarModificarServicio() {
                if (tabServicios.selectedIndex != -1) {
                    var control = new Date();
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE41&operacion=cargarModifServicio&tipo=0&numero=<%=numExpediente%>&id=' + listaServicios[tabServicios.selectedIndex][0] + '&control=' + control.getTime(), 400, 850, 'no', 'no', function (result) {
                        if (result != undefined) {
                            if (result[0] == '0') {
                                recargarTablaServicios(result);
                            }
