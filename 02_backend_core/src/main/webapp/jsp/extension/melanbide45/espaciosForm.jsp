...
<link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/scripts/DataTables/datatables.min.css"/>
    ...
    <body>
            ...
            <script type="text/javascript">
                ...
                tabEspacioForm = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.getElementById('listaEspacioForm'), 762);
            ...
            <script type="text/javascript">
                ...
                function pulsarAltaEspacioForm(){
                    ...
                    if(navigator.appName.indexOf("Internet Explorer")!=-1){
						...
						lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE45&operacion=cargarNuevoEspacioForm&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),600,900,'no','no', function(result){
								if (result != undefined){
									if(result[0] == '0'){
                ...
                function pulsarModificarEspacioForm(){
                    if(tabEspacioForm.selectedIndex != -1) {
                        ...
                        if(navigator.appName.indexOf("Internet Explorer")!=-1){
							...
							lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE45&operacion=cargarModifEspacioForm&tipo=0&numero=<%=numExpediente%>&id='+listaEspacioForm[tabEspacioForm.selectedIndex][0]+'&control='+control.getTime(),600,900,'no','no', function(result){
								if (result != undefined){
									if(result[0] == '0'){
										recargarTablaEspacioForm(result);								
									}
