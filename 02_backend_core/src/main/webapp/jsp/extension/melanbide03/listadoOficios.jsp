...
<html>
    <head>
        ...
        <title><%=meLanbide03I18n.getMensaje(idiomaUsuario, "tituloPantalla.listadoExpedienteOficio")%></title>
        ...
        <script type="text/javascript">
            ...
            %>
                datosExpedienteAPA[j] = [check(seleccionado, contador),
                    '<%=(String)expedienteAPA.getAtributo("numExpediente")%>', '<%=(String)expedienteAPA.getAtributo("interesados")%>', '<%=(String)expedienteAPA.getAtributo("numRegistro")%>', '<%=(String)expedienteAPA.getAtributo("fechaEntrada")%>', '<%=(String)expedienteAPA.getAtributo("fecPres")%>'];
                        ...
                        try {
                            ...
                            if (codigoOperacion == "0") {
                                ...
                                for (var i = 0; i < listaDocumentos.length; i++) {
                                    ...
                                    lineas[i] = [filaAct[0], filaAct[1], '<a href=\"#\" title=\"' + '<%=meLanbide03I18n.getMensaje(idiomaUsuario, "label.verExpFicheroImpresion")%>' + '\" onclick=\"listadoExpedientesImpresion(' + i + ');\">' + '<%=meLanbide03I18n.getMensaje(idiomaUsuario, "label.ver")%>' + '</a>'];
                                }
                                tabFichExelAPA.lineas = lineas;
                                refresca(tabFichExelAPA);
                                lineas = new Array();
    ...
    <body class="bandaBody" onload="javascript:{
                ...
                if (window.top.principal.frames[0] && window.top.principal.frames['menu'].Go)
                    window.top.principal.frames['menu'].Go();
                inicializar();
            }">
                            ...
                            <tr>
                                <td>
                                    <table class="bordeInteriorPantalla" cellspacing="1px">
                                        <tr>
                                            <td id="titulo" style="width: 100%; height: 30px" class="txttitblanco"><%= meLanbide03I18n.getMensaje(idiomaUsuario, "tituloPantalla.listadoExpedienteOficio")%></td>
                                        </tr>
                                        <tr>
                                            <td class="separadorTituloPantalla"></td>
                                        </tr>
                                        <tr>
                                            <td><%= meLanbide03I18n.getMensaje(idiomaUsuario, "tituloPantalla.fechaDesde")%></td>
                                            <td><input type="text" name="fechaDesde"  id="fechaRD" size="10" class="inputTexto" readonly="true" value=""/></td>
                                            <td><%= meLanbide03I18n.getMensaje(idiomaUsuario, "tituloPantalla.fechaHasta")%></td>
                                            <td><input type="text" name="fechaHasta"  id="fechaRD" size="10" class="inputTexto" readonly="true" value=""/></td>                                            
                                        </tr>
                                        <tr>
                                            <td><%= meLanbide03I18n.getMensaje(idiomaUsuario, "tituloPantalla.tipoSolicitud")%></td>
                                            <td>
                                                <input type="text" name="codTipoSolicitud" id="codTipoSolicitud" size="10" class="inputTexto" value="" onkeyup="xAMayusculas(this);"/>
                                                <input type="text" name="descTipoSolicitud"  id="descTipoSolicitud" size="112" class="inputTexto" readonly="true" value=""/>
                                                <a href="" id="anchorTipoSolicitud" name="anchorTipoSolicitud">
                                        ...
                                        <tr>
                                            <td>
                                                <input type= "button" class="botonLargo" value="<%=meLanbide03I18n.getMensaje(idiomaUsuario, "btn.buscar")%>" name="buscarExp" onClick="buscarExp();">
                                            </td>
                                        </tr>
                                        <tr>
        ...
        <script type="text/javascript">
            ...
            if (document.all)
                tabImpresionExpAPA = new Tabla(document.all.tablaImpresionExpCEPAP);
            else
                tabImpresionExpAPA = new Tabla(document.getElementById('tablaImpresionExpCEPAP'));
            ...
            tabImpresionExpAPA.addColumna('165', 'center', '<%=meLanbide03I18n.getMensaje(idiomaUsuario, "lbl.anotRegistro")%>');
            tabImpresionExpAPA.addColumna('165', 'center', '<%=meLanbide03I18n.getMensaje(idiomaUsuario, "lbl.fecPresRegistro")%>');
            tabImpresionExpAPA.addColumna('155', 'center', '<%=meLanbide03I18n.getMensaje(idiomaUsuario, "lbl.fecSolicitud")%>');
            ...
            if (document.all) {
                tabFichExelAPA = new Tabla(document.all.tablaFichExelCEPAP);
            } else {
                tabFichExelAPA = new Tabla(document.getElementById('tablaFichExelCEPAP'));
            }
            tabFichExelAPA.addColumna('450', 'center', '<%=meLanbide03I18n.getMensaje(idiomaUsuario, "lbl.nomFicheroGenerado")%>');
            tabFichExelAPA.addColumna('250', 'center', '<%=meLanbide03I18n.getMensaje(idiomaUsuario, "lbl.fechaGeneracion")%>');
            tabFichExelAPA.addColumna('198', 'center', '<%=meLanbide03I18n.getMensaje(idiomaUsuario, "lbl.expedientesRelacionados")%>');

            listaCodigosTipoSolicitud[0] = "";
            listaDescripcionesTiposSolicitud[0] = "";

            /* Lista con los certificados recuperados de la BBDD */
            var contador = 0;

            <logic:iterate id="tipoSolicitud" name="listaTipoSolicitud" scope="request">
            listaCodigosTipoSolicitud[contador] = ['<bean:write name="tipoSolicitud" property="codTipoSolicitud" />'];
            listaDescripcionesTiposSolicitud[contador] = ['<bean:write name="tipoSolicitud" property="descTipoSolicitud" />'];
            contador++;
            </logic:iterate>

            comboTipoSolicitud = new Combo("TipoSolicitud");
            comboTipoSolicitud.addItems(listaCodigosTipoSolicitud, listaDescripcionesTiposSolicitud);
            comboTipoSolicitud.change = cargarDatosSolicitud;
