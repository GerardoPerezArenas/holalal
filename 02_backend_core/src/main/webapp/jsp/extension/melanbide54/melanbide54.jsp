...
<script type="text/javascript">
    function recargarTabla(result) {
        ...
        for (var i = 1; i < result.length; i++) {
            fila = result[i];
            lista[i - 1] = [fila[0], fila[3], fila[5], fila[6], fila[7], fila[8], fila[9]];
    ...
    function pulsarAlta() {
        var control = new Date();
        lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE54&operacion=cargarNuevoCentro&tipo=0&numero=<%=numExpediente%>&control=' + control.getTime(), 480, 800, 'yes', 'no', function (result) {
            if (result != undefined) {
                if (result[0] == '0') {
    ...
    function pulsarModificar() {
        if (tab.selectedIndex != -1) {
            var control = new Date();
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE54&operacion=cargarModifCentro&tipo=0&numero=<%=numExpediente%>&id=' + lista[tab.selectedIndex][0] + '&control=' + control.getTime(), 480, 800, 'yes', 'no', function (result) {
                if (result != undefined) {
                    if (result[0] == '0') {
                        recargarTabla(result);
                    }
