...
<%@page import="es.altia.flexia.integracion.moduloexterno.melanbide86.vo.FilaMinimisVO"%>
...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            function pulsarNuevoMinimis() {
                lanzarPopUpModal(url + '?tarea=preparar&modulo=MELANBIDE86&operacion=cargarNuevoMinimis&tipo=0&nuevo=1&numExp=<%=numExpediente%>', 750, 1200, 'no', 'no', function (result) {
                    if (result != undefined) {
                        if (result[0] == '0') {
            ...
            function pulsarModificarMinimis() {
                if (tablaMinimis.selectedIndex != -1) {
                    lanzarPopUpModal(url + '?tarea=preparar&modulo=MELANBIDE86&operacion=cargarModificarMinimis&tipo=0&nuevo=0&numExp=<%=numExpediente%>&id=' + listaMinimis[tablaMinimis.selectedIndex][0], 750, 1200, 'no', 'no', function (result) {
                        if (result != undefined) {
                            if (result[0] == '0') {
                                recargarTablaMinimis(result[1]);
                            }
            ...
            function pulsarEliminarMinimis() {
                if (tablaMinimis.selectedIndex != -1) {
                    ...
                    if (resultado == 1) {
                        ...
                        var parametros = 'tarea=preparar&modulo=MELANBIDE86&operacion=eliminarMinimis&tipo=0&numExp=<%=numExpediente%>&id=' + listaMinimis[tablaMinimis.selectedIndex][0];
                        try {
                            $.ajax({
                                url: url,
                                type: 'POST',
                                async: true,
                                data: parametros,
                                success: procesarRespuestaActualizarPestanaMinimis,
            ...
            tablaMinimis = new Tabla(true, '<%=descriptor.getDescripcion("buscar")%>', '<%=descriptor.getDescripcion("anterior")%>', '<%=descriptor.getDescripcion("siguiente")%>', '<%=descriptor.getDescripcion("mosFilasPag")%>', '<%=descriptor.getDescripcion("msgNoResultBusq")%>', '<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>', '<%=descriptor.getDescripcion("filtrDeTotal")%>', '<%=descriptor.getDescripcion("primero")%>', '<%=descriptor.getDescripcion("ultimo")%>', document.getElementById('listaMinimis'));
                ...
                tablaMinimis.addColumna('0', 'center', "<%=meLanbide86I18n.getMensaje(idiomaUsuario,"minimis.tablaMinimis.id")%>");
                tablaMinimis.addColumna('100', 'center', "<%=meLanbide86I18n.getMensaje(idiomaUsuario,"minimis.tablaMinimis.estado")%>");
                tablaMinimis.addColumna('400', 'center', "<%=meLanbide86I18n.getMensaje(idiomaUsuario,"minimis.tablaMinimis.organismo")%>");
                tablaMinimis.addColumna('820', 'center', "<%=meLanbide86I18n.getMensaje(idiomaUsuario,"minimis.tablaMinimis.objeto")%>");
                tablaMinimis.addColumna('100', 'center', "<%=meLanbide86I18n.getMensaje(idiomaUsuario,"minimis.tablaMinimis.importe")%>");
                tablaMinimis.addColumna('100', 'center', "<%=meLanbide86I18n.getMensaje(idiomaUsuario,"minimis.tablaMinimis.fecha")%>");
            ...
            function actualizarPestanaMinimis() {
                var parametros = 'tarea=preparar&modulo=MELANBIDE86&operacion=actualizarPestanaMinimis&tipo=0&numExp=<%=numExpediente%>';
                try {
                    $.ajax({
                        url: url,
                        type: 'POST',
            ...
            function procesarRespuestaActualizarPestanaMinimis(ajaxResult) {
    ...
    <body class="bandaBody" onload="javascript:{
        ...
        <div class="tab-page" id="tabPage862" style="height:520px; width: 100%;">
            <h2 class="tab" id="pestana862"><%=meLanbide86I18n.getMensaje(idiomaUsuario,"label.subvencion.pestanaSubvencion")%></h2> 
            <script type="text/javascript">tp1.addTabPage(document.getElementById("tabPage862"));</script>
           <div id="barraProgreso" style="display : none;">
            ...
            <br/>
            <h2 class="legendAzul" id="pestanaPrinc"><%=meLanbide86I18n.getMensaje(idiomaUsuario,"label.tituloPrincipalSub")%></h2>
            <div>    
                <br>
                <div id="divGeneral">     
                ...
                <br>
                <div class="legendRojo" id="notaFecha"><%=meLanbide86I18n.getMensaje(idiomaUsuario,"label.notaFecha")%></div>
                <div class="botonera" style="text-align: center;">
                    <input type="button" id="btnNuevoMinimis" name="btnNuevoMinimis" class="botonGeneral"  value="<%=meLanbide86I18n.getMensaje(idiomaUsuario, "btn.nuevo")%>" onclick="pulsarNuevoMinimis();">
                    <input type="button" id="btnModificarMinimis" name="btnModificarMinimis" class="botonGeneral" value="<%=meLanbide86I18n.getMensaje(idiomaUsuario, "btn.modificar")%>" onclick="pulsarModificarMinimis();">
                    <input type="button" id="btnEliminarMinimis" name="btnEliminarMinimis"   class="botonGeneral" value="<%=meLanbide86I18n.getMensaje(idiomaUsuario, "btn.eliminar")%>" onclick="pulsarEliminarMinimis();">
                </div>
            </div>  
        </div>
               ...
               if (List!= null && List.size() >0){
                   ...
                   for (int indice=0;indice<List.size();indice++)
                   {
                        ...
                        if(objectVO.getDesEstado()!=null){
                            ...
                            String barraSeparadoraDobleIdiomaDesple = ConfigurationParameter.getParameter(ConstantesMeLanbide86.BARRA_SEPARADORA_IDIOMA_DESPLEGABLES, ConstantesMeLanbide86.FICHERO_PROPIEDADES);
                            String[] descripcionDobleIdioma = (descripcion!=null?descripcion.split(barraSeparadoraDobleIdiomaDesple):null);
                            if(descripcionDobleIdioma!=null && descripcionDobleIdioma.length>1){
                                if(idiomaUsuario==ConstantesMeLanbide86.CODIGO_IDIOMA_EUSKERA){
