...
<html>
 ...
 String css = "";
	 ...
	 css = usuario.getCss();
            ...
            String tituloPagina = meLanbide35I18n.getMensaje(idiomaUsuario,"label.justificacion.inserciones.tituloPagina");
...
<link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/scripts/DataTables/datatables.min.css"/>
        ...
        <script type="text/javascript">
            ...
            function pulsarNuevoSeguimientoJus(){
                    ...
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarNuevoSeguimientoPreparador&tipo=0&numero=<%=numExpediente%>&opcion=nuevo&tiposeg=1&preparador=<%=preparador!=null?preparador.getJusPreparadoresCod():""  %>&colectivo=<%=colectivo%>&sexo=<%=sexo%>&control='+control.getTime(),450,850,'no','no', function(result){
						if (result != undefined){
							if(result[0] == '0'){
            ...
            function pulsarModifSegPreparador(){                
                if(tabSeguimientos_prep.selectedIndex != -1) {
                    ...
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarNuevoSeguimientoPreparador&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&preparador=<%=preparador!=null?preparador.getJusPreparadoresCod():""  %>&colectivo=<%=colectivo%>&sexo=<%=sexo%>&idSegModificar='+listaSeguimientos_prep[tabSeguimientos_prep.selectedIndex][0]+'&control='+control.getTime(),510,860,'no','no', function(result){
						if (result != undefined){
							if(result[0] == '0'){
								self.parent.opener.retornoXanelaAuxiliar(true);
								recargarTablaSegPreparadores(result);
            ...
            function pulsarEliminarSegPreparador(){
                    if(tabSeguimientos_prep.selectedIndex != -1) {
                        ...
                        if (resultado == 1){
                            ...
                            parametros = 'tarea=preparar&modulo=MELANBIDE35&operacion=eliminarSegPreparador&tipo=0&numero=<%=numExpediente%>&idSegPrep='+listaSeguimientos_prep[tabSeguimientos_prep.selectedIndex][0]+'&tipoSeg=<%=ConstantesMeLanbide35.TIPO_INSERCION_MELANBIDE35%>&colectivo=<%=colectivo%>&sexo=<%=sexo%>&filtrar=<%=preparador != null ? "1" : "0"%>&control='+control.getTime();
                            try{
           ...
           // var camposErrores = new Array();
                ...
                for(var i = 1;i< result.length; i++){
                    ...
                    <%
                        ...
                        {
                    %>
                            listaSeguimientos_prepTabla[i-1] = [fila[17] != undefined && fila[17].length > 0 ? '<img src="<%=request.getContextPath()%>/images/extension/melanbide35/alert.png"/>' : '', fila[1], fila[2], fila[3], fila[4], fila[5], fila[6], fila[7], fila[8], fila[9], fila[10], fila[11], fila[12], fila[13], fila[14], fila[15], fila[17]];
                    ...
                    <%
                        ...
                        {
                    %>
                            listaSeguimientos_prepTabla[i-1] = [fila[17] != undefined && fila[17].length > 0 ? '<img src="<%=request.getContextPath()%>/images/extension/melanbide35/alert.png"/>' : '', fila[1], fila[2], fila[3], fila[4], fila[6], fila[7], fila[8], fila[9], fila[10], fila[11], fila[12], fila[13], fila[14], fila[15], fila[17]];
                            listaSeguimientos_prepTabla_titulos[i-1] = [getListAsText(fila[17]), fila[1], fila[2], fila[3],fila[4],                 
                ...
                tabSeguimientos_prep.addColumna('30','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col0")%>');        
                tabSeguimientos_prep.addColumna('80','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col2")%>');
                tabSeguimientos_prep.addColumna('150','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col3")%>');
               
                tabSeguimientos_prep.addColumna('80','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col4")%>');
                tabSeguimientos_prep.addColumna('60','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col5")%>');
                
                <%
                    if(colectivo == null || colectivo.equals(""))
                    {
                %>
                        tabSeguimientos_prep.addColumna('70','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col6")%>');
                <%
                    }
                %>
                
                tabSeguimientos_prep.addColumna('90','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col7")%>');
                tabSeguimientos_prep.addColumna('100','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col8")%>');
                tabSeguimientos_prep.addColumna('100','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col9")%>');
                tabSeguimientos_prep.addColumna('70','right','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col10")%>');
                tabSeguimientos_prep.addColumna('80','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col11")%>');
                tabSeguimientos_prep.addColumna('80','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col12")%>');
                tabSeguimientos_prep.addColumna('100','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col13")%>');
                tabSeguimientos_prep.addColumna('130','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col14")%>');
                //tabSeguimientos_prep.addColumna('150','left','<%//=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col15")%>');
                tabSeguimientos_prep.addColumna('90','right','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col16")%>');
                tabSeguimientos_prep.addColumna('250','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col17")%>');
    ...
    <body onload="inicio();" class="contenidoPantalla etiqueta" style="margin:0" >
        <form>
            ...
            <div style="width: 95%; padding: 10px; text-align: left;">
                ...
                <div class="lineaFormulario">
                    ...
                    %>
                            <div style="width:50%;float:left">
                                <%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.justificacion.preparadores.colectivo").toUpperCase()%>:
        ...
        <script type="text/javascript">
            ...
            tabSeguimientos_prep.addColumna('30','center','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col0")%>','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.solicitud.preparadores.tabla.col0.title")%>');    
            tabSeguimientos_prep.addColumna('80','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col2")%>');
            tabSeguimientos_prep.addColumna('150','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.inserciones.tabla.col3")%>');
            ...
            %>
                ...
                %>
                        ...
                        listaSeguimientos_prepTabla[<%=i%>] = ['<%=act.getErrores() != null && act.getErrores().size() > 0 ? "<img src=\""+request.getContextPath()+"/images/extension/melanbide35/alert.png\" />" : ""%>', '<%=act.getNif()%>', '<%=act.getNombreApe()%>', '<%=act.getFecNacimiento()%>',
                        '<%=act.getSexo().equals("0")?meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.seguimiento.sexo0").toUpperCase():meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.seguimiento.sexo1").toUpperCase()%>','<%=act.getColectivo()%>','<%=act.getTipoDiscapacidad()%>','<%=act.getGravedad()%>','<%=act.getTipoContrato()%>','<%=act.getPorcJornada()%>',
                        '<%=act.getFecIni()%>', '<%=act.getFecFin()%>', '<%=act.getNifPreparador()%>', '<%=act.getEmpresa()%>',  '<%=act.getHorasCont()%>', '<%=act.getObservaciones()%>'];
                <%
                    }
                    else
                ...
                %>
                        ...
                        '<%=act.getSexo().equals("0")?meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.seguimiento.sexo0"):meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.seguimiento.sexo1")%>','<%=act.getColectivo()%>','<%=act.getTipoDiscapacidad()%>','<%=act.getGravedad()%>','<%=act.getTipoContrato()%>','<%=act.getPorcJornada()%>',
                        '<%=act.getFecIni()%>', '<%=act.getFecFin()%>', '<%=act.getNifPreparador()%>', '<%=act.getEmpresa()%>',  '<%=act.getHorasCont()%>', '<%=act.getObservaciones()%>'];


                        listaSeguimientos_prepTabla[<%=i%>] = ['<%=act.getErrores() != null && act.getErrores().size() > 0 ? "<img src=\""+request.getContextPath()+"/images/extension/melanbide35/alert.png\" />" : ""%>','<%=act.getNif()%>', '<%=act.getNombreApe()%>', '<%=act.getFecNacimiento()%>',
                        '<%=act.getSexo().equals("0")?meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.seguimiento.sexo0").toUpperCase():(act.getSexo().equals("1")?meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.seguimiento.sexo1").toUpperCase():act.getSexo())%>','<%=act.getColectivo()%>','<%=act.getTipoDiscapacidad()%>','<%=act.getGravedad()%>','<%=act.getTipoContrato()%>','<%=act.getPorcJornada()%>',
                        '<%=act.getFecIni()%>', '<%=act.getFecFin()%>', '<%=act.getNifPreparador()%>', '<%=act.getEmpresa()%>',  '<%=act.getHorasCont()%>', '<%=act.getObservaciones()%>'];
