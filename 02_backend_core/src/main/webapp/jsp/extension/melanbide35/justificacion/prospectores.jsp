...
<script type="text/javascript">
    ...
    function recargarTablaProspectoresJustificacionEca(result, actualizarOtrasPestanas){
        ...
        //}
        tabProspectores_jus.addColumna('250','left','<%=meLanbide35I18n.getMensaje(idiomaUsuario, "label.justificacion.prospectores.tabla.col2")%>');
    ...
    function pulsarNuevoProspectorJustificacionEca(){
        if(tabProspectores_jus.selectedIndex != -1) {  
            ...
            {  
                ...
                var result = null;
                if(navigator.appName.indexOf("Internet Explorer")!=-1){
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarNuevoProspectorJustificacion&tipo=0&numero=<%=numExpediente%>&opcion=nuevo&idProsOrigen='+listaProspectores_jus[tabProspectores_jus.selectedIndex][0]+'&control='+control.getTime(),400,950,'no','no', function(result){
					if (result != undefined){
                    				if(result[0] == '0'){								
    ...
    function pulsarModifProspectorJustificacionEca(){
        if(tabProspectores_jus.selectedIndex != -1) {
            ...
            if(navigator.appName.indexOf("Internet Explorer")!=-1){                
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarNuevoProspectorJustificacion&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&idProsModif='+listaProspectores_jus[tabProspectores_jus.selectedIndex][0]+'&control='+control.getTime(),450,950,'no','no', function(result){
					if (result != undefined){
                    				if(result[0] == '0'){								
							recargarTablaProspectoresJustificacionEca(result,true);
						}
    ...
    <%--function mostrarBotonCargarFicheroProspectores(){
        ...
        function cargarTodasVisitasJus(){        
            ...
            if(navigator.appName.indexOf("Internet Explorer")!=-1){
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarVisitas&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&control='+control.getTime(),590,900,'no','no', function(result){
        ...
        function cargarVisitasProspector(idProspector){
            ...
            if(navigator.appName.indexOf("Internet Explorer")!=-1){
                lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarVisitas&tipo=0&numero=<%=numExpediente%>&idProspector='+idProspector+'&opcion='+opcion+'&control='+control.getTime(),605,950,'no','no', function(result){
					if(result != undefined && result == true){
               					var lista = getListaProspectoresJustificacionEca();
               					recargarTablaProspectoresJustificacionEca(lista, true)
            				}
