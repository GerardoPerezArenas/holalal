        ...
        <script type="text/javascript">
            ...
            function pulsarModifSegPreparador(){                
                if(tabSeguimientos_prep.selectedIndex != -1) {
                    ...
                    var result = null;
                    if(navigator.appName.indexOf("Internet Explorer")!=-1){                        
                        lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarNuevoSeguimientoPreparador&tipo=0&numero=<%=numExpediente%>&opcion=<%=consulta ? "consultar" : "modificar"%>&preparador=<%=preparador!=null?preparador.getJusPreparadoresCod():""  %>&colectivo=<%=colectivo%>&sexo=<%=sexo%>&idSegModificar='+listaSeguimientos_prep[tabSeguimientos_prep.selectedIndex][0]+'&control='+control.getTime(),650,860,'no','no', function(result){
					if (result != undefined){
                    				if(result[0] == '0'){								
							self.parent.opener.retornoXanelaAuxiliar(true);
                            				recargarTablaSegPreparadores(result);
