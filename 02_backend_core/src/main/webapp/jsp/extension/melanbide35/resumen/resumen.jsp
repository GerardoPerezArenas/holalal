...
<%@page import="es.altia.flexia.integracion.moduloexterno.melanbide35.vo.resumen.EcaResumenVO" %>
...
<script type="text/javascript">         
    ...
    function actualizarDatosResumen(){
        ...
        parametros = 'tarea=preparar&modulo=MELANBIDE35&operacion=actualizarDatosResumen&tipo=0&numero=<%=numExpediente%>&control='+control.getTime();
        try{
            ...
            if(mensaje != null && mensaje != ''){
                if(mensaje.toUpperCase() == '<%=ConstantesMeLanbide35.OK%>'){
                    jsp_alerta("A", '<%=meLanbide35I18n.getMensaje(idiomaUsuario,"msg.resumen.actualizarDatos.ok")%>');
                }else{
                    jsp_alerta("A",'<%=meLanbide35I18n.getMensaje(idiomaUsuario,"msg.resumen.errorActualizarDatos")%>:'+'<br/><br/>'+mensaje);
                }
            }else{
                jsp_alerta("A",'<%=meLanbide35I18n.getMensaje(idiomaUsuario,"msg.resumen.actualizarDatos.error.desconocido")%>');
            }
        }
        catch(err){
            alert(err);
    ...
    function getDatosResumenEca(){
        ...
        var control = new Date();
        var datos = new Array();
        parametros = 'tarea=preparar&modulo=MELANBIDE35&operacion=getDatosResumenEca&tipo=0&numero=<%=numExpediente%>&control='+control.getTime();
        try{
            ajax.open("POST",url,false);
            ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=ISO-8859-1");
            ajax.setRequestHeader("Accept", "text/xml, application/xml, text/plain");     
            ...
            if(hijos[j].nodeName=="CODIGO_OPERACION"){                            
                ...
                if(hijos[j].nodeName=="VALORES_SOLICITADO"){
                    for(var cont = 0; cont < hijosFila.length; cont++){
                        ...
                        }else if(hijosFila[cont].nodeName=="TOT_SUBV_APROBADA"){
                            nodoCampo = hijosFila[cont];
                            if(nodoCampo.childNodes.length > 0){
    ...
    function recargarDatosResumenEca(datos){
            if(datos)
            {
                ...
                document.getElementById('totSubAprobadaRes').value = columna[9];

                //rellenamos los concedidos con lo solicitado
                document.getElementById('impOrgPublicosConcResumen').value = columna[6];//5
                document.getElementById('impOrgPrivadosConcResumen').value = columna[7];
                ...
                document.getElementById('totConcResumen').value = columna[5];
    ...
    function cargarDetalleProspectoresResumen(){
        var control = new Date();   
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarDetalleProspectoresResumen&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),480,992,'no','no', function(result){
				});
        }else{
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarDetalleProspectoresResumen&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),500,1022,'no','no', function(result){
				});
        }
    ...
    function cargarDetallePreparadoresResumen(){
        var control = new Date();   
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarDetallePreparadoresResumen&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),480,1100,'no','no', function(result){
				});
        }else{
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE35&operacion=cargarDetallePreparadoresResumen&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),500,1130,'no','no', function(result){
				});
        }
    ...
    function guardarDatosResumen(){
        if(validarResumenEca()){
            ...
            parametros = 'tarea=preparar&modulo=MELANBIDE35&operacion=guardarDatosResumen&tipo=0&numero=<%=numExpediente%>'
            ...
            try{
                ...
                if(codigoOperacion=="0"){
                    ...
                    jsp_alerta("A",'<%=meLanbide35I18n.getMensaje(idiomaUsuario,"error.noSolic")%>');
                }else{
                    jsp_alerta("A",'<%=meLanbide35I18n.getMensaje(idiomaUsuario,"error.errorGen")%>');
                }//if(
            }
            ...
            try{
                if(!validarNumericoDecimalEca(document.getElementById('impOrgPublicosJusResumen'), 10, 2)){
                    ...
                    if(mensajeValidacion == '')
                        mensajeValidacion = '<%=meLanbide35I18n.getMensaje(idiomaUsuario, "msg.resumen.impPubJusIncorrecto")%>';
                }else{
        ...
        if(document.getElementById('impOrgPrivadosJusResumen').value != ''){
            try{
                if(!validarNumericoDecimalEca(document.getElementById('impOrgPrivadosJusResumen'), 10, 2)){
                    ...
                    if(mensajeValidacion == '')
                        mensajeValidacion = '<%=meLanbide35I18n.getMensaje(idiomaUsuario, "msg.resumen.impPrivJusIncorrecto")%>';
                }else{
    ...
    function ajustarDecimalesImportesResumen(refrescando){
        ...
        if(refrescando){
            ...
            campo = document.getElementById('totSubAprobadaRes');
            reemplazarPuntosEca(campo);
            ajustarDecimalesCampo(campo, 2);
        }
...
<body class="contenidoPantalla etiqueta">
    <div class="tab-page" id="tabPage353" style="height:520px; width: 100%;">
        <h2 class="tab" id="pestana353"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.tituloPestana")%></h2>
        ...
        <div style="clear: both;">
            ...
            <div style="width: 920px; float: left; text-align: left; clear: both; overflow-x: hidden; overflow-y: auto; border: 1px solid #999999" onscroll="deshabilitarRadiosSolicitudEca();">
                <table style="width: 910px; font-size: 12px; text-align: center; border-collapse: collapse;" cellpadding="1px">
                    ...
                    <tr>
                        <td colspan="13" style="text-align: left;" class="sub3titulo"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.actuacionSubvencionable")%></td>
                    </tr>
                    <tr>
                        <td colspan="10">
                    ...
                    <tr class="negrita">
                        ...
                        <td colspan="3" class="sub3titulo"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.solicitado")%></td>
                        <td colspan="3" class="sub3titulo"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.concedido")%></td>
                        <td colspan="3" class="sub3titulo"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.justificadoObjetivos")%></td>
                        <td colspan="3" class="sub3titulo"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.subvencionable")%></td>
                    </tr>
                    <tr class="negrita">
                        <!--td style="text-align: left;"></td-->
                        <td class="xCabecera"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.numero")%></td>
                        <td colspan="2" class="xCabecera"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.solicitud.importe")%></td>               
                        <td class="xCabecera"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.numero")%></td>
                        <td colspan="2" class="xCabecera"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.solicitud.importe")%></td>                
                        <td class="xCabecera"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.numero")%></td>
                        <td colspan="2" class="xCabecera"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.importe")%></td>              
                        <td class="xCabecera"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.numero")%></td>
                        <td colspan="2" class="xCabecera"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.importe")%></td>
                   ...
                   <tr>
                    ...
                    <tr>
                        <td style="text-align: left;"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.nProspectores")%></td>
                        ...
                        <td><a href="#" onclick="cargarDetalleProspectoresResumen()"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.detalle")%></a></td>
                    </tr>
                    <tr>
                        <td style=" text-align: left;"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.nPreparadores")%></td>
                        ...
                        <td><a href="#" onclick="cargarDetallePreparadoresResumen();"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.detalle")%></a></td>
                    </tr>
                    <tr>
                        <td style="text-align: left;"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.gastosGeneralesAdm")%></td>
                        <td></td>
                        <td colspan="2" style="border-right: 1px solid #1766A7;">
                        ...
                        <td></td>
                        <td colspan="2"><input id="gastosGeneralesPagadoResumen" name="gastosGeneralesPagadoResumen" type="text" size="10" class="inputTexto textoNumerico readOnly" readOnly="readonly" onkeyup="calcularTotSubvResumen();FormatNumber(this.value, 8, 2, this.id);"
                    ...
                    <tr>
                        <td style="text-align: left;"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.otrasSubvenciones")%></td>
                        <td colspan="3" style="text-align: left; border-right: 1px solid #1766A7;">
                            <table style="width: 100%; font-size: 12px;text-align: center;">
                                <tr>
                                    <input type="radio" name="radioOtrasAyudasSResumen" id="radioOtrasAyudasSResumen" class="readOnly" style="background-color: white;" readOnly="readonly" value="S"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.general.si").toUpperCase()%>
                                    <input type="radio" name="radioOtrasAyudasSResumen" id="radioOtrasAyudasNResumen" class="readOnly" style="background-color: white;" readOnly="readonly" value="N"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.general.no").toUpperCase()%>
                                </tr>
                            </table>
                        </td>
                    ...
                    <tr id="filaOtrosOrganismos2Resumen">
                        <td style="text-align: left; padding-left: 45px;"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.cantOrgPublicos")%></td>
                        <td ></td>                
                    ...
                    <tr id="filaOtrosOrganismos3Resumen">
                        <td style="text-align: left; padding-left: 45px;"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.cantOrgPrivados")%></td>
                        <td ></td>                
                    ...
                    <tr>
                        <td style=" text-align: left;" class="negrita"><%=meLanbide35I18n.getMensaje(idiomaUsuario,"label.resumen.totSubSolicitada")%></td>
                        <td ></td>
                    ...
                    <tr>
                        ...
                        <td></td>
                        <td colspan="2" style="border-right: 1px solid #1766A7;"><input id="totSubAprobadaRes" name="totSubAprobadaRes" type="text" size="10" class="inputTexto textoNumerico readOnly" readOnly="readonly"></td>
                        <td colspan="3" ></td>
                        <td colspan="3"></td>
                    </tr>
                    <tr>
            ...
            <div class="botonera" style="height: 50px; padding-top: 20px;">
                <input type="button" id="btnGuardarResumen" name="btnGuardarResumen" class="botonGeneral" value="<%=meLanbide35I18n.getMensaje(idiomaUsuario, "btn.general.guardar")%>" onclick="guardarDatosResumen();">
                <input type="button" id="btnActualizarResumen" name="btnActualizarResumen" class="botonGeneral" value="<%=meLanbide35I18n.getMensaje(idiomaUsuario, "btn.solicitud.resumen.actualizarResumen")%>" onclick="actualizarDatosResumen();">
            </div> 
        </div>
    </div>
</body>
...
<script type="text/javascript">
            ...
            %>
                    ...
                    document.getElementById('totSolResumen').value = '<%=datosResumen.getTotSubv_solic() != null ? formateador.format(datosResumen.getTotSubv_solic()) : ""%>';
                    document.getElementById('totSubAprobadaRes').value='<%=datosResumen.getTotAprobado_solic() !=null ? formateador.format(datosResumen.getTotAprobado_solic()) : ""%>';
                    ...
                    document.getElementById('totConcResumen').value = '<%=datosResumen.getTotSubv_conc() != null ? formateador.format(datosResumen.getTotSubv_conc()) : ""%>';
