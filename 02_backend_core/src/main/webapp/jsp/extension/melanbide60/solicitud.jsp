...
<script type="text/javascript"> 
    ...
    function pulsarAltaPuestoCme(){
        ...
        lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE60&operacion=cargarNuevoPuesto&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),600,980,'yes','yes', function(result){
        if (result != undefined){
            if(result[0] == '0'){
    ...
    function pulsarModificarPuestoCme(){
        ...
        if(tabPuestosCme.selectedIndex != -1) {
            ...
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE60&operacion=cargarModificarPuesto&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&idPuesto='+listaPuestosCme[fila][0]+'&control='+control.getTime(),600,980,'yes','yes', function(result){
            if (result != undefined){
                if(result[0] == '0'){
                    recargarTablaPuestosCme(result);
                }
    ...
    function recargarTablaPuestosCme(result){
        ...
        for(var i = 2;i< result.length; i++){
			...
			listaPuestosCme[i-2] =[fila[0],fila[1], fila[2], fila[3], fila[4], fila[5], fila[6], fila[7], fila[8], fila[9], fila[10], fila[11]];
        ...
        tabPuestosCme = new FixedColumnTable(document.getElementById('puestosCme'), 850, 876, 'puestosCme'); // modificado FixedColumTable para que coja 100%
        tabPuestosCme.addColumna('300','left',"<%= meLanbide60I18n.getMensaje(idiomaUsuario,"label.solicitud.tabla.puestos.col1")%>", "<%= meLanbide60I18n.getMensaje(idiomaUsuario,"label.solicitud.tabla.puestos.col1.title")%>");
        ...
        tabPuestosCme.scrollWidth = 1400;
...
<body>
    ...
    <div style="clear: both;">
        <div>
            <div id="puestosCme" style="padding: 5px; width:86.8%; height: 159px; text-align: center; margin:0px;margin-top:0px;" align="center"></div>
