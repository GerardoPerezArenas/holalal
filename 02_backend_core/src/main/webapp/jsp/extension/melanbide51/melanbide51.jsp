...
<head>
    ...
    <%
        ...
        //System.out.println("IDIOMA-> "+sIdioma);
        int idiomaUsuario = 0;//Integer.parseInt(sIdioma);
        int apl = 5;
        String css = "";
                            ...
                            <td style="width: 80%">
                                ...
                                <a href="" id="anchorListaMotivoVisita_busq" name="anchorListaMotivoVisita_busq">
                                    <span class="fa fa-chevron-circle-down" aria-hidden="true" id="botonAplicacion"
                                         name="botonAplicacion" style="cursor:hand; border: 0px;"></span>
                                </a>
    ...
    <script type="text/javascript">
        ...
        function pulsarNuevoAcceso() {
            ...
            if (navigator.appName.indexOf("Internet Explorer") != -1) {
                lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE51&operacion=cargarNuevoAcceso&tipo=0&nuevo=1&control='+control.getTime(), 650, 1000, 'no', 'no',function(result){
                    if (result != undefined) {
                        if (result[0] == '0') {
        ...
        function pulsarModificarAcceso() {
            if (tabaAccesos.selectedIndex != -1) {
                ...
                var result = null;
                if (navigator.appName.indexOf("Internet Explorer") != -1) {
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE51&operacion=cargarModificarAcceso&tipo=0&nuevo=0&id=' + listaAccesos[tabaAccesos.selectedIndex][0] + '&control=' + control.getTime(), 650, 1000, 'no', 'no',function(result){
                        if (result != undefined) {
                            if (result[0] == '0') {
                                recargarTablaAccesos(result);
                            }
