...
<script type="text/javascript">
    ...
    function recargarTablaProspectoresSolicitudEca(result, actualizarOtrasPestanas) {
        ...
        for (var i = 1; i < result.length; i++) {
            ...
            if (fila[15] != undefined && fila[15].length > 0) {
                ...
                camposErroresProspectores_sol = fila[17];
            ...
            if (fila[16] != undefined && fila[16] == '1') {
                ...
                if (fila[15] == undefined || fila[15].length == 0 || listaEstilosFila == undefined || listaEstilosFila == null) {
                    listaEstilosFila = new Array();
                }
                var estilo;
        ...
        //}
        tabProspectores_sol.addColumna('250', 'left', '<%=meLanbide44I18n.getMensaje(idiomaUsuario, "label.solicitud.prospectores.tabla.col2")%>', '<%=meLanbide44I18n.getMensaje(idiomaUsuario, "label.solicitud.prospectores.tabla.col2.title")%>');
        ...
        if (ejercicio < 2019) {
            ...
            tabProspectores_sol.addColumna('70', 'right', '<%=meLanbide44I18n.getMensaje(idiomaUsuario, "label.solicitud.prospectores.tabla.col111")%>', '<%=meLanbide44I18n.getMensaje(idiomaUsuario, "label.solicitud.prospectores.tabla.col111.title")%>');
        }
    ...
    function pulsarNuevoProspectorSolicitudEca() {
        var control = new Date();
        lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE44&operacion=cargarNuevoProspectorSolicitud&tipo=0&numero=<%=numExpediente%>&opcion=nuevo&control=' + control.getTime(), 380, 950, 'no', 'no', function (result) {
            if (result != undefined) {
    ...
    function pulsarSustituirProspectorSolicitudEca() {
        if (tabProspectores_sol.selectedIndex != -1) {
            if (listaProspectores_sol[tabProspectores_sol.selectedIndex][16] == undefined || listaProspectores_sol[tabProspectores_sol.selectedIndex][16] == null || listaProspectores_sol[tabProspectores_sol.selectedIndex][16] != '1')
            {
                var control = new Date();
                var result = null;
                lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE44&operacion=cargarSustituirProspectorSolicitud&tipo=0&numero=<%=numExpediente%>&idProsSustituir=' + listaProspectores_sol[tabProspectores_sol.selectedIndex][0] + '&control=' + control.getTime(), 510, 870, 'no', 'no', function (result) {
                    if (result != undefined) {
                        if (result[0] == '0') {
    ...
    function pulsarModifProspectorSolicitudEca() {
        if (tabProspectores_sol.selectedIndex != -1) {
            ...
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE44&operacion=cargarNuevoProspectorSolicitud&tipo=0&numero=<%=numExpediente%>&opcion=' + opcion + '&idProsModif=' + listaProspectores_sol[tabProspectores_sol.selectedIndex][0] + '&control=' + control.getTime(), 450, 950, 'no', 'no', function (result) {
                if (result != undefined) {
                    recargarTablaProspectoresSolicitudEca(result, true);
                }
            });
    ...
    <div style="width: 100%; float: left; text-align: left; clear: both; height: 100%; overflow-x: hidden; overflow-y: auto;">
        ...
        <div style="clear: both;">
            ...
            <div style="width: 900px; text-align: right; height: 70px;">
                ...
                <% } else {%> 
                    <div style="float: left; text-align: right; width: 135px;">
                        <a href="#" onclick="descargarPlantillaEca('plantilla_prospectores_solicitud2019.xls');"><%=meLanbide44I18n.getMensaje(idiomaUsuario,"btn.general.descargarPlantilla")%></a>
                    </div>
                <% }%>                                 
            </div>
