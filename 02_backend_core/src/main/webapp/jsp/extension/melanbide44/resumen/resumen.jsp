...
<script type="text/javascript">         
    ...
    function cargarDetalleProspectoresResumen(){
        var control = new Date();   
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE44&operacion=cargarDetalleProspectoresResumen&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),480,992,'no','no', function(result){
				});
        }else{
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE44&operacion=cargarDetalleProspectoresResumen&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),500,1022,'no','no', function(result){
				});
        }
    ...
    function cargarDetallePreparadoresResumen(){
        var control = new Date();   
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE44&operacion=cargarDetallePreparadoresResumen&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),480,1100,'no','no', function(result){
				});
        }else{
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE44&operacion=cargarDetallePreparadoresResumen&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),500,1130,'no','no', function(result){
				});
        }
...
<body class="contenidoPantalla etiqueta">
    <div class="tab-page" id="tabPage353" style="height:520px; width: 100%;">
        ...
        <div style="clear: both;">
            ...
            <div style="width: 930px; float: left; text-align: left; clear: both;  overflow-x: hidden; overflow-y: auto; border: 1px solid #999999" onscroll="deshabilitarRadiosSolicitudEca();">
                        ...
                        <td colspan="3" style="text-align: left; border-right: 1px solid #1766A7;">
                            <table style="width: 100%; font-size: 12px;text-align: center;">
                                <tr>
                                    ...
                                    <input type="radio" name="radioOtrasAyudasSResumen" id="radioOtrasAyudasNResumen" class="readOnly" style="background-color: white;" readOnly="readonly" value="N"><%=meLanbide44I18n.getMensaje(idiomaUsuario,"label.general.no").toUpperCase()%>
                                </tr>
                            </table>
                        </td>
