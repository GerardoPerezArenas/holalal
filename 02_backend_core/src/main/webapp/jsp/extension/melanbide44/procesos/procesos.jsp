...
<%@page import="es.altia.flexia.integracion.moduloexterno.melanbide44.vo.procesos.FilaAuditoriaProcesosVO" %>
...
<script type="text/javascript">
    ...
    function cargarComboProcesos(){
            ...
            parametros = 'tarea=preparar&modulo=MELANBIDE44&operacion=cargarComboProcesos&tipo=0&control='+control.getTime();
            try{
    ...
    function filtrarAuditorias(){
        if(validarDatosFiltro()){
            ...
            parametros = 'tarea=preparar&modulo=MELANBIDE44&operacion=filtrarAuditoriaProcesos&tipo=0&nomApellidos='+nomApellidos
    ...
    function validarDatosFiltro(){
        ...
        if(txtDesde != null && txtDesde != ''){
            if(!ValidarFechaConFormato(document.forms[0],document.getElementById('fechaDesde'))){
                jsp_alerta("A","<%=meLanbide44I18n.getMensaje(idiomaUsuario, "msg.fechaDesdeIncorrecto")%>");
                return false;
            }else{
                hayFechaDesde = true;
            }
        ...
        if(txtHasta != null && txtHasta != ''){
            if(!ValidarFechaConFormato(document.forms[0],document.getElementById('fechaHasta'))){
                jsp_alerta("A","<%=meLanbide44I18n.getMensaje(idiomaUsuario, "msg.fechaHastaIncorrecto")%>");
                return false;
            }else{
                hayFechaHasta = true;
            }
        ...
        if(hayFechaDesde && hayFechaHasta){
            ...
            var result = n2 - n1;
            if(result < 0){
                jsp_alerta("A","<%=meLanbide44I18n.getMensaje(idiomaUsuario, "msg.fechaDesdeAnteriorHasta")%>");
                correcto = false;
            }
        }
        return correcto;
    ...
    function solicitarAno(){
        ...
        var result = null;
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            lanzarPopUpModal('<%=request.getContextPath()%>/jsp/extension/melanbide44/procesos/solicitarAno.jsp?idioma=<%=idiomaUsuario%>&control='+control.getTime(),120,200,'no','no',function(){});
        }else{
            lanzarPopUpModal('<%=request.getContextPath()%>/jsp/extension/melanbide44/procesos/solicitarAno.jsp?idioma=<%=idiomaUsuario%>&control='+control.getTime(),120,250,'no','no',function(){});
        }
        return result;
    }
    ...
    function solicitarAnoYFormato(){
        ...
        var result = null;
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            result = lanzarPopUpModal('<%=request.getContextPath()%>/jsp/extension/melanbide44/procesos/solicitarAnoYFormato.jsp?idioma=<%=idiomaUsuario%>&control='+control.getTime(),120,200,'no','no');
        }else{
            result = lanzarPopUpModal('<%=request.getContextPath()%>/jsp/extension/melanbide44/procesos/solicitarAnoYFormato.jsp?idioma=<%=idiomaUsuario%>&control='+control.getTime(),120,250,'no','no');
        }
        return result;
    }
    ...
    function ejecutarListadoDesglose(){
        ...
        //var datos = solicitarAnoYFormato();
        var control = new Date();     
        lanzarPopUpModal('<%=request.getContextPath()%>/jsp/extension/melanbide44/procesos/solicitarAnoYFormato.jsp?idioma=<%=idiomaUsuario%>&control='+control.getTime(),120,200,'no','no', function(datos){
                if(datos != null && datos != undefined && datos.length == 2){
                    ...
                    if(ano != null && ano != undefined && ano != ''){
                        if(formato != null && formato != undefined && formato != ''){
                            ...
                            var parametros = "";
                            var control = new Date();
                            parametros = '?tarea=preparar&modulo=MELANBIDE44&operacion=generarInformeDesglose&tipo=0&ano='+ano+'&formato='+formato+'&control='+control.getTime();
                            inicio = false;
        ...
        if(datos != null && datos != undefined && datos.length == 2){
            ...
            if(ano != null && ano != undefined && ano != ''){
                if(formato != null && formato != undefined && formato != ''){
                    ...
                    var parametros = "";
                    var control = new Date();
                    parametros = '?tarea=preparar&modulo=MELANBIDE44&operacion=generarInformeProyectos&tipo=0&ano='+ano+'&formato='+formato+'&control='+control.getTime();
                    inicio = false;
                    window.open(url+parametros, "_blank");
                }
            }
    ...
    <form id="formProcesos">
        <div class="tab-page" id="tabPage323" style="height:550px; width: 100%;">
            <table width="100%" style="height: 550px;" cellpadding="0px" cellspacing="0px" border="0px">
                <tr>
                    <td style="width:100%;  height:30px; padding-left: 15px" class="txttitblanco"><%=meLanbide44I18n.getMensaje(idiomaUsuario, "tit_procesos")%></td>
                </tr>                
                <tr>
                    <td class="contenidoPantalla" valign="top" style="padding-top: 5px; padding-bottom: 10px;">
                        <div id="contenidoProc" class="cuadroFondoBlanco" style="width:970px; height: 550px; overflow-x: auto; overflow-y: auto; padding: 10px;margin:0px;margin-top:0px;" align="center">
                            <fieldset style="width: 47%; float: left; padding-left: 10px; padding-right: 10px;">
                                <legend class="legendAzul"><%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.legend.solicitudEca")%></legend>
                                <div class="lineaFormularioFont">
                                    <div style="width: 300px; float: left; text-align: left;">
                                        <%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.label.RPSE")%>
                                    </div>
                                    <div style="width: 100px; float: left;">
                                        <div style="float: left;">
                                            <input type="button" id="btnRPSE" name="btnRPSE" class="botonGeneral" value="<%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.btn.ejecutar")%>" onclick="ejecutarResolucionProvisionalSolicitudEca();">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="lineaFormularioFont">
                                    <div style="width: 300px; float: left; text-align: left;">
                                        <%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.label.DRSE")%>
                                    </div>
                                    <div style="width: 100px; float: left;">
                                        <div style="float: left;">
                                            <input type="button" id="btnDRSE" name="btnDRSE" class="botonGeneral" value="<%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.btn.generar")%>" onclick="generarDocumentacionResolucionSolicitudEca();">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="lineaFormularioFont">
                                    <div style="width: 300px; float: left; text-align: left;">
                                        <%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.label.CSE")%>
                                    </div>
                                    <div style="width: 100px; float: left;">
                                        <div style="float: left;">
                                            <input type="button" id="btnCSE" name="btnCSE" class="botonGeneral" value="<%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.btn.ejecutar")%>" onclick="consolidarSolicitudEca();">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="lineaFormularioFont">
                                    <div style="width: 300px; float: left; text-align: left;">
                                        <%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.label.DCSE")%>
                                    </div>
                                    <div style="width: 100px; float: left;">
                                        <div style="float: left;">
                                            <input type="button" id="btnDCSE" name="btnDCSE" class="botonGeneral" value="<%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.btn.ejecutar")%>" onclick="deshacerConsolidacionSolicitudEca();">
                                        </div>
                                    </div>
                                </div>
                            </fieldset>
                            ...
                            <fieldset style="width: 47%; float: left; padding-left: 10px; padding-right: 10px;">
                                <legend class="legendAzul"><%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.legend.otros")%></legend>
                                <div class="lineaFormularioFont">
                                    <div style="width: 300px; float: left; text-align: left;">
                                        <%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.label.LD")%>
                                    </div>
                                    <div style="width: 100px; float: left;">
                                        <div style="float: left;">
                                            <input type="button" id="btnLD" name="btnLD" class="botonGeneral" value="<%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.btn.generar")%>" onclick="ejecutarListadoDesglose();">
                                        </div>
                                    </div>
                                </div>
                                <div class="lineaFormularioFont">
                                    <div style="width: 300px; float: left; text-align: left;">
                                        <%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.label.LP")%>
                                    </div>
                                    <div style="width: 100px; float: left;">
                                        <div style="float: left;">
                                            <input type="button" id="btnLP" name="btnLP" class="botonGeneral" value="<%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.btn.generar")%>" onclick="ejecutarListadoProyectos();">
                                        </div>
                                    </div>
                                </div>
                            </fieldset>
                                <div style="clear: both; float: left; width: 100%; padding-top: 10px;">
                                    <fieldset style="width: 97%; float: left; padding-top: 10px;">
                                        <legend class="legendAzul"><%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.legend.auditoriaProcesos")%></legend>
                                        <div style="clear: both; float: left; width: 100%;">
                                            <table style="width: 100%; font-size: 12px;">
                                                <tr>
                                                    <td>
                                                        <b><u><%=meLanbide44I18n.getMensaje(idiomaUsuario,"proc.label.criteriosBusq")%></u></b>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        <%=meLanbide44I18n.getMensaje(idiomaUsuario,"proc.label.nomApellidos")%>
                                                        <input id="nomApellidos" name="nomApellidos" type="text" class="inputTexto" size="52" maxlength="40" />
                                                        <%=meLanbide44I18n.getMensaje(idiomaUsuario,"proc.label.fDesde")%>
                                                        <input type="text" class="inputTxtFecha" size="10" maxlength="10" id="fechaDesde" name="fechaDesde" onblur="javascript:return comprobarFechaEca(this, '<%=meLanbide44I18n.getMensaje(idiomaUsuario, "msg.fechaNoVal")%>');" onkeyup="return SoloCaracteresFecha(this);" onfocus="this.select();" />
                                                        <A href="javascript:calClick(event);" onclick="mostrarCalFechaDesde(event);return false;" style="text-decoration:none; padding-right: 10px;" >
                                                            <IMG style="border: 0" height="17" id="calFechaDesde" name="calFechaDesde" alt="<%=meLanbide44I18n.getMensaje(idiomaUsuario,"label.fNac")%>" src="<c:url value='/images/calendario/icono.gif'/>">
                                                        </A>
                                                        <%=meLanbide44I18n.getMensaje(idiomaUsuario,"proc.label.fHasta")%>
                                                        <input type="text" class="inputTxtFecha" size="10" maxlength="10" id="fechaHasta" name="fechaHasta" onblur="javascript:return comprobarFechaEca(this, '<%=meLanbide44I18n.getMensaje(idiomaUsuario, "msg.fechaNoVal")%>');" onkeyup="return SoloCaracteresFecha(this);" onfocus="this.select();" />
                                                        <A href="javascript:calClick(event);" onclick="mostrarCalFechaHasta(event);return false;" style="text-decoration:none; padding-right: 10px;" >
                                                            <IMG style="border: 0" height="17" id="calFechaHasta" name="calFechaHasta" alt="<%=meLanbide44I18n.getMensaje(idiomaUsuario,"label.fNac")%>" src="<c:url value='/images/calendario/icono.gif'/>">
                                                        </A>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        <%=meLanbide44I18n.getMensaje(idiomaUsuario,"proc.label.proceso")%>
                                                        ...
                                                        <a id="anchorProceso" name="anchorProceso" href=""><span class="fa fa-chevron-circle-down" aria-hidden="true" id="botonProceso" name="botonProceso" style="cursor:hand;" alt="<%=meLanbide44I18n.getMensaje(idiomaUsuario, "altDesplegable")%>" title="<%=meLanbide44I18n.getMensaje(idiomaUsuario, "altDesplegable")%>"></span></a>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        <div class="botonera">
                                                            <input type="button" id="btnBuscar" name="btnBuscar" class="botonGeneral" value="<%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.btn.buscar")%>" onclick="buscar();">
                                                            <input type="button" id="btnLimpiar" name="btnLimpiar" class="botonGeneral" value="<%=meLanbide44I18n.getMensaje(idiomaUsuario, "proc.btn.limpiar")%>" onclick="limpiarFiltro();">
                                                        </div>
                                                    </td>
                                                </tr>
                                            </table>
...
// JAVASCRIPT DE LA TABLA AUDITORIA
    ...
    //TOTAL ANCHO: 928
    tabAud.addColumna('245','left','<%= meLanbide44I18n.getMensaje(idiomaUsuario, "proc.auditoria.nombre")%>');
    tabAud.addColumna('250','left','<%= meLanbide44I18n.getMensaje(idiomaUsuario, "proc.auditoria.proceso")%>');
    tabAud.addColumna('165','center','<%= meLanbide44I18n.getMensaje(idiomaUsuario, "proc.auditoria.fecha")%>');
    tabAud.addColumna('244','center','<%= meLanbide44I18n.getMensaje(idiomaUsuario, "proc.auditoria.resultado")%>');
