...
<script type="text/javascript">   
    function pulsarAltaPuestoCpe(){
        ...
        lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE59&operacion=cargarNuevoPuesto&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),600,980,'yes','yes', function(result){
        if (result != undefined){
            if(result[0] == '0'){
...
function pulsarModificarPuestoCpe(){
        ...
        if(tabPuestosCpe.selectedIndex != -1) {
            ...
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE59&operacion=cargarModificarPuesto&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&idPuesto='+listaPuestosCpe[fila][0]+'&control='+control.getTime(),600,980,'yes','yes', function(result){
            if (result != undefined){
                if(result[0] == '0'){
                    recargarTablaPuestosCpe(result);
                }
    ...
    function recargarTablaPuestosCpe(result){
        ...
        for(var i = 2;i< result.length; i++){
			...
			listaPuestosCpe[i-2] = [fila[0],fila[1], fila[2], fila[3], fila[4], fila[5], fila[6], fila[7], fila[8], fila[9]];
        ...
        tabPuestosCpe.scrollWidth = 1220;
...
<body>
    ...
    <div style="clear: both;">
        <div>
            <div id="puestosCpe" style="padding: 5px; width:76.4%; height: 159px; text-align: center; margin:0px;margin-top:0px;" align="center"></div>
            <div class="botonera">
...
<script type="text/javascript">   
    ...
    %>
        ...
        listaPuestosCpe[<%=i%>] = ['<%=voP.getCodPuesto()%>','<%=voP.getDescPuesto()%>', '<%=voP.getPais()%>', '<%=voP.getNivelFor()%>', '<%=titulacion%>', '<%=voP.getSubvSolic()%>', '<%=voP.getSubvAprob()%>', '<%=voP.getResultado()%>', '<%=voP.getMotivo()%>', '<%=voP.getObservaciones()%>'];
