...
<head><jsp:include page="/jsp/sge/tpls/app-constants.jsp" />
    <title>"<%= descriptor.getDescripcion("eMovExpIntNotifElec")%>"</title>        
    ...
    <script type="text/javascript" src="<c:url value='/scripts/extension/pluginnotificacionplatea/notificacion.js'/>"></script>
 ...
 {    
    if('<%=form.getCaducidadNotificacion()%>'=='-1') document.forms[0].caducidadNotificacion.value='';
        ...
        document.forms[0].textoNotificacion.value = unescape('<bean:write name="NotificacionForm" property="textoNotificacion"/>').replace(/&quot;/g,'"');
    ...
    <%ArrayList adjuntos = (ArrayList) form.getAdjuntos();
     if (adjuntos != null) {
        for (int i = 0; i < adjuntos.size(); i++) {
            ...
            if( (null==adjuntosVO.getEstadoFirma()) || ("F".equals(adjuntosVO.getEstadoFirma())) || ("V".equals(adjuntosVO.getEstadoFirma())) || ("M".equals(adjuntosVO.getEstadoFirma())) ){
                if("SI".equals(adjuntosVO.getSeleccionado())){
                %>
                    listaAdjuntos[j]=["<input type='checkbox' name='adjunto"+j+"' checked onchange='javascript:existenDatosSinGrabar();'>" ,'<%=adjuntosVO.getNombre()%>', getEstadoFirmaVisual('<%=adjuntosVO.getEstadoFirma()%>')];
                <%}else{%>
                    listaAdjuntos[j]=["<input type='checkbox' name='adjunto"+j+"' onchange='javascript:existenDatosSinGrabar();'>" ,'<%=adjuntosVO.getNombre()%>', getEstadoFirmaVisual('<%=adjuntosVO.getEstadoFirma()%>')];
            <%} }
            else{
                if("SI".equals(adjuntosVO.getSeleccionado())){
                %>
                    listaAdjuntos[j]=["<input type='checkbox' name='adjunto"+j+"' disabled='disabled' checked onchange='javascript:existenDatosSinGrabar();'>" ,'<%=adjuntosVO.getNombre()%>', getEstadoFirmaVisual('<%=adjuntosVO.getEstadoFirma()%>')];
                <%}else{%>
                    listaAdjuntos[j]=["<input type='checkbox' name='adjunto"+j+"' disabled='disabled' onchange='javascript:existenDatosSinGrabar();'>" ,'<%=adjuntosVO.getNombre()%>', getEstadoFirmaVisual('<%=adjuntosVO.getEstadoFirma()%>')];
            <%} }%>
            j++;
   ...
   if (autorizados != null) {
       for (int i = 0; i < autorizados.size(); i++) {
           ...
           // EN PANTALLA MOSTRAMOS EL CHECK SIEMPRE MARCADO. DA IGUAL SI TIENE EL ATRIBUTO seleccionado CON VALOR SI
           //if("SI".equals(autorizadosVO.getSeleccionado()))
           //{
           %>
                listaAutorizados[k]=["<input type='checkbox' name='autorizado"+k+"' disabled checked onchange='javascript:existenDatosSinGrabar();'>",'<%=autorizadosVO.getNif()%>',"<%=autorizadosVO.getNombreCompleto()%>"];
           <%//}else{%>
               // listaAutorizados[k]=["<input type='checkbox' name='autorizado"+k+"' readonly checked onchange='javascript:existenDatosSinGrabar();'>",'<%=autorizadosVO.getNif()%>',"<%=autorizadosVO.getNombreCompleto()%>"];
            <%//}%>
           k++;
      <% }
    }
   ...
   <%    
        ...
        for(int i=0;adjuntosExternos!=null && i<adjuntosExternos.size();i++){
            ...
            if(codNotificacionGeneral!=-1 && estadoFirma!=null && estadoFirma.equals("O") && !"F".equals(form.getEstadoNotificacion())){
                enlace = "<a href=\"#\" onclick=\"javascript:enviarFirmar(" + codExterno + ");\">Firmar</a>";
            }
   %>        
             
            listaAdjuntosExternos[h] = ['<%=fileNameExt%>','<%=estadoFirma%>','<%=codExterno%>'];     
            listaAdjuntosExternosTabla[h] = ['<%=fileNameExt%>','<%=descEstadoFirma%>','<%=descTipoCertificado%>','<%=enlace%>'];
  ...
  function verFirma(codAdjunto){     
      ...
      var result = abrirPopupFirma(url);
  }
  ...
  function firmarNotificacion(){    
      var codNotificacion = document.forms[0].codNotificacion.value;
      var url = "<%=request.getContextPath()%>/portafirmas/documentoportafirmas/PrepareFirmaNotificacion.do?codNotificacion=" + codNotificacion;
      var result = abrirPopupFirma(url);      
  ...
  function abrirPopupFirma(url){
        var opt ='unadorned:yes;resizable:1;dialogHeight:300px;dialogwidth:640px;scroll:no;status=no;resizable:no;';
        var result = window.showModalDialog("<html:rewrite page='/jsp/sge/mainVentana.jsp'/>?source=" + url,'ventana',opt);
        return result;
  }

  function enviarFirmar(adjunto){ 
      ...
      if(adjunto!=null){
          ...
          if(admiteFirmaCertificadoOrganismo=="true"){
              if(jsp_alerta("C","<%=descriptor.getDescripcion("msgTipoCertificadoNotificacion")%>")){
                  ...
                  var codNotificacion = document.forms[0].codNotificacion.value;
                  var codigoMunicipio = document.forms[0].codigoMunicipio.value;                  
                  document.forms[0].target = "oculto";
                  document.forms[0].action = "<%=request.getContextPath()%>/Notificacion.do?opcion=firmaCertificadoOrganismo&codAdjunto=" + adjunto + "&codNotificacion=" + codNotificacion + "&codOrganizacion=" + codigoMunicipio;
                  document.forms[0].submit();
              }else{
                  var codNotificacion = document.forms[0].codNotificacion.value;
                  ...
                  var url = "<%=request.getContextPath()%>/portafirmas/documentoportafirmas/PrepareFirmaDocExternoNotificacion.do?codAdjunto=" + adjunto + "&codNotificacion=" + codNotificacion;
                  var opt ='unadorned:yes;resizable:1;dialogHeight:300px;dialogwidth:640px;scroll:no;status=no;resizable:no;';                  
                  var result = abrirPopupFirma(url);
                  ...
                  if(result!=null && result.length>0){                      
                      ...
                      if(result[0]=="FIRMA_OK"){
                          ...
                          barraProgresoDocumento("on");
                          document.forms[0].target="oculto";
                          document.forms[0].action="<%=request.getContextPath()%>/Notificacion.do?opcion=actualizarListaAdjuntos&codNotificacion=" + codNotificacion;
                          document.forms[0].submit();
                      }else
                      if(result[0]=="FIRMA_NO_ALMACENADA"){
                          jsp_alerta("A","<%=descriptor.getDescripcion("msgErrGuardarFirmaDocNotif")%>");
            ...
            var url = "<%=request.getContextPath()%>/portafirmas/documentoportafirmas/PrepareFirmaDocExternoNotificacion.do?codAdjunto=" + adjunto + "&codNotificacion=" + codNotificacion + "&codProcedimiento=" + codProcedimiento + "&numExpediente=" + numExpediente + "&codigoTramite=" + codigoTramite + "&ocurrenciaTramite=" + ocurrenciaTramite + "&ejercicio=" + ejercicio + "&codMunicipio=" + codMunicipio;              
  ...
  function actualizarListaAdjuntosExternos(listaAdjuntos,listaAdjuntosTabla){
      ...
      document.forms[0].ficheroExterno.value = "";
  }
        ...
        jsp_alerta("A","<%=descriptor.getDescripcion("msgErrFirmarNotif")%>");
         ...
         for (i=0; i < listaAdjuntos.length; i++) {
             ...
             if(eval("document.forms[0]." + cajaAdjunto + ".checked") == true)
             {
                 ...
                 listaAdjuntosSeleccionados += 'SI'+'<%=separador%>';
             }
             else listaAdjuntosSeleccionados += 'NO'+'<%=separador%>';
         }
         ...
         for (i=0; i < listaAutorizados.length; i++) {
             ...
             if(eval("document.forms[0]." + cajaAutorizado + ".checked") == true)
             {
                ...
                listaAutorizadosSeleccionados += 'SI'+'<%=separador%>';
             }
             else listaAutorizadosSeleccionados += 'NO'+'<%=separador%>';
         }
 ...
 {   
     ...
     {     
        ...
        if (document.forms[0].actoNotificado.value == "" || document.forms[0].textoNotificacion.value == ""){
            jsp_alerta("A","<%=descriptor.getDescripcion("msgErrGuardarNotif")%>");             
        }else{
			 var selecAdjunto;
			 var selecAutoriz;
			 listaAdjuntosSeleccionados='';
			 ...
			 document.forms[0].textoNotificacion.value = reemplazarTodo(document.forms[0].textoNotificacion.value, "\r\n", ". ");
			 document.forms[0].textoNotificacion.value = reemplazarTodo(document.forms[0].textoNotificacion.value, "\n", ". ");
			 document.forms[0].textoNotificacion.value = reemplazarTodo(document.forms[0].textoNotificacion.value, "\r", ". ");
			 //alert(document.forms[0].textoNotificacion.value);
...
//Comprueba si existen adjuntos seleccionados
function enviarValidarAdjuntos() {
            var resultado=true;  
            var resultadoExterno= true;
            for (i=0; i < listaAdjuntos.length; i++) {
     ...
     if(eval("document.forms[0]." + cajaAdjunto + ".checked") == true) {
         ...
         for (i=0; i < listaAdjuntosExternos.length; i++) {
             ...
             var cajaAdjunto = "adjunto" + i;
               ...
               resultadoExterno=false;
               
         }
         
         
         
         if (resultado && resultadoExterno){
         return true;    
        }
         return false;
    }
...
function pulsarEnviar() {    
                if (document.forms[0].textoNotificacion.value == '') {
                    jsp_alerta('A', '<%=descriptor.getDescripcion("msjEnvioTextoVacio")%>');
                    parent.principal.mainFrame.barraProgresoDocumento("off");
                } else if (enviarValidarAdjuntos()) {
                    jsp_alerta("A", '<%=descriptor.getDescripcion("msjNotifAdjuntoVacio")%>');
    } else if (jsp_alerta("C", '<%=descriptor.getDescripcion("msjEnviarNotificacion")%>')) {
                    barraProgresoDocumento("on");
...
function adjuntarFicheroExterno(){
    ...
    if(dato!=null && dato!=undefined && dato!=""){
        var extension = dato.split('.').pop().toLowerCase();
        if (!(extension == 'xlsx' || extension == 'pdf')) {
            var resultado = jsp_alerta('A', '<%=descriptor.getDescripcion("msgExtArchivosNotif")%> <%=extensionesValidas %>');
        } else {
            barraProgresoDocumento("on");   
            document.forms[0].target = "oculto";
    ...
    function getEstadoFirmaVisual( codigoEstadoFirma ) {
        var result = "";
        
        // Si no requiere firma
        if ( ('null'==codigoEstadoFirma) || (null===codigoEstadoFirma) || (!codigoEstadoFirma) || (codigoEstadoFirma=='') ){
            result = "<fmt:message key='Sge.TramitacionExpedientesForm.EstadoFirma.Null'/>";
        }else {
            if (codigoEstadoFirma == 'E') {
                result = "<fmt:message key='Sge.TramitacionExpedientesForm.EstadoFirma.E'/>";
            } else if (codigoEstadoFirma == 'O') {
                result = "<fmt:message key='Sge.TramitacionExpedientesForm.EstadoFirma.O'/>";
            } else if (codigoEstadoFirma == 'T') {
                result = "<fmt:message key='Sge.TramitacionExpedientesForm.EstadoFirma.T'/>";
                ...
                result = "<fmt:message key='Sge.TramitacionExpedientesForm.EstadoFirma.F'/>";
            } else if (codigoEstadoFirma == 'R') {
                result = "<fmt:message key='Sge.TramitacionExpedientesForm.EstadoFirma.R'/>";
...
<div id="cuerpo">
    <div id="hidepage" >
        <table width="100%" border="0px" cellpadding="0px" cellspacing="0px" border="0px">
            <tr>
                <td align="center" valign="middle" style="height: 650px">
...
<table id="tabla1" style="width: 100%; height: 250px" cellpadding="0px" cellspacing="0px">
    <tr>
        <td>
            <table class="bordeExteriorPantalla" style="width: 100%" cellspacing="0px" cellpadding="1px">
                <tr>
                    <td>
                        <table class="bordeInteriorPantalla" style="width: 100%" cellspacing="1px">
                            <tr>
                                <td id="titulo" style="width: 100%; height: 25px" class="txttitblanco">&nbsp;<%=descriptor.getDescripcion("etiqDatosNotificacion")%></td>
                            </tr>
                            <tr>
                                ...
                                <td valign="top" style="padding-top: 5px">                                    
                                    <table cellpadding="1px" cellspacing="0px" border="0px">
                                                ...
                                                <tr>
                                                    ...
                                                    <td colspan="2" align="left">
														<html:text styleId="actoNotificado" property="actoNotificado" styleClass="inputTexto" size="60" readonly="" onchange="javascript:existenDatosSinGrabar(); quitarComillasSimples(this); limitar2000Caracteres(this);" onkeyup="quitarComillasSimples(this); limitar2000Caracteres(this);" onmouseover="titulo(this);"/>														
                                                    </td>
                                                ...
                                                <tr>
													...
													<html:textarea styleClass="textareaTexto" cols="125" rows="4"  property="textoNotificacion" onkeyup="quitarComillasSimples(this); limitar2000Caracteres(this);" onchange="javascript:existenDatosSinGrabar(); quitarComillasSimples(this); limitar2000Caracteres(this);"
                                                    value=""></html:textarea>
                                                    </td>
                                        ...
                                        <TR>
                                            <TD class="sub3titulo" colspan="4"><%=descriptor.getDescripcion("msgArchivosExtNotifPlatea")%></TD>                                            
                                        </TR>                                                                                    
                                        <tr>
                                        ...
                                        <tr>
                                            <td colspan="4" align="right">
                                                <input type="button" name="mostrarFicheros" id="mostrarFicheros" value="<%=descriptor.getDescripcion("etiqDocsExp")%>"  class="botonMasLargo" style="width:300px" onclick="pulsarDocumentosExpediente();"/>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td colspan="4" align="right">
                                                <input type="file" name="ficheroExterno" size="30"  accept=".xlsx,.pdf"/>
                                        ...
                                        <!-- prueba -->
                                        <TR>
                                            <TD class="sub3titulo" style="height: 18px"  align="left" colspan="4">&nbsp;<%=descriptor.getDescripcion("etiqAdjuntosPlatea")%></TD>
                                        </TR>
                                        <tr>
                                        ...
                                        <TR>
                                            <TD class="sub3titulo" style="height: 18px"  align="left" colspan="4">&nbsp;<%=descriptor.getDescripcion("etiqAutorizados")%>:</TD>
                                        </TR>
                                        <tr>
            ...
            <td class="sombraLateral">
                <table width="1px" cellpadding="0px" cellspacing="0px" border="0px">
                    ...
                    <tr>
                        <td style="height: 100%" class="parteInferior"></td>
                    </tr>
                </table>
            </td>
            <!-- Fin sombra lateral. -->
            ...
            <td colspan="2" class="sombraInferior">
                <table cellpadding="0px" cellspacing="0px" border="0px">
                    <tr>
                        ...
                        <td style="width: 100%" class="parteDerecha"></td>
                    </tr>
                </table>
            </td>
        </tr>
    ...
    <table border="0">                                        
        <tr>
            ...
            <td>                        
                <table height="4px" width="85%" cellpadding="0px" cellspacing="3px" border="0px">
                   <tr>
                       <td>
                       ...
                       <TD style="width:2px"></TD>
                       <td>
                           <input type= "button" class="botonGeneral" accesskey="E" value="<%=descriptor.getDescripcion("etiqEnviar")%>" name="cmdEnviar" id="cmdEnviar" onclick="pulsarEnviar();" disabled="true">
                       </td>
                       <TD style="width:2px"></TD>
                       <td>
...
<!-------------------------------------- BOTONES. ------------------------------------------>
    ...
    if(document.getElementById("actoNotificado").value != "")
        document.getElementById("cmdEnviar").disabled = false;
    //alert("disabled " + document.getElementById("cmdEnviar").disabled);
        
      var tablaAutorizados;
      if(document.all) tablaAutorizados = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.all.tabla);
      else tablaAutorizados = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.getElementById('tabla'));
      ...
      function checkKeysLocal(evento,tecla){
        ...
        var tablaDocumentos;
        if(document.all) tablaDocumentos = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.all.tablaDocumentos);
        else tablaDocumentos = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.getElementById('tablaDocumentos'));

        tablaDocumentos.addColumna('100','center',"");
        tablaDocumentos.addColumna('600','left',"<%= descriptor.getDescripcion("etiqNombreDocumento")%>");
        tablaDocumentos.addColumna('200','left',"<%= descriptor.getDescripcion("etiq_estadoFirma")%>");

        tablaDocumentos.displayCabecera=true;
        tablaDocumentos.height = 70;
        tablaDocumentos.displayDatos = pintaDatosDocumentos;

        /**** TABLA DOCUMENTOS ADJUNTOS EXTERNOS **/
        var tablaDocumentosExternos;
        if(document.all) tablaDocumentosExternos = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.all.tablaDocumentosExternos);
        else tablaDocumentosExternos = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.getElementById('tablaDocumentosExternos'));
        
        tablaDocumentosExternos.addColumna('900','left',"<%= descriptor.getDescripcion("etiqNombreDocumento")%>");
        tablaDocumentosExternos.addColumna('0','center',"<%=descriptor.getDescripcion("etiqEstadoFirmaNotif")%>");
        tablaDocumentosExternos.addColumna('0','center',"<%=descriptor.getDescripcion("etiqTipoCertificado")%>");
        tablaDocumentosExternos.addColumna('0','center',"");
        tablaDocumentosExternos.displayCabecera=true;
        tablaDocumentosExternos.height = 70; 
        tablaDocumentosExternos.displayDatos = pintaDatosDocumentosExternos;
        ...
        function pintaDatosDocumentosExternos(rowID){
            var i = tablaDocumentosExternos.selectedIndex;
            if(i != -1) {
    ...
    function pulsarDocumentosExpediente() {
        var codMunicipio = document.forms[0].codigoMunicipio.value;
        var codProcedimiento = document.forms[0].codigoProcedimiento.value;

        var ejercicio = document.forms[0].ejercicio.value;
        var numero = document.forms[0].numExpediente.value;
        var codTramite = document.forms[0].codigoTramite.value;
        var ocurrenciaTramite = document.forms[0].ocurrenciaTramite.value;
        var codNotificacion  = document.forms[0].codNotificacion.value;

        source = "<html:rewrite page='/Notificacion.do'/>?opcion=mostrarPantallaDocumentosExpediente" +
                "&codMunicipio=" + codMunicipio + "&codProcedimiento=" + codProcedimiento +
                "&ejercicio=" + ejercicio + "&numero=" + numero + "&codTramite=" + codTramite +
                "&ocurrenciaTramite=" + ocurrenciaTramite +  "&recargar=1";

        abrirXanelaAuxiliar('<%=request.getContextPath()%>/jsp/sge/mainVentana.jsp?source=' + source, 'ventana2', 'width=900,height=850', function(result){
      	            if(result!=null && result.length>0) {
      	                console.log("result: "+ result);
      	                var url = "<%=request.getContextPath()%>/Notificacion.do?opcion=seleccionarDocumentosExpediente" + result;
    ...
    <div id="comboDesplegable" style="overflow-y: auto; overflow-x: no; visibility: hidden; BORDER: 0px"></div>
