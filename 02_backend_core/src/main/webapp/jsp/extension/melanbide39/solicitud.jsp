...
<script type="text/javascript">   
    function pulsarAltaPuestoCpe(){
        ...
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE39&operacion=cargarNuevoPuesto&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),560,980,'yes','no', function(result){
							if (result != undefined){
								if(result[0] == '0'){
            ...
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE39&operacion=cargarNuevoPuesto&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),620,980,'yes','no', function(result){
							if (result != undefined){
								if(result[0] == '0'){
									recargarTablaPuestosCpe(result);								
								}
    ...
    function recargarTablaPuestosCpe(result){
        ...
        for(var i = 2;i< result.length; i++){
			...
			listaPuestosCpe[i-2] = [fila[0],fila[1], fila[2], fila[3], fila[4], fila[5], fila[6], fila[7], fila[8], fila[9], fila[10]];
