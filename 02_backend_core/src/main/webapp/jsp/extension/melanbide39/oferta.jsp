...
<script type="text/javascript">   
    function pulsarAltaOfertaCpe(){
        ...
        if(navigator.appName.indexOf("Internet Explorer")!=-1){
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE39&operacion=cargarNuevaOferta&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),560,980,'no','no', function(nodos){
							if (nodos != undefined){
            ...
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE39&operacion=cargarNuevaOferta&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),620,980,'no','no', function(nodos){
							if (nodos != undefined){
								result = extraerListadoOfertasCpe(nodos);
								if(result[0] == '0'){
									recargarTablaOfertasCpe(result);								
    ...
    function pulsarModificarOfertaCpe(){
        ...
        if(tabOfertasCpe.selectedIndex != -1) {
            ...
            if(navigator.appName.indexOf("Internet Explorer")!=-1){                
                lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE39&operacion=cargarModificarOferta&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&idOferta='+listaOfertasCpe[fila][0]+'&idPuesto='+listaOfertasCpe[fila][1]+'&control='+control.getTime(),560,980,'yes','no', function(nodos){
							if (nodos != undefined){
								result = extraerListadoOfertasCpe(nodos);
								if(result[0] == '0'){
									recargarTablaOfertasCpe(result);								
                 ...
                 lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE39&operacion=cargarModificarOferta&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&idOferta='+listaOfertasCpe[fila][0]+'&idPuesto='+listaOfertasCpe[fila][1]+'&control='+control.getTime(),620,980,'yes','no', function(nodos){
							if (nodos != undefined){
								result = extraerListadoOfertasCpe(nodos);
								if(result[0] == '0'){
									recargarTablaOfertasCpe(result);								
    ...
    function recargarTablaOfertasCpe(result){
        ...
        tabOfertasCpe.addColumna('250','left',"<%= meLanbide39I18n.getMensaje(idiomaUsuario,"label.solicitud.tabla.ofertas.col8")%>");     
