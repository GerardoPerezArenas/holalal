...
<script type="text/javascript">   
    ...
    function mostrarImpConCalculado(event){
        ...
        if(document.forms[0].modoConsulta.value == "si"){
            if(minimis != undefined && minimis != ''){
                resultado = jsp_alerta('A','<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.datosCme.impConCalculadoEs")%>: <br/>'+document.getElementById('concedidoReal').value+' '+'<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.euros")%>'+'<br/>'+'<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.avisoMinimis")%>'+' '+document.getElementById('minimisConce').value);
            }else{
                resultado = jsp_alerta('A','<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.datosCme.impConCalculadoEs")%>: <br/>'+document.getElementById('concedidoReal').value+' '+'<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.euros")%>');
            }
        }else{
            if(minimis != undefined && minimis != ''){
                resultado = jsp_alerta('','<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.datosCme.impConCalculadoEs")%>: <br/>'+document.getElementById('concedidoReal').value+' '+'<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.euros")%>'+'<br/>'+'<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.avisoMinimis")%>'+' '+document.getElementById('minimisConce').value+'<br/><br/>'+'<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.datosCme.preguntaAceptarImporte")%>');
            }else{
                resultado = jsp_alerta('','<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.datosCme.impConCalculadoEs")%>: <br/>'+document.getElementById('concedidoReal').value+' '+'<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.euros")%>'+'<br/><br/>'+'<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.datosCme.preguntaAceptarImporte")%>');
            }
        }
        if (resultado == 1){
    ...
    function mostrarImpPag2Calculado(event){
        ...
        if(document.forms[0].modoConsulta.value == "si"){    
            resultado = jsp_alerta('A','<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.datosCme.impPag2CalculadoEs")%>: <br/>'+valor+' '+'<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.euros")%>');
        }else{
            resultado = jsp_alerta('','<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.datosCme.impPag2CalculadoEs")%>: <br/>'+valor+' '+'<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.euros")%>'+'<br/><br/>'+'<%=meLanbide46I18n.getMensaje(idiomaUsuario, "msg.datosCme.preguntaAceptarImporte")%>');
        }
        if (resultado == 1){
     ...
     function consultarImportesResolucion(res){
        try{
            if(res != undefined && res != ''){
                var control = new Date();
                if(navigator.appName.indexOf("Internet Explorer")!=-1){
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE46&operacion=cargarImportesResolucion&res='+res+'&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),380,395,'yes','no',function(){
					});
                }else{
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE46&operacion=cargarImportesResolucion&res='+res+'&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),400,440,'yes','no',function(){
					});
                }
            }else{
...
<body>
    <div class="tab-page" id="tabPage391" style="height:480px; width: 100%; ">
        <h2 class="tab" id="pestana391" style="font-size:1em;"><%=meLanbide46I18n.getMensaje(idiomaUsuario,"label.datosCme.tituloPestanaHist")%></h2>
