...
<script type="text/javascript">   
    function pulsarAltaPuestoCme(){
        ...
        lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE46&operacion=cargarNuevoPuesto&tipo=0&numero=<%=numExpediente%>&control='+control.getTime(),600,980,'yes','no', function(result){
			if (result != undefined){
				if(result[0] == '0'){
    ...
    function pulsarModificarPuestoCme(){
       ...
       /* var idTabla = document.getElementById('puestosCme').children[0].children[1].children[0].children[0].id;
        ...
        if(tabPuestosCme.selectedIndex != -1) {
            ...
            lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE46&operacion=cargarModificarPuesto&tipo=0&numero=<%=numExpediente%>&opcion='+opcion+'&idPuesto='+listaPuestosCme[fila][0]+'&control='+control.getTime(),600,980,'yes','no', function(result){
				if (result != undefined){
					if(result[0] == '0'){
						recargarTablaPuestosCme(result);
					}
    ...
    function recargarTablaPuestosCme(result){
        ...
        tabPuestosCme.addColumna('300','left',"<%= meLanbide46I18n.getMensaje(idiomaUsuario,"label.solicitud.tabla.puestos.col1")%>", "<%= meLanbide46I18n.getMensaje(idiomaUsuario,"label.solicitud.tabla.puestos.col1.title")%>");
