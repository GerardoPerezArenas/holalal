...
<%@page import="es.altia.flexia.integracion.moduloexterno.melanbide90.vo.FilaMinimisVO"%>
...
<html>
    <head>
        ...
        <!--<script type="text/javascript" src="<%=request.getContextPath()%>/scripts/extension/melanbide90/FixedColumnsTable.js"></script>-->
        ...
        <script type="text/javascript">
            ...
            function pulsarNuevoMinimis() {
                lanzarPopUpModal(url + '?tarea=preparar&modulo=MELANBIDE90&operacion=cargarNuevoMinimis&tipo=0&nuevo=1&numExp=<%=numExpediente%>', 500, 1200, 'no', 'no', function (result) {
                    if (result != undefined) {
                        if (result[0] == '0') {
            ...
            function pulsarModificarMinimis() {
                if (tablaMinimis.selectedIndex != -1) {
                    lanzarPopUpModal(url + '?tarea=preparar&modulo=MELANBIDE90&operacion=cargarModificarMinimis&tipo=0&nuevo=0&numExp=<%=numExpediente%>&id=' + listaMinimis[tablaMinimis.selectedIndex][0], 500, 1200, 'no', 'no', function (result) {
                        if (result != undefined) {
                            if (result[0] == '0') {
                                recargarTablaMinimis(result[1]);
                            }
            ...
            function pulsarEliminarMinimis() {
                if (tablaMinimis.selectedIndex != -1) {
                    var resultado = jsp_alerta('', '<%=meLanbide90I18n.getMensaje(idiomaUsuario, "msg.preguntaEliminar")%>');
                    if (resultado == 1) {
                        pleaseWait('on');
                        var parametros = 'tarea=preparar&modulo=MELANBIDE90&operacion=eliminarMinimis&tipo=0&numExp=<%=numExpediente%>&id=' + listaMinimis[tablaMinimis.selectedIndex][0];
                        try {
                            $.ajax({
                                url: url,
                                type: 'POST',
                                async: true,
                                data: parametros,
                                success: procesarRespuestaEliminarMinimis,
            ...
            function iniciarTablaMinimis() {
                ...
                tablaMinimis.addColumna('0', 'center', "<%=meLanbide90I18n.getMensaje(idiomaUsuario,"minimis.tablaMinimis.id")%>");
                tablaMinimis.addColumna('100', 'center', "<%=meLanbide90I18n.getMensaje(idiomaUsuario,"minimis.tablaMinimis.estado")%>");
                tablaMinimis.addColumna('400', 'left', "<%=meLanbide90I18n.getMensaje(idiomaUsuario,"minimis.tablaMinimis.organismo")%>");
                tablaMinimis.addColumna('800', 'left', "<%=meLanbide90I18n.getMensaje(idiomaUsuario,"minimis.tablaMinimis.objeto")%>");
                tablaMinimis.addColumna('100', 'right', "<%=meLanbide90I18n.getMensaje(idiomaUsuario,"minimis.tablaMinimis.importe")%>");
                tablaMinimis.addColumna('100', 'center', "<%=meLanbide90I18n.getMensaje(idiomaUsuario,"minimis.tablaMinimis.fecha")%>" + "<%=meLanbide90I18n.getMensaje(idiomaUsuario,"minimis.tablaMinimis.asterisco")%>");
...
//            }
            ...
            function procesarRespuestaEliminarMinimis(ajaxResult) {
                ...
                if (codigoOperacion == "0") {
                    ...
                    if($('#MINIMIS').length > 0 ) {
                        var valorMinimisAplicarPantalla = $('#MINIMIS').val();
                        ...
                        if(valorMinimisAplicarPantalla != undefined && valorMinimisAplicarPantalla != valorMinimisAplicarActualizado)
                             $('#MINIMIS').val(valorMinimisAplicarActualizado);
                    }
                } else {
            ...
            function mostrarErrorPeticion(codigo) {
                ...
                if (codigo == "1") {
                    ...
                    jsp_alerta("A", '<%=meLanbide90I18n.getMensaje(idiomaUsuario,"error.errorGen")%>', msgtitle);
                } else if (codigo == "3") {
                    ...
                    jsp_alerta("A", '<%=meLanbide90I18n.getMensaje(idiomaUsuario,"error.operacionIncorrecta")%>', msgtitle);
                } else {
                    jsp_alerta("A", '<%=meLanbide90I18n.getMensaje(idiomaUsuario,"error.errorGen")%>', msgtitle);
                }
            }
    ...
    <body class="bandaBody">
        <div class="tab-page" id="tabPage901" style="height:520px; width: 100%;">
            <h2 class="tab" id="pestana901"><%=meLanbide90I18n.getMensaje(idiomaUsuario,"label.titulo.pestana")%></h2> 
            <script type="text/javascript">tp1.addTabPage(document.getElementById("tabPage901"));</script>            
            <br/>
            <h2 class="legendTema" align="center" id="pestanaPrinc"><%=meLanbide90I18n.getMensaje(idiomaUsuario,"label.tituloPrincipal")%></h2>
            <div>    
                <div id="divGeneral">     
                    <div id="listaMinimis"  align="center"></div>
                </div>
                <br>
                <div class="legendRojo" id="notaFecha"><%=meLanbide90I18n.getMensaje(idiomaUsuario,"label.notaFecha")%></div>
                <div class="botonera" style="text-align: center;">
                    <input type="button" id="btnNuevoMinimis" name="btnNuevoMinimis" class="botonGeneral"  value="<%=meLanbide90I18n.getMensaje(idiomaUsuario, "btn.nuevo")%>" onclick="pulsarNuevoMinimis();">
                    <input type="button" id="btnModificarMinimis" name="btnModificarMinimis" class="botonGeneral" value="<%=meLanbide90I18n.getMensaje(idiomaUsuario, "btn.modificar")%>" onclick="pulsarModificarMinimis();">
                    <input type="button" id="btnEliminarMinimis" name="btnEliminarMinimis"   class="botonGeneral" value="<%=meLanbide90I18n.getMensaje(idiomaUsuario, "btn.eliminar")%>" onclick="pulsarEliminarMinimis();">
                </div>
            </div>  
        </div>
        <script  type="text/javascript">
               ...
               if (List!= null && List.size() >0){
                   ...
                   for (int indice=0;indice<List.size();indice++)
                   {
                        ...
                        if(objectVO.getDesEstado()!=null){
                            ...
                            String barraSeparadoraDobleIdiomaDesple = ConfigurationParameter.getParameter(ConstantesMeLanbide90.BARRA_SEPARADORA_IDIOMA, ConstantesMeLanbide90.FICHERO_PROPIEDADES);
                            String[] descripcionDobleIdioma = (descripcion!=null?descripcion.split(barraSeparadoraDobleIdiomaDesple):null);
                            if(descripcionDobleIdioma!=null && descripcionDobleIdioma.length>1){
                                if(idiomaUsuario==ConstantesMeLanbide90.CODIGO_IDIOMA_EUSKERA){
