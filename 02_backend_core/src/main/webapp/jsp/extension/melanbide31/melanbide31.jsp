...
<script type="text/javascript">
    function actualizarValidacionesDatosSuplementarios(){
        ...
        parametros = 'tarea=preparar&modulo=MELANBIDE31&operacion=actualizarValidaciones&tipo=<%=codOrganizacion%>&numero=<%=numExpediente%>&control='+control.getTime();
        try{
    ...
    function recargarTabla(result){
        ...
        tabValidaciones = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.all.validaciones, 894);
    ...
    <div class="tab-page" id="tabPage311" style="height:520px; width: 100%;">
        ...
        <div style="clear: both;">
            <div>
                ...
                <div id="validaciones" align="center"></div>
            </div>
        </div>
    </div>
</body>
