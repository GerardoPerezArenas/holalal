        ...
        <script type="text/javascript">
            ...
            function ejecutarListadoCP() {
                ...
                if (navigator.appName.indexOf("Internet Explorer") != -1) {
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE37&operacion=cargarCriteriosFiltroListadoCP&tipo=0&numero=<%=numExpediente%>&control=' + control.getTime(), 330, 840, 'no', 'no', function (result) {
                        if (result != undefined) {
                            if (result[0] == '0') {
                    ...
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE37&operacion=cargarCriteriosFiltroListadoCP&tipo=0&numero=<%=numExpediente%>&control=' + control.getTime(), 400, 870, 'no', 'no', function (result) {
                        if (result != undefined) {
                            if (result[0] == '0') {
                                //recargarCriteriosListadoCP(result);
                                var fila;
            ...
            function ejecutarListadoComprobacionCP() {
                var control = new Date();
                if (navigator.appName.indexOf("Internet Explorer") != -1) {
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE37&operacion=cargarCriteriosFiltroListadoCompCP&tipo=0&numero=<%=numExpediente%>&control=' + control.getTime(), 330, 840, 'no', 'no', function (result) {
                        if (result != undefined) {
                    ...
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE37&operacion=cargarCriteriosFiltroListadoCompCP&tipo=0&numero=<%=numExpediente%>&control=' + control.getTime(), 400, 870, 'no', 'no', function (result) {
                        if (result != undefined) {
                            for (var i = 0; i < result.length; i++) {
                                var codigoCP = result[0];
                                var fecdesde = result[1];
            ...
            function solicitarTipoAcreditacion() {
                var control = new Date();
                if (navigator.appName.indexOf("Internet Explorer") != -1) {
                    lanzarPopUpModal('<%=request.getContextPath()%>/jsp/extension/melanbide37/procesos/solicitarTipoAcreditacion.jsp?idioma=<%=idiomaUsuario%>&control=' + control.getTime(), 120, 200, 'no', 'no', function (result) {

                    });
                } else {
                    lanzarPopUpModal('<%=request.getContextPath()%>/jsp/extension/melanbide37/procesos/solicitarTipoAcreditacion.jsp?idioma=<%=idiomaUsuario%>&control=' + control.getTime(), 120, 250, 'no', 'no', function (result) {
        ...
        <form id="formProcesos">
            <div style="height:550px; width: 100%;">
                <table width="100%" style="height: 550px;" cellpadding="0px" cellspacing="0px" border="0px">
                    <tr>
                        <td style="width:100%;  height:30px; padding-left: 15px" class="txttitblanco">CEPAP <%=meLanbide37I18n.getMensaje(idiomaUsuario, "proc.legend.listados")%></td>
                    </tr>                
                    <tr>
