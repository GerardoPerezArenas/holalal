...
<%@page import="es.altia.flexia.integracion.moduloexterno.melanbide50.vo.identificacionesp.IdentificacionEspVO"%>
...
<div class="tab-page" id="tabPage501" style="height:420px; width: 90%;">
    <div style="clear: both;">
        <label class="legendAzul" style="text-align: center; position: relative; left: 28%;"><%=meLanbide50I18n.getMensaje(idiomaUsuario, "identificacionesp.legend.titulo")%></label>
        <div id="divGeneral"  style="overflow-y: auto; overflow-x: hidden; height: 420px;">   
            <!--div id="listaIdentificacionEsp"></div-->
            <div>
                <jsp:include page="<%=urlNuevaIdentificacionEsp%>" flush="true"/>
            </div>
            <div class="botonera">
                <input type="button" id="btnNuevaIdentificacionEsp" name="btnNuevaIdentificacionEsp" style="display: none" class="botonGeneral" value="<%=meLanbide50I18n.getMensaje(idiomaUsuario, "btn.nuevo")%>" onclick="pulsarAltaIdentificacionEsp();">
                <input type="button" id="btnEliminarIdentificacionEsp" name="btnEliminarIdentificacionEsp"   style="display: none" class="botonGeneral" value="<%=meLanbide50I18n.getMensaje(idiomaUsuario, "btn.eliminar")%>" onclick="pulsarEliminarIdentificacionEsp();">
                <input type="button" id="btnModificarIdentificacionEsp" name="btnModificarIdentificacionEsp" style="display: none" class="botonGeneral" value="<%=meLanbide50I18n.getMensaje(idiomaUsuario, "btn.modificar")%>" onclick="pulsarModificarIdentificacionEsp();">
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    /*     //Tabla Identifiacacnioes
     ...
     tabIdentificacionEsp = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.getElementById('listaIdentificacionEsp'), 2000);
     tabIdentificacionEsp.addColumna('100','center',"<%=meLanbide50I18n.getMensaje(idiomaUsuario,"identificacionesp.tablaIdentificacionEsp.col1")%>");
     tabIdentificacionEsp.addColumna('500','left',"<%=meLanbide50I18n.getMensaje(idiomaUsuario,"identificacionesp.tablaIdentificacionEsp.col2")%>");
     tabIdentificacionEsp.addColumna('150','right',"<%=meLanbide50I18n.getMensaje(idiomaUsuario,"identificacionesp.tablaIdentificacionEsp.col3")%>");
     tabIdentificacionEsp.addColumna('150','right',"<%=meLanbide50I18n.getMensaje(idiomaUsuario,"identificacionesp.tablaIdentificacionEsp.col4")%>");
     tabIdentificacionEsp.addColumna('200','center',"<%=meLanbide50I18n.getMensaje(idiomaUsuario,"identificacionesp.tablaIdentificacionEsp.col5")%>");
     tabIdentificacionEsp.addColumna('400','left',"<%=meLanbide50I18n.getMensaje(idiomaUsuario,"identificacionesp.tablaIdentificacionEsp.col6")%>");
     tabIdentificacionEsp.addColumna('400','left',"<%=meLanbide50I18n.getMensaje(idiomaUsuario,"identificacionesp.tablaIdentificacionEsp.col7")%>");
...
<script type="text/javascript">
    ...
    function pulsarAltaIdentificacionEsp() {
        ...
        var result = null;
        if (navigator.appName.indexOf("Internet Explorer") != -1) {
            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarNuevaIdentificacionEsp&tipo=0&numero=<%=numExpediente%>&control=' + control.getTime(), 400, 850, 'no', 'no');
        } else {
            result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarNuevaIdentificacionEsp&tipo=0&numero=<%=numExpediente%>&control=' + control.getTime(), 400, 850, 'no', 'no');
        }
        if (result != undefined) {
    ...
    function pulsarModificarIdentificacionEsp() {
        if (tabIdentificacionEsp.selectedIndex != -1) {
            ...
            var result = null;
            if (navigator.appName.indexOf("Internet Explorer") != -1) {
                result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarModifIdentificacionEsp&tipo=0&numero=<%=numExpediente%>&id=' + listaIdentificacionEsp[tabIdentificacionEsp.selectedIndex][0] + '&idEspSol=' + listaIdentificacionEsp[tabIdentificacionEsp.selectedIndex][1] + '&control=' + control.getTime(), 400, 850, 'no', 'no');
            } else {
                result = lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarModifIdentificacionEsp&tipo=0&numero=<%=numExpediente%>&id=' + listaIdentificacionEsp[tabIdentificacionEsp.selectedIndex][0] + '&idEspSol=' + listaIdentificacionEsp[tabIdentificacionEsp.selectedIndex][1] + '&control=' + control.getTime(), 400, 850, 'no', 'no');
            }
            if (result != undefined) {
    ...
    function pulsarEliminarIdentificacionEsp() {
        if (tabIdentificacionEsp.selectedIndex != -1) {
            ...
            if (resultado == 1) {
                ...
                var parametros = "";
                var control = new Date();
                parametros = 'tarea=preparar&modulo=MELANBIDE50&operacion=eliminarIdentificacionEsp&tipo=0&numero=<%=numExpediente%>&id=' + listaIdentificacionEsp[tabIdentificacionEsp.selectedIndex][0] + '&control=' + control.getTime();
                try {
