    ...
    <body>
        <div class="tab-page" id="tabPage501" style="height:420px; width: 98%;">
            <div style="clear: both;">
                <label class="legendAzul" style="text-align: center; position: relative; left: 18%;"><%=meLanbide50I18n.getMensaje(idiomaUsuario, "material.legend.titulo")%></label>
        ...
        <script type="text/javascript">
            ...
            function pulsarAltaMaterial() {
                ...
                var result = null;
                lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarNuevoMaterial&tipo=0&numero=<%=numExpediente%>&control=' + control.getTime() + '&idEpsol=<%=datoEspecialidad.getId()%>', 550, 900, 'no', 'no', function (result) {
                    if (result != undefined) {
                        if (result[0] == '0') {
            ...
            function pulsarModificarMaterial() {
                if (tabMaterial.selectedIndex != -1) {
                    var control = new Date();
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarModifMaterial&tipo=0&numero=<%=numExpediente%>&id=' + listaMaterial[tabMaterial.selectedIndex][0] + '&control=' + control.getTime() + '&idEpsol=<%=datoEspecialidad.getId()%>', 550, 900, 'no', 'no', function (result) {
                        if (result != undefined) {
                            if (result[0] == '0') {
                                recargarTablaMaterial(result);
                            }
            ...
            function recargarTablaMaterial(result) {
                ...
                for (var i = 1; i < result.length; i++) {
                    fila = result[i];
                    //listaMaterial[i-1] = fila;//no funciona en IE9
                    listaMaterial[i - 1] = [fila[0], fila[1], fila[2]];
