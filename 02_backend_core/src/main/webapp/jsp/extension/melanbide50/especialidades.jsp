    ...
    <body>
        ...
        <script type="text/javascript">
            ...
            /*//IE9 comentamos
             ...
             try {
             var div = document.getElementById('divlistaEspecialidades');
             ...
             document.getElementById('divlistaEspecialidades').children[0].children[1].children[0].children[0].ondblclick = function (event) {
        ...
        <script type="text/javascript">
            function pulsarAltaEspecialidad() {
                ...
                var result = null;
                lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarNuevaEspecialidad&tipo=0&numero=<%=numExpediente%>&control=' + control.getTime(), 550, 900, 'no', 'no', function (result) {
                    if (result != undefined) {
                        if (result[0] == '0') {
            ...
            function pulsarModificarEspecialidad() {
                if (tabEspecialidades.selectedIndex != -1) {
                    var control = new Date();
                    var result = null;
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarModifEspecialidad&tipo=0&numero=<%=numExpediente%>&id=' + listaEspecialidades[tabEspecialidades.selectedIndex][0] + '&control=' + control.getTime(), 550, 900, 'no', 'no', function (result) {
                        if (result != undefined) {
                            if (result[0] == '0') {
                                recargarTablaEspecialidades(result);
                                actualizarDatosDisponibilidad();
            ...
            function recargarTablaEspecialidades(result) {
                ...
                for (var i = 1; i < result.length; i++) {
                    fila = result[i];
                    //listaEspecialidades[i-1] = fila;//NO FUNCIONA IE9
            ...
            function pulsarListasxEspecialidad(event) {
                ...
                if (fila >= 0 && fila < listaEspecialidades.length) {
                    var control = new Date();
                    //var result = null;
                    lanzarPopUpModal('<%=request.getContextPath()%>/PeticionModuloIntegracion.do?tarea=preparar&modulo=MELANBIDE50&operacion=cargarListasxEspecialidad&tipo=0&numero=<%=numExpediente%>&id=' + listaEspecialidades[fila][0] + '&control=' + control.getTime(), 600, 1400, 'no', 'no', function (result) {
                        if (result != undefined) {
                            if (result[0] == '0') {
                                recargarTablaEspecialidades(result);
                            }
