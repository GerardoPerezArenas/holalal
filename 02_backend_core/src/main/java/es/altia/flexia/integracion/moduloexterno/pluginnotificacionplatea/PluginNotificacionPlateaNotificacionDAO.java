...
public class PluginNotificacionPlateaNotificacionDAO {
    ...
    protected static Logger m_Log =
            LogManager.getLogger(PluginNotificacionPlateaNotificacionDAO.class.getName());
...
//#282252
      ...
      int resultado = st.executeUpdate(sql);
            m_Log.debug("PluginNotificacionPlateaNotificacionDAO.updateNotificacion notificación actualizadas: " + resultado);
 ...
 public boolean updateNotificacionXML(NotificacionBVO notificacion, String objetoNotificacion, AdaptadorSQLBD obd) throws Exception {
    ...
    try{
      ...
      String numeroExpediente=notificacion.getNumExpediente();
      int codNotificacion = notificacion.getCodigoNotificacion();
            Reader xml = new StringReader(objetoNotificacion);

      //      sql = "UPDATE NOTIFICACION SET XML_NOTIFICACION = '" + objetoNotificacion + "'" + " WHERE CODIGO_NOTIFICACION=" + codNotificacion + " AND NUM_EXPEDIENTE='" + numeroExpediente + "'";
            sql = "UPDATE NOTIFICACION SET XML_NOTIFICACION = ? WHERE CODIGO_NOTIFICACION = ? AND NUM_EXPEDIENTE = ?";
            m_Log.debug(sql);
            int i = 1;
            ...
            m_Log.debug("PluginNotificacionPlateaNotificacionDAO.updateNotificacionXML notificación xml actualizado: " + resultado);
            if (resultado >= 1) {
                exito = true;
            }
  ...
  public NotificacionVO getNotificacion(NotificacionVO notificacion, Connection con) throws TechnicalException {
    ...
    public ArrayList<NotificacionBVO> recogerNotificacionesPendientes(AdaptadorSQLBD adapt) throws Exception  {
        ...
        try{
            ...
            if(m_Log.isDebugEnabled()) m_Log.debug("Recogiendo notificaciones pendientes: " + sql);
            st = con.createStatement();
            rs = st.executeQuery(sql);
            ...
            if (notificaciones.size() > 0){
                m_Log.debug("Se actualizaran " + notificaciones.size() + " notificaciobnes");
                actualizarNotificacionesEnTratamiento(notificaciones, con);
            }else{
                m_Log.debug("No hay notificaciones a tratar");
            }
    ...
    public boolean documentoNotificado(String numExp, String oid, AdaptadorSQLBD adapt) throws BDException  {
        ...
        try{
            ...
            if(m_Log.isDebugEnabled()) m_Log.debug("Comprobando si el documento ha sido notificado" + sql);
            st = con.createStatement();
            rs = st.executeQuery(sql);

            if(rs.next()){ 
                String notif = rs.getString("RELDOC_PREPNOTIF");
                m_Log.debug("RELDOC_PREPNOTIF: " + notif);
                if(("1").equals(notif)){
                    notificado = true;
                    m_Log.debug("Documento notificado");
                }
                m_Log.debug("Documento notificado: " + notificado);
            }
    ...
    */
     ...
     public boolean obtieneLocalizador (String proce, int codTramite, String numExp, Connection con) throws Exception
    {
        ...
        try
        {
            ...
            m_Log.debug("Documento con localizador: " + localizador);
        }
        catch(Exception ex)
        {
            //insertaError(con, ex.toString(), id, "actualizarProcesados");
    ...
    public String getValorCampoSuplementarioExpteTipoTextoxCodNumExp(String numExpediente, String codigoCampo,Connection con) throws Exception  {
        ...
        Statement st = null;
        String valorCampo="";
        m_Log.debug("getValorCampoSuplementarioExpteTipoTextoxCodNumExp - Begin() " + numExpediente + "-" + codigoCampo);
        try{
        ...
        m_Log.debug("getValorCampoSuplementarioExpteTipoTextoxCodNumExp - End () " + valorCampo);
        return valorCampo;
    }
    ...
    public static ArrayList<AutorizadoNotificacionVO> getInteresadosExpedientePluginNotificacion(String numExpediente, int codigoNotificacion, Connection con) throws TechnicalException {
        ...
        try {
            ...
            while (rs.next()) {
                m_Log.debug("-- Traza leyendo Datos Terceros Expedientes Notificacion " + numExpediente);
            ...
            rs.close();
            m_Log.debug("-- Traza leyendo Datos Terceros Expedientes Notificacion - ArrayInteresado " + arrayAux.size());
        ...
        m_Log.debug("-- Traza leyendo Datos Terceros Expedientes Notificacion - arrayRetorno " + arrayAux.size());
        return arrayRetorno;
    }
