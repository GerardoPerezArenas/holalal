...
public class MeLanbideInteropManager {
    ...
    //Logger
    private static Logger log = LogManager.getLogger(MeLanbideInteropManager.class);
    ...
    public List<RegistroVidaLaboralVO> getDatosVidaLaboralIntermediacion(final String numExp, final String documentacion,
        ...
        try {
            ...
            if (registros.isEmpty() && meLanbideInteropVidaLaboralDAO.peticionPreviaVidaLaboral(numExp,documentacion, con)) {
                registros = meLanbideInteropVidaLaboralDAO.getRegistrosVidaLaboralByDoc(documentacion, con);
            }
        } catch (BDException e) {
            log.error("Se ha producido una excepcion recuperando la conexión a BBDD", e);
            throw new Exception(e);
    ...
    // Remiendo para LAK para que haya retrocompatibilidad para cuando no existia la columna NUMEXP en INTEROP_VIDALABORAL
    public String getDatosSuplementariosNIFPersonaContratada(String numeroExpediente, AdaptadorSQLBD adaptador) throws Exception {
        ...
        Connection con = null;
        MeLanbideInteropVidaLaboralDAO meLanbideInteropVidaLaboralDAO = MeLanbideInteropVidaLaboralDAO.getInstance();

        try {
            con = adaptador.getConnection();
            nifPersonaContratada = meLanbideInteropVidaLaboralDAO.getNIFPersonaContratada(numeroExpediente, con);
        } catch (BDException e) {
            log.error("Se ha producido una excepcion recuperando la conexión a BBDD", e);
            throw new Exception(e);
        } catch (Exception ex) {
