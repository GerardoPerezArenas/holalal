...
public class MeLanbide43Manager {
    ...
    //Logger
    private static Logger log = LogManager.getLogger(MeLanbide43Manager.class);
...
//    private static final Lan6Config lan6Config = new Lan6Config();
    ...
    public String generarMisGestiones(int codOrganizacion, int codTramite, int ocurrenciaTramite, String numExp, String evento, AdaptadorSQLBD adaptador) throws Exception {
        if (log.isDebugEnabled()) {
            log.info("generarMisGestiones ( codOrganizacion = " + codOrganizacion + " codTramite = " + codTramite + " ocurrenciaTramite = " + ocurrenciaTramite + " numExp = " + numExp + " evento = " + evento + " ) : BEGIN");
        }
        ...
        try {
            if (numExp != null && !"".equals(numExp)) {
                ...
                if (instancia || sinSolicitud || (!evento.equals("I") && !instancia)) {
                    ...
                    if (codProcedimiento.equalsIgnoreCase("RGI") || codProcedimiento.equalsIgnoreCase("IMV")) {
                        publicar = getCampoNumericoEnteroExpediente(numExp, "PUBSEDE", adaptador);
                    }
                    log.info("Publicar en Sede: " + publicar);
                    if (publicar == 1) {
                        Date date = new Date();
                        ...
                        if (par.getTipoID() != 0) {
                            ...
                            if (fecha != "") {
                                ...
                                if (date.after(fec)) {
                                    boolean iniciadoEnMiCarpeta = MeLanbide43Manager.getInstance().iniciadoEnMiCarpeta(numExp, adaptador);
                                    if (evento.equals("C")) {
                                        if (!meLanbide43Manager.expedienteCerrado(numExp, adaptador)) {
                                            ...
                                            codigoOperacion = avanzarGestiones(id, String.valueOf(codOrganizacion), adaptador);
                                                ...
                                                log.info("El expediente: " + numExp + " no está en MiCarpeta");
                                            }
                                        } else {
                                            log.info("El expediente: " + numExp + " ya había sido cerrado");
                                        }
                                    } else if (evento.equals("I")) {
                                        //boolean iniciadoEnMiCarpeta = MeLanbide43Manager.getInstance().iniciadoEnMiCarpeta(numExp, adaptador);
                                        boolean realizarGestión = true;
                                        if (iniciadoEnMiCarpeta) {
                                            // compruebo si ademas de Iniciado esta Cerrado                                            
                                            if (cerradoEnMiCarpeta(numExp, adaptador)) {
                                                //evento "P" para reapertura de expediente
                                                evento = "P";
                                            } else {
                                                // no se llama a ninguna operacion
                                                realizarGestión = false;
                                            }
                                        }
                                        if (realizarGestión) {
                                            id = guardarGestiones(numExp, codOrganizacion, adaptador, codTramite, evento);
                        ...
                        log.error("El expediente " + numExp + " no se publica en sede.");
                    }
            ...
            log.error("Error en la función generarMisGestiones: ", ex);
            String error = "Error en la función generarMisGestiones: " + ex.getMessage() != null ? ex.getMessage().toString() : "null";

            ErrorBean errorB = new ErrorBean();
            errorB.setIdError("MISGEST_012");
            errorB.setMensajeError("Error al iniciar un expediente en la función generarMisGestiones");
        ...
        if (log.isDebugEnabled()) {
            log.debug("generarMisGestiones() : END");
        }
    ...
    public boolean comprobarExpGenerado(int codOrganizacion, int codTramite, int ocurrenciaTramite, String numExp, Connection con) throws Exception {
        if (log.isDebugEnabled()) {
            log.info("comprobarExpGenerado ( codOrganizacion = " + codOrganizacion + " codTramite = " + codTramite + " ocurrenciaTramite = " + ocurrenciaTramite + " numExp = " + numExp + " ) : BEGIN");
        }
        MeLanbide43DAO meLanbide43DAO = MeLanbide43DAO.getInstance();

        if (log.isDebugEnabled()) {
            log.debug("comprobarExpGenerado() : END");
        }
                    ...
                    if (unidad.getIntentos() < 3) {
                        ...
                        switch (tipo[0]) {
                            case 'I':   //inicio expediente
                                ...
                                break;
                            case 'P':   //reapertura expediente
                                codOperacion = reaperturaExpConsulta(unidad, codOrg, adaptador);
                                break;
    ...
    public String iniciarExpConsulta(FilaListadoMisGestiones gestion, String codOrg, AdaptadorSQLBD adaptador) throws Exception {
        ...
        Connection con = null;
        try {
            log.info("En función iniciarExpConsulta()");
...
//                idProcedimiento = convierteProcedimiento(pepe[1]);
            ...
            log.info("new informar consulta servicios - " + pepe[1]);
            Lan6DokusiServicios dokusi = new Lan6DokusiServicios(pepe[1]);
            ...
            if (pepe[1].equals("RECUR")) {
                ...
                if (exp.getNumExpRel() != null && !exp.getNumExpRel().equals("")) {
                    log.debug("Expediente relacionado: " + exp.getNumExpRel());
...
//                        idProcedimiento = convierteProcedimiento(proc[1]);
                        idProcedimiento = gestionAdaptadoresLan6Config.getCodProcedimientoPlatea(proc[1]); //convierteProcedimiento(codProcedimiento);
            ...
            //tramite - mÃ­nimo el de apertura
            ...
            log.debug("Apellido 1 : " + par.getApe1());
...
//                lan6Expediente.setAportacionesCiudadano(aportacionesCiudadano);
            ...
            if (null == doclan6) {
                ...
                if (null != docAportados) {
                    ...
                    for (String aport : docAportados) {
                        ...
                        log.debug("hay documento. Datos-: " + aport);
                ...
                lan6Aportacion.setTipoAportacion(Lan6Constantes.TIPO_APORTACION_APPLICANT);
                lan6Aportacion.setTipoTramite(Lan6Constantes.TRAMITE_ABRIR_EXPEDIENTE);
                log.debug("documentos aportados");
                lan6Aportacion.setDocumentos(documentos);
                lan6Aportacion.setFechaAportacion(fechaApertura);
                log.debug("fecha solicitud");
            ...
            String error = "Error en la función iniciarExpConsulta: " + ex.getException().getMessage();
            ...
            String error = "Error en la función iniciarExpConsulta: " + ex.getMessage();
            ...
            try {
                ...
                log.error("Error en funcion iniciarExpConsulta: ", ex);
            }
        }
        return codOperacion;
    }

    public String reaperturaExpConsulta(FilaListadoMisGestiones gestion, String codOrg, AdaptadorSQLBD adaptador) throws Exception {
        String codOperacion = "0";
        String expe = gestion.getNumExp();
        String[] pepe = expe.split("/");
        int id = gestion.getId();
        ...
        try {
            log.info("En función reaperturaExpConsulta()");
...
//                idProcedimiento = convierteProcedimiento(pepe[1]);
            ...
            //llamada al metodo
            //numExpediente, ejercicio y observaciones (motivo de cancelación de cierre o reapertura)
            lan6Expediente.setObservaciones("Reapertura de expediente por necesidades de gestión");
            lan6Expediente.setObservacionesEu("Espedientea berriz irekitzea kudeaketa-beharrengatik");
            log.info("Parametros enviados cancelarCerrarExpediente - lan6Expediente : " + gson.toJson(lan6Expediente));
            servicios.cancelarCerrarExpediente(lan6Expediente);

            eliminarGestionesExpedienteCerradoMiCarpeta(expe, con);
            ...
            ErrorBean errorB = new ErrorBean();
            errorB.setIdError("MISGEST_013");
            errorB.setMensajeError("Error Lan6InformarConsultaExcepcion al reabrir un expediente");
            errorB.setSituacion("reaperturaExpConsulta");

            MeLanbide43Manager.grabarError(errorB, error, causa, gestion.getNumExp());
            throw ice;
        } catch (Lan6Excepcion ex) {
            codOperacion = "2";
            log.error("Error en la función reaperturaExpConsulta: " + ex.getException().getMessage());
            String error = "Error en la función reaperturaExpConsulta: " + ex.getException().getMessage();

            ErrorBean errorB = new ErrorBean();
            errorB.setIdError("MISGEST_012");
            errorB.setMensajeError("Error al reabrir un expediente");
            errorB.setSituacion("reaperturaExpConsulta");
            ...
            codOperacion = "2";
            log.error("Error en la función reaperturaExpConsulta: ", ex);
            String error = "Error en la función reaperturaExpConsulta: " + ex.getMessage();
            ...
            try {
                ...
                log.error("Error en funcion reaperturaExpConsulta: ", ex);
            }
        }
        return codOperacion;
    }
...
//                idProcedimiento = convierteProcedimiento(pepe[1]);
            ...
            log.info("Recogemos los datos del trámite de inicio " + gestion.getTramiteInicio().toString());
            ...
            log.debug("Comenzamos a llenar lan6Expediente. Trámite: " + tra.getCod().toString());
            ...
            log.debug("Expedientes relacionados");
            if (idProcedimiento.equals(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_PROC_RECUR))) {
...
//                        idProcedimiento = convierteProcedimiento(proc[1]);
                    ...
                    log.info("Parametros enviados relacionarExpedientes - lan6Expediente : " + gson.toJson(lan6Expediente));
            ...
            //Llamada metodo    
            log.debug("Procedemos a volcar la informacion de los tramites");
            ...
            try {
                ...
                log.error("Error en la función avanceExpConsulta: " + e.getMessage());
            ...
            codOperacion = "2";
            log.error("Error en la función avanceExpConsulta: " + ex.getException().getMessage());
            String error = "Error en la función avanceExpConsulta: " + ex.getException().getMessage();
            ...
            try {
                ...
                log.error("Error en funcion avanceExpConsulta: ", ex);
            }
        }
        return codOperacion;
...
//                    idProcedimiento = convierteProcedimiento(pepe[1]);
            ...
            //Llamada metodo       	
            log.info("Parametros enviados cerrarExpediente - tramites : " + gson.toJson(lan6Expediente));
            ...
            try {
                ...
                log.error("Error en la función cierreConsulta: " + e.getMessage());
            }
            ErrorBean errorB = new ErrorBean();
            ...
            codOperacion = "2";
            log.error("Error en la función cierreExpConsulta: " + ex.getException().getMessage());
            String error = "Error en la función cierreExpConsulta: " + ex.getException().getMessage();
            ...
            codOperacion = "2";
            log.error("Error en la función cierreExpConsulta: " + ex.getMessage());
            try {
                int intentos = gestion.getIntentos() + 1;
            ...
            try {
                ...
                log.error("Error en funcion cierreExpConsulta: ", ex);
            }
        }
        return codOperacion;
    }
    ...
    public Lan6Tramite setLan6TramiteEsperas(String numExp, int codTramite, Lan6Tramite lan6Tramite) {
        ...
        try {
            ...
            //estaba asi, por defecto se toman estos para los que no tienen
            ...
            lan6Tramite.setDescripcion(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION_DESC"));
            lan6Tramite.setDescripcionEu(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION_DESC_EU"));
            ...
            if (exp[1].equals("CEI")) {
                ...
                log.debug("tramSeg: " + tramSeg);
                log.debug("tramTer: " + tramTer);
                // Segun tramite
                if (codTramite == tramSeg) {//JUSTIFICACION CONTABLE PARA SEGUNDO PAGO
                    log.debug("tramite 23");
//                    lan6Tramite.setInstanciaId(ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_PAGO2_LAN68_CEI, ConstantesMeLanbide43.FICHERO_ADAPTADORES));// Apertura del tramite para el segundo pago
                    lan6Tramite.setInstanciaId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_PAGO2_LAN68_CEI));// Apertura del tramite para el segundo pago
...
//                    lan6Tramite.setInstanciaId(ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_PAGO_SEM1_ANU4_LAN68_SEI, ConstantesMeLanbide43.FICHERO_ADAPTADORES));
                    ...
                    lan6Tramite.setInstanciaId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_PAGO_SEM2_ANU4_LAN68_SEI)); //2.2
...
//                    lan6Tramite.setInstanciaId(ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_PAGO_FINAL_ANU4_LAN68_SEI, ConstantesMeLanbide43.FICHERO_ADAPTADORES));
                ...
                if (codTramite == tramRecepDocpago2) {// Tramite 399 â€“ Recepcion telematica documentacion 2º pago
                    log.debug("tramite 399");
...
//                    lan6Tramite.setInstanciaId(ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_DOC_AVAL_LAN62_UAAP, ConstantesMeLanbide43.FICHERO_ADAPTADORES));
                    lan6Tramite.setInstanciaId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_DOC_AVAL_LAN62_UAAP)); //2.2
...
//                    lan6Tramite.setInstanciaId(ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_DOC_LIQ_LAN62_UAAP, ConstantesMeLanbide43.FICHERO_ADAPTADORES));
                ...
                log.debug("procedimiento " + exp[1] + "- tramite:" + codTramite);
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_ALTA_PER_ORI14); // 2.2
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_DOC_TRAM_ORI14); // 2.2
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
                    lan6Tramite.setInstanciaId(instancia);
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    lan6Tramite.setDescripcion(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_APORTACION_DOC_DESC"));
                    lan6Tramite.setDescripcionEu(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_APORTACION_DOC_DESC_EU"));
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_INI_ACTIVIDAD_ORI14); // 2.2
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
                    lan6Tramite.setInstanciaId(instancia);
...
//                    String instancia=ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_SUB_INI_ACTIVIDAD_LAN62_ORI14, ConstantesMeLanbide43.FICHERO_ADAPTADORES);
                    lan6Tramite.setDescripcion(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_SUBSANACION_DOC_DESC"));
                    lan6Tramite.setDescripcionEu(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_SUBSANACION_DOC_DESC_EU"));
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_SUB_INI_ACTIVIDAD_LAN62_ORI14); // 2.2
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_SUBSANACION"));
                    lan6Tramite.setInstanciaId(instancia);
...
//                    lan6Tramite.setDescripcionEu(Lan6Constantes.TRAMITE_ALEGACION_DOC_DESC);
                    ...
                    //se deja la espera inicial porque no se permite desde Platea más de 1 instancia en Alegaciones, sí en Requerimiento, lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION);//112
                    ...
                    //String instancia=ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_APOR_DOC_TRAM_ORI14, ConstantesMeLanbide43.FICHERO_ADAPTADORES);
//                    String instancia=ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_APOR_ALEGA_RESOLPROV_ORI14, ConstantesMeLanbide43.FICHERO_ADAPTADORES);
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    lan6Tramite.setDescripcion(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ALEGACION_DOC_DESC"));
                    lan6Tramite.setDescripcionEu(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ALEGACION_DOC_DESC_EU"));
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_ALEGA_RESOLPROV_ORI14); // 2.2
                    lan6Tramite.setInstanciaId(instancia);
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_RECURSO_POTESTATIVO);
                    ...
                    lan6Tramite.setDescripcion(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_RECURSO_POTESTATIVO_DOC_DESC"));
                    lan6Tramite.setDescripcionEu(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_RECURSO_POTESTATIVO_DOC_DESC_EU"));
                }
            } else if (exp[1].equals("AERTE")) {
                // Solo usamos para abrir Presentacion de recursos -Invocacio
                log.debug("procedimiento AERTE - tramite:" + codTramite);
...
//                    lan6Tramite.setDescripcionEu(Lan6Constantes.TRAMITE_RECURSO_POTESTATIVO_DOC_DESC);
                ...
                log.debug("procedimiento CEMP- tramite:" + codTramite);
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    lan6Tramite.setDescripcion(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_APORTACION_DOC_DESC"));
                    lan6Tramite.setDescripcionEu(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_APORTACION_DOC_DESC_EU"));
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_ALTA_PER_CEMP); // 2.2
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
                    lan6Tramite.setInstanciaId(instancia);
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_DOC_TRAM_CEMP); // 2.2
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
                    lan6Tramite.setInstanciaId(instancia);
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    lan6Tramite.setDescripcion(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_APORTACION_DOC_DESC"));
                    lan6Tramite.setDescripcionEu(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_APORTACION_DOC_DESC_EU"));
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_INI_ACTIVIDAD_CEMP); // 2.2
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
                    lan6Tramite.setInstanciaId(instancia);
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_SUBSANACION);
                    lan6Tramite.setDescripcion(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_SUBSANACION_DOC_DESC"));
                    lan6Tramite.setDescripcionEu(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_SUBSANACION_DOC_DESC_EU"));
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_SUB_INI_ACTIVIDAD_LAN62_CEMP); // 2.2
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_SUBSANACION"));
                    lan6Tramite.setInstanciaId(instancia);
...
//                    lan6Tramite.setDescripcionEu(Lan6Constantes.TRAMITE_ALEGACION_DOC_DESC);
                    ...
                    //se deja la espera inicial porque no se permite desde Platea más de 1 instancia en Alegaciones, sí en Requerimiento, lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION);//112
                    //String instancia=ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_APOR_DOC_TRAM_CEMP, ConstantesMeLanbide43.FICHERO_ADAPTADORES);
//                    String instancia=ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_APOR_ALEGA_RESOLPROV_CEMP, ConstantesMeLanbide43.FICHERO_ADAPTADORES);
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_ALEGA_RESOLPROV_CEMP); // 2.2
                    lan6Tramite.setInstanciaId(instancia);
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
                    // Fecha Limite Aportacion
                    String fechaStr = ConfigurationParameter.getParameter(exp[1] + BARRA + exp[0] + BARRA + codTramite, ConstantesMeLanbide43.FICHERO_PROPIEDADES);
                    log.debug("Fecha limite aportación: " + fechaStr);
                    Calendar fechaLimiteAportacion = Calendar.getInstance();
                    fechaLimiteAportacion.setTime(dateFormat.parse(fechaStr));
                    lan6Tramite.setFechaLimiteAportacion(fechaLimiteAportacion);
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_RECURSO_POTESTATIVO);
                ...
                log.debug("procedimiento COLEC- tramite:" + codTramite);
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    lan6Tramite.setDescripcion(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_APORTACION_DOC_DESC"));
                    lan6Tramite.setDescripcionEu(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_APORTACION_DOC_DESC_EU"));
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_ALTA_PER_COLEC); // 2.2
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
                    lan6Tramite.setInstanciaId(instancia);
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_DOC_TRAM_COLEC); // 2.2
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
                    lan6Tramite.setInstanciaId(instancia);
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    lan6Tramite.setDescripcion(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_APORTACION_DOC_DESC"));
                    lan6Tramite.setDescripcionEu(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_APORTACION_DOC_DESC_EU"));
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_INI_ACTIVIDAD_COLEC); // 2.2
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
                    lan6Tramite.setInstanciaId(instancia);
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_SUBSANACION);
                    ...
                    lan6Tramite.setDescripcion(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_SUBSANACION_DOC_DESC"));
                    lan6Tramite.setDescripcionEu(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_SUBSANACION_DOC_DESC_EU"));
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_SUB_INI_ACTIVIDAD_LAN62_COLEC); // 2.2
                    lan6Tramite.setInstanciaId(instancia);
...
//                    lan6Tramite.setDescripcionEu(Lan6Constantes.TRAMITE_ALEGACION_DOC_DESC);
                    ...
                    //se deja la espera inicial porque no se permite desde Platea más de 1 instancia en Alegaciones, sí en Requerimiento, lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION);//112
                    //String instancia=ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_APOR_DOC_TRAM_COLEC, ConstantesMeLanbide43.FICHERO_ADAPTADORES);
//                    String instancia=ConfigurationParameter.getParameter(ConstantesMeLanbide43.ID_INSTANCIA_APOR_ALEGA_RESOLPROV_COLEC, ConstantesMeLanbide43.FICHERO_ADAPTADORES);
//                     lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_ALEGA_RESOLPROV_COLEC); // 2.2
                    lan6Tramite.setInstanciaId(instancia);
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
                    lan6Tramite.setDescripcion(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ALEGACION_DOC_DESC"));
                    lan6Tramite.setDescripcionEu(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ALEGACION_DOC_DESC_EU"));
...
//                    lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_RECURSO_POTESTATIVO);
                ...
                log.debug("procedimiento DECEX- trámite:" + codTramite);
...
//                         lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_MOD_LAN62_DECEX); // 2.2
                    lan6Tramite.setInstanciaId(instancia);
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
...
//                             lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_3MESES_LAN62_DECEX); // 2.2
                    lan6Tramite.setInstanciaId(instancia);
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
...
//                            lan6Tramite.setId(Lan6Constantes.TRAMITE_ESPERA_REQUERIMIENTO_APORTACION + "_" + instancia);
                    String instancia = gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties(ConstantesMeLanbide43.ID_INSTANCIA_APOR_ANU1_LAN62_DECEX); // 2.2
                    lan6Tramite.setInstanciaId(instancia);
                    lan6Tramite.setId(gestionAdaptadoresLan6Config.getElementoConfigFicheroAdaptadoresProperties("TRAMITE_ESPERA_REQUERIMIENTO_APORTACION") + "_" + instancia);
                }
            }
            if (conFechaLimitAportacion) {
                log.debug("fecha limite aportacion : " + lan6Tramite.getFechaLimiteAportacion());
            }
        } catch (Exception e) {
...
//                    idProcedimiento = MeLanbide43Manager.getInstance().convierteProcedimiento(exp[1]);
            ...
            if (fecha != "") {
                ...
                if (date.after(fec)) {
                    ...
                    //DATOS
                    log.debug("ejercicio: " + lan6Tramite.getEjercicio());
                    log.debug("expediente: " + lan6Tramite.getIdExpediente());
                    log.debug("fecha Actualizacion: " + lan6Tramite.getFechaActualizacion().toString());
                    log.debug("id: " + lan6Tramite.getId());
                    log.debug("tipoTramite:" + lan6Tramite.getTipoTramite());
                    log.debug("desc:" + lan6Tramite.getDescripcion());
                    log.debug("InstanciaId:" + lan6Tramite.getInstanciaId());
                    /*if (conFechaLimitAportacion){
            ...
            if (log.isDebugEnabled()) {
                log.debug("justificantesPagos() : END");
            }
        } catch (Lan6InformarConsultaExcepcion ice) {
            ...
            log.error("Error en la función justificantesPagos: ", ex);
            String error = "Error en la función justificantesPagos: " + ex.getException().getMessage().toString();
        ...
        //       }
            ...
            try {
                ...
                log.error("Error en funcion justificantesPagos: ", ex);
            }
        }
        return mensaje;
    }
    ...
    public String esperaAportDocPostal(int codOrganizacion, int codTramite, String numExp, AdaptadorSQLBD adaptador) throws Exception {
        String mensaje = "0";
        log.debug("esperaAportDocPostal BEGIN: " + numExp);
        Connection con = null;
        try {
            con = adaptador.getConnection();
            MeLanbide43DAO melanbide43DAO = MeLanbide43DAO.getInstance();
            ...
            try {
                ...
                log.error("Error en funcion verificarFecha: ", ex);
            }
        }
        return mensaje;
    }
    ...
    public ArrayList<FilaListadoMisGestiones> getInfoGestiones(String numExp, int codOrganizacion, AdaptadorSQLBD adaptador, int codTramite, String evento) throws Exception {
        ...
        try {
            ...
            log.error("Se ha producido una excepción en la BBDD en getInfoGestiones para el expediente " + numExp, e);
    ...
    public String guardarQuartz(AdaptadorSQLBD adaptador) throws Exception {
        ...
        try {
            ...
            log.error("Se ha producido una excepción en la BBDD guardando la clave registral APA para el expediente ", e);
            throw new Exception(e);
        } catch (Exception e) {
    ...
    public static void grabarError(ErrorBean error, String excepError, String traza, String numExpediente) {
        try {
            ...
            String idProc = "";
            if (valor.length > 1) {
                idProc = gestionAdaptadoresLan6Config.getCodProcedimientoPlatea(valor[1]); //convierteProcedimiento(codProcedimiento);
            }
    ...
    public String obtenerUsuarioUltimoTramite(String numExpediente, AdaptadorSQLBD adaptador) throws Exception {
        ...
        try {
            ...
            try {
                ...
                log.error("Error en funcion obtenerUsuarioUltimoTramite: ", ex);
            }
        }
        return usuario;
    }
    ...
    public String fechaInicioExpediente(String procedimiento, AdaptadorSQLBD adaptador) throws Exception {
        ...
        try {
            ...
            try {
                ...
                log.error("Error en funcion fechaInicioExpediente: ", ex);
            }
        }
        return fecha;
    }
...
//#233079 Mis gestiones - Mantenimiento tabla de fases - principio
    public List<ProcedimientoVO> getProcedimientos(AdaptadorSQLBD adaptador) throws Exception {
        log.debug(" Entrando a Manager.GetProcedimientos");
        Connection con = null;
        try {
            if (adaptador != null) {
                log.debug(" adapt es dif de null -- antes de crear conexion" + adaptador.toString());
                con = adaptador.getConnection();
            }
            MeLanbide43DAO meLanbide43DAO = MeLanbide43DAO.getInstance();
    ...
    public FaseVO getFaseProcedimientoTramiteyFase(String codProc, String codTram, String codFase, AdaptadorSQLBD adaptador) throws Exception {
        ...
        try {
            ...
            log.error("Se ha producido un error recuperando fase para Procedimiento " + codProc + " Trámite " + codTram + " y Fase " + codFase, e);
            throw new Exception(e);
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando fase para Procedimiento " + codProc + " Trámite " + codTram + " y Fase " + codFase, ex);
            throw new Exception(ex);
        } finally {
            try {
                adaptador.devolverConexion(con);
    ...
    //#233082 Consulta y reenvÃ­o de Notificaciones - inicio  
    ...
    //#233082 Consulta y reenvÃ­o de Notificaciones - fin 
    public String getInsertarCodigoInerno(AdaptadorSQLBD adaptador, String codigointernodescrip, String numExpediente) throws Exception {
        ...
        try {
            if (adaptador != null) {
                log.debug(" num expediente" + numExpediente);
                con = adaptador.getConnection();
            }
            MeLanbide43DAO meLanbide43DAO = MeLanbide43DAO.getInstance();
    ...
    public ArrayList<Participantes> leerListaParticipantesRepresentantesRdRxExp(String numExpediente, int codRolrepre, Connection con) throws Exception {
        log.debug("leerListaParticipantesRepresentantesRdRxExp - Manager - Begin()");
        ArrayList<Participantes> exp = new ArrayList<Participantes>();
        try {
            MeLanbide43DAO melanbide43DAO = MeLanbide43DAO.getInstance();
            ...
            log.debug("leerListaParticipantesRepresentantesRdRxExp - Manager - End()");
        }
        return exp;
    }

    public TercerosValueObject getDatosBasicosterceroRepreRdR(String nif, Connection con) throws Exception {
        log.debug("getDatosBasicosterceroRepreRdR - Manager - Begin()");
        try {
            MeLanbide43DAO melanbide43DAO = MeLanbide43DAO.getInstance();
            ...
            log.debug("getDatosBasicosterceroRepreRdR - Manager - End()");
        }
    }
    ...
    public TercerosValueObject getDatosTerceroRdRxNifExpteRol(String numExpediente, String nif, Integer rol, Connection con) throws Exception {
        log.debug("getDatosTerceroRdRxNifExpteRol - Manager - Begin()");
        try {
            MeLanbide43DAO melanbide43DAO = MeLanbide43DAO.getInstance();
            ...
            log.debug("getDatosTerceroRdRxNifExpteRol - Manager - End()");
        }
    }
    ...
    public boolean insertarRepresentanteLegalExpedienteRdR(TercerosValueObject tercero, int codOrganizacion, String numExpediente, String codProcedimiento, int codRolRepreProcedimientoInt, Connection con) throws Exception {
        log.debug("insertarRepresentanteLegalExpedienteRdR - Manager - Begin()");
        try {
            MeLanbide43DAO melanbide43DAO = MeLanbide43DAO.getInstance();
            ...
            log.debug("insertarRepresentanteLegalExpedienteRdR - Manager - End()");
        }
    }
    ...
    public boolean eliminarRepresentanteLegalExpedienteRdR(TercerosValueObject tercero, int codOrganizacion, String numExpediente, String codProcedimiento, int codRolRepreProcedimientoInt, Connection con) throws Exception {
        log.debug("eliminarRepresentanteLegalExpedienteRdR - Manager - Begin()");
        try {
            MeLanbide43DAO melanbide43DAO = MeLanbide43DAO.getInstance();
            ...
            log.debug("eliminarRepresentanteLegalExpedienteRdR - Manager - End()");
        }
    }
    ...
    public boolean existeNIFRepresentanteLegalEnExpedienteRdR(String lan6ApoderadoTypeID, int codOrganizacion, int codRolRepreProcedimientoInt, String codProcedimiento, String numExpediente, Connection con) throws Exception {
        log.debug("existeNIFRepresentanteLegalEnExpedienteRdR - Manager - Begin()");
        try {
            MeLanbide43DAO melanbide43DAO = MeLanbide43DAO.getInstance();
            ...
            log.debug("existeNIFRepresentanteLegalEnExpedienteRdR - Manager - End()");
        }
    }
    ...
    public String getDatosNotificacionElectronicaAVISOSxExptexCodCampo(String numExpediente, String codCampo, Connection con) throws Exception {
        log.debug("getDatosNotificacionElectronicaAVISOSxExptexCodCampo - Manager - Begin()");
        try {
            MeLanbide43DAO melanbide43DAO = MeLanbide43DAO.getInstance();
            ...
            log.debug("getDatosNotificacionElectronicaAVISOSxExptexCodCampo - Manager - End()");
        }
    }
    ...
    public boolean updateDatosNotificacionElectronicaAVISOSxExptexCodCampo(int codOrganizacion, String numExpediente, String codCampo, String nuevoValorCampo, Connection con) throws Exception {
        log.debug("updateDatosNotificacionElectronicaAVISOSxExptexCodCampo - Manager - Begin()");
        try {
            MeLanbide43DAO melanbide43DAO = MeLanbide43DAO.getInstance();
            ...
            log.debug("updateDatosNotificacionElectronicaAVISOSxExptexCodCampo - Manager - End()");
        }
    }
    ...
    public boolean borarDatosNotificacionElectronicaAVISOSxExptexCodCampo(int codOrganizacion, String numExpediente, String codCampo, Connection con) throws Exception {
        log.debug("borarDatosNotificacionElectronicaAVISOSxExptexCodCampo - Manager - Begin()");
        try {
            ...
            log.debug("borarDatosNotificacionElectronicaAVISOSxExptexCodCampo - Manager - End()");
        }
    }

    public Integer esDesarrolloCero(String numExp, Connection con) throws Exception {
        if (log.isDebugEnabled()) {
            log.debug("esDesarrolloCero ( numExp = " + numExp + " ) : BEGIN");
        }
        MeLanbide43DAO meLanbide43DAO = MeLanbide43DAO.getInstance();

        if (log.isDebugEnabled()) {
            log.debug("esDesarrolloCero() : END");
        }
    ...
    public HSSFWorkbook descargarExcelLlamadasMisGest(String numExped, String fecDesde, String fecHasta, String dniTercero, String codProcedimiento, AdaptadorSQLBD adaptador) throws Exception {
        ...
        try {
            ...
            adaptador.inicioTransaccion(con);
            MeLanbide43DAO meLanbide43DAO = MeLanbide43DAO.getInstance();
            log.debug("Recogemos lo datos con los parametros seleccionados ");
            ...
            log.debug("Datos Recogidos para generar el Excel : " + (lstLlamadas != null ? lstLlamadas.size() : 0));
            ...
            try {
                ...
                log.debug("Limite filas recogido Properties : " + limite);
            ...
            log.debug("Limite Lineas por hoja : " + limiteLineasHoja);
            ...
            for (int i = 0; i < lstLlamadas.size(); i++) {
                log.debug("sheetRowCounter  " + sheetRowCounter + " limiteLineasHoja " + limiteLineasHoja);
                if (sheetRowCounter == 0 || sheetRowCounter > limiteLineasHoja) { // Cero para la primera hoja primera linea
                    log.debug("Crea nueva hoja...");
                ...
                log.debug("sheetRowCounter  " + sheetRowCounter);
                if (sheet != null) {
    ...
    private static void crearCabeceraHojaExceImprimirLlamadasMisGestiones(HSSFWorkbook workbook, HSSFSheet sheet, String[] headers) {
        log.debug("Creamos la cabecera para : " + (sheet != null ? sheet.getSheetName() : "Null"));
        try {
    ...
    public List<ProcedimientoVO> getComboProcedimiento(int codOrganizacion, AdaptadorSQLBD adaptador) throws Exception {
        log.debug("getComboProcedimiento - Begin () " + formatDateLog.format(new Date()));
        Connection con = null;
        try {
            con = adaptador.getConnection();
    ...
    public void actualizarProcesadoActInteresados(int id, AdaptadorSQLBD adaptador) throws Exception {
        ...
        try {
            ...
            try {
                ...
                log.error("Error al cerrar conexión la BBDD: " + e.getMessage());
            }
        }
    }

    /**
     * Recibiendo Adaptador BD - Obtiene los datos de Aviso de un expediente.
     * Campos Suplementarios:
     * AVISOEMAIL,AVISOEMAILTIT,AVISOSMS,AVISOSMSTIT,AVISOIDCTAINTE,AVISOIDCTAINTETIT,IDIOMAAVISOS
     *
     * @param numExpediente
     * @param adaptador
     * @return Mapa con key/valor (codCampo/valorCampo) : AVISOEMAIL /
     * ejempmlo@ejemplo.com , IDIOMAAVISOS / es, etc.
     */
    ...
    /**
     * Recibiendo Conexion BD - Obtiene los datos de Aviso de un expediente.
     * Campos Suplementarios:
     * AVISOEMAIL,AVISOEMAILTIT,AVISOSMS,AVISOSMSTIT,AVISOIDCTAINTE,AVISOIDCTAINTETIT,IDIOMAAVISOS
     *
     * @param numExpediente
    ...
    public String getValorCampoSuplementarioExpedienteTDESP(String numExpediente, String codCampo, Connection con) throws Exception {
        log.debug("getValorCampoSuplementarioExpedienteTDESP - Manager - Begin()");
        try {
            MeLanbide43DAO melanbide43DAO = MeLanbide43DAO.getInstance();
            ...
            log.debug("getValorCampoSuplementarioExpedienteTDESP - Manager - End()");
        }
    }

    public int getCampoNumericoEnteroExpediente(String numExpediente, String codCampo, AdaptadorSQLBD adaptador) throws Exception {
        log.debug("getCampoNumericoEnteroExpediente - Manager - Begin()");
        Connection con = null;
        try {
            con = adaptador.getConnection();
            MeLanbide43DAO melanbide43DAO = MeLanbide43DAO.getInstance();
            return melanbide43DAO.getCampoNumericoEnteroExpediente(numExpediente, codCampo, con);
        } catch (Exception e) {
            log.error("Error al recuperar el campo " + codCampo + " del expediente : " + numExpediente, e);
            throw e;
        } finally {
            log.debug("getCampoNumericoEnteroExpediente - Manager - End()");
            try {
                adaptador.devolverConexion(con);
            } catch (BDException e) {
                log.error("Error al cerrar conexión a la BBDD: " + e.getMessage());
    ...
    public List<Tram9099CerrarVO> getExpedientesFallidos9099(AdaptadorSQLBD adaptador) throws Exception {
        log.debug("getExpedientesFallidos9099 - Manager - Begin()");
        Connection con = null;
        try {
            con = adaptador.getConnection();
            MeLanbide43Tramite9099DAO meLanbide43Tramite9099DAO = MeLanbide43Tramite9099DAO.getInstance();
            return meLanbide43Tramite9099DAO.getExpedientesFallidos9099(con);
        } catch (Exception e) {
            log.error("Error al recuperar expedientes fallidos tramite 9099 ", e);
            throw e;
        } finally {
            if (con != null) {
                con.close();
            }
            log.debug("getExpedientesFallidos9099 - Manager - End()");
        }
    }

    public List<Tram9099CerrarVO> getExpedientesCerrar9099(AdaptadorSQLBD adaptador) throws Exception {
        log.debug("getExpedientesCerrar9099 - Manager - Begin()");
        Connection con = null;
        try {
            con = adaptador.getConnection();
            MeLanbide43Tramite9099DAO meLanbide43Tramite9099DAO = MeLanbide43Tramite9099DAO.getInstance();
            return meLanbide43Tramite9099DAO.getExpedientesCerrar9099(con);
        } catch (Exception e) {
            log.error("Error al recuperar expedientes a cerrar tramite 9099 ", e);
            throw e;
        } finally {
            if (con != null) {
                con.close();
            }
            log.debug("getExpedientesCerrar9099 - Manager - End()");
        }
    }

    public int insertarLineasLogJob9099(AdaptadorSQLBD adaptador, String numeroExpediente, String estado, String mensaje, String enMicarpeta, int traEspera, int ocuEspera, int traSalida, int ocuSalida) throws Exception {
        log.debug("insertarLineasLogJob - Begin () ");
        Integer retorno = 0;
        Connection con = null;
        try {
            con = adaptador.getConnection();
            MeLanbide43Tramite9099DAO meLanbide43Tramite9099DAO = MeLanbide43Tramite9099DAO.getInstance();
            retorno = meLanbide43Tramite9099DAO.insertarLineasLogJob(con, numeroExpediente, estado, mensaje, enMicarpeta, traEspera, ocuEspera, traSalida, ocuSalida);
        ...
        log.debug("insertarLineasLogJob - End () ");
        return retorno;
    }
    ...
    public Date getValorCampoSuplementarioTramiteE_TFET(String numExpediente, String codCampo, int tramite, int ocurrencia, AdaptadorSQLBD adaptador) throws Exception {
        log.debug("getValorCampoSuplementarioTramiteE_TFET - Manager - Begin()");
        Connection con = null;
        try {
            con = adaptador.getConnection();
            ...
            log.debug("getValorCampoSuplementarioTramiteE_TFET - Manager - End()");
        }
    }

    public boolean iniciadoEnMiCarpeta(String numExpediente, AdaptadorSQLBD adaptador) {
        Boolean enMiC = false;
        Connection con = null;
        try {
            con = adaptador.getConnection();
            enMiC = MeLanbide43DAO.getInstance().iniciadoEnMiCarpeta(numExpediente, con);
        } catch (Exception ex) {
            log.error("Error en iniciadoEnMiCarpeta: ", ex);
        } finally {
            try {
                if (con != null) {
                    con.close();
    ...
    public boolean cerradoEnMiCarpeta(String numExpediente, AdaptadorSQLBD adaptador) throws Exception {
        boolean cerrado = false;
        Connection con = null;
        try {
            con = adaptador.getConnection();
            cerrado = MeLanbide43DAO.getInstance().cerradoEnMiCarpeta(numExpediente, con);
        } catch (Exception ex) {
            log.error("Error en cerradoEnMiCarpeta: ", ex);
        } finally {
            try {
                if (con != null) {
                    con.close();
    ...
    public void eliminarGestionesExpedienteCerradoMiCarpeta(String numExpediente, Connection con) throws Exception {
        log.debug("eliminarGestionesExpedienteCerradoMiCarpeta - Manager - Begin()");
        try {
            MeLanbide43DAO melanbide43DAO = MeLanbide43DAO.getInstance();
            melanbide43DAO.eliminarGestionesExpedienteCerradoMiCarpeta(numExpediente, con);
        } catch (Exception e) {
            log.error("Error al eliminar gestión de expediente cerrado, en el expediente " + numExpediente, e);
            throw e;
        } finally {
            log.debug("eliminarGestionesExpedienteCerradoMiCarpeta - Manager - End()");
        }
    }
