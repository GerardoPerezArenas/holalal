 ...
 * <ol>
 *  <li>david.caamano * 12-09-2012 * #86969 Edici�n inicial</li>
 * </ol> 
 */
public class CerModulosFormativosDAO {
    ...
    public ArrayList<CerModuloFormativoVO> getModulosFormativos (String codCertificado, Connection con) throws Exception{
        ...
        if(log.isDebugEnabled()) log.debug("Primero recuperamos los c�digos de los modulos formativos");
        codigosModulosFormativos = getCodigosModulosFormativos(codCertificado, con);
        if(codigosModulosFormativos.size() > 0){
            if(log.isDebugEnabled()) log.debug("El certificado tiene m�dulos formativos");
            Statement st = null;
            ResultSet rs = null;
            try{
                String sql = "Select * from " + GlobalNames.ESQUEMA_GENERICO
                    + ConfigurationParameter.getParameter(ConstantesMeLanbide40.CER_MODULOS_FORMATIVOS, 
                        ConstantesMeLanbide40.FICHERO_PROPIEDADES)
                    + " where CODMODULO in ( ";
    ...
    private ArrayList<String> getCodigosModulosFormativos(String codCertificado, Connection con) throws Exception{
        ...
        try{
            String sql = "Select * from " + GlobalNames.ESQUEMA_GENERICO
                + ConfigurationParameter.getParameter(ConstantesMeLanbide40.CER_REL_MOD_CERT, ConstantesMeLanbide40.FICHERO_PROPIEDADES)
