 ...
 */
package es.altia.flexia.integracion.moduloexterno.melanbide17.dao;
...
import es.altia.flexia.integracion.moduloexterno.melanbide17.util.MeLanbide17MappingUtils;
 ...
 * @author santiagoc
 */
public class MeLanbide17DAO {

    //Logger
    private static Logger log = LogManager.getLogger(MeLanbide17DAO.class);

    //Instancia
    private static MeLanbide17DAO instance = null;

    private MeLanbide17DAO() {

    }

    public static MeLanbide17DAO getInstance() {
        if (instance == null) {
            synchronized (MeLanbide17DAO.class) {
                instance = new MeLanbide17DAO();
            }
        }
        return instance;
    }
    ...
    public List<FilaMinimisVO> getDatosMinimis(String numExp, int codOrganizacion, Connection con) throws Exception {
        ...
        try {
            String query = null;
            query = "SELECT * FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide17.MELANBIDE17_SUBSOLIC, ConstantesMeLanbide17.FICHERO_PROPIEDADES)
            ...
            while (rs.next()) {
                minimis = (FilaMinimisVO) MeLanbide17MappingUtils.getInstance().map(rs, FilaMinimisVO.class);
    ...
    public int eliminarMinimis(String id, Connection con) throws Exception {
        ...
        try {
            String query = null;
            query = "DELETE FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide17.MELANBIDE17_SUBSOLIC, ConstantesMeLanbide17.FICHERO_PROPIEDADES)
                    + " WHERE ID=" + (id != null && !id.isEmpty() ? id : "null");
            if (log.isDebugEnabled()) {
                log.debug("sql = " + query);
            }
    ...
    public boolean crearNuevoMinimis(FilaMinimisVO nuevaMinimis, Connection con) throws Exception {
        ...
        try {
            int id = recogerIDInsertar(ConfigurationParameter.getParameter(ConstantesMeLanbide17.SEQ_MELANBIDE17_SUBSOLIC, ConstantesMeLanbide17.FICHERO_PROPIEDADES), con);
            query = "INSERT INTO " + ConfigurationParameter.getParameter(ConstantesMeLanbide17.MELANBIDE17_SUBSOLIC, ConstantesMeLanbide17.FICHERO_PROPIEDADES)
    ...
    public boolean modificarMinimis(FilaMinimisVO datModif, Connection con) throws Exception {
        ...
        try {
            query = "UPDATE " + ConfigurationParameter.getParameter(ConstantesMeLanbide17.MELANBIDE17_SUBSOLIC, ConstantesMeLanbide17.FICHERO_PROPIEDADES)
    ...
    public List<DesplegableAdmonLocalVO> getValoresDesplegablesAdmonLocalxdes_cod(String des_cod, Connection con) throws Exception {
        ...
        try {
            String query = null;
            query = "SELECT * FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide17.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide17.FICHERO_PROPIEDADES)
                    + " WHERE DES_COD='" + des_cod + "' order by DES_NOM";
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            while (rs.next()) {
                valoresDesplegable = (DesplegableAdmonLocalVO) MeLanbide17MappingUtils.getInstance().map(rs, DesplegableAdmonLocalVO.class);
