/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */
package es.altia.flexia.integracion.moduloexterno.melanbide38.vo;

import java.math.BigDecimal;
import java.util.Date;

/**
 *
 * @author santiagoc
 */
public class ContratoVO
{
    private Integer mun;
    private Integer eje;
    private String pro;
    private String num;
    private Integer nCon;
    private String doc;
    private Long terCod;
    private String nom;
    private String ap1;
    private String ap2;
    private Date fna;
    private Integer sex;
    private String min;
    private String inm;
    private String pld;
    private String rml;
    private String otr;
    private Integer col;
    private Integer mde;
    private Integer nes;
    private Integer tco;
    private Integer tic;
    private Date fac;
    private String tjo;
    private BigDecimal pjt;
    private Date ffc;
    private Integer dco;
    private String cnoe;
    private BigDecimal sal;
    private BigDecimal css;
    private BigDecimal imp;
    private BigDecimal imr;
    private BigDecimal ire;
    private Integer cPro;
    
    public ContratoVO()
    {
        
    }

    public Integer getMun() {
        return mun;
    }

    public void setMun(Integer mun) {
        this.mun = mun;
    }

    public Integer getEje() {
        return eje;
    }

    public void setEje(Integer eje) {
        this.eje = eje;
    }

    public String getPro() {
        return pro;
    }

    public void setPro(String pro) {
        this.pro = pro;
    }

    public String getNum() {
        return num;
    }

    public void setNum(String num) {
        this.num = num;
    }

    public Integer getnCon() {
        return nCon;
    }

    public void setnCon(Integer nCon) {
        this.nCon = nCon;
    }

    public String getDoc() {
        return doc;
    }

    public void setDoc(String doc) {
        this.doc = doc;
    }

    public Long getTerCod() {
        return terCod;
    }

    public void setTerCod(Long terCod) {
        this.terCod = terCod;
    }

    public String getNom() {
        return nom;
    }

    public void setNom(String nom) {
        this.nom = nom;
    }

    public String getAp1() {
        return ap1;
    }

    public void setAp1(String ap1) {
        this.ap1 = ap1;
    }

    public String getAp2() {
        return ap2;
    }

    public void setAp2(String ap2) {
        this.ap2 = ap2;
    }

    public Date getFna() {
        return fna;
    }

    public void setFna(Date fna) {
        this.fna = fna;
    }

    public Integer getSex() {
        return sex;
    }

    public void setSex(Integer sex) {
        this.sex = sex;
    }

    public String getMin() {
        return min;
    }

    public void setMin(String min) {
        this.min = min;
    }

    public String getInm() {
        return inm;
    }

    public void setInm(String inm) {
        this.inm = inm;
    }

    public String getPld() {
        return pld;
    }

    public void setPld(String pld) {
        this.pld = pld;
    }

    public String getRml() {
        return rml;
    }

    public void setRml(String rml) {
        this.rml = rml;
    }

    public String getOtr() {
        return otr;
    }

    public void setOtr(String otr) {
        this.otr = otr;
    }

    public Integer getCol() {
        return col;
    }

    public void setCol(Integer col) {
        this.col = col;
    }

    public Integer getMde() {
        return mde;
    }

    public void setMde(Integer mde) {
        this.mde = mde;
    }

    public Integer getNes() {
        return nes;
    }

    public void setNes(Integer nes) {
        this.nes = nes;
    }

    public Integer getTco() {
        return tco;
    }

    public void setTco(Integer tco) {
        this.tco = tco;
    }

    public Integer getTic() {
        return tic;
    }

    public void setTic(Integer tic) {
        this.tic = tic;
    }

    public Date getFac() {
        return fac;
    }

    public void setFac(Date fac) {
        this.fac = fac;
    }

    public String getTjo() {
        return tjo;
    }

    public void setTjo(String tjo) {
        this.tjo = tjo;
    }

    public BigDecimal getPjt() {
        return pjt;
    }

    public void setPjt(BigDecimal pjt) {
        this.pjt = pjt;
    }

    public Date getFfc() {
        return ffc;
    }

    public void setFfc(Date ffc) {
        this.ffc = ffc;
    }

    public Integer getDco() {
        return dco;
    }

    public void setDco(Integer dco) {
        this.dco = dco;
    }

    public String getCnoe() {
        return cnoe;
    }

    public void setCnoe(String cnoe) {
        this.cnoe = cnoe;
    }

    public BigDecimal getSal() {
        return sal;
    }

    public void setSal(BigDecimal sal) {
        this.sal = sal;
    }

    public BigDecimal getCss() {
        return css;
    }

    public void setCss(BigDecimal css) {
        this.css = css;
    }

    public BigDecimal getImp() {
        return imp;
    }

    public void setImp(BigDecimal imp) {
        this.imp = imp;
    }

    public BigDecimal getImr() {
        return imr;
    }

    public void setImr(BigDecimal imr) {
        this.imr = imr;
    }

    public BigDecimal getIre() {
        return ire;
    }

    public void setIre(BigDecimal ire) {
        this.ire = ire;
    }

    public Integer getcPro() {
        return cPro;
    }

    public void setcPro(Integer cPro) {
        this.cPro = cPro;
    }
}