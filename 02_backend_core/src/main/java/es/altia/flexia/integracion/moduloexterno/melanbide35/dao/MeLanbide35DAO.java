 ...
 */
package es.altia.flexia.integracion.moduloexterno.melanbide35.dao;
 ...
 * @author santiagoc
 */
public class MeLanbide35DAO {

    //Logger
    private static final Logger log = LogManager.getLogger(MeLanbide35DAO.class);

    //Instancia
    private static MeLanbide35DAO instance = null;

    private MeLanbide35DAO() {

    }

    public static MeLanbide35DAO getInstance() {
        if (instance == null) {
            synchronized (MeLanbide35DAO.class) {
                instance = new MeLanbide35DAO();
            }
        }
        return instance;
    }
    ...
    public EcaSolicitudVO getDatosSolicitud(String numExpediente, Connection con) throws Exception {
        ...
        try {
            String query = null;
            query = "select * from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOLICITUD, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_NUMEXP = '" + numExpediente + "'";
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            if (rs.next()) {
                retSol = (EcaSolicitudVO) MeLanbide35MappingUtils.getInstance().map(rs, EcaSolicitudVO.class);
            }
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando  getDatosSolicitud", ex);
        ...
        if (solAsociada != null && solAsociada.getSolicitudCod() != null) {
            ...
            try {
                if (solAsociada.getNumExp() != null) {
                    String[] datos = solAsociada.getNumExp().split(ConstantesMeLanbide35.BARRA_SEPARADORA);
            ...
            try {
                ...
                query = "select tmp1.*, tmp2.*, tmp3.*,  tmp4.*, tmp5.*, "
                        + " case when tmp6.max_subvencionable < 0 then 0"
                        + " else tmp6.max_subvencionable end as max_subvencionable"
                        + " from "
                        + "  ("
                        + "       select"
                        ...
                        + "       from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + "       a where a.eca_prep_solicitud = " + solAsociada.getSolicitudCod()
                        + "  )tmp1,"
                        + "  ("
                        + "       SELECT"
                        + "       sum(nvl(b.ECA_PREP_SEG_ANT,0)) seguimientos_ant"
                        + "       FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOLICITUD, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " A"
                        + "       INNER JOIN " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " B"
                        ...
                        + "       FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOLICITUD, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " A"
                        + "       INNER JOIN " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " B"
                        + "       ON A.ECA_SOLICITUD_COD=B.ECA_PROS_SOLICITUD"
                        + "       where A.ECA_SOLICITUD_COD = " + solAsociada.getSolicitudCod()
                        + "       and B.ECA_PROS_CODORIGEN is null"
                        + "  )tmp4,"
                        + "  ("
                        ...
                        + "  )tmp5,"
                        + "  ("
                        + "       SELECT imp-NVL(ECA_SUB_PUB,0)-NVL(ECA_SUB_PRIV,0)max_subvencionable FROM ECA_SOLICITUD AA"
                        + "       INNER JOIN"
                        + "       ("
                ...
                rs = st.executeQuery(query);
                if (rs.next()) {
                    datos = (DatosAnexosVO) MeLanbide35MappingUtils.getInstance().map(rs, DatosAnexosVO.class);
                }
            } catch (Exception ex) {
                log.error("Se ha producido un error recuperando getDatosSolicitudAnexos", ex);
                throw new Exception(ex);
            } finally {
                try {
                    if (st != null) {
    ...
    public DatosAnexosVO getDatosSolicitudCarga(EcaSolicitudVO solAsociada, Connection con) throws Exception {
        if (solAsociada != null && solAsociada.getSolicitudCod() != null) {
            ...
            try {
                ...
                query = "select tmp1.*, tmp2.*, tmp3.*,  tmp4.*, tmp5.*, tmp6.*"
                        + " from "
                        + "  ("
                        + "       select"
                ...
                log.error("Se ha producido un error recuperando getDatosSolicitudCarga", ex);
                throw new Exception(ex);
            } finally {
                try {
                    if (st != null) {
    ...
    public EcaSolicitudVO guardarDatosSolicitud(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        try {
            ...
            if (sol.getSolicitudCod() == null) {
                ...
                query = "update " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOLICITUD, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + " set"
            ...
            log.error("Se ha producido un error guardarDatosSolicitud", ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public EcaSolPreparadoresVO guardarEcaSolPreparadoresVO(EcaSolPreparadoresVO prep, Connection con) throws Exception {
        ...
        try {
            ...
            if (prep.getSolPreparadoresCod() == null) {
                prep.setSolPreparadoresCod(this.getNextId(ConfigurationParameter.getParameter(ConstantesMeLanbide35.SEQ_ECA_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES), con));
                //Es una solicitud nueva
                query = "insert into " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + "(ECA_SOL_PREPARADORES_COD, ECA_PREP_NIF, ECA_PREP_NOMBRE, ECA_PREP_FECINI, ECA_PREP_FECFIN, ECA_PREP_HORAS_JC, ECA_PREP_HORAS_CONT, ECA_PREP_HORAS_ECA, ECA_PREP_IMP_SS_JC, ECA_PREP_IMP_SS_JR, ECA_PREP_IMP_SS_ECA, ECA_PREP_SEG_ANT, ECA_PREP_IMP_SEG_ANT, ECA_PREP_INS_C1_H, ECA_PREP_INS_C1_M, ECA_PREP_INS_C1, ECA_PREP_INS_C2_H, ECA_PREP_INS_C2_M, ECA_PREP_INS_C2, ECA_PREP_INS_C3_H, ECA_PREP_INS_C3_M, ECA_PREP_INS_C3, ECA_PREP_INS_C4_H, ECA_PREP_INS_C4_M, ECA_PREP_INS_C4, ECA_PREP_INS_IMPORTE, ECA_PREP_INS_SEG_IMPORTE, ECA_PREP_COSTE, ECA_PREP_SOLICITUD,"
                        + " ECA_PREP_IMPTE_CONCEDIDO, "
                        + " ECA_PREP_NIF_CARGA, ECA_PREP_NOMBRE_CARGA, ECA_PREP_FECINI_CARGA, ECA_PREP_FECFIN_CARGA, ECA_PREP_HORAS_JC_CARGA, ECA_PREP_HORAS_CONT_CARGA, ECA_PREP_HORAS_ECA_CARGA, ECA_PREP_IMP_SS_JC_CARGA, ECA_PREP_IMP_SS_JR_CARGA, ECA_PREP_IMP_SS_ECA_CARGA, ECA_PREP_SEG_ANT_CARGA, ECA_PREP_IMP_SEG_ANT_CARGA, ECA_PREP_INS_C1_H_CARGA, ECA_PREP_INS_C1_M_CARGA, ECA_PREP_INS_C1_CARGA, ECA_PREP_INS_C2_H_CARGA, ECA_PREP_INS_C2_M_CARGA, ECA_PREP_INS_C2_CARGA, ECA_PREP_INS_C3_H_CARGA, ECA_PREP_INS_C3_M_CARGA, ECA_PREP_INS_C3_CARGA, ECA_PREP_INS_C4_H_CARGA, ECA_PREP_INS_C4_M_CARGA, ECA_PREP_INS_C4_CARGA, ECA_PREP_INS_IMPORTE_CARGA, ECA_PREP_INS_SEG_IMPORTE_CARGA, ECA_PREP_COSTE_CARGA,"
                ...
                //Es una solicitud que ya existe
                query = "update " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + " set"
    ...
    public List<FilaPreparadorSolicitudVO> getListaPreparadoresSolicitud(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        try {
            String query = "select p.*,"
                    ...
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " p"
            ...
            while (rs.next()) {
                fila = (FilaPreparadorSolicitudVO) MeLanbide35MappingUtils.getInstance().map(rs, FilaPreparadorSolicitudVO.class);
                retList.add(fila);
            }
            return retList;
        } catch (Exception ex) {
    ...
    public int eliminarPreparadorSolicitud(EcaSolPreparadoresVO prep, Connection con) throws Exception {
        Statement st = null;
        try {
            String query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
    ...
    public EcaSolPreparadoresVO getPreparadorSolicitudPorId(EcaSolPreparadoresVO prep, Connection con) throws Exception {
        ...
        try {
            String query = "select * from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_SOL_PREPARADORES_COD = " + prep.getSolPreparadoresCod().toString();
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            if (rs.next()) {
                retPrep = (EcaSolPreparadoresVO) MeLanbide35MappingUtils.getInstance().map(rs, EcaSolPreparadoresVO.class);
            }
            return retPrep;
        } catch (Exception ex) {
            log.error("Se ha producido un error eliminando preparador solicitud " + (prep != null ? prep.getSolPreparadoresCod() : "(preparador = null)"), ex);
    ...
    public EcaSolProspectoresVO guardarEcaSolProspectoresVO(EcaSolProspectoresVO pros, Connection con) throws Exception {
        ...
        try {
            ...
            if (pros.getSolProspectoresCod() == null) {
                pros.setSolProspectoresCod(this.getNextId(ConfigurationParameter.getParameter(ConstantesMeLanbide35.SEQ_ECA_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES), con));
                //Es una solicitud nueva
                query = "insert into " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                ...
                //Es una solicitud que ya existe
                query = "update " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + " set"
    ...
    public List<FilaProspectorSolicitudVO> getListaProspectoresSolicitud(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        try {
            String query = "select p.*,"
                    ...
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " p"
            ...
            while (rs.next()) {
                fila = (FilaProspectorSolicitudVO) MeLanbide35MappingUtils.getInstance().map(rs, FilaProspectorSolicitudVO.class);
                //Si bubiera que mostrar las filas con colores o similares (porque tengan errores) se haría aqui
                retList.add(fila);
            }
            return retList;
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando getListaProspectoresSolicitud", ex);
            throw new Exception(ex);
        } finally {
            try {
    ...
    public EcaSolProspectoresVO getProspectorSolicitudPorId(EcaSolProspectoresVO pros, Connection con) throws Exception {
        ...
        try {
            String query = "select * from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_SOL_PROSPECTORES_COD = " + pros.getSolProspectoresCod().toString();
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            if (rs.next()) {
                retPros = (EcaSolProspectoresVO) MeLanbide35MappingUtils.getInstance().map(rs, EcaSolProspectoresVO.class);
            }
            return retPros;
        } catch (Exception ex) {
    ...
    public int eliminarProspectorSolicitud(EcaSolProspectoresVO pros, Connection con) throws Exception {
        Statement st = null;
        try {
            String query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_SOL_PROSPECTORES_COD = " + pros.getSolProspectoresCod().toString();
            log.debug("sql = " + query);
            st = con.createStatement();
            return st.executeUpdate(query);
    ...
    public EcaSolValoracionVO getValoracionSolicitud(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        try {
            String query = "select * from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_VALORACION, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_SOL_VAL_SOLICITUD = " + sol.getSolicitudCod();
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            while (rs.next()) {
                valoracion = (EcaSolValoracionVO) MeLanbide35MappingUtils.getInstance().map(rs, EcaSolValoracionVO.class);
            }
            return valoracion;
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando getValoracionSolicitud", ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public EcaSolValoracionVO guardarDatosValoracionSolicitud(EcaSolValoracionVO val, Connection con) throws Exception {
        ...
        try {
            ...
            if (val.getSolValoracionCod() == null) {
                val.setSolValoracionCod(this.getNextId(ConfigurationParameter.getParameter(ConstantesMeLanbide35.SEQ_ECA_SOL_VALORACION, ConstantesMeLanbide35.FICHERO_PROPIEDADES), con));
                //Es una solicitud nueva
                query = "insert into " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_VALORACION, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                ...
                //Es una solicitud que ya existe
                query = "update " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_VALORACION, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + " set"
            ...
            log.error("Se ha producido un error guardarDatosValoracionSolicitud", ex);
            throw new Exception(ex);
        } finally {
            try {
    ...
    public EcaConfiguracionVO getConfiguracionEca(int ano, Connection con) throws Exception {
        ...
        try {
            String query = null;
            query = "select * from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_CONFIGURACION, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ANO = " + ano;
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            if (rs.next()) {
                conf = (EcaConfiguracionVO) MeLanbide35MappingUtils.getInstance().map(rs, EcaConfiguracionVO.class);
            }
            return conf;
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando getConfiguracionEca", ex);
            throw new Exception(ex);
        } finally {
            try {
    ...
    public int eliminarTodosPreparadoresSol_Solicitud(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        try {
            String query = null;
            query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_PREP_SOLICITUD = " + sol.getSolicitudCod();
            log.debug("sql = " + query);
            st = con.createStatement();
            return st.executeUpdate(query);
        } catch (SQLException ex) {
            log.error("Se ha producido un error eliminarTodosPreparadoresSol_Solicitud", ex);
            throw new Exception(ex);
        } finally {
            try {
    ...
    public int eliminarTodosProspectoresSol_Solicitud(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        try {
            String query = null;
            query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_PROS_SOLICITUD = " + sol.getSolicitudCod();
            log.debug("sql = " + query);
            st = con.createStatement();
            return st.executeUpdate(query);
        } catch (SQLException ex) {
            log.error("Se ha producido un error eliminarTodosProspectoresSol_Solicitud", ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public EcaJusPreparadoresVO getPreparadorJustificacionPorId(EcaJusPreparadoresVO prep, Connection con) throws Exception {
        ...
        try {
            String query = "select * from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_JUS_PREPARADORES_COD = " + prep.getJusPreparadoresCod().toString();
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            if (rs.next()) {
                retPrep = (EcaJusPreparadoresVO) MeLanbide35MappingUtils.getInstance().map(rs, EcaJusPreparadoresVO.class);
            }
            return retPrep;
        } catch (Exception ex) {
            ...
            try {
                ...
                if (rs != null) {
                    try {
                        ...
                        log.debug("error al cerra resulset" + e.getMessage());
                    }
    ...
    public EcaJusPreparadoresVO getPreparadorJustificacionPorNIF(EcaSolicitudVO sol, String nif, Date feInicio, /*String feFin*/ Connection con) throws Exception {
        ...
        if (sol != null && sol.getSolicitudCod() != null && !sol.getSolicitudCod().equals("") && nif != null && !nif.equals("")) {
            ...
            try {
                String query = "select * from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + " where upper(ECA_PREP_NIF) = '" + nif.toUpperCase() + "' AND ECA_PREP_SOLICITUD=" + sol.getSolicitudCod() //+ " AND ("
    ...
    public EcaJusProspectoresVO getProspectorJustificacionSustituto(Integer codPrep, Connection con) throws Exception {
        if (codPrep != null) {
            ...
            try {
                String query = "select * from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                ...
                rs = st.executeQuery(query);
                if (rs.next()) {
                    retPros = (EcaJusProspectoresVO) MeLanbide35MappingUtils.getInstance().map(rs, EcaJusProspectoresVO.class);
                }
                return retPros;
            } catch (Exception ex) {
    ...
    public List<FilaPreparadorJustificacionVO> getListaPreparadoresJustificacion(EcaSolicitudVO sol, Integer idioma, Connection con) throws Exception {
        ...
        try {
            if (sol.getNumExp() != null) {
                String[] datos = sol.getNumExp().split(ConstantesMeLanbide35.BARRA_SEPARADORA);
                conf = this.getConfiguracionEca(Integer.parseInt(datos[0]), con);
            }
        } catch (Exception ex) {
            log.debug("No existe configuraciónd e datos para ese anio");
        ...
        try {
            String query = "select PJUS.ECA_JUS_PREPARADORES_COD,PJUS.ECA_SOL_PREPARADORES_COD,PJUS.ECA_PREP_NIF,PJUS.ECA_PREP_NOMBRE, PJUS.ECA_PREP_FECINI, PJUS.ECA_PREP_FECFIN, PJUS.ECA_PREP_HORAS_JC, PJUS.ECA_PREP_CODORIGEN,"
                    ...
                    + (idioma == 1
                            ? " (SELECT NOMBRE_C FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_TIPOCONTRATO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " "
                            + "   WHERE COD_TIPOCONTRATO = PJUS.ECA_PREP_TIPOCONTRATO  ) AS TIPOCONTRATO, "
                            : " (SELECT NOMBRE_E FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_DISCAPGRAVEDAD, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " "
                            + "   WHERE COD_TIPOCONTRATO = PJUS.ECA_PREP_TIPOCONTRATO  ) AS TIPOCONTRATO, ")
                    ...
                    + " C1H_JUS, C1M_JUS, C1_JUS, C2H_JUS, C2M_JUS, C2_JUS, C3H_JUS, C3M_JUS, C3_JUS, C4H_JUS, C4M_JUS, C4_JUS, NUMSEG,"
                    ...
                    + " (C3H_JUS * " + conf.getImC3h() + ") + (C3M_JUS * " + conf.getImC3m() + ")" + " + (C4H_JUS * " + conf.getImC4h() + ") + (C4M_JUS * " + conf.getImC4m() + ")  AS IMPINSERCIONES "
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " PJUS "
                    + " left join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " PSOL "
                    ...
                    + "SUM(c4h) as C4H_JUS, SUM(c4m) as C4M_JUS, SUM(c4) AS C4_JUS, SUM(seguimientos) AS NUMSEG "
                    + "FROM ( "
                    ...
                    + /*"case when eca_seg_sexo=0 and eca_seg_tipodiscapacidad = 1 and eca_seg_discgravedad = 2 and months_between(eca_seg_fecfin,eca_seg_fecini) >=12 and eca_seg_tipo = 1  then round(eca_seg_porcjornada/100,2) else 0 end as c1h,"+
                                            ...
                                            "case when  to_char(ECA_SEG_FECINI,'YYYY') < '"+datosexp[0]+"' and eca_seg_tipo = 0  then round(eca_seg_porcjornada/100,2) else 0 end as seguimientos "+*/ "case when eca_seg_sexo=0 and eca_seg_tipodiscapacidad = 1 and (eca_seg_discgravedad = 2 or eca_seg_discgravedad=3) and (months_between(eca_seg_fecfin+1,eca_seg_fecini) >=12 or eca_seg_fecfin is null) and eca_seg_tipo = 1  then round(eca_seg_porcjornada/100,4) else 0 end as c1h,"
                    ...
                    /*
                                             ...
                                             * codigo se quitaran los que contengan errores. El hecho de que la fecha de inicio de un seguimiento no sea < que el anio del expediente
            ...
            while (rs.next()) {
                fila = (FilaPreparadorJustificacionVO) MeLanbide35MappingUtils.getInstance().map(rs, FilaPreparadorJustificacionVO.class);
                //Si bubiera que mostrar las filas con colores o similares (porque tengan errores) se haría aqui
                retList.add(fila);
            }
            return retList;
    ...
    public EcaJusPreparadoresVO guardarEcaJusPreparadoresVO(EcaJusPreparadoresVO prep, Connection con) throws Exception {
        ...
        try {
            ...
            if (prep.getJusPreparadoresCod() == null) {
                prep.setJusPreparadoresCod(this.getNextId(ConfigurationParameter.getParameter(ConstantesMeLanbide35.SEQ_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES), con));
                //Es una solicitud nueva
                query = "insert into " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                ...
                //Es una solicitud que ya existe
                query = "update " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + " set"
                        + " ECA_PREP_NIF = " + (prep.getNif() != null ? "'" + prep.getNif() + "'" : "null")
                        + ", ECA_PREP_NOMBRE = " + (prep.getNombre() != null ? "'" + prep.getNombre() + "'" : "null")
                        + ", ECA_PREP_FECINI = " + (prep.getFecIni() != null ? "TO_DATE('" + format.format(prep.getFecIni()) + "', 'dd/mm/yyyy')" : "null")
    ...
    public int eliminarPreparadorJustificacion(EcaJusPreparadoresVO prep, Connection con) throws Exception {
        Statement st = null;
        try {
            String query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_JUS_PREPARADORES_COD = " + prep.getJusPreparadoresCod().toString();
            log.debug("sql = " + query);
            st = con.createStatement();
            return st.executeUpdate(query);
    ...
    public Integer obtenerSiguienteCodSeguimiento(Integer codPrep, Connection con) {
        ...
        try {
            String query = "SELECT MAX(ECA_SEG_PREPARADORES_COD) "
                    + " FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SEG_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
    ...
    public EcaSegPreparadoresVO guardarEcaSegPreparadoresVO(EcaSegPreparadoresVO seg, Connection con) throws Exception {
        ...
        try {
            ...
            if (seg.getSegPreparadoresCod() == null) {
                ...
                //seg.setSegPreparadoresCod(this.getNextId(ConfigurationParameter.getParameter(ConstantesMeLanbide35.SEQ_ECA_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES), con));
                seg.setSegPreparadoresCod(obtenerSiguienteCodSeguimiento(seg.getJusPreparadoresCod(), con));
                //Es un seguimiento nuevo
                query = "insert into " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SEG_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                ...
                log.debug("UPDATE");
                //Es una solicitud que ya existe
                query = "update " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SEG_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + "  set "
    ...
    public List<FilaSegPreparadoresVO> getListaSeguimientos(EcaSolicitudVO sol, EcaJusPreparadoresVO prep, String tipo, String colectivo, String sexo, Integer idioma, Connection con) throws Exception {
        ...
        try {
            String query = "select ECA_SEG_PREPARADORES_COD, B.ECA_JUS_PREPARADORES_COD, ECA_SEG_TIPO, ECA_PREP_NIF, ECA_SEG_TIPODISCAPACIDAD,"
                    ...
                    + (idioma == 1
                            ? " (SELECT NOMBRE_C FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_TIPODISCAPACIDAD, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " "
                            + "   WHERE COD_TDISCAPACIDAD = ECA_SEG_TIPODISCAPACIDAD  ) AS TDISCAPACIDAD, "
                            : " (SELECT NOMBRE_E FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_TIPODISCAPACIDAD, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " "
                            + "   WHERE COD_TDISCAPACIDAD = ECA_SEG_TIPODISCAPACIDAD  ) AS TDISCAPACIDAD, ")
                    + (idioma == 1
                            ? " (SELECT NOMBRE_C FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_DISCAPGRAVEDAD, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " "
                    ...
                    + " ECA_SEG_FECSEGUIMIENTO,  ECA_SEG_OBSERVACIONES "
                    + //ECA_SEG_PERSCONTACTO, 
                    " , ECA_SEG_FINCONTRATO "
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SEG_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " A "
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " B "
            ...
            while (rs.next()) {
                fila = (FilaSegPreparadoresVO) MeLanbide35MappingUtils.getInstance().map(rs, FilaSegPreparadoresVO.class);
                //Si bubiera que mostrar las filas con colores o similares (porque tengan errores) se haría aqui
                retList.add(fila);
            }
            return retList;
    ...
    public List<FilaInsPreparadoresVO> getListaInserciones(EcaSolicitudVO sol, EcaJusPreparadoresVO prep, String tipo, String colectivo, String sexo, Integer idioma, Connection con) throws Exception {
        ...
        try {
            ...
            while (rs.next()) {
                fila = (FilaInsPreparadoresVO) MeLanbide35MappingUtils.getInstance().map(rs, FilaInsPreparadoresVO.class);
                //Si bubiera que mostrar las filas con colores o similares (porque tengan errores) se haría aqui
                retList.add(fila);
            }
            return retList;
    ...
    public List<SelectItem> getListaTipoContrato(Integer idioma, Connection con) throws Exception {
        ...
        String query = "select COD_TIPOCONTRATO, UPPER(NOMBRE_" + (idioma == 1 ? "C" : "E") + ") as NOMBRE from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_TIPOCONTRATO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "  ";
    ...
    public List<SelectItem> getListaTipodiscapacidad(Integer idioma, Connection con) throws Exception {
        ...
        String query = "select COD_TDISCAPACIDAD, UPPER(NOMBRE_" + (idioma == 1 ? "C" : "E") + ") as NOMBRE from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_TIPODISCAPACIDAD, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "  ";
    ...
    public List<SelectItem> getListaGravedad(Integer idioma, Connection con) throws Exception {
        ...
        ResultSet rs = null;
        ArrayList lista = new ArrayList();
        String query = "select COD_GRAVEDAD, UPPER(NOMBRE_" + (idioma == 1 ? "C" : "E") + ") as NOMBRE from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_DISCAPGRAVEDAD, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "  ";
        log.debug("sql = " + query);
        st = con.createStatement();
        rs = st.executeQuery(query);
        SelectItem si = null;/*new SelectItem();
    ...
    public List<FilaAuditoriaProcesosVO> filtrarAuditoriaProcesos(String nomApellidos, Date feDesde, Date feHasta, Integer codProc, int codIdioma, Connection con) throws Exception {
        ...
        try {
            ...
            query = "select au.*, u.usu_nom from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_AUDITORIA, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " au"
                    + " left join " + GlobalNames.ESQUEMA_GENERICO + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_USUARIOS, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " u"
            ...
            while (rs.next()) {
                ...
                descProceso = MeLanbide35Utils.obtenerNombreProceso(cproceso, codIdioma);
    ...
    public int eliminarTodosSeguimientosPrep_Solicitud(EcaSolicitudVO sol, String tipo, Connection con) throws Exception {
        ...
        try {
            String query = null;
            query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SEG_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " s "
                    + " where exists ("
                    + " select  * from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " p where s.eca_jus_preparadores_cod=p.eca_jus_preparadores_cod and p.eca_prep_solicitud=" + sol.getSolicitudCod()
    ...
    public int eliminarSegPreparador(EcaSegPreparadoresVO seg, Connection con) throws Exception {
        Statement st = null;
        try {
            String query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SEG_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
            ...
            try {
                /*String query = "select"
                                ...
                                +" ("
                                  ...
                                  +" where ext.ext_rol = "+ConstantesMeLanbide35.ROLES.INTERESADO
                                +" )tmp"
                ...
                String query = "SELECT eca_solicitud_cod,eca_numexp, ECA_SEG_EMPRESA,\r\n"
                        ...
                        + " --,\r\n"
                        + " --b.*,c.*\r\n";
                if (ejercicio != null && !ejercicio.equals("") && Integer.parseInt(ejercicio) > 2014) {
                    query += " from eca14_solicitud a\r\n"
                            + " left join eca14_jus_preparadores b on b.ECA_PREP_SOLICITUD=a.ECA_SOLICITUD_COD\r\n"
                            + " left join (SELECT * FROM eca14_seg_preparadores ) c on b.ECA_JUS_PREPARADORES_COD=c.ECA_JUS_PREPARADORES_COD\r\n";
                } else {
                    query += " from eca_solicitud a\r\n"
                ...
                query += " --LEFT JOIN T_TER D ON D.TER_DOC=C.ECA_SEG_NIF\r\n"
                        + " where \r\n"
                        + " --ECA_NUMEXP='2013/ECA2/000004'and \r\n"
                        + " ECA_NUMEXP like '" + ejercicio + "/" + ConstantesMeLanbide35.NOMBRE_PROCEDIMIENTO_MELANBIDE35 + "/%' and \r\n"
                ...
                st = con.createStatement();
                rs = st.executeQuery(query);
                retList = MeLanbide35InformeUtils.getInstance().extraerDatosInformeDesglose(rs);
                ...
                for (FilaInformeDesglose fila : retList) {
                    if (datosTerceros.containsKey(fila.getNumExp())) {
                        ...
                        datosTercero = this.getDatosTercerosPorExpedienteYRol(fila.getNumExp(), ConstantesMeLanbide35.ROLES.INTERESADO, con);
    ...
    public EcaSegPreparadoresVO getSeguimientoPreparadorPorId(EcaSegPreparadoresVO seg, Connection con) throws Exception {
        ...
        try {
            String query = "select S.*,"
                    + " (select ECA_PREP_NIF from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " "
                    + "         where  ECA_JUS_PREPARADORES_COD =" + seg.getJusPreparadoresCod() + ") as NIFPREPARADOR "
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SEG_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " S "
            ...
            rs = st.executeQuery(query);
            if (rs.next()) {
                retSeg = (EcaSegPreparadoresVO) MeLanbide35MappingUtils.getInstance().map(rs, EcaSegPreparadoresVO.class);
            }
            return retSeg;
        } catch (Exception ex) {
    ...
    public List<FilaProspectorJustificacionVO> getListaProspectoresJustificacion(EcaSolicitudVO sol, Integer idioma, Connection con) throws Exception {
        ...
        try {
            String query = "select PJUS.ECA_JUS_PROSPECTORES_COD,PJUS.ECA_SOL_PROSPECTORES_COD,PJUS.ECA_PROS_NIF,PJUS.ECA_PROS_NOMBRE, PJUS.ECA_PROS_FECINI, PJUS.ECA_PROS_FECFIN, PJUS.ECA_PROS_HORAS_JC, PJUS.ECA_PROS_CODORIGEN,"
                    ...
                    :" (SELECT NOMBRE_E FROM "+ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_DISCAPGRAVEDAD, ConstantesMeLanbide35.FICHERO_PROPIEDADES)+" "+
                            "   WHERE COD_TIPOCONTRATO = PJUS.ECA_PROS_TIPOCONTRATO  ) AS TIPOCONTRATO, ")+    */ " case when nvl(VISITASREAL,0)<" + conf.getMinEmpVisit() + " then 0 "
                    ...
                    + "  VISITASREAL "
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " PJUS "
                    + " left join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " PSOL "
                    ...
                    + "from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_VIS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " group by ECA_JUS_PROSPECTORES_COD "
            ...
            while (rs.next()) {
                fila = (FilaProspectorJustificacionVO) MeLanbide35MappingUtils.getInstance().map(rs, FilaProspectorJustificacionVO.class);
                //Si bubiera que mostrar las filas con colores o similares (porque tengan errores) se haría aqui
                retList.add(fila);
            }
            return retList;
    ...
    public EcaJusProspectoresVO guardarEcaJusProspectoresVO(EcaJusProspectoresVO pros, Connection con) throws Exception {
        ...
        try {
            ...
            if (pros.getJusProspectoresCod() == null) {
                pros.setJusProspectoresCod(this.getNextId(ConfigurationParameter.getParameter(ConstantesMeLanbide35.SEQ_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES), con));
                //Es una solicitud nueva
                query = "insert into " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                ...
                //Es una solicitud que ya existe
                query = "update " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + " set"
                        + " ECA_PROS_NIF = " + (pros.getNif() != null ? "'" + pros.getNif() + "'" : "null")
                        + ", ECA_PROS_NOMBRE = " + (pros.getNombre() != null ? "'" + pros.getNombre() + "'" : "null")
                        + ", ECA_PROS_FECINI = " + (pros.getFecIni() != null ? "TO_DATE('" + format.format(pros.getFecIni()) + "', 'dd/mm/yyyy')" : "null")
    ...
    public int eliminarProspectorJustificacion(EcaJusProspectoresVO pros, Connection con) throws Exception {
        Statement st = null;
        try {
            String query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_JUS_PROSPECTORES_COD = " + pros.getJusProspectoresCod().toString();
            log.debug("sql = " + query);
            st = con.createStatement();
            return st.executeUpdate(query);
    ...
    public List<FilaVisProspectoresVO> getListaVisitas(EcaSolicitudVO sol, EcaJusProspectoresVO prep, Integer idioma, Connection con) throws Exception {
        ...
        try {
            String query = "select ECA_VIS_PROSPECTORES_COD, B.ECA_JUS_PROSPECTORES_COD, ECA_PROS_NIF, ECA_VIS_CIF, ECA_VIS_EMPRESA, "
                    ...
                    + GlobalNames.ESQUEMA_GENERICO + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_PROVINCIAS, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " P "
                    + " WHERE PRV_PAI=108  and prv_aut=14 order by PRV_COD)"
                    + " WHERE  ECA_VIS_PROVINCIA = COD) AS PROVINCIA, "
                    + /* (idioma==1?
                            " (SELECT PRV_NOM FROM "+GlobalNames.ESQUEMA_GENERICO +ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_PROVINCIAS, ConstantesMeLanbide35.FICHERO_PROPIEDADES)+" "+
                            "   WHERE PRV_COD = ECA_VIS_PROVINCIA  ) AS PROVINCIA, "
                    :" (SELECT PRV_NOL FROM "+GlobalNames.ESQUEMA_GENERICO+ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_PROVINCIAS, ConstantesMeLanbide35.FICHERO_PROPIEDADES)+" "+
                            "   WHERE PRV_COD = ECA_VIS_PROVINCIA  ) AS PROVINCIA, ")+
                     */ (idioma == 1
                            ? " (SELECT NOMBRE_C FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SECTORACTIVIDAD, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " "
                            + "   WHERE COD_SECTOR = ECA_VIS_SECTOR  ) AS SECTOR, "
                            : " (SELECT NOMBRE_E FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SECTORACTIVIDAD, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " "
                            + "   WHERE COD_SECTOR = ECA_VIS_SECTOR  ) AS SECTOR, ")
                    + (idioma == 1
                            ? " (SELECT NOMBRE_C FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_CUMPLELISMI, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " "
                            + "   WHERE COD_CUMPLE = ECA_VIS_CUMPLELISMI  ) AS CUMPLELISMI, "
                            : " (SELECT NOMBRE_E FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_CUMPLELISMI, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " "
                            + "   WHERE COD_CUMPLE = ECA_VIS_CUMPLELISMI  ) AS CUMPLELISMI, ")
                    + (idioma == 1
                            ? " (SELECT NOMBRE_C FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_RESULTADO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " "
                            + "   WHERE COD_RESULTADO = ECA_VIS_RESULTADOFINAL  ) AS RESULTADO, "
                            : " (SELECT NOMBRE_E FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_RESULTADO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " "
                            + "   WHERE COD_RESULTADO = ECA_VIS_RESULTADOFINAL  ) AS RESULTADO, ")
                    ...
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_VIS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " A "
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " B "
            ...
            while (rs.next()) {
                fila = (FilaVisProspectoresVO) MeLanbide35MappingUtils.getInstance().map(rs, FilaVisProspectoresVO.class);
                //Si bubiera que mostrar las filas con colores o similares (porque tengan errores) se haría aqui
                retList.add(fila);
            }
            return retList;
    ...
    public List<FilaResumenInformeProyectos> getDatosResumenInformeProyectos(String ejercicio, Connection con) throws Exception {
        ...
        if (ejercicio != null && !ejercicio.equals("")) {
            ...
            try {
                ...
                if (ejercicio != null && (!"".equals(ejercicio)) && Integer.parseInt(ejercicio) > 2014) {
                    query += " from eca14_solicitud ";
                } else {
                    query += " from eca_solicitud ";
                }
                query += "where substr(ECA_NUMEXP, 1,4) = '" + ejercicio + "'";
                log.debug("sql = " + query);
                st = con.createStatement();
                rs = st.executeQuery(query);

                retList = MeLanbide35InformeUtils.getInstance().extraerDatosResumenInformeProyectos(0, rs);
    ...
    public List<FilaInformeProyectos> getDatosInformeProyectosParte2(String numExp, Connection con) throws Exception {
        ...
        if (numExp != null && !numExp.equals("")) {
            int ejercicio = MeLanbide35Utils.getEjercicioDeExpediente(numExp);
            Statement st = null;
            ResultSet rs = null;
            try {
                ...
                if (ejercicio > 2014) {
                    query += "          from eca14_solicitud a\r\n"
                            + "          left join eca14_jus_preparadores b on b.ECA_PREP_SOLICITUD=a.ECA_SOLICITUD_COD\r\n"
                            + "          left join eca14_seg_preparadores \r\n";
                } else {
                    query += "          from eca_solicitud a\r\n"
                            + "          left join eca_jus_preparadores b on b.ECA_PREP_SOLICITUD=a.ECA_SOLICITUD_COD\r\n"
                            + "          left join eca_seg_preparadores \r\n";
                }
                query += "          --ECA_SEG_TIPO=1) \r\n"
                ...
                st = con.createStatement();
                rs = st.executeQuery(query);
                retList = MeLanbide35InformeUtils.getInstance().extraerDatosInformeProyectos(2, rs);
            } catch (Exception ex) {
                log.error("Se ha producido un error recuperando datos del informe proyectos", ex);
                throw new Exception(ex);
            } finally {
    ...
    public List<FilaInformeProyectos> getDatosInformeProyectosParte3(String numExp, Connection con) throws Exception {
        ...
        if (numExp != null && !numExp.equals("")) {
            ...
            try {
                ...
                st = con.createStatement();
                rs = st.executeQuery(query);
                retList = MeLanbide35InformeUtils.getInstance().extraerDatosInformeProyectos(3, rs);
            } catch (Exception ex) {
                log.error("Se ha producido un error recuperando datos del informe proyectos", ex);
                throw new Exception(ex);
            } finally {
    ...
    public FilaResumenInformeProyectos getDatosInformeProyectosParte4(String numExp, Connection con) throws Exception {
        ...
        if (numExp != null && !numExp.equals("")) {
            ...
            try {
                ...
                if (ejercicio > 2014) {
                    query += " from eca14_solicitud a\r\n"
                            + " left join eca14_jus_preparadores b ";
                } else {
                    query += " from eca_solicitud a\r\n"
                            + " left join eca_jus_preparadores b ";
                }

                query += "on b.ECA_PREP_SOLICITUD=a.ECA_SOLICITUD_COD\r\n"
                        + " where\r\n"
                        + " ECA_NUMEXP='" + numExp + "'\r\n"
                        + " --and\r\n"
                        + " --ECA_SEG_TIPO=1 AND\r\n"
                ...
                st = con.createStatement();
                rs = st.executeQuery(query);
                retList = MeLanbide35InformeUtils.getInstance().extraerDatosResumenInformeProyectos(4, rs);
            } catch (Exception ex) {
                log.error("Se ha producido un error recuperando datos del informe proyectos", ex);
                throw new Exception(ex);
            } finally {
    ...
    public List<SelectItem> getListaSectorActividad(Integer idioma, Connection con) throws Exception {
        ...
        ResultSet rs = null;
        List<SelectItem> lista = new ArrayList<SelectItem>();
        String query = "select COD_SECTOR, UPPER(NOMBRE_" + (idioma == 1 ? "C" : "E") + ") as NOMBRE from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SECTORACTIVIDAD, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "  ";
        try {
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            ...
            log.error("Se ha producido un error al obtener lista de sectores de actividad", ex);
            throw new Exception(ex);
        } finally {
            if (st != null) {
                st.close();
    ...
    public List<Integer> getListaCodigosSectorActividad(Connection con) throws Exception {
        ...
        try {
            String query = "select COD_SECTOR from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SECTORACTIVIDAD, ConstantesMeLanbide35.FICHERO_PROPIEDADES);
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            Integer act = null;
    ...
    public List<SelectItem> getListaCumpleLismi(Integer idioma, Connection con) throws Exception {
        ...
        try {
            String query = "select COD_CUMPLE, UPPER(NOMBRE_" + (idioma == 1 ? "C" : "E") + ") as NOMBRE from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_CUMPLELISMI, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "  ";
            ...
            log.error("Se ha producido un error al obtener CumpleLismi", ex);
            throw new Exception(ex);
        } finally {
    ...
    public List<Integer> getListaCodigosCumple(Connection con) throws Exception {
        ...
        List<Integer> retList = new ArrayList<Integer>();
        try {
            String query = "select COD_CUMPLE from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_CUMPLELISMI, ConstantesMeLanbide35.FICHERO_PROPIEDADES);
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            Integer act = null;
    ...
    public List<SelectItem> getListaResultadoFinal(Integer idioma, Connection con) throws Exception {
        ...
        List<SelectItem> lista = new ArrayList<SelectItem>();
        try {
            String query = "select COD_RESULTADO, UPPER(NOMBRE_" + (idioma == 1 ? "C" : "E") + ") as NOMBRE from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_RESULTADO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "  ";
            ...
            log.error("Se ha producido un error al obtener ListaResultadoFinal", ex);
            throw new Exception(ex);
        } finally {
            if (st != null) {
                st.close();
    ...
    public List<Integer> getListaCodigosResultado(Connection con) throws Exception {
        ...
        List<Integer> retList = new ArrayList<Integer>();
        try {
            String query = "select COD_RESULTADO from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_RESULTADO, ConstantesMeLanbide35.FICHERO_PROPIEDADES);
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            Integer act = null;
    ...
    public int eliminarTodasVisitasPros(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        try {
            String query = null;
            query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_VIS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " s "
                    + " where exists ("
                    + " select  * from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " p where s.eca_jus_prospectores_cod=p.eca_jus_prospectores_cod and p.eca_pros_solicitud=" + sol.getSolicitudCod()
                    + " )";
    ...
    public EcaVisProspectoresVO guardarEcaVisProspectoresVO(EcaVisProspectoresVO vis, Statement st, Connection con) throws Exception {
        ...
        try {
            ...
            if (vis.getVisProspectoresCod() == null) {
                ...
                //Es un seguimiento nuevo
                query = "insert into " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_VIS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                ...
                log.debug("UPDATE");
                //Es una solicitud que ya existe
                query = "update " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_VIS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + "  set "
    ...
    public Integer obtenerSiguienteCodVisita(Integer codPrep, Statement st, Connection con) throws SQLException {
        Integer num = 0;
        ResultSet rs = null;
        try {
            String query = "SELECT MAX(ECA_VIS_PROSPECTORES_COD) "
                    + " FROM " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_VIS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
    ...
    public EcaVisProspectoresVO getVisitaProspectorPorId(EcaVisProspectoresVO vis, Connection con) throws Exception {
        ...
        try {
            String query = "select S.*,"
                    + " (select ECA_PROS_NIF from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " "
                    + "         where  ECA_JUS_PROSPECTORES_COD =" + vis.getJusProspectoresCod() + ") as NIFPROSPECTOR "
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_VIS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " S "
            ...
            rs = st.executeQuery(query);
            if (rs.next()) {
                retVis = (EcaVisProspectoresVO) MeLanbide35MappingUtils.getInstance().map(rs, EcaVisProspectoresVO.class);
            }
            return retVis;
        } catch (Exception ex) {
    ...
    public List<Integer> getListaCodigosTipoContrato(Connection con) throws Exception {
        ...
        List<Integer> retList = new ArrayList<Integer>();
        try {
            String query = "select COD_TIPOCONTRATO from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_TIPOCONTRATO, ConstantesMeLanbide35.FICHERO_PROPIEDADES);
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            Integer act = null;
    ...
    public List<Integer> getListaCodigosTipoDiscapacidad(Connection con) throws Exception {
        ...
        List<Integer> retList = new ArrayList<Integer>();
        try {
            String query = "select COD_TDISCAPACIDAD from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_TIPODISCAPACIDAD, ConstantesMeLanbide35.FICHERO_PROPIEDADES);
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            Integer act = null;
    ...
    public List<Integer> getListaCodigosGravedad(Connection con) throws Exception {
        ...
        List<Integer> retList = new ArrayList<Integer>();
        try {
            String query = "select COD_GRAVEDAD from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_DISCAPGRAVEDAD, ConstantesMeLanbide35.FICHERO_PROPIEDADES);
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            Integer act = null;
    ...
    public int eliminarVisitaProspectorJustificacion(EcaVisProspectoresVO vis, Connection con) throws Exception {
        ...
        EcaSolProspectoresVO retPros = null;
        try {
            String query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_VIS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
    ...
    public Map<String, Integer> getNumeroSeguimientosInsercionesPreparador(EcaJusPreparadoresVO prep, Connection con) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String query = "select ECA_SEG_TIPO as TIPO, count(*) as SUMA from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SEG_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_JUS_PREPARADORES_COD = " + prep.getJusPreparadoresCod().toString()
    ...
    public Integer getNumeroVisitasProspector(EcaJusProspectoresVO pros, Connection con) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String query = "select count(*) as SUMA from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_VIS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_JUS_PROSPECTORES_COD = " + pros.getJusProspectoresCod().toString();
            log.debug("sql = " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
    ...
    public int eliminarTodosPreparadoresJus(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        try {
            String query = null;
            query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_PREP_SOLICITUD = " + sol.getSolicitudCod();
            log.debug("sql = " + query);
            st = con.createStatement();
            return st.executeUpdate(query);
    ...
    public int copiarPreparadoresFromSolToJus(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        try {
            String querySeq = "select " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.SEQ_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + ".nextval NEW_ID from dual";

            log.debug("sqlSeq = " + querySeq);

            String querySol = "select * from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where eca_prep_solicitud = " + sol.getSolicitudCod()
                ...
                if (!rsSol.wasNull()) {
                    ...
                    if (rsSeq.next()) {
                        ...
                        if (!rsSeq.wasNull()) {
                            ...
                            queryJus = "insert into " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                                    ...
                                    + ", " + (rsSol.getDate("ECA_PREP_FECINI") != null ? "TO_DATE('" + formatoFecha.format(rsSol.getDate("ECA_PREP_FECINI")) + "', '" + ConstantesMeLanbide35.FORMATO_FECHA + "')" : "null")
                                    + ", " + (rsSol.getDate("ECA_PREP_FECFIN") != null ? "TO_DATE('" + formatoFecha.format(rsSol.getDate("ECA_PREP_FECFIN")) + "', '" + ConstantesMeLanbide35.FORMATO_FECHA + "')" : "null")
                            ...
                            throw new Exception("No se ha podido obtener id de " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES));
                        }
                    } else {
                        throw new Exception("No se ha podido obtener id de " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES));
                    }
    ...
    public int eliminarTodosProspectoresJus(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        try {
            String query = null;
            query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_PROS_SOLICITUD = " + sol.getSolicitudCod();
            log.debug("sql = " + query);
            st = con.createStatement();
            return st.executeUpdate(query);
    ...
    public boolean hayProspectoresParaCopiar(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        try {
            ...
            query = "select count(*) as NUM_PROSPECTORES "
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ECA_PROS_SOLICITUD = " + sol.getSolicitudCod();
    ...
    public int copiarProspectoresFromSolToJus(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        int filasInsertadas = 0;
        try {
            String querySeq = "select " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.SEQ_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + ".nextval NEW_ID from dual";

            log.debug("sqlSeq = " + querySeq);

            String querySol = "select * from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where eca_pros_solicitud = " + sol.getSolicitudCod()
                ...
                if (!rsSol.wasNull()) {
                    ...
                    if (rsSeq.next()) {
                        ...
                        if (!rsSeq.wasNull()) {
                            ...
                            queryJus = "insert into " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                                    ...
                                    + ", " + (rsSol.getDate("ECA_PROS_FECINI") != null ? "TO_DATE('" + formatoFecha.format(rsSol.getDate("ECA_PROS_FECINI")) + "', '" + ConstantesMeLanbide35.FORMATO_FECHA + "')" : "null")
                                    + ", " + (rsSol.getDate("ECA_PROS_FECFIN") != null ? "TO_DATE('" + formatoFecha.format(rsSol.getDate("ECA_PROS_FECFIN")) + "', '" + ConstantesMeLanbide35.FORMATO_FECHA + "')" : "null")
                            ...
                            throw new Exception("No se ha podido obtener id de " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES));
                        }
                    } else {
                        throw new Exception("No se ha podido obtener id de " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES));
                    }
    ...
    public List<EcaJusPreparadoresVO> getPreparadoresJustificacion(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        try {
            ...
            while (rs.next()) {
                fila = (EcaJusPreparadoresVO) MeLanbide35MappingUtils.getInstance().map(rs, EcaJusPreparadoresVO.class);
                //Si bubiera que mostrar las filas con colores o similares (porque tengan errores) se haría aqui
                retList.add(fila);
            }
            return retList;
    ...
    public List<EcaJusProspectoresVO> getProspectoresJustificacion(EcaSolicitudVO sol, Connection con) throws Exception {
        ...
        try {
            ...
            while (rs.next()) {
                fila = (EcaJusProspectoresVO) MeLanbide35MappingUtils.getInstance().map(rs, EcaJusProspectoresVO.class);
                //Si bubiera que mostrar las filas con colores o similares (porque tengan errores) se haría aqui
                retList.add(fila);
            }
            return retList;
    ...
    public List<FilaEcaResProspectoresVO> getListaDetalleProspectoresResumen(String numExp, Connection con) throws Exception {
        ...
        try {
            /*String query = "select r.*, j.ECA_PROS_TIPO_SUST from "+ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_RES_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)+" r"
                          +" left join "+ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)+" j"
            ...
            String query = "select r.*, j.ECA_PROS_TIPO_SUST,"
                    ...
                    + " j.ECA_PROS_FECINI, j.ECA_PROS_FECFIN, j.ECA_PROS_NIF, IMPORTE_VISITAS_CONC"
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_RES_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " r"
                    + " left join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " j on r.ECA_JUS_PROSPECTORES_COD = j.ECA_JUS_PROSPECTORES_COD"
                    + " where ECA_NUMEXP = '" + numExp + "'"
            ...
            while (rs.next()) {
                pros = (FilaEcaResProspectoresVO) MeLanbide35MappingUtils.getInstance().map(rs, FilaEcaResProspectoresVO.class);
                retList.add(pros);
            }
            return retList;
        } catch (Exception ex) {
    ...
    public List<FilaEcaResPreparadoresVO> getListaDetallePreparadoresResumen(String numExp, Connection con) throws Exception {
        ...
        try {
            /*String query = "select r.*, j.ECA_PREP_TIPO_SUST from "+ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_RES_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)+" r"
                          +" left join "+ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)+" j"
            ...
            String query = "select r.*, j.ECA_PREP_TIPO_SUST ,"
                    ...
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_RES_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " r"
                    + " left join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " j on r.ECA_JUS_PREPARADORES_COD = j.ECA_JUS_PREPARADORES_COD"
                    + " where ECA_NUMEXP = '" + numExp + "'"
                    + " order by ORIGEN,  j.ECA_PREP_FECINI, j.ECA_PREP_FECFIN, j.ECA_PREP_NIF";
            log.debug("sql getListaDetallePreparadoresResumen= " + query);
            st = con.createStatement();
            rs = st.executeQuery(query);
            FilaEcaResPreparadoresVO prep = null;
            while (rs.next()) {
                prep = (FilaEcaResPreparadoresVO) MeLanbide35MappingUtils.getInstance().map(rs, FilaEcaResPreparadoresVO.class);
                retList.add(prep);
            }
            return retList;
        } catch (Exception ex) {
    ...
    public EcaResDetalleInsercionesVO cargarDetalleInsercionesPrep(String ejercicio, EcaJusPreparadoresVO prep, Connection con) throws Exception {
        ...
        try {
            String query = "select\r\n"
                    ...
                    + "   from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SOLICITUD, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " dd inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " cc on cc.ECA_PREP_SOLICITUD=dd.ECA_SOLICITUD_COD\r\n"
                    + "   inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_SEG_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " aa on cc.ECA_JUS_PREPARADORES_COD=aa.ECA_JUS_PREPARADORES_COD\r\n"
                    + "   inner join\r\n"
                    + "   (select IM_SEGUIMIENTO,IM_C1_H,IM_C1_M,IM_C2_H,IM_C2_M,IM_C3_H,IM_C3_M,IM_C4_H,IM_C4_M from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_CONFIGURACION, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "\r\n"
                    + "   where ano=" + ejercicio + "\r\n"
                    + "   ) importe on 1=1\r\n"
                    + "   where\r\n"
                    + "     ECA_SEG_CORRECTO ='S'\r\n"
    ...
    public String obtieneSeguimientosCorrectos(String numExp, Connection con) throws Exception {
        ...
        try {
            String query = null;
            query = "call obtiene_seguimientos_correctos(?,?)";
            log.debug("sql = " + query);
    ...
    public String obtieneVisitasCorrectas(String numExp, Connection con) throws Exception {
        ...
        try {
            String query = null;
            query = "call obtiene_visitas_correctas(?,?)";
            log.debug("sql = " + query);
            st = con.prepareCall(query);
            st.setString(1, numExp);
            st.registerOutParameter(2, java.sql.Types.VARCHAR);
    ...
    public EcaResumenVO getResumenSolicitud(EcaSolicitudVO solicitud, Connection con) throws Exception {
        if (solicitud != null && solicitud.getSolicitudCod() != null) {
            ...
            try {
                ...
                query = "select * from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_RESUMEN, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + " where ECA_RES_SOLICITUD = " + solicitud.getSolicitudCod();
                st = con.createStatement();
                rs = st.executeQuery(query);
                if (rs.next()) {
                    resumen = (EcaResumenVO) MeLanbide35MappingUtils.getInstance().map(rs, EcaResumenVO.class);
                }
                return resumen;
            } catch (Exception ex) {
                log.error("Se ha producido un error recuperando getResumenSolicitud", ex);
                throw new Exception(ex);
            } finally {
                try {
    ...
    public EcaResumenVO guardarResumenSolicitud(EcaResumenVO resumen, Connection con) throws Exception {
        ...
        try {
            ...
            if (resumen.getEcaCodResumen() == null) {
                resumen.setEcaCodResumen(this.getNextId(ConfigurationParameter.getParameter(ConstantesMeLanbide35.SEQ_ECA_RESUMEN, ConstantesMeLanbide35.FICHERO_PROPIEDADES), con));
                //Es una solicitud nueva
                query = "insert into " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_RESUMEN, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + "(ECA_COD_RESUMEN, ECA_RES_SOLICITUD, ECA_RES_SUB_PRIV, ECA_RES_SUB_PUB, ECA_RES_TOT_SUBV)"
                ...
                //Es una solicitud que ya existe
                query = "update " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA_RESUMEN, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                        + " set"
    ...
    public List<FilaInsercionesECA23VO> getListaInsercionesSolicitudECA23(EcaSolicitud23VO sol, Connection con) throws Exception {
        log.debug("getListaInsercionesSolicitudECA23 - DAO");
        Statement st = null;
        ResultSet rs = null;
        List<FilaInsercionesECA23VO> retList = new ArrayList<FilaInsercionesECA23VO>();
        try {
            String query = "select *  from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOL_INSERCIONES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where NUMEROEXPEDIENTE = '" + sol.getNumeroExpediente() + "' order by ID";
            log.debug("sql = " + query);

            st = con.createStatement();
            rs = st.executeQuery(query);
            FilaInsercionesECA23VO fila = null;
            while (rs.next()) {
                fila = (FilaInsercionesECA23VO) MeLanbide35MappingUtils.getInstance().map(rs, FilaInsercionesECA23VO.class);
                retList.add(fila);
            }
            return retList;
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando getListaInsercionesSolicitudECA23", ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public List<FilaProspectorECA23VO> getListaProspectoresSolicitudECA23(EcaSolicitud23VO sol, Connection con) throws Exception {
        log.debug("getListaProspectoresSolicitudECA23 - DAO");
        Statement st = null;
        ResultSet rs = null;
        List<FilaProspectorECA23VO> retList = new ArrayList<FilaProspectorECA23VO>();
        try {
            String query = "select *  from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " p"
                    + " where NUMEROEXPEDIENTE = '" + sol.getNumeroExpediente() + "' order by ID";
            log.debug("sql = " + query);

            st = con.createStatement();
            rs = st.executeQuery(query);
            FilaProspectorECA23VO fila = null;
            while (rs.next()) {
                fila = (FilaProspectorECA23VO) MeLanbide35MappingUtils.getInstance().map(rs, FilaProspectorECA23VO.class);

                retList.add(fila);
            }
            return retList;
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando getListaProspectoresSolicitudECA23", ex);
            throw new Exception(ex);
        } finally {
            try {
    ...
    public List<FilaPreparadorECA23VO> getListaPreparadoresSolicitudECA23(EcaSolicitud23VO sol, Connection con) throws Exception {
        log.debug("getListaPreparadoresSolicitudECA23 - DAO");
        Statement st = null;
        ResultSet rs = null;
        List<FilaPreparadorECA23VO> retList = new ArrayList<FilaPreparadorECA23VO>();
        try {
            String query = "select *  from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " p"
                    + " where NUMEROEXPEDIENTE = '" + sol.getNumeroExpediente() + "' order by ID";
            log.debug("sql = " + query);

            st = con.createStatement();
            rs = st.executeQuery(query);
            FilaPreparadorECA23VO fila = null;
            while (rs.next()) {
                fila = (FilaPreparadorECA23VO) MeLanbide35MappingUtils.getInstance().map(rs, FilaPreparadorECA23VO.class);
                retList.add(fila);
            }
            return retList;
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando getListaPreparadoresSolicitudECA23", ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public EcaSolicitud23VO getSolicitudECA23(String numeroExpediente, Connection con) throws Exception {
        log.debug("getSolicitudECA23 - DAO");
        Statement st = null;
        ResultSet rs = null;
        EcaSolicitud23VO ret = new EcaSolicitud23VO();
        try {
            String query = "select *  from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOLICITUD, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
            ...
            rs = st.executeQuery(query);
            if (rs.next()) {
                ret = (EcaSolicitud23VO) MeLanbide35MappingUtils.getInstance().map(rs, EcaSolicitud23VO.class);
            } else {
                insertSolicitudECA23(numeroExpediente, con);
                getSolicitudECA23(numeroExpediente, con);
            }
            return ret;
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando getSolicitudECA23", ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public Eca23ConfiguracionVO getConfiguracionECA23(String anio, Connection con) throws Exception {
        log.debug("getConfiguracionECA23 - DAO");
        Statement st = null;
        ResultSet rs = null;
        Eca23ConfiguracionVO ret = new Eca23ConfiguracionVO();
        try {
            String query = "select *  from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_CONFIGURACION, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ANO = '" + anio + "'";
            log.debug("sql = " + query);
            ...
            rs = st.executeQuery(query);
            if (rs.next()) {
                ret = (Eca23ConfiguracionVO) MeLanbide35MappingUtils.getInstance().map(rs, Eca23ConfiguracionVO.class);
            }
            return ret;
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando getImportesDiscapacidadECA23", ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public void insertSolicitudECA23(String numeroExpediente, Connection con) throws Exception {
        log.debug("insertSolicitudECA23 - DAO");
        Statement st = null;
        ResultSet rs = null;
        try {
            String query = "INSERT INTO " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOLICITUD, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " (NUMEROEXPEDIENTE, TOTALNUMPERSONAS, TOTALIMPORTESOLIC, MUJERESSEG, HOMBRESSEG, CUANTIASOLIC, IMPORTESOLICSEG, TOTALIMPCAL1ANO)"
                    + " VALUES ('" + numeroExpediente + "', '0', '0', '0', '0', '0', '0', '0')";
            log.debug("sql = " + query);
            ...
            log.error("Se ha producido un error insertSolicitudECA23", ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public int eliminarInsercionSolicitud23(FilaInsercionesECA23VO inser, Connection con) throws Exception {
        log.debug("eliminarInsercionSolicitud23 - DAO");
        Statement st = null;
        try {
            String query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOL_INSERCIONES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where id = " + inser.getId();
            log.debug("sql = " + query);

            st = con.createStatement();
            return st.executeUpdate(query);
        } catch (SQLException ex) {
            log.error("Se ha producido un error eliminando preparador solicitud " + (inser != null ? inser.getNumeroExpediente() : "(insercion = null)"), ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public int eliminarProspectorSolicitud23(FilaProspectorECA23VO inser, Connection con) throws Exception {
        log.debug("eliminarProspectorSolicitud23 - DAO");
        Statement st = null;
        try {
            String query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where id = " + inser.getId();
            log.debug("sql = " + query);
    ...
    public int eliminarPreparadorSolicitud23(FilaPreparadorECA23VO prep, Connection con) throws Exception {
        log.debug("eliminarPreparadorSolicitud23 - DAO");
        Statement st = null;
        try {
            String query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where id = " + prep.getId();
            log.debug("sql = " + query);
            ...
            log.error("Se ha producido un error eliminando preparador solicitud " + (prep != null ? prep.getNumeroExpediente() : "(preparador = null)"), ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public FilaInsercionesECA23VO guardarEca23SolInsercionVO(FilaInsercionesECA23VO inser, Connection con) throws Exception {
        log.debug("guardarEca23SolInsercionVO - DAO");
        Statement st = null;
        try {
            String query = null;
            query = "insert into " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOL_INSERCIONES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + "(NUMEROEXPEDIENTE, TIPODISCAPACIDAD, TIPOPORSEXO, NUMPERSONAS, PORCTRABAJO, IMPORTESOLIC, IMPORTECAL1ANO)"
                    + " values('" + inser.getNumeroExpediente() + "',"
                    + " '" + inser.getTipoDiscapacidad() + "',"
                    + " '" + inser.getTipoSexoEdad() + "',"
                    + " " + inser.getNumeroPersonas() + ","
                    + " " + inser.getPorcentajeTrabajo() + ","
                    + " " + inser.getImporteCalculadoUnAnio() + ", "
                    + " " + inser.getImporteSolicitado() + ")";
            ...
            log.error("Se ha producido un error guardando los datos del preparador " + (inser != null ? inser.getNumeroExpediente() : "(insercion = null)"), ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public EcaSolicitud23VO actualizaCuantiaSolic(EcaSolicitud23VO sol, Connection con) throws Exception {
        log.debug("actualizaCuantiaSolic - DAO");
        Statement st = null;
        try {
            String query = null;
            query = "UPDATE " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOLICITUD, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " SET " + " CUANTIASOLIC=" + sol.getCuantiaSolicitada()
                    + " WHERE NUMEROEXPEDIENTE='" + sol.getNumeroExpediente() + "'";
            log.debug("sql = " + query);
            ...
            log.error("Se ha producido un error guardando los datos del guardarEca23Importes " + (sol != null ? sol.getNumeroExpediente() : "(guardarEca23Importes = null)"), ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public EcaSolicitud23VO guardarEca23SeguimientosVO(EcaSolicitud23VO segui, Connection con) throws Exception {
        log.debug("guardarEca23SeguimientosVO - DAO");
        Statement st = null;
        try {
            String query = null;
            query = "UPDATE " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOLICITUD, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " SET TOTALSEG=" + segui.getTotalNumeroPersonas() + ", IMPORTESOLICSEG=" + segui.getImporteSolicitadoSeguimiento()
                    + ", MUJERESSEG=" + segui.getMujeresSeguimiento() + ", HOMBRESSEG=" + segui.getHombresSeguimiento()
                    + ", TOTALIMPCAL1ANO=" + segui.getTotalImporteCalculadoUnAnio()
                    + " WHERE NUMEROEXPEDIENTE='" + segui.getNumeroExpediente() + "'";
            log.debug("sql = " + query);
            ...
            log.error("Se ha producido un error guardando los datos del seguimiento " + (segui != null ? segui.getNumeroExpediente() : "(seguimiento = null)"), ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public EcaSolicitud23VO guardarHorasPreparadorSolicitud23(EcaSolicitud23VO segui, Connection con) throws Exception {
        log.debug("guardarHorasPreparadorSolicitud23 - DAO");
        Statement st = null;
        try {
            String query = null;
            query = "UPDATE " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOLICITUD, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " SET HORASJORNCOMPPREP=" + segui.getHorasConvenio() + ", SUMHORASPREP=" + segui.getSumatoriaHorasTotales()
                    + " WHERE NUMEROEXPEDIENTE='" + segui.getNumeroExpediente() + "'";
            log.debug("sql = " + query);
            ...
            log.error("Se ha producido un error guardando los datos de las horas de los preparadores " + (segui != null ? segui.getNumeroExpediente() : "( horas de los preparadores = null)"), ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public EcaSolicitud23VO updateSolicitudTotalInsercionesECA23(Integer numeroPersonas, BigDecimal totalImporteSolUnAnio, BigDecimal totalImporte, EcaSolicitud23VO segui, Connection con) throws Exception {
        log.debug("updateSolicitudTotalInsercionesECA23 - DAO");
        Statement st = null;
        try {
            String query = null;
            query = "UPDATE " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOLICITUD, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " SET TOTALNUMPERSONAS=" + numeroPersonas + ", TOTALIMPCAL1ANO=" + totalImporteSolUnAnio + ", TOTALIMPORTESOLIC=" + totalImporte
                    + " WHERE NUMEROEXPEDIENTE='" + segui.getNumeroExpediente() + "'";
            log.debug("sql = " + query);
    ...
    public FilaProspectorECA23VO guardarEca23SolProspectorVO(FilaProspectorECA23VO prospec, Connection con) throws Exception {
        log.debug("guardarEca23SolProspectorVO - DAO");
        Statement st = null;
        try {
            String query = null;
            query = "insert into " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOL_PROSPECTORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + "(NUMEROEXPEDIENTE, NOMBRE, DNI) values('" + prospec.getNumeroExpediente() + "', '" + prospec.getNombre() + "', '" + prospec.getDni() + "')";
            log.debug("sql = " + query);
            ...
            log.error("Se ha producido un error guardando los datos del prospector " + (prospec != null ? prospec.getNumeroExpediente() : "(prospec = null)"), ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public FilaPreparadorECA23VO guardarEca23SolPreparadorVO(FilaPreparadorECA23VO prep, Connection con) throws Exception {
        log.debug("guardarEca23SolPreparadorVO - DAO");
        Statement st = null;
        try {
            String query = null;
            query = "insert into " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_SOL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + "(NUMEROEXPEDIENTE, NOMBRE, DNI, HORAS_ANIO) values('" + prep.getNumeroExpediente() + "', '" + prep.getNombre() + "', '" + prep.getDni() + "', " + prep.getHorasECA() + ")";
            log.debug("sql = " + query);
            ...
            log.error("Se ha producido un error guardando los datos del preparador " + (prep != null ? prep.getNumeroExpediente() : "(prospec = null)"), ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public List<SelectItem> getListaDesplegable(Connection con, String idLista, Integer idioma) throws Exception {
        Statement st = null;
        ResultSet rs = null;
        List<SelectItem> lista = new ArrayList<SelectItem>();
        String query = "select DES_VAL_COD, DES_NOM as NOMBRE from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
        ...
        try {
            ...
            log.error("Se ha producido un error recuperando el desplegable " + idLista, ex);
            throw new Exception(ex);
        } finally {
            if (st != null) {
                st.close();
    ...
    public double getTotalJusInsercionesECA23(String numExp, Connection con) throws Exception {
        log.info("getTotalJusInsercionesECA23 - " + numExp);
        double total = 0;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "select sum(IMPORTE_INSER) TOTAL from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_JUS_INSERCIONES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where NUM_EXP = ?";
            log.debug("sql = " + query);
            int i = 1;
            ...
            log.error("Se ha producido un error recuperando el TOTAL de justificación de INSERCIONES ", ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) {
                ps.close();
    ...
    public double getTotalValInsercionesECA23(String numExp, Connection con) throws Exception {
        log.info("getTotalJusInsercionesECA23 - " + numExp);
        double total = 0;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "select sum(IMPORTE_INSER) TOTAL from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_INSERCIONES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where NUM_EXP = ?";
            log.debug("sql = " + query);
            int i = 1;
            ps = con.prepareStatement(query);
            ...
            log.error("Se ha producido un error recuperando el TOTAL de validación de INSERCIONES ", ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) {
                ps.close();
    ...
    public double getTotalValSeguimientosECA23(String numExp, Connection con) throws Exception {
        log.info("getTotalValSeguimientosECA23 - " + numExp);
        double total = 0;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "select sum(IMPORTE_SEGUI) TOTAL from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_SEGUIMIENTOS, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where NUM_EXP = ?";
            log.debug("sql = " + query);
            int i = 1;
            ps = con.prepareStatement(query);
    ...
    public List<JusPreparadoresECA23VO> getValPreparadoresECA23(String numExp, Connection con) throws Exception {
        log.info("getValPreparadoresECA23 - " + numExp);
        List<JusPreparadoresECA23VO> listaJusPrep = new ArrayList<JusPreparadoresECA23VO>();
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "select pre.* from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " pre"
                    + " where pre.NUM_EXP = ?"
                    + " order by pre.ID";
            log.debug("sql = " + query);
            int i = 1;
            ...
            ps.setString(i++, numExp);
            rs = ps.executeQuery();
            JusPreparadoresECA23VO fila = null;
            while (rs.next()) {
                fila = (JusPreparadoresECA23VO) MeLanbide35MappingUtils.getInstance().map(rs, JusPreparadoresECA23VO.class);
                listaJusPrep.add(fila);
            }
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando la lista de validación de PREPARADORES ", ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) {
                ps.close();
    ...
    public JusPreparadoresECA23VO getValPreparadoresECA23ById(Integer id, Connection con) throws Exception {
        log.info("getValPreparadoresECA23ById - " + id);
        JusPreparadoresECA23VO valPrep = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "select pre.* from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " pre"
                    + " where pre.ID_JUS_PREPARADORES = ?";
            log.debug("sql = " + query);
            int i = 1;
            ps = con.prepareStatement(query);
            ps.setInt(i++, id);
            ...
            if (rs.next()) {
                valPrep = (JusPreparadoresECA23VO) MeLanbide35MappingUtils.getInstance().map(rs, JusPreparadoresECA23VO.class);
            }
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando la lista de validación de PREPARADORES ", ex);
            throw new Exception(ex);
    ...
    public void insertValPreparadoresECA23(JusPreparadoresECA23VO ins, Connection con) throws Exception {
        log.debug("insertValPreparadoresECA23 - DAO");
        Statement st = null;
        ResultSet rs = null;
        SimpleDateFormat formatoFecha = new SimpleDateFormat(ConstantesMeLanbide35.FORMATO_FECHA);
        try {
            Integer id = this.getNextId(ConfigurationParameter.getParameter(ConstantesMeLanbide35.SEQ_ECA_23_VAL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES), con);
            String query = "INSERT INTO " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " (ID, NUM_EXP, NIF_PREPARADOR, JORNADA, PERMITIDOS, IMPORTE_PREP, ID_JUS_PREPARADORES)"
                    + " VALUES (" + id
                    + ", " + (ins.getNumExpediente() != null ? "'" + ins.getNumExpediente() + "'" : "null")
                    + ", " + (ins.getNifPreparador() != null ? "'" + ins.getNifPreparador() + "'" : "null")
                    + ", " + (ins.getJornada() != null ? ins.getJornada() : null)
                    + ", " + (ins.getPermitidos() != null ? ins.getPermitidos() : null)
                    + ", " + (ins.getImportePrep() != null ? ins.getImportePrep() : null)
                    + ", " + (ins.getId() != null ? ins.getId() : null) + ")";
            ...
            log.error("Se ha producido un error insertValPreparadoresECA23", ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public int deleteValPreparadorECA23(Integer id, Connection con) throws Exception {
        Statement st = null;
        try {
            String query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ID = " + id;
            log.debug("sql = " + query);
            st = con.createStatement();
            return st.executeUpdate(query);
        } catch (SQLException ex) {
            log.error("Se ha producido un error eliminando preparador " + id, ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public boolean updatePreparadorValECA23(JusPreparadoresECA23VO validacion, Connection con) throws Exception {
        log.debug("updatePreparadorValECA23 - DAO");
        Statement st = null;
        try {
            SimpleDateFormat format = new SimpleDateFormat(ConstantesMeLanbide35.FORMATO_FECHA);
            String query = null;
            query = "UPDATE " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " SET NIF_PREPARADOR = '" + validacion.getNifPreparador() + "',"
                    + " JORNADA = " + validacion.getJornada() + ","
                    + " PERMITIDOS = " + validacion.getPermitidos() + ","
                    + " IMPORTE_PREP = " + validacion.getImportePrep()
                    + " WHERE ID = " + validacion.getId();
            log.debug("sql = " + query);
            ...
            log.error("Se ha producido un error guardando los datos del preparador " + validacion != null ? validacion.getNumExpediente() : "(seguimiento = null)", ex.getMessage());
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public int getNumValSeguimientosPreparadorECA23(String numExp, String dniPreparador, Connection con) throws Exception {
        log.info("getNumValSeguimientosPreparadorECA23 - " + numExp);
        int total = 0;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "select count(*) TOTAL from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_SEGUIMIENTOS, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where NUM_EXP = ? and NIF_PREPARADOR = ?";
            log.debug("sql = " + query);
            int i = 1;
            ps = con.prepareStatement(query);
            ps.setString(i++, numExp);
            ps.setString(i++, dniPreparador);
            rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public List<JusSeguimientosECA23VO> getValSeguimientosECA23(String numExp, Connection con) throws Exception {
        log.info("getJustSeguimientosECA23 - " + numExp);
        List<JusSeguimientosECA23VO> listaJusSeg = new ArrayList<JusSeguimientosECA23VO>();
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "SELECT seg.*, sex.DES_NOM DES_SEXO, dis.DES_NOM DES_DIS, cole.DES_NOM DES_COL, cont.DES_NOM DES_CON, tse.DES_NOM DES_TSE, cna.LAN_CNAE_DESC_C CNAE_C, cna.LAN_CNAE_DESC_E CNAE_E"
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_SEGUIMIENTOS, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " seg"
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " sex"
                    + " on sex.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_SEXO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and sex.DES_VAL_COD = seg.SEXO "
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " dis "
                    + " on dis.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_TIPO_DIS, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and dis.DES_VAL_COD = seg.TIPODISC "
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " cole"
                    + " on cole.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_COLECTIVO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and cole.DES_VAL_COD = seg.COLECTIVO"
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " cont"
                    + " on cont.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_CONTRATO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and cont.DES_VAL_COD = seg.TIPOCONTRATO "
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " tse"
                    + " on tse.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_EDAD_SEXO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and tse.DES_VAL_COD = seg.TIPO_EDAD_SEXO "
                    + " inner join FLBGEN.S_LAN_CNAE_09 cna on cna.LAN_CNAE_CORR = seg.CNAE"
                    + " where seg.NUM_EXP = ?"
                    + " order by seg.ID";
            log.debug("sql = " + query);
            int i = 1;
            ...
            ps.setString(i++, numExp);
            rs = ps.executeQuery();
            JusSeguimientosECA23VO fila = null;
            while (rs.next()) {
                fila = (JusSeguimientosECA23VO) MeLanbide35MappingUtils.getInstance().map(rs, JusSeguimientosECA23VO.class);
                listaJusSeg.add(fila);
            }
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando la lista de justificación de SEGUIMIENTOS ", ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) {
                ps.close();
    ...
    public JusSeguimientosECA23VO getValSeguimientosECA23ById(Integer id, Connection con) throws Exception {
        log.info("getValSeguimientosECA23ById - " + id);
        JusSeguimientosECA23VO valSeg = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            ...
            rs = ps.executeQuery();
            if (rs.next()) {
                valSeg = (JusSeguimientosECA23VO) MeLanbide35MappingUtils.getInstance().map(rs, JusSeguimientosECA23VO.class);
            }
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando la lista de justificación de SEGUIMIENTOS ", ex);
            throw new Exception(ex);
    ...
    public double getTotalValSeguimientosPreparadorECA23(String numExp, String dniPreparador, Connection con) throws Exception {
        log.info("getTotalValSeguimientosPreparadorECA23 - " + numExp);
        double total = 0;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
    ...
    public void insertValSeguimientosECA23(JusSeguimientosECA23VO seguiValidacion, Connection con) throws Exception {
        log.debug("insertValSeguimientosECA23 - DAO");
        PreparedStatement st = null;
        SimpleDateFormat formatoFecha = new SimpleDateFormat(ConstantesMeLanbide35.FORMATO_FECHA);
        try {
            Integer id = this.getNextId(ConfigurationParameter.getParameter(ConstantesMeLanbide35.SEQ_ECA_23_VAL_SEGUIMIENTOS, ConstantesMeLanbide35.FICHERO_PROPIEDADES), con);
            String query = "INSERT INTO " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_SEGUIMIENTOS, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " (ID, NUM_EXP, DNI, NOMBRE, APELLIDO1, APELLIDO2, "
                    + "SEXO, FEC_NAC, TIPODISC, GRADO, COLECTIVO, TIPOCONTRATO, "
                    + "JORNADA, FEC_INICIO, TIPO_EDAD_SEXO,"
                    + "FEC_FIN, EMPRESA, NIF_EMPRESA, CNAE, IMPORTE_SEGUI, "
                    + "NIF_PREPARADOR, ID_JUS_SEGUIMIENTOS)"
                    + " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            st.setInt(1, id);
            st.setString(2, seguiValidacion.getNumExpediente());
            st.setString(3, seguiValidacion.getDni());
            st.setString(4, seguiValidacion.getNombre());
            st.setString(5, seguiValidacion.getApellido1());
            st.setString(6, seguiValidacion.getApellido2());
            st.setString(7, seguiValidacion.getSexo());
            if (seguiValidacion.getfNacimiento() != null)
                st.setDate(8, new java.sql.Date(seguiValidacion.getfNacimiento().getTime()));
            else
                st.setNull(8, java.sql.Types.DATE);
            st.setString(9, seguiValidacion.getTipoDisc());
            st.setDouble(10, seguiValidacion.getGrado());
            st.setString(11, seguiValidacion.getColectivo());
            st.setString(12, seguiValidacion.getTipoContrato());
            st.setDouble(13, seguiValidacion.getJornada());
            if (seguiValidacion.getfInicio() != null)
                st.setDate(14, new java.sql.Date(seguiValidacion.getfInicio().getTime()));
            else
                st.setNull(14, java.sql.Types.DATE);
            st.setString(15, seguiValidacion.getTipoEdadSexo());
            if (seguiValidacion.getfFin() != null)
                st.setDate(16, new java.sql.Date(seguiValidacion.getfFin().getTime()));
            else
                st.setNull(16, java.sql.Types.DATE);
            st.setString(17, seguiValidacion.getEmpresa());
            st.setString(18, seguiValidacion.getNifEmpresa());
            st.setString(19, seguiValidacion.getCnae());
            st.setDouble(20, seguiValidacion.getImporteSegui());
            st.setString(21, seguiValidacion.getNifPreparador());
            st.setNull(22, java.sql.Types.INTEGER);;

            st.executeUpdate();

        } catch (Exception ex) {
            log.error("Se ha producido un error insertValSeguimientosECA23", ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public int deleteValSeguimientoECA23(Integer id, Connection con) throws Exception {
        Statement st = null;
        try {
            String query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_SEGUIMIENTOS, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ID = " + id;
            log.debug("sql = " + query);
            st = con.createStatement();
            return st.executeUpdate(query);
        } catch (SQLException ex) {
            log.error("Se ha producido un error eliminando seguimiento " + id, ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public boolean updateSeguimientoValECA23(JusSeguimientosECA23VO seg, Connection con) throws Exception {
        log.debug("updateSeguimientoValECA23 - DAO");
        PreparedStatement st = null;
        try {
            String sql = "UPDATE " +
                    ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_SEGUIMIENTOS, ConstantesMeLanbide35.FICHERO_PROPIEDADES) +
                    " SET DNI = ?, NOMBRE = ?, APELLIDO1 = ?, APELLIDO2 = ?, SEXO = ?, " +
                    " FEC_NAC = ?, TIPODISC = ?, GRADO = ?, COLECTIVO = ?, TIPOCONTRATO = ?, " +
                    " JORNADA = ?, FEC_INICIO = ?, TIPO_EDAD_SEXO = ?, FEC_FIN = ?, EMPRESA = ?, " +
                    " NIF_EMPRESA = ?, CNAE = ?, IMPORTE_SEGUI = ? WHERE ID = ?";
            log.debug("sql = " + sql);

            st = con.prepareStatement(sql);
            st.setString(1, seg.getDni());
            st.setString(2, seg.getNombre());
            st.setString(3, seg.getApellido1());
            st.setString(4, seg.getApellido2());
            st.setString(5, seg.getSexo());

            if (seg.getfNacimiento() != null) st.setDate(6, new java.sql.Date(seg.getfNacimiento().getTime()));
            else st.setNull(6, java.sql.Types.DATE);

            st.setString(7, seg.getTipoDisc());


            if (seg.getGrado() != null) st.setObject(8, seg.getGrado(), java.sql.Types.DOUBLE);
            else st.setNull(8, java.sql.Types.DOUBLE);

            st.setString(9, seg.getColectivo());
            st.setString(10, seg.getTipoContrato());

            if (seg.getJornada() != null) st.setObject(11, seg.getJornada(), java.sql.Types.DOUBLE);
            else st.setNull(11, java.sql.Types.DOUBLE);

            if (seg.getfInicio() != null) st.setDate(12, new java.sql.Date(seg.getfInicio().getTime()));
            else st.setNull(12, java.sql.Types.DATE);

            st.setString(13, seg.getTipoEdadSexo());

            if (seg.getfFin() != null) st.setDate(14, new java.sql.Date(seg.getfFin().getTime()));
            else st.setNull(14, java.sql.Types.DATE);

            st.setString(15, seg.getEmpresa());
            st.setString(16, seg.getNifEmpresa());
            st.setString(17, seg.getCnae());

            if (seg.getImporteSegui() != null) st.setObject(18, seg.getImporteSegui(), java.sql.Types.DOUBLE);
            else st.setNull(18, java.sql.Types.DOUBLE);

            st.setInt(19, seg.getId());

            int res = st.executeUpdate();
            return res > 0;
        } catch (SQLException ex) {
            log.error("Error guardando seguimiento: " + (seg != null ? seg.getNumExpediente() : "(seguimiento = null)"), ex);
            throw ex;
        } finally {
            if (st != null) try { st.close(); } catch (SQLException e) { log.warn("Error cerrando PreparedStatement", e); }
        }
    }

    public JusInsercionesECA23VO getValInsercionesECA23ById(Integer id, Connection con) throws Exception {
        log.info("getValInsercionesECA23ById - " + id);
        JusInsercionesECA23VO valIns = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "select ins.*, sex.DES_NOM DES_SEXO, dis.DES_NOM DES_DIS, cole.DES_NOM DES_COL, cont.DES_NOM DES_CON, tse.DES_NOM DES_TSE, cna.LAN_CNAE_DESC_C CNAE_C, cna.LAN_CNAE_DESC_E CNAE_E"
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_INSERCIONES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " ins"
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " sex"
                    + " on sex.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_SEXO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and sex.DES_VAL_COD = ins.SEXO "
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " dis "
                    + " on dis.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_TIPO_DIS, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and dis.DES_VAL_COD = ins.TIPODISC "
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " cole"
                    + " on cole.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_COLECTIVO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and cole.DES_VAL_COD = ins.COLECTIVO"
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " cont"
                    + " on cont.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_CONTRATO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and cont.DES_VAL_COD = ins.TIPOCONTRATO "
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " tse"
                    + " on tse.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_EDAD_SEXO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and tse.DES_VAL_COD = ins.TIPO_EDAD_SEXO "
                    + " inner join FLBGEN.S_LAN_CNAE_09 cna on cna.LAN_CNAE_CORR = ins.CNAE"
                    + " where ins.ID_JUS_INSERCIONES = ?"
                    + " order by ins.ID";
            log.debug("sql = " + query);
            int i = 1;
            ps = con.prepareStatement(query);
            ...
            rs = ps.executeQuery();
            if (rs.next()) {
                valIns = (JusInsercionesECA23VO) MeLanbide35MappingUtils.getInstance().map(rs, JusInsercionesECA23VO.class);
            }
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando la lista de justificación de INSERCIONES ", ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) {
                ps.close();
    ...
    public List<JusInsercionesECA23VO> getValInsercionesECA23(String numExp, Connection con) throws Exception {
        log.info("getValInsercionesECA23 - " + numExp);
        List<JusInsercionesECA23VO> listaJusIns = new ArrayList<JusInsercionesECA23VO>();
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            ...
            ps.setString(i++, numExp);
            rs = ps.executeQuery();
            JusInsercionesECA23VO fila = null;
            while (rs.next()) {
                fila = (JusInsercionesECA23VO) MeLanbide35MappingUtils.getInstance().map(rs, JusInsercionesECA23VO.class);
                listaJusIns.add(fila);
            }
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando la lista de justificación de INSERCIONES ", ex);
            throw new Exception(ex);
    ...
    public void insertValInsercionesECA23(JusInsercionesECA23VO ins, Connection con) throws Exception {
        log.debug("insertValInsercionesECA23 - DAO");
        PreparedStatement ps = null;
        try {
            Integer id = this.getNextId(ConfigurationParameter.getParameter(ConstantesMeLanbide35.SEQ_ECA_23_VAL_INSERCIONES, ConstantesMeLanbide35.FICHERO_PROPIEDADES), con);
            String query = "INSERT INTO " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_INSERCIONES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " (ID, NUM_EXP, DNI, NOMBRE, APELLIDO1, APELLIDO2, "
                    + "SEXO, FEC_NAC, TIPODISC, GRADO, COLECTIVO, TIPOCONTRATO, "
                    + "JORNADA, FEC_INICIO, TIPO_EDAD_SEXO, "
                    + "FEC_FIN, DIAS, EDAD, EMPRESA, NIF_EMPRESA, CNAE, "
                    + "NIF_PREPARADOR, IMPORTE_INSER)"
                    + " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

            log.debug("sql = " + query);
            ps = con.prepareStatement(query);
            ps.setInt(1, id);
            ps.setString(2, ins.getNumExpediente());
            ps.setString(3, ins.getDni());
            ps.setString(4, ins.getNombre());
            ps.setString(5, ins.getApellido1());
            ps.setString(6, ins.getApellido2());
            ps.setString(7, ins.getSexo());
            if (ins.getfNacimiento() != null)
                ps.setDate(8, new java.sql.Date(ins.getfNacimiento().getTime()));
            else
                ps.setNull(8, java.sql.Types.DATE);
            ps.setString(9, ins.getTipoDisc());
            ps.setDouble(10, ins.getGrado());
            ps.setString(11, ins.getColectivo());
            ps.setString(12, ins.getTipoContrato());
            ps.setDouble(13, ins.getJornada());
            if (ins.getfInicio() != null)
                ps.setDate(14, new java.sql.Date(ins.getfInicio().getTime()));
            else
                ps.setNull(14, java.sql.Types.DATE);
            ps.setString(15, ins.getTipoEdadSexo());
            if (ins.getfFin() != null)
                ps.setDate(16, new java.sql.Date(ins.getfFin().getTime()));
            else
                ps.setNull(16, java.sql.Types.DATE);
            if (ins.getDias() != null)
                ps.setInt(17, ins.getDias());
            else
                ps.setNull(17, java.sql.Types.INTEGER);
            ps.setInt(18, ins.getEdad());
            ps.setString(19, ins.getEmpresa());
            ps.setString(20, ins.getNifEmpresa());
            ps.setString(21, ins.getCnae());
            ps.setString(22, ins.getNifPreparador());
            ps.setDouble(23, ins.getImporteInser());

            ps.executeUpdate();

        } catch (Exception ex) {
            log.error("Se ha producido un error insertValInsercionesECA23 id= " + ins.getId(), ex);
            throw new Exception(ex);
        } finally {
            try {
                if (ps != null) {
    ...
    public int deleteValInsercionesECA23(Integer id, Connection con) throws Exception {
        Statement st = null;
        try {
            String query = "delete from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_VAL_INSERCIONES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where ID = " + id;
            log.debug("sql = " + query);
            st = con.createStatement();
            return st.executeUpdate(query);
        } catch (SQLException ex) {
            log.error("Se ha producido un error eliminando Insercion " + id, ex);
            throw new Exception(ex);
        } finally {
            try {
                if (st != null) {
    ...
    public boolean updateInsercionValECA23(JusInsercionesECA23VO insercion, Connection con) throws Exception {
        log.debug("updateInsercionValECA23 - DAO");

        PreparedStatement ps = null;
        try {
            String tabla = ConfigurationParameter.getParameter(
                    ConstantesMeLanbide35.TABLA_ECA23_VAL_INSERCIONES,
                    ConstantesMeLanbide35.FICHERO_PROPIEDADES
            );

            String query = "UPDATE " + tabla + " SET "
                    + "DNI = ?, NOMBRE = ?, APELLIDO1 = ?, APELLIDO2 = ?, SEXO = ?, "
                    + "FEC_NAC = ?, TIPODISC = ?, GRADO = ?, COLECTIVO = ?, TIPOCONTRATO = ?, "
                    + "JORNADA = ?, FEC_INICIO = ?, TIPO_EDAD_SEXO = ?, FEC_FIN = ?, DIAS = ?, "
                    + "EDAD = ?, EMPRESA = ?, NIF_EMPRESA = ?, CNAE = ?, NIF_PREPARADOR = ?, "
                    + "IMPORTE_INSER = ? "
                    + "WHERE ID = ?";
            ...
            // Set parameters
            ps.setString(1, insercion.getDni());
            ps.setString(2, insercion.getNombre());
            ps.setString(3, insercion.getApellido1());
            ps.setString(4, insercion.getApellido2());
            ps.setString(5, insercion.getSexo());

            // Fechas
            if (insercion.getfNacimiento() != null)
                ps.setDate(6, new java.sql.Date(insercion.getfNacimiento().getTime()));
            else
                ps.setNull(6, java.sql.Types.DATE);

            ps.setString(7, insercion.getTipoDisc());
            ps.setDouble(8, insercion.getGrado());
            ps.setString(9, insercion.getColectivo());
            ps.setString(10, insercion.getTipoContrato());

            ps.setDouble(11, insercion.getJornada());

            if (insercion.getfInicio() != null)
                ps.setDate(12, new java.sql.Date(insercion.getfInicio().getTime()));
            else
                ps.setNull(12, java.sql.Types.DATE);

            ps.setString(13, insercion.getTipoEdadSexo());

            if (insercion.getfFin() != null)
                ps.setDate(14, new java.sql.Date(insercion.getfFin().getTime()));
            else
                ps.setNull(14, java.sql.Types.DATE);

            if (insercion.getDias() != null)
                ps.setInt(15, insercion.getDias());
            else
                ps.setNull(15, java.sql.Types.INTEGER);
            ps.setInt(16, insercion.getEdad());
            ps.setString(17, insercion.getEmpresa());
            ps.setString(18, insercion.getNifEmpresa());
            ps.setString(19, insercion.getCnae());
            ps.setString(20, insercion.getNifPreparador());
            ps.setDouble(21, insercion.getImporteInser());

            ps.setLong(22, insercion.getId()); // ID in WHERE clause

            log.debug("Executing update for ID = " + insercion.getId());
            int result = ps.executeUpdate();

            return result > 0;

        } catch (SQLException ex) {
            log.error("Error actualizando inserción: " +
                    (insercion != null ? insercion.getNumExpediente() : "(inserción = null)"), ex);
            throw new Exception("Error en updateInsercionValECA23", ex);
        } finally {
            if (ps != null) {
                try {
                    ps.close();
                } catch (SQLException e) {
                    log.error("Error cerrando PreparedStatement", e);
                }
            }
        }
    }

    public List<JusSeguimientosECA23VO> getValSeguimientosPreparadorECA23(String numExp, String dniPreparador, Connection con) throws Exception {
        log.info("getValSeguimientosPreparadorECA23 - " + numExp);
        List<JusSeguimientosECA23VO> listaJusSeg = new ArrayList<JusSeguimientosECA23VO>();
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "SELECT seg.*, sex.DES_NOM DES_SEXO, dis.DES_NOM DES_DIS, cole.DES_NOM DES_COL, cont.DES_NOM DES_CON, tse.DES_NOM DES_TSE, cna.LAN_CNAE_DESC_C CNAE_C, cna.LAN_CNAE_DESC_E CNAE_E"
                    ...
                    + " on tse.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_EDAD_SEXO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and tse.DES_VAL_COD = seg.TIPO_EDAD_SEXO "
                    + " inner join FLBGEN.S_LAN_CNAE_09 cna on cna.LAN_CNAE_CORR = seg.CNAE"
                    + " where seg.NUM_EXP = ? and seg.NIF_PREPARADOR = ?"
                    + " order by seg.ID";
            log.debug("sql = " + query);
            int i = 1;
            ps = con.prepareStatement(query);
            ...
            log.error("Se ha producido un error recuperando la lista de justificación de SEGUIMIENTOS por PREPARADOR ", ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) {
                ps.close();
    ...
    public List<GeneralComboVO> getValuesCombos(final String desCod, Connection con) throws Exception {
        log.info("getValuesCombos - " + desCod);
        List<GeneralComboVO> lista = new ArrayList<GeneralComboVO>();
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "select DES_VAL_COD, DES_NOM"
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " WHERE DES_COD = '" + desCod + "'";
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                GeneralComboVO fila = new GeneralComboVO();
                fila.setCodigo(rs.getString("DES_VAL_COD"));
                fila.setDescripcion(rs.getString("DES_NOM"));
                //fila = (GeneralComboVO) MeLanbide35MappingUtils.getInstance().map(rs, GeneralComboVO.class);
                lista.add(fila);
            }
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando la lista con los valores del combo " + desCod, ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) {
                ps.close();
    ...
    public List<GeneralComboVO> getValuesCombosExterno(final String nombreVista, Connection con) throws Exception {
        log.info("getValuesCombosExterno - " + nombreVista);
        List<GeneralComboVO> lista = new ArrayList<GeneralComboVO>();
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "SELECT LAN_CNAE_CORR, CONCAT(CONCAT(LAN_CNAE_DESC_C, '|'), LAN_CNAE_DESC_E) VALORES"
                + " FROM " + nombreVista;
            ...
            while (rs.next()) {
                GeneralComboVO fila = new GeneralComboVO();
                fila.setCodigo(rs.getString("LAN_CNAE_CORR"));
                fila.setDescripcion(rs.getString("VALORES"));
                //fila = (GeneralComboVO) MeLanbide35MappingUtils.getInstance().map(rs, GeneralComboVO.class);
                lista.add(fila);
            }
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando la lista con los valores del combo " + nombreVista, ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) {
                ps.close();
    ...
    public double getTotalJusSeguimientosECA23(String numExp, Connection con) throws Exception {
        log.info("getTotalJusSeguimientosECA23 - " + numExp);
        double total = 0;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "select sum(IMPORTE_SEGUI) TOTAL from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_JUS_SEGUIMIENTOS, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where NUM_EXP = ?";
            log.debug("sql = " + query);
            int i = 1;
            ps = con.prepareStatement(query);
     ...
     * @throws Exception
     */
    public double getTotalJusSeguimientosPreparadorECA23(String numExp, String dniPreparador, Connection con) throws Exception {
        log.info("getTotalJusSeguimientosPreparadorECA23 - " + numExp);
        double total = 0;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
    ...
    public int getNumJusSeguimientosECA23(String numExp, Connection con) throws Exception {
        log.info("getTotalJusSeguimientosPreparadorECA23 - " + numExp);
        int total = 0;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "select count(*) TOTAL from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_JUS_SEGUIMIENTOS, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where NUM_EXP = ? ";
            log.debug("sql = " + query);
            int i = 1;
            ps = con.prepareStatement(query);
     ...
     * @throws Exception
     */
    public int getNumJusSeguimientosPreparadorECA23(String numExp, String dniPreparador, Connection con) throws Exception {
        log.info("getTotalJusSeguimientosPreparadorECA23 - " + numExp);
        int total = 0;
        PreparedStatement ps = null;
        ResultSet rs = null;
    ...
    public List<JusInsercionesECA23VO> getJustInsercionesECA23(String numExp, Connection con) throws Exception {
        log.info("getJustInsercionesECA23 - " + numExp);
        List<JusInsercionesECA23VO> listaJusIns = new ArrayList<JusInsercionesECA23VO>();
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "select ins.*, sex.DES_NOM DES_SEXO, dis.DES_NOM DES_DIS, cole.DES_NOM DES_COL, cont.DES_NOM DES_CON, tse.DES_NOM DES_TSE, cna.LAN_CNAE_DESC_C CNAE_C, cna.LAN_CNAE_DESC_E CNAE_E"
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_JUS_INSERCIONES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " ins"
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " sex"
                    + " on sex.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_SEXO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and sex.DES_VAL_COD = ins.SEXO "
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " dis "
                    + " on dis.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_TIPO_DIS, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and dis.DES_VAL_COD = ins.TIPODISC "
     ...
     * @throws Exception
     */
    public JusInsercionesECA23VO getJusInsercionECA23(String numExp, String id, Connection con) throws Exception {
        log.info("getJustInsercionECA23 - " + id);
        JusInsercionesECA23VO insercion = new JusInsercionesECA23VO();
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            ...
            rs = ps.executeQuery();
            if (rs.next()) {
                insercion = (JusInsercionesECA23VO) MeLanbide35MappingUtils.getInstance().map(rs, JusInsercionesECA23VO.class);
            }
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando la justificación de INSERCION ", ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) {
                ps.close();
    ...
    public List<JusPreparadoresECA23VO> getJusPreparadoresECA23(String numExp, Connection con) throws Exception {
        log.info("getJustSeguimientosECA23 - " + numExp);
        List<JusPreparadoresECA23VO> listaJusPrep = new ArrayList<JusPreparadoresECA23VO>();
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "select pre.* from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_JUS_PREPARADORES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " pre"
                    + " where pre.NUM_EXP = ?"
                    + " order by pre.ID";
            log.debug("sql = " + query);
            int i = 1;
            ...
            log.error("Se ha producido un error recuperando la lista de justificación de PREPARADORES ", ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) {
                ps.close();
    ...
    public JusPreparadoresECA23VO getJusPreparadorECA23(String numExp, String id, Connection con) throws Exception {
        log.info("getJustInsercionECA23 - " + id);
        ...
        try {
            ...
            rs = ps.executeQuery();
            if (rs.next()) {
                preparador = (JusPreparadoresECA23VO) MeLanbide35MappingUtils.getInstance().map(rs, JusPreparadoresECA23VO.class);
            }
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando la justificación de INSERCION ", ex);
            throw new Exception(ex);
    ...
    public List<JusSeguimientosECA23VO> getJustSeguimientosECA23(String numExp, Connection con) throws Exception {
        log.info("getJustSeguimientosECA23 - " + numExp);
        List<JusSeguimientosECA23VO> listaJusSeg = new ArrayList<JusSeguimientosECA23VO>();
        PreparedStatement ps = null;
        ResultSet rs = null;
        ...
        try {
            query = "SELECT seg.*, sex.DES_NOM DES_SEXO, dis.DES_NOM DES_DIS, cole.DES_NOM DES_COL, cont.DES_NOM DES_CON, tse.DES_NOM DES_TSE, cna.LAN_CNAE_DESC_C CNAE_C, cna.LAN_CNAE_DESC_E CNAE_E"
                    + " from " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_ECA23_JUS_SEGUIMIENTOS, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " seg"
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " sex"
                    + " on sex.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_SEXO, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and sex.DES_VAL_COD = seg.SEXO "
                    + " inner join " + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_VALORES_DESPLEGABLES, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + " dis "
                    + " on dis.DES_COD = '" + ConfigurationParameter.getParameter(ConstantesMeLanbide35.COD_DES_TIPO_DIS, ConstantesMeLanbide35.FICHERO_PROPIEDADES) + "' and dis.DES_VAL_COD = seg.TIPODISC "
    ...
    public List<JusSeguimientosECA23VO> getJustSeguimientosPreparadorECA23(String numExp, String dniPreparador, Connection con) throws Exception {
        log.info("getJustSeguimientosECA23 - " + numExp);
        List<JusSeguimientosECA23VO> listaJusSeg = new ArrayList<JusSeguimientosECA23VO>();
        PreparedStatement ps = null;
        ResultSet rs = null;
     ...
     * @throws Exception
     */
    public  double getSuplementarioImporte(String numExp, String campo, Connection con) throws Exception {
        log.info("getSuplementarioImporte " + campo + " - " + numExp);
        double importe = 0;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = null;
        try {
            query = "select tnu_valor from "
                   + ConfigurationParameter.getParameter(ConstantesMeLanbide35.TABLA_NUMERICOS_EXP, ConstantesMeLanbide35.FICHERO_PROPIEDADES)
                    + " where TNU_NUM = ? and TNU_COD = ?";
            log.debug("sql = " + query);
            int i = 1;
            ps = con.prepareStatement(query);
            ps.setString(i++, numExp);
            ps.setString(i++, campo);
            rs = ps.executeQuery();
            if (rs.next()) {
                importe = rs.getDouble("TNU_VALOR");
            }
        } catch (Exception ex) {
            log.error("Se ha producido un error recuperando un importe ", ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) {
                ps.close();
