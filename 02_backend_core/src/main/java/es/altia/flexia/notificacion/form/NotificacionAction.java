...
private static final String MESSAGE_SUCCESS = "Sge.FirmaDocumentoTramitacionForm.FirmaGuardadaCorrectamente";
      ...
      if (opcion.equals("mostrarPantallaNotificacion")) {
          ...
          if (session != null) {
           ...
           try{
                ...
                autorizadosSel = new StringTokenizer(listaAutorizadosSeleccionados,ConstantesDatos.SEPARADOR,false);
                adjuntosSel = new StringTokenizer(listaAdjuntosSeleccionados,ConstantesDatos.SEPARADOR,false);
    ...
    */
      ...
      if("detalleNotificacion".equals(opcion)){
       ...
       if(opcion.equals("descargarDocumentoTramitacion")){
          ...
          if(unidad!=null && !"".equals(unidad) && codTramite!=null && !"".equals(codTramite) && ocuTramite!=null && !"".equals(ocuTramite) && numExpediente!=null && !"".equals(numExpediente) && nombre!=null && !"".equals(nombre)){
       ...
       else if (opcion.equals("mostrarPantallaDocumentosExpediente")){
           m_Log.debug("Notificacion mostrarPantallaDocumentosExternos");
           if (session != null) {
               notificacionForm = (NotificacionForm) session.getAttribute("notificacionForm");
           }
           ...
           String numExpediente = request.getParameter("numero");
           try {
               ArrayList<AdjuntoNotificacionVO> ficherosExpediente = AdjuntoNotificacionManager.getInstance().getDocumentosSupExpediente(numExpediente,params);
               ArrayList<AdjuntoNotificacionVO> ficherosTramite = AdjuntoNotificacionManager.getInstance().getDocumentosSupTramite(numExpediente,params);
               ArrayList<AdjuntoNotificacionVO> ficherosExternos = AdjuntoNotificacionManager.getInstance().getDocumentosExternos(numExpediente,params);
               //Cargar datos en el formulario
               request.setAttribute("ListaDocsExpediente", ficherosExpediente);
               request.setAttribute("ListaDocsTramite", ficherosTramite);
               request.setAttribute("ListaDocsExternos",ficherosExternos);

               m_Log.debug("Cod notif: " + notificacionForm.getCodNotificacion());
               m_Log.debug("Cod tram : " + notificacionForm.getCodigoTramite());
               notificacionForm.setNombreExpediente(codProcedimiento);
               notificacionForm.setNumExpediente(numExpediente);
               session.setAttribute("notificacionForm", notificacionForm);
           }catch(Exception e){
               e.printStackTrace();
           }
       }
       else if (opcion.equals("seleccionarDocumentosExpediente")) {
           m_Log.debug("Notificacion seleccionarDocumentosExpediente");
           String codMunicipio = request.getParameter("codMunicipio");
           String numExpediente = request.getParameter("numero");
           String codTramite = request.getParameter("codTramite");
           String ocurrenciaTramite = request.getParameter("ocurrenciaTramite");
           String codNotificacion = request.getParameter("codNotificacion");
           String listaExpedienteSelec = request.getParameter("listaExpedienteSelec");
           String listaTramiteSelec = request.getParameter("listaTramiteSelec");
           String listaExternosSelec = request.getParameter("listaExternosSelec");

           ArrayList<AdjuntoNotificacionVO> lote = new ArrayList<AdjuntoNotificacionVO>();

           try {
               String[] datosExp = numExpediente.split(ConstantesDatos.BARRA);
               // expediente
               if (listaExpedienteSelec != null && !listaExpedienteSelec.isEmpty() && !listaExpedienteSelec.equals("")) {
                   String[] parts = listaExpedienteSelec.split(ConstantesDatos.GUION_BAJO);
                   for (String oid:parts) {
                       AdjuntoNotificacionVO doc = AdjuntoNotificacionManager.getInstance().getDocumentoExpedienteOid(numExpediente, oid, params);
                       doc.setNumeroExpediente(numExpediente);
                       doc.setCodigoMunicipio(Integer.parseInt(codMunicipio));
                       doc.setCodigoProcedimiento(datosExp[1]);
                       doc.setEjercicio(Integer.parseInt(datosExp[0]));
                       doc.setCodigoNotificacion(Integer.parseInt(codNotificacion));
                       doc.setCodigoTramite(Integer.parseInt(codTramite));
                       doc.setOcurrenciaTramite(Integer.parseInt(ocurrenciaTramite));
                       lote.add(doc);
                   }
               }
               // tramite
               if (listaTramiteSelec != null && !listaTramiteSelec.isEmpty() && !listaTramiteSelec.equals("")) {
                   String[] parts = listaTramiteSelec.split(ConstantesDatos.GUION_BAJO);
                   for (String oid:parts) {
                       AdjuntoNotificacionVO doc = AdjuntoNotificacionManager.getInstance().getDocumentoTramiteOid(numExpediente, oid, params);
                       doc.setNumeroExpediente(numExpediente);
                       doc.setCodigoMunicipio(Integer.parseInt(codMunicipio));
                       doc.setCodigoProcedimiento(datosExp[1]);
                       doc.setEjercicio(Integer.parseInt(datosExp[0]));
               ...
               // externos
               if (listaExternosSelec != null && !listaExternosSelec.isEmpty() && !listaExternosSelec.equals("")) {
                   String[] parts = listaExternosSelec.split(ConstantesDatos.GUION_BAJO);
                   for (String oid:parts) {
                       AdjuntoNotificacionVO doc = AdjuntoNotificacionManager.getInstance().getDocumentoExternoOid(numExpediente, oid, params);
                       doc.setNumeroExpediente(numExpediente);
                       doc.setCodigoMunicipio(Integer.parseInt(codMunicipio));
                       doc.setCodigoProcedimiento(datosExp[1]);
                       doc.setEjercicio(Integer.parseInt(datosExp[0]));
               ...
               if(!lote.isEmpty() && lote.size()>0){
                   exito = AdjuntoNotificacionManager.getInstance().insertarDocumentosSeleccionados(lote, params);
               }
               if (exito) {
                   m_Log.debug("Insertado(s) "+ lote.size() + " documento(s)");
                   // Se recupera la lista de adjuntos para actualizar la vista
                   ArrayList<AdjuntoNotificacionVO> adjuntosExternos = AdjuntoNotificacionManager.getInstance().getListaAdjuntosExterno(Integer.parseInt(codNotificacion),params);
                   request.setAttribute("LISTA_ADJUNTOS_EXTERNOS",adjuntosExternos);
               } else {
           ...
           return mapping.findForward("seleccionarDocumentosExpediente");
       }
      } else { // No hay usuario.
