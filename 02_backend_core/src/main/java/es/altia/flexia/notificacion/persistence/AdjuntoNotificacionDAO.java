...
public class AdjuntoNotificacionDAO {
    ...
    protected static Logger m_Log = LogManager.getLogger(AdjuntoNotificacionDAO.class.getName());
...
//Recupera los documentos adjuntos asociados a una determinada notificación de la tabla ADJUNTO_NOTIFICACION
 ...
 /**
  ...
  public AdjuntoNotificacionVO getAdjuntoById(int codAdjunto,boolean expedienteHistorico,Connection con){
    ...
    public ArrayList<AdjuntoNotificacionVO> getDocumentosSupExpediente(String numExpediente, Connection con){
        ResultSet rs = null;
        Statement st = null;
        String sql = null;
        ArrayList<AdjuntoNotificacionVO> arrayRetorno=new ArrayList<AdjuntoNotificacionVO>();
        try {
            // Ficheros en Campo Suplementario Expediente
            sql =  "SELECT E.TFI_NOMFICH, E.TFI_MIME, D.RELDOC_OID FROM E_TFI E " +
                    " INNER JOIN MELANBIDE_DOKUSI_RELDOC_DOCCSE D " +
                    " ON E.TFI_MUN = D.RELDOC_MUN AND E.TFI_EJE = D.RELDOC_EJE AND E.TFI_NUM = D.RELDOC_NUM AND E.TFI_COD = D.RELDOC_COD  " +
                    " WHERE E.TFI_NUM = '" + numExpediente + "' AND E.TFI_MIME IN ('application/pdf', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')";
            m_Log.debug(sql);
            st = con.createStatement();
            rs = st.executeQuery(sql);
            while(rs.next()){
                AdjuntoNotificacionVO adjunto = new AdjuntoNotificacionVO();
                adjunto.setNombre(rs.getString("TFI_NOMFICH"));
                adjunto.setContentType(rs.getString("TFI_MIME"));
                adjunto.setOidDokusi(rs.getString("RELDOC_OID"));
                arrayRetorno.add(adjunto);
            }

        }catch(Exception e){
            e.printStackTrace();
            m_Log.error("Excepcion al recuperar Ficheros de expediente y trámite: " + e.getMessage());
        }finally{
            try{
                if(st!=null) st.close();
                if(rs!=null) rs.close();
                ...
                bde.printStackTrace();
                m_Log.error("Excepcion capturada en: " + getClass().getName() + ": " + bde.getMessage());
            }
        }
        return arrayRetorno;
    }
     ...
     * @return
     */
    public ArrayList<AdjuntoNotificacionVO> getDocumentosSuplementariosTram(String numExpediente, Connection con){
        ResultSet rs = null;
        Statement st = null;
        String sql = null;
        ArrayList<AdjuntoNotificacionVO> arrayRetorno=new ArrayList<AdjuntoNotificacionVO>();
        try {
            // Ficheros en Campo Suplementario Tramite
            sql =  "SELECT E.TFIT_NOMFICH, E.TFIT_MIME, D.RELDOC_OID FROM E_TFIT E " +
                    " INNER JOIN MELANBIDE_DOKUSI_RELDOC_DOCCST D " +
                    " ON E.TFIT_PRO = D.RELDOC_PRO AND E.TFIT_MUN = D.RELDOC_MUN AND E.TFIT_EJE = D.RELDOC_EJE AND E.TFIT_NUM = D.RELDOC_NUM " +
                    " AND E.TFIT_COD = D.RELDOC_COD AND E.TFIT_TRA = D.RELDOC_TRA AND E.TFIT_OCU = D.RELDOC_OCU " +
                    " WHERE E.TFIT_NUM = '" + numExpediente + "' AND E.TFIT_MIME IN ('application/pdf', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')";
            m_Log.debug(sql);
            st = con.createStatement();
            rs = st.executeQuery(sql);
            while (rs.next()) {
                AdjuntoNotificacionVO adjunto = new AdjuntoNotificacionVO();
                adjunto.setNombre(rs.getString("TFIT_NOMFICH"));
                adjunto.setContentType(rs.getString("TFIT_MIME"));
                adjunto.setOidDokusi(rs.getString("RELDOC_OID"));
                adjunto.setTipoDocumento(null);
                arrayRetorno.add(adjunto);
            }
     ...
     * @return
     */
    public ArrayList<AdjuntoNotificacionVO> getDocumentosExternos(String numExpediente, Connection con) {
        ResultSet rs = null;
        Statement st = null;
        String sql = null;
        ArrayList<AdjuntoNotificacionVO> arrayRetorno=new ArrayList<AdjuntoNotificacionVO>();
        try {
            sql =  "select E.DOC_EXT_NOM, E.DOC_EXT_TIP, D.RELDOC_OID  FROM E_DOC_EXT E " +
                    " inner join MELANBIDE_DOKUSI_RELDOC_DOCEXT D " +
                    " ON E.DOC_EXT_MUN = D.RELDOC_MUN and E.DOC_EXT_EJE = D.RELDOC_EJE and E.DOC_EXT_NUM = D.RELDOC_NUM and E.DOC_EXT_COD = D.RELDOC_COD " +
                    " WHERE E.DOC_EXT_NUM = '" + numExpediente + "' AND E.DOC_EXT_TIP IN ('application/pdf', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')";
            m_Log.debug(sql);
            st = con.createStatement();
            rs = st.executeQuery(sql);
            while(rs.next()){
                ...
                adjunto.setNombre(rs.getString("DOC_EXT_NOM"));
                adjunto.setContentType(rs.getString("DOC_EXT_TIP"));
                adjunto.setOidDokusi(rs.getString("RELDOC_OID"));
                arrayRetorno.add(adjunto);
            }
        } catch(Exception e){
            e.printStackTrace();
            m_Log.error("Excepcion al recuperar Ficheros externos de expediente : " + e.getMessage());
        }finally{
            try{
                if(st!=null) st.close();
                if(rs!=null) rs.close();
     ...
     * @return
     */
    public AdjuntoNotificacionVO getDocumentoExpedienteOid(String numExpediente,String oid, Connection con){
        ResultSet rs = null;
        Statement st = null;
        String sql = null;
        AdjuntoNotificacionVO adjunto = new AdjuntoNotificacionVO();
        try {
            sql =  "SELECT E.TFI_NOMFICH, E.TFI_MIME, D.RELDOC_OID FROM E_TFI E " +
                    " INNER JOIN MELANBIDE_DOKUSI_RELDOC_DOCCSE D " +
                    " ON E.TFI_MUN = D.RELDOC_MUN AND E.TFI_EJE = D.RELDOC_EJE AND E.TFI_NUM = D.RELDOC_NUM AND E.TFI_COD = D.RELDOC_COD  and D.RELDOC_OID = '" + oid + "' " +
                    " WHERE E.TFI_NUM = '" + numExpediente + "' AND E.TFI_MIME IN ('application/pdf', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')";
            m_Log.debug(sql);
            st = con.createStatement();
            rs = st.executeQuery(sql);
            ...
            e.printStackTrace();
            m_Log.error("Excepcion al recuperar Fichero de expediente por OID: " + e.getMessage());
        } finally{
            try{
                if(st!=null) st.close();
                if(rs!=null) rs.close();
                ...
                bde.printStackTrace();
                m_Log.error("Excepcion capturada en: " + getClass().getName() + " al recuperar Fichero de expediente por OID: " + bde.getMessage());
            }
        }
    ...
    public AdjuntoNotificacionVO getDocumentoTramiteOid(String numExpediente,String oid, Connection con){
        ResultSet rs = null;
        Statement st = null;
        String sql = null;
        AdjuntoNotificacionVO adjunto = new AdjuntoNotificacionVO();
        try {
            sql =  "SELECT E.TFIT_NOMFICH, E.TFIT_MIME, D.RELDOC_OID FROM E_TFIT E " +
                    " INNER JOIN MELANBIDE_DOKUSI_RELDOC_DOCCST D " +
                    " ON E.TFIT_PRO = D.RELDOC_PRO AND E.TFIT_MUN = D.RELDOC_MUN AND E.TFIT_EJE = D.RELDOC_EJE AND E.TFIT_NUM = D.RELDOC_NUM and D.RELDOC_OID = '"  + oid +  "' " +
                    " WHERE E.TFIT_NUM = '" + numExpediente + "' AND E.TFIT_MIME IN ('application/pdf', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')";
            m_Log.debug(sql);
            st = con.createStatement();
            rs = st.executeQuery(sql);
            ...
            e.printStackTrace();
            m_Log.error("Excepcion al recuperar Fichero de trámite por OID: " + e.getMessage());
        } finally{
            try{
                if(st!=null) st.close();
                if(rs!=null) rs.close();
    ...
    public AdjuntoNotificacionVO getDocumentoExternoOid(String numExpediente,String oid, Connection con){
        ResultSet rs = null;
        Statement st = null;
        String sql = null;
        AdjuntoNotificacionVO adjunto = new AdjuntoNotificacionVO();
        try {
            sql =  "select E.DOC_EXT_NOM, E.DOC_EXT_TIP, D.RELDOC_OID  FROM E_DOC_EXT E " +
                    " inner join MELANBIDE_DOKUSI_RELDOC_DOCEXT D " +
                    " ON E.DOC_EXT_MUN = D.RELDOC_MUN and E.DOC_EXT_EJE = D.RELDOC_EJE and E.DOC_EXT_NUM = D.RELDOC_NUM and E.DOC_EXT_COD = D.RELDOC_COD and D.RELDOC_OID = '" + oid + "' " +
                    " WHERE E.DOC_EXT_NUM = '" + numExpediente + "' AND E.DOC_EXT_TIP IN ('application/pdf', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')";
            m_Log.debug(sql);
            st = con.createStatement();
            rs = st.executeQuery(sql);
            ...
            e.printStackTrace();
            m_Log.error("Excepcion al recuperar Fichero externo de expediente por OID: " + e.getMessage());
        }finally{
            try{
                if(st!=null) st.close();
                if(rs!=null) rs.close();
     ...
     * @return
     */
    public boolean insertarAdjuntoExternoDokusi(AdjuntoNotificacionVO adjuntoNotificacionVO,  Connection con) {
        m_Log.info("insertarAdjuntoExternoDokusi  " );
        PreparedStatement ps = null;
        String sql = "";
        boolean exito = false;
        try {
            con.setAutoCommit(false);
            sql = "insert into ADJUNTO_EXT_NOTIFICACION(ID, COD_MUNICIPIO, NUM_EXPEDIENTE, COD_TRAMITE, OCU_TRAMITE, FECHA, ID_NOTIFICACION, NOMBRE, TIPO_MIME, ESTADO_FIRMA)"
            ...
            ps.setString(i++, "0");
            int rowsInserted = ps.executeUpdate();
            m_Log.debug(" Filas insertadas en adjunto_ext_notificacion:  " + rowsInserted);
            if (rowsInserted == 1) {
                int primkey = 0;
                ...
                m_Log.debug(" Id generado:  " + primkey);
                i = 1;
                ...
                ps.setInt(i++, adjuntoNotificacionVO.getOcurrenciaTramite());
                ps.setInt(i++, primkey);
                ps.setString(i++, adjuntoNotificacionVO.getOidDokusi());
//                ps.executeUpdate();
            ...
            e.printStackTrace();
            m_Log.error(("Excepcion capturada en: " + this.getClass().getName() + e.getMessage()));
            try {
                con.rollback();
            }  catch (SQLException bde) {
                bde.printStackTrace();
                m_Log.error(("Excepcion capturada en: " + this.getClass().getName()));
                return false;
            }
            exito = false;
        } finally {
