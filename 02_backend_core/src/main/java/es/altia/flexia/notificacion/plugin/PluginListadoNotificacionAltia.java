...
public class PluginListadoNotificacionAltia implements PluginListadoNotificacion{

    protected static Config conf =
            ConfigServiceHelper.getConfig("notificaciones");
    
    private Logger log = LogManager.getLogger(PluginListadoNotificacionAltia.class);
    ...
    public ArrayList<NotificacionVO> getNotificacionesExpediente(int codOrganizacion, int idioma, String numExpediente, String expHistorico, Connection con) throws TechnicalException {
        ...
        try{
            notificaciones = NotificacionDAO.getInstance().getNotificacionesExpediente(codOrganizacion,idioma,numExpediente,expHistorico,con, true);
        }catch (Exception e) {
            e.printStackTrace();
	}finally{
