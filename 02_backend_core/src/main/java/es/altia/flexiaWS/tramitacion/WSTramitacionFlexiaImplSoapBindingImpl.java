...
import es.altia.flexiaWS.tramitacion.bd.acceso.CamposSuplementariosDAO;
...
public class WSTramitacionFlexiaImplSoapBindingImpl {
    ...
    Logger m_Log = LogManager.getLogger(es.altia.flexiaWS.tramitacion.WSTramitacionFlexiaImplSoapBindingImpl.class);

    /**
     * Devuelve los tramites iniciados en el expediente.
     * @param idExpediente
     * @param idTramite
     * @param condFinalizacionVO
     * @param origenLlamada
     * @param infoConexion
     * @return
                                        ...
                                        if (resultadoTramitesIniciar.size() == 0 && 
                                            ...
                                            if (teVO.getRespOpcion() != null && teVO.getRespOpcion().equals("tramiteFinalizado")) {
                                                ...
                                                if (!listaTramitesIniciar.isEmpty() && listaTramitesIniciarOriginal != null && listaTramitesNoIniciados != null && 
                                                    ...
                                                    paramsRegistroOperacion.setAtributo("origenLlamada", teVO.getOrigenLlamada());
    ...
    public RespuestasTramitacionVO iniciarExpedienteOperacion(ExpedienteVO expVO, InfoConexionVO infoConexion)
        ...
        Map<String, String> camposExpediente = null;
        Map<String, String> camposTramite = null;
        Map<String, String> camposTercero = null;
            ...
            if (!aplicacionTrue) {
                ...
                if (expedienteValido == 1) {
                   ...
                   InteresadoVO[] interesados = expVO.getTercero();
                    ...
                    if (interesados != null) {
                        ...
                        for (int x = 0; x < interesados.length; x++) {
                          ...
                          boolean continuar = true;
                            ...
                            int rolInteresado=interesadoExpedienteVO.getRol();
                            ...
                            String verTercero=null;
                            String codDomicilio=null;

                            try {

                                //El codigo de interesado no viene informado: se inicia expediente desde WSTramitacion, o
                                //El codigo de interesado viene informado con -1: se inicia desde el batch de KONTU y el interesado se recupera del XML de datos de integracion
                                if(interesadoExpedienteVO.getCodigo() == null || interesadoExpedienteVO.getCodigo().equals("-1")){
                                    ...
                                    //El dato de tipoVia en la request desde WSTramitacion se informa en dicha propiedad pero para insertar en DNN_TVI se obtiene la propiedad IdTipoVia
                                    domVO.setIdTipoVia(Integer.toString(interesadoExpedienteVO.getDomicilio().getTipoVia()));
                                    ...
                                    codTercero = terVO.getIdentificador();
                                    verTercero = terVO.getVersion();
                                    codDomicilio = terVO.getIdDomicilio();

                                    // Se graban los campos suplementarios del tercero si este se recupera del XML de datos de integracion
                                    // como parte del proceso del batch de KONTU
                                    if(interesadoExpedienteVO.getCodigo()!=null && interesadoExpedienteVO.getCodigo().equals("-1")) {
                                        interesadoExpedienteVO.setCodigo(terVO.getIdentificador());
                                        String campos = interesadoExpedienteVO.getCampos();
                                        if (campos != null) {
                                            Map <String, Map <String, String>> mapa = XMLTraductor.traduccionCSXmlToHashMap(campos);
                                            camposTercero = mapa.get("datosCampos");
                                            //Tratamos los CS: del expediente primero y del tramite de inicio después
                                            tratarCamposSuplementarios(expVO, null, interesadoExpedienteVO, camposTercero,  con);

                                        } else {
                                            m_Log.debug("No hay campos suplementarios para el tercero");
                                        }
                                    }
                                } else { //El codigo viene informado: alta desde batch de KONTU - interesado del registro
                                    codTercero = interesadoExpedienteVO.getCodigo();
                                    verTercero = interesadoExpedienteVO.getVersion();
                                    codDomicilio = interesadoExpedienteVO.getCoddomicilio();
                                }
                                ...
                                salida.setCodigoError(6);
                                salida.setIncidencias("Ha ocurrido un error: " + e.getMessage());
                                bd.rollBack(con);
                                respuesta.setSalida(salida);
                                continuar = false;
                                return (respuesta);
                            ...
                            //Creamos un objeto GeneralValueObject con los datos particulares de este interesado y lo anadimos a la lista
                            GeneralValueObject gVOIntActual = new GeneralValueObject();
                            gVOIntActual.setAtributo("codTercero", codTercero);
                            gVOIntActual.setAtributo("version", verTercero);
                            gVOIntActual.setAtributo("codDomicilio", codDomicilio);
                            gVOIntActual.setAtributo("codRol", Integer.toString(rolInteresado));

                            listaInteresados.add(gVOIntActual);
                        }
                       //El alta de interesado ya se hace en el método de iniciar expediente
                       gVO.setAtributo("listaInteresados", listaInteresados);
                   }
                    ...
                    if(expVO.getTramite() != null){
                        gVO.setAtributo("codTramiteInicio", expVO.getTramite().getCodTramite());
                    }
//
                    ...
                    // Se graban los campos suplementarios que vienen en ExpedienteVO (del expediente y/o del tramite de inicio)
                    String campos = expVO.getCampos();
                    if (campos != null) {
                        Map <String, Map <String, String>> mapa = XMLTraductor.traduccionCSXmlToHashMap(campos);
                        camposExpediente = mapa.get("datosCampos");
                        camposTramite = mapa.get("datosExtra");
                        //Tratamos los CS: del expediente primero y del tramite de inicio después
                        tratarCamposSuplementarios(expVO, null, null, camposExpediente,  con);
                        tratarCamposSuplementarios(expVO, expVO.getTramite(), null, camposTramite,  con);
...
//                        
    ...
    private void tratarCamposSuplementarios(ExpedienteVO expVO, TramiteVO tramVO, InteresadoVO interesadoVO,
                                            Map<String, String> datos, Connection con) {
        String textoEntidad = "";
        if(interesadoVO != null) {
            textoEntidad = "tercero";
        } else if(tramVO != null) {
            textoEntidad = "tramite";
        } if(expVO != null) {
            textoEntidad = "expediente";
        }
        m_Log.debug("-->tratarCamposSuplementarios() init - de " + textoEntidad);

        java.util.Iterator it = datos.keySet().iterator();
        try {
            while (it.hasNext()) {
                String cod = (String) it.next();
                m_Log.debug("-->cod campo recuperado de hashmap " + cod);
                String valor = datos.get(cod);
                String tit = null;
                String tip = null;
                ...
                if (valor != null && !valor.equals("")) {
                    ...
                    m_Log.debug("-->tip  " + tip);
                    if(interesadoVO == null) {
                        m_Log.debug("-->Antes de entrar en grabarDatoSuplementario: Campos hashmap  ");
                        ExpedienteDAO.getInstance().grabarDatoSuplementarioDesdeXML(expVO, tramVO, cod, valor, tit, tip, con);
                    } else {
                        m_Log.debug("-->Antes de entrar en grabarCSTerceroDesdeXML: Campos hashmap  ");
                        CamposSuplementariosDAO.getInstance().grabarCSTerceroDesdeXML(expVO.getOrganizacionUsuario(),
                                Integer.parseInt(interesadoVO.getCodigo()), cod, valor, con);
                    }
                    m_Log.debug("--> Inserción de datos suplementarios de " + textoEntidad+ " correcto.");
                }
            }
        } catch (Exception e) {
            m_Log.error("Ha ocurrido un error durante la inserción de datos suplementarios de " + textoEntidad+ ".");
        }
    }
