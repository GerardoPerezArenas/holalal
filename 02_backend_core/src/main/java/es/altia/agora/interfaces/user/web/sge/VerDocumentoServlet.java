...
public class VerDocumentoServlet extends HttpServlet
{
    ...
    protected static Logger log
            = LogManager.getLogger(VerDocumentoServlet.class.getName());
  ...
  public void defaultAction(HttpServletRequest req,HttpServletResponse res) throws Exception {
      ...
      if(opcion.equals("8")){
          ...
          String[] parametros = codigo.split(ConstantesDatos.SEPARADOR);
          ...
          if(parametros!=null && parametros.length==7){
              ...
              //Construimos el objeto RegistroValueObject necesario
              RegistroValueObject documentoVO = crearDocumentoVO(codDepartamento,Integer.parseInt(codUor),tipoEntrada,Integer.parseInt(ejercicio),
                      Long.parseLong(numeroAnotacion),nombreDocumento,tipoMime);

              AlmacenDocumentoHelper almacenDocumentoHelper = new AlmacenDocumentoHelper(usuario.getOrgCod());
              Documento iDocumento = null;
              try {
                  iDocumento = almacenDocumentoHelper.getDocumentoRegistro(documentoVO, params);

                  if(iDocumento!=null && iDocumento.getFichero()!=null && iDocumento.getFichero().length>0){
                      fichero = iDocumento.getFichero();
                      nombreFichero = String.format("%s.%s", iDocumento.getNombreDocumento(), iDocumento.getExtension());
                  }
              } catch (AlmacenDocumentoTramitacionException e) {
                  e.printStackTrace();
                  log.error("Error al recuperar documento : ", e);
  ...
  private Hashtable<String,String> obtenerTramiteOcurrenciaCampo(String codigo){
    ...
    private RegistroValueObject crearDocumentoVO(String codDepartamento, int codUor, String tipoEntrada, int ejercicio, long numeroAnotacion, String nombreDoc, String tipoMimeDoc){
        RegistroValueObject documento = new RegistroValueObject();

        documento.setAnoReg(ejercicio);
        documento.setNumReg(numeroAnotacion);
        documento.setTipoReg(tipoEntrada);
        documento.setNombreDoc(nombreDoc);
        documento.setDptoUsuarioQRegistra(codDepartamento);
        documento.setUnidadOrgan(codUor);
        documento.setTipoDoc(tipoMimeDoc);
