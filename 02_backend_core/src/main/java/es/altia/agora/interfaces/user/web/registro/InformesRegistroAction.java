...
m_Log.debug("el codUnidad es ***************************** " + codUnidad);
        ...
        Vector<RegistroValueObject> listado = new Vector<RegistroValueObject>();
         ...
         if ((desde != null) && (!desde.equals(""))) {
            if (desde.compareTo(hasta) <= 0) {
             ...
             retorno=consulta_fecha(0, unidad, fechaInicio, fechaFin, orden, ejercicio, false, null);
             sql=(String) retorno.get(0);

         }
         try{
             listado=InformesManager.getInstance().getListadoInformeRegistro(sql, (Vector<String>) retorno.get(1), params);
         }catch(Exception e ){
             e.printStackTrace();
         }
        return listado;   
    ...
    private String informeLibroRegistro(String unidad,String claveIdioma,String[] params) {
        ...
        if ((desde != null) && (!desde.equals(""))) {
            ...
            if(elementosImprimir!=null && elementosImprimir!="" && elementosImprimir.length()>0 ){
                Vector <Object>retorno = consulta_fecha(0, unidad, desde, hasta, orden, ejercicio, false, null);
                    file = pdf.construyeTabla(consulta_fecha2((String) retorno.get(0),elementosImprimir), xsl,listaNumAnotacionImpresas, (Vector<String>) retorno.get(1));
                }else{
                Vector <Object>retorno = consulta_fecha(0, unidad, fechaInicio, fechaFin, orden, ejercicio, false, null);
                    file = pdf.construyeTabla((String) retorno.get(0), xsl,listaNumAnotacionImpresas, (Vector<String>) retorno.get(1));
                }
            if (file != null) {
                ficheros.add(file);
            }

            Vector <Object>retorno = consulta_fecha(1, unidad, fechaInicio, fechaFin, orden, ejercicio, false, null);
             ficheros.add(pdf.construyeTabla((String) retorno.get(0), "diligencias_"+claveIdioma, null, (Vector<String>) retorno.get(1)));//añado la diligencia
                if (m_Conf.getString("PDF.pie_registro").equals("si")) {
                    Hashtable tabla = new Hashtable();
                    ficheros.add(pdf.construyeTablaVacia2("pieHoja3_"+claveIdioma, tabla));
        ...
        if (unidades.isEmpty()) {
            ...
            for (int numUnidades = 0; numUnidades < unidades.size(); numUnidades++) {
                ...
                if ((desde != null) && (!desde.equals(""))) {
                    ...
                    Vector <Object>retorno = consulta_fecha(2, (String) unidades.get(numUnidades), fechaInicio, fechaFin, orden, ejercicio, false, null);
                    file = pdf.construyeTabla((String) retorno.get(0), xsl, listaNumAnotacionImpresas, (Vector<String>) retorno.get(1));
                ...
                for (int numUnidades = 0; numUnidades < unidades.size(); numUnidades++) {
                    ...
                    try {
                        if ((desde != null) && (!desde.equals(""))) {
                            ...
                            Vector <Object> retorno= consulta_fecha(2, (String) unidades.get(numUnidades), fechaInicio, fechaFin, orden, ejercicio, true, codOrganizacionExterna);
                            file = pdf.construyeTabla((String) retorno.get(0), xsl, listaNumAnotacionImpresas, (Vector<String>) retorno.get(1));
...
//        return max;
  ...
  */
     ...
     * @return La sql requerida
     */
    private Vector<Object> consulta_fecha(int t, String codigoUnidad, String fecha_desde, String fecha_hasta, boolean orden,
            String ejercicio, boolean unidadesExternas, String codOrganizacion) {
        m_Log.debug("ENTRO AKI--> consulta_fecha");
        Vector<String> filtrosFecha= new Vector<String>();
        Vector <Object>retorno=new Vector();
        String sql = "";
        AdaptadorSQLBD oad = new AdaptadorSQLBD(params);
        String nombreLargo = " ";
        ...
        switch (t) {
            ...
            case 2:
                ...
                if ((fecha_desde != null) && (!fecha_desde.equals(""))) {
                    parteWhere += " AND " + conf.getString("SQL.R_RES.fechaAnotacion") + " >= "
                            + oad.convertir("?", AdaptadorSQL.CONVERTIR_COLUMNA_FECHA, "DD/MM/YYYY");
                    filtrosFecha.add(fecha_desde);
                }

                if ((fecha_hasta != null) && (!fecha_hasta.equals(""))) {
                    parteWhere += " AND " + conf.getString("SQL.R_RES.fechaAnotacion") + " <= "
                            + oad.convertir("?", AdaptadorSQL.CONVERTIR_COLUMNA_FECHA, "DD/MM/YYYY HH24:MI:SS");
                    filtrosFecha.add(fecha_hasta + " 23:59:59");
                }

                if (t == 0) {
                    parteWhere += " AND " + conf.getString("SQL.R_RES.numeroAnotacion") + " >= " + request.getParameter("numDesde");
                } else if (t == 2) {
                    if (!codigoUnidad.equals("")) { // tenemos que tener en cuenta las unidades externas
                        if (unidadesExternas) {
                            parteWhere += " AND RES_UCD = " + codigoUnidad + " AND RES_OCD = " + codOrganizacion; // unidad y org destino externa
                        } else {
                            parteWhere += " AND " + conf.getString("SQL.R_RES.unidOrigDest") + " = " + codigoUnidad;
                ...
                if (params[0].equals("sqlserver") || params[0].equals("SQLSERVER")) {
                    ...
                    joins[12] = conf.getString("SQL.R_RES.codUnidad") + " = " + conf.getString("SQL.R_DIL.codUnidad")
                            + " AND " + conf.getString("SQL.R_RES.tipoReg") + " = " + conf.getString("SQL.R_DIL.tipo")
                            + " AND " + oad.convertir(oad.convertir(conf.getString("SQL.R_RES.fechaAnotacion"), AdaptadorSQL.CONVERTIR_COLUMNA_TEXTO, "DD/MM/YYYY"), AdaptadorSQL.CONVERTIR_COLUMNA_FECHA, "DD/MM/YYYY") + " = "
                            + conf.getString("SQL.R_DIL.fecha");
                }
                joins[13] = "false";
                ...
                if ((fecha_desde != null) && (!fecha_desde.equals(""))) {
                    parteWhere1 += " AND " + conf.getString("SQL.R_RES.fechaAnotacion") + " >= "
                        + oad.convertir("?", AdaptadorSQL.CONVERTIR_COLUMNA_FECHA, "DD/MM/YYYY");
                    filtrosFecha.add(fecha_desde);
                }
                ...
                if (unidadesExternas) { // si se va a imprimir un informe de las unidades externas necesitamos consultar la tabla A_UOREX
                    joins1[10] = "LEFT";
            ...
            case 1:
                sql = "SELECT " + oad.convertir(conf.getString("SQL.R_DIL.fecha"),
                        ...
                        + " AND " + conf.getString("SQL.R_DIL.tipo") + " = '" + request.getParameter("tipo") + "'";
                if ((fecha_desde != null) && (!fecha_desde.equals(""))) {
                    sql += " AND " + conf.getString("SQL.R_DIL.fecha") + " >= TO_DATE(?, 'DD/MM/YYYY')";
                    filtrosFecha.add(fecha_desde);
                }

                if ((fecha_hasta != null) && (!fecha_hasta.equals(""))) {
                    sql += " AND " + conf.getString("SQL.R_DIL.fecha") + " <= TO_DATE(?, 'DD/MM/YYYY')";
                    filtrosFecha.add(fecha_hasta);
                }
                break;

        }
        retorno.add(sql);
        retorno.add(filtrosFecha);
