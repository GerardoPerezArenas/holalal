...
public final class MantAnotacionRegistroAction extends ActionSession {
    ...
    public ActionForward performSession(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
        ...
        if (session.getAttribute("usuario") != null
            ...
            //Comprobamos si en el fichero de propiedades de registro existe una entrada que nos indice si el campo asunto será obligatorio
            ...
            if ("init_alta_entrada".equals(opcion) || "init_alta_salida".equals(opcion)) {
                ...
                try {
                    ...
                    /**
                     * * SE ALMACENA EN EL OBJETO elRegistroESVO EL CÓDIGO DE LA
                    ...
                    valores = new StringTokenizer(txtListaDocEntregados, ConstantesDatos.SEPARADOR, false);
                    ...
                    if (!"duplicar".equals(opcion)) {
                        m_Log.error("(NO ERROR) Traza de control - No estamos duplicando: " + opcion + ". Digitalización: " + elRegistroESVO.isFinDigitalizacion());
                        ...
                        //si esta el servicio de digitalización activo  y se finaliza el proceso de digtalización de documentos
                        ...
                        if (digitalizacionActivo.equalsIgnoreCase("si")) {
                            if (elRegistroESVO.isFinDigitalizacion()) {
                                ...
                                // se cambia el documento de contexto Registro a Tramitación
                ...
                // Comprobación de si la anotación de registro se ha dado de alta desde el servicio web WSRegistroES
                ...
                try {
                    ...
                    //DIGITALIZACIÓN LANBIDE
                    if (servicioDigitalizacionActivo.equalsIgnoreCase("si")) {
                        //comprueba que se finaliza la digitalización por primera vez
                        ...
                        if (registro.isFinDigitalizacion() && !finDigitalizacionAntesMod) {
                            ...
                            m_Log.info(session.getId() + " " + usuarioActualLog + " El registro ya había sido finalizado con anterioridad.");
                        }
                    }
                ...
                try {
                    //Boolean nos dice si hay digitalización o no
                ...
                if ("S".equals(request.getParameter("desdeEntradasRechazadas")) || "S".equals(request.getParameter("desdePendientesFinalizar"))) {
                    ...
                    if ((request.getParameter("ano") != null) && (request.getParameter("numero") != null)) {
                        if ((!"".equals(request.getParameter("ano").trim())) && (!"".equals(request.getParameter("numero").trim()))) {
                            ...
                            m_Log.debug("año anotacion " + request.getParameter("ano"));
                            storedConsulta.setNumReg(Long.parseLong(request.getParameter("numero")));
                            m_Log.debug("numero anotación " + request.getParameter("numero"));
                        }
            ...
            //RelacionBusqueda_E es cuando viene de busqueda de anotaciones
                ...
                * Fecha: 22/07/2003 */
                ...
                try {
                    ...
                    m_Log.error("Saltó excepción formateando la fecha, en ManAntoacionRegistroAction", e);
                    e.printStackTrace();
                ...
                //El hijo que vamos a desplegar será el que tenga la clasificación, a la que pertenezca
                ...
                m_Log.debug("A árbore ten un tamaño de.... :" + arbolClasifAsuntos.size());
                ...
                try {
                    ...
                    // Se recupera el justificante de registro personalizado que está activo, si lo hay
                ...
                if (justif != null) {
                    // Se realiza la redirección al action que se encarga de mostrar el justificante
                    ...
                    if (errorJustificante) {
                        ...
                        if (pdf == null) {
                            ...
                            for (List numero : listaNumReg) {
                                ...
                                // Obtenemos el xml con los datos de la anotación
                                xml = AnotacionRegistroManager.getInstance().consultaXML(gVO, params);
                                ...
                                if (informe != null) {
                                    m_Log.debug(" =========== MantAnotacionRegistroAction tamaño byte[] pdf generado: " + informe.length);
                            ...
                            if (listaPdfs.size() > 0) {
                                ...
                                m_Log.debug("No se ha generado el pdf de justificante de registro para ninguna anotación. No se puede hacer el merge.");
                                request.setAttribute("ERROR_JUSTIFICANTE_REGISTRO_PERSONALIZADO", "GENERACION_PDF_INCORRECTA");
                            }// else
                        }
                    }
                ...
                // #239565: Es necesario añadir un atributo a gVO que indique que el tipo de informe es 'justificante' para que consultaXML funcione correctamente
                ...
                StringTokenizer rolesAnteriores = new StringTokenizer(listaDescRol, ConstantesDatos.SEPARADOR, false);
                ...
                if ("recargarComboAsuntos".equals(opcion)) {
                    ...
                    m_Log.info(session.getId() + " " + usuarioActualLog + " (NO ERROR)Traza control opción  cargarHistorico antes de llamar a obtenerHistoricoAnotacion ");
                    ...
                    m_Log.info(session.getId() + " " + usuarioActualLog + " (NO ERROR)Traza control opción  cargarHistorico despues de llamar a obtenerHistoricoAnotacion ");
...
//                    traductor.setIdi_cod(usuarioVO.getIdioma());
                    ...
                    m_Log.info(session.getId() + " " + usuarioActualLog + " (NO ERROR)Traza control opción  cargarHistorico antes de llamar a getDescripcionMovimiento ");
                    ...
                    m_Log.info(session.getId() + " " + usuarioActualLog + " (NO ERROR)Traza control opción  cargarHistorico END ");
...
//                    mensajes.setIdi_cod(usuarioVO.getIdioma());
                    ...
                    if (regCargaBusqueda.getFallo().equals("fallo")) {
                        ...
                        m_Log.debug("ERROR AL CARGAR LA ANOTACIÃ?N. NO EXISTE");
                        return mapping.findForward(opcion);
                    }
                    ...
                    try {
                        ...
                        if (opcionPermanencia) {
                            if (permanencia.equalsIgnoreCase("SI")) {
                                ...
                                if (m_Log.isDebugEnabled()) {
                                    m_Log.debug("Recuperamos el número de expediente seleccionado = " + numExpediente);
                                }
                                if (numExpediente != null && !"".equalsIgnoreCase(numExpediente)) {
                                    ...
                                    if (m_Log.isDebugEnabled()) {
                                        m_Log.debug("Comprobamos que el origen de la anotación este incluido en los servicios válidos");
                                    }
                                    ...
                                    for (int x = 0; x < origenes.length; x++) {
                                        if (registroOrigen.equalsIgnoreCase(origenes[x])) {
                                            if (m_Log.isDebugEnabled()) {
                                                m_Log.debug("El origen de del registro es válido");
                                            }
                                            origenValido = true;
                                        }//if(registroOrigen.equalsIgnoreCase(origenes[x]))
                                    }//for(int x=0; x<origenes.length; x++)
                                    ...
                                    if (!origenValido) {
                                        if (m_Log.isDebugEnabled()) {
                                            m_Log.debug("El origen de la anotación no es válido para mantener la anotación en el buzón -> " + registroOrigen);
                                        }
                                        opcionPermanencia = false;
                                    }//if(!origenValido)
                                    ...
                                    if (opcionPermanencia) {
                                        ...
                                        if (m_Log.isDebugEnabled()) {
                                            m_Log.debug("código de procedimiento seleccionado = " + codProc);
                                        }
                                        if (m_Log.isDebugEnabled()) {
                                            m_Log.debug("Recuperamos los expedientes para los que no se puede "
                                                    + "mantener la anotación en el buzón");
                                        }
                                        try {
                                            ...
                                            if (m_Log.isDebugEnabled()) {
                                                m_Log.debug("Códigos procedimientos restringidos = " + codProcsRestringidos);
                                            }
                                            ...
                                            m_Log.error("Se ha producido un error recuperando los procedimientos restringidos "
                                                    + "que no pueden mantener la anotación en el buzón", ex);
                                        }//try-catch
                                    }//if(opcionPermanencia)
                                } else {
                                    opcionPermanencia = false;
    ...
    private Vector listaTemasSeleccionados(String listTemasSelecc) {
        ...
        if (listTemasSelecc != null) {
            codigos = new StringTokenizer(listTemasSelecc, ConstantesDatos.SEPARADOR+",", false);
    ...
    private Hashtable listaCriterios(String listCriterios) {
        ...
        if (listCriterios != null) {
            elementos = new StringTokenizer(listCriterios, ConstantesDatos.SEPARADOR+",", false);
    ...
    /**
     * Método que inicializa las listas del formulario: tipo de documentos, tipo
     * de remitentes, tipo de transportes, tipo de actuaciones, tipo de temas,
     *
     * @param elRegistroESVO: RegistroValueObject
     * @param recuperarTodosAsuntos: si está a true , al recuperar los asuntos
     * de registro, se recuperan todos los asuntos, incluidos los que han sido
     * dados de baja.Si está a false, sólo se recuperan los que no han sido
     * eliminados
    ...
    private void notificarUnidadesOrganicas(MantAnotacionRegistroForm registroForm) {
        ...
        // En dicho cliente el envío es independiente de que la unidad de registro tenga o no configurado un mail.
    ...
    private Vector listaInteresadosSeleccionados(String listSelecc) {
        ...
        if (listSelecc != null) {
            codigos = new StringTokenizer(listSelecc, ConstantesDatos.SEPARADOR, false);
    ...
    /**
     * Método auxiliar para la funcionalidad de ImprimirCuneus Ahora se necesita
    ...
    /**
     * Envía los datos necesarios (PK) de un registro para recuperar toda la
     * información pertinente de la base de datos y enviarla a la oficina
     * apropiada.
     *
     * @param mapping
     * @param request
    ...
    private ActionForward enviarRegistro(ActionMapping mapping, HttpServletRequest request,
        ...
        int departamento = 1; // Por ahora siempre es así
