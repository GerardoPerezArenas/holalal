...
import es.altia.util.html.Conversor;
...
public final class FichaExpedienteAction extends ActionSession {
    ...
    public ActionForward performSession(	ActionMapping mapping,
        ...
        //********************************************************************            
        ...
        } else if (("generarExcelExpediente".equals(opcion))) {
            generarExcelExpediente(params, expForm.getNumExpediente(), response, (FichaExpedienteForm) form, usuarioVO);
            ...
            if ("iniciarExpedienteAsiento".equals(opcion)) {                
                ...
                try {
                    ...
                    //Si por error envian varios documentos XML, quedara con el ultimo.
                    for( int i=0; i<documentos.size() ; i ++ ){
                        ...
                        if (documentoAux.getNombreDoc().equalsIgnoreCase(ConstantesDatos.NOMBRE_FICHERO_DATOS_INTEGRACION_SOLICITUD)){
                            ...
                            AlmacenDocumentoHelper almacenDocumentoHelper = new AlmacenDocumentoHelper(usuarioVO.getOrgCod());
                            Documento iDocumento = null;
                            try {
                                iDocumento = almacenDocumentoHelper.getDocumentoRegistro(documentoAux, params);
                            } catch (AlmacenDocumentoTramitacionException e) {
                                e.printStackTrace();
                            }

                            if(iDocumento!=null && iDocumento.getFichero()!=null && iDocumento.getFichero().length>0){
                                documentoAux.setDoc(iDocumento.getFichero());
                            }
                    ...
                    if (documento != null){
                        ExtractorDatosXMLIntegracion lector = new ExtractorDatosXMLIntegracion(documento.getNombreDoc(), documento.getDoc());
                        request.setAttribute("nombreDocumento", documento.getNombreDoc());
                        
                        //Recupera los datos suplementarios del documento xml
                        lector.setEtiquetaPadre(ConstantesDatos.TAG_XML_CAMPOSVARIABLES);
                        Map<String, List<GeneralValueObject>> camposSuplementarios = lector.leerDatosCamposSuplementarios();
                        ...
                        lector.setEtiquetaPadre(ConstantesDatos.TAG_XML_INTERESADOS);
                        listaInteresados = lector.leerDatosInteresados();
                        
                        //Recupera el nombre del modulo del documento xml
                        lector.setEtiquetaPadre(ConstantesDatos.TAG_XML_FLX_EXTENSION);
                        nombreModulo = lector.leerValorAtributo(ConstantesDatos.ATRIBUTO_XML_COD);
                        
                        // Se recupera el número de expediente relacionado si es que ha sido comunicado
                        lector.setEtiquetaPadre(ConstantesDatos.TAG_XML_EXPEDIENTE_RELACIONADO);
                        String numExpedienteRelacionado = lector.leerValorNodo();
...
//                String numero = request.getParameter("numeroExp");
      ...
      m_Log.debug("Ver toda la documentacion del expediente " + expForm.getNumExpediente());   
          ...
          for (FicheroVO fichero : ficherosRegistroEntrada) {                                                        
              fichero.setCodigo(fichero.getEjercicio()+ ConstantesDatos.SEPARADOR + fichero.getNumero() + ConstantesDatos.SEPARADOR + fichero.getTipo() + ConstantesDatos.SEPARADOR + fichero.getNombre() + ConstantesDatos.SEPARADOR + fichero.getUor() + ConstantesDatos.SEPARADOR + fichero.getDep() + ConstantesDatos.SEPARADOR + fichero.getTipoContenido());
...
//                String numero = request.getParameter("numeroExp");
        ...
        traductor.setIdi_cod(usuarioVO.getIdioma());
            m_Log.info("PARAMETROS LISTADO OPERACIONES INICIAL"+usuarioVO.getOrgCod() + expForm.getNumExpediente()+isExpHistorico+traductor+params);
        ...
        if (idOperacion != null && !idOperacion.equals("")){
            ...
            try {
                ...
                LogManager.getLogger(FichaExpedienteAction.class.getName()).error( ex);
            }
        }
 ...
 /**
    ...
    private Vector listaTemasSeleccionados(String listTemasSelecc) {
        ...
        if (listTemasSelecc != null) {
            valores = new StringTokenizer(listTemasSelecc, ConstantesDatos.SEPARADOR, false);
    ...
    public ActionForward generarExcelExpediente(String[] params, String numExpediente, HttpServletResponse response, FichaExpedienteForm form, UsuarioValueObject usuarioVO) {
        AdaptadorSQLBD adapt = null;
        Connection con = null;
        String rutaArchivoSalida = null;

        try {
            //inizializar adaptdor y obtener conexion
            adapt = new AdaptadorSQLBD(params);
            con = adapt.getConnection();
            //obtener lista operaciones expedienrte dede request
            TraductorAplicacionBean traductor = new TraductorAplicacionBean();
            traductor.setApl_cod(ConstantesDatos.APP_GESTION_EXPEDIENTES);
            traductor.setIdi_cod(usuarioVO.getIdioma());


            List<OperacionExpedienteVO> listaOperaciones = OperacionesExpedienteManager.getInstance().recuperarOperacionesExpediente(usuarioVO.getOrgCod(), numExpediente, form.isExpHistorico(), traductor, params);

            if (listaOperaciones == null) {
                listaOperaciones = new ArrayList<OperacionExpedienteVO>();
            }

            //bloque para generar excel

            FileInputStream istr = null;
            try {
                // CREAR LIBRO EXCEL DE EXPEDIENTE
                HSSFWorkbook libro = new HSSFWorkbook();
                HSSFSheet hoja = libro.createSheet("Operaciones Expediente");

                //tituo principal (fila 0)
                HSSFRow filaTitulo = hoja.createRow(0);
                HSSFCell celdaTitulo = filaTitulo.createCell(0);
                celdaTitulo.setCellValue("Listado de Operaciones del Expediente");


                // Unir celdas para el título (columnas 0 a 2)
                hoja.addMergedRegion(new CellRangeAddress(0, 0, 0, 2));
                filaTitulo.setHeightInPoints(40);


                // Fila de cabecera (fila 1)
                HSSFRow filaCabecera = hoja.createRow(1);
                String[] cabeceras = { "ID Operación", "Código Usuario", "Nombre Usuario", "Login Usuario", "Número expediente", "Fecha Operación", "Tipo Operación", "Descripción Operación" };
                ...
                int numFila = 2;
                for (OperacionExpedienteVO op : listaOperaciones) {
                    HSSFRow fila = hoja.createRow(numFila++);
                    HSSFCell celda0 = fila.createCell(0);
                    celda0.setCellValue(op.getIdOperacion());
                    HSSFCell celda1 = fila.createCell(1);
                    celda1.setCellValue(op.getCodUsuario());
                    HSSFCell celda2 = fila.createCell(2);
                    celda2.setCellValue(op.getNombreUsuario());
                    HSSFCell celda3 = fila.createCell(3);
                    celda3.setCellValue(op.getLoginUsuario());
                    HSSFCell celda4 = fila.createCell(4);
                    celda4.setCellValue(op.getNumExpediente());
                    HSSFCell celda5 = fila.createCell(5);
                    celda5.setCellValue(DateOperations.toString(op.getFechaOperacion(), "dd-MM-yyyy HH:mm:ss"));
                    HSSFCell celda6 = fila.createCell(6);
                    celda6.setCellValue(op.getTipoOperacionTxt());
                    String detalleDescription = Conversor.ConversorHTML(op.getDescripcionOperacion());
                    int k = detalleDescription.indexOf("}", 0);
                    detalleDescription = detalleDescription.substring(k + 1, detalleDescription.length() - 1);
                    detalleDescription = OperacionesExpedienteTraductor.traducir(traductor, detalleDescription);
                    HSSFCell celda7 = fila.createCell(7);
                    celda7.setCellValue(detalleDescription);
                    HSSFCellStyle estiloDato = libro.createCellStyle();
                    estiloDato.setBorderTop(HSSFCellStyle.BORDER_THIN);
                    estiloDato.setBorderBottom(HSSFCellStyle.BORDER_THIN);
                    estiloDato.setBorderLeft(HSSFCellStyle.BORDER_THIN);
                    estiloDato.setBorderRight(HSSFCellStyle.BORDER_THIN);
                    estiloDato.setAlignment(HSSFCellStyle.ALIGN_LEFT);
                    celda0.setCellStyle(estiloDato);
                    celda1.setCellStyle(estiloDato);
                    celda2.setCellStyle(estiloDato);
                    celda3.setCellStyle(estiloDato);
                    celda4.setCellStyle(estiloDato);
                    celda5.setCellStyle(estiloDato);
                    celda6.setCellStyle(estiloDato);
                    celda7.setCellStyle(estiloDato);
                }
                ...
                File directorioTemp = new File(String.valueOf(m_Conf.getString("PDF.base_dir")));

                File informe = File.createTempFile("listado_operaciones", ".xls", directorioTemp);
                ...
                m_Log.error("EXCEPCION informe resumenDatosEmpresas", ioe);
            } finally {
                if (istr != null) {
                    try {
                        istr.close();
                    } catch (IOException ex) {
                        m_Log.error("Error al intentar cerrar el InputStreamReader", ex);
                    }
                }
            }
        } catch (BDException e) {
