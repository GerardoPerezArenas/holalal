...
public class InformacionSistemaAction extends ActionSession
{
    ...
    throws IOException, ServletException
  {
    ...
    m_Log.debug("perform");
      APL_REAL_PATH = this.getServlet().getServletContext().getRealPath("");
    ...
    if ("cargar".equalsIgnoreCase(opcion))
    {
      ...
      try {
            ...
            formulario.setWebAppBaseDir(this.APL_REAL_PATH);
            formulario.setVersion(confVersion.getString("version.aplicacion"));
            formulario.setNivelLog(LogManager.getRootLogger().getLevel().toString());
            formulario.setSalidaLog(LogManager.getRootLogger().getName()/*confLog.getString("log4j.rootCategory")*/);
    ...
    else if("descargar".equalsIgnoreCase(opcion))
    {
        ...
        m_Log.debug("El fichero que se ha indicado en la request que se quiere descargar es: " + fichero);;
        if("ficheroLog".equals(fichero)) {
            ...
            nomFile ="web.xml" ;
            ruta=this.APL_REAL_PATH+fileSeparator+"WEB-INF"+fileSeparator+nomFile;
            response.setHeader("Content-Disposition","attachment;filename=web.xml");
        } else if("context".equals(fichero)) {
            nomFile ="context.xml" ;
            ruta=this.APL_REAL_PATH+fileSeparator+"META-INF"+fileSeparator+nomFile;
            response.setHeader("Content-Disposition","attachment;filename=context.xml");
        } else if("log4j".equals(fichero)) {
            nomFile ="log4j2.xml" ;
            ruta=this.APL_REAL_PATH+fileSeparator+"WEB-INF"+fileSeparator+"classes"+fileSeparator+nomFile;
    ...
    else if("otroFicheroRuta".equalsIgnoreCase(opcion))
    {
        ...
        String fichero = request.getParameter("ficheroDescarga");
        nomFile = fichero;
        ruta=this.APL_REAL_PATH+nomFile;
        response.setHeader("Content-Disposition","attachment;filename=fichero.txt");
        ...
        try{
             ...
             for( int i=0; i<resultados.size();i++)
             {
               ...
               for( int j=0; j<fila.size();j++)
                ...
                if((i+1)<resultados.size())resultadoString=resultadoString+ConstantesDatos.SEPARADOR;
        ...
        String jndi = UsuariosGruposManager.getInstance().obtenerJNDI(
             ...
             for( int i=0; i<resultadosExp.size();i++){
               ...
               for( int j=0; j<fila.size();j++)
                ...
                if((i+1)<resultadosExp.size())resultadoStringExp=resultadoStringExp+ConstantesDatos.SEPARADOR;
             }
