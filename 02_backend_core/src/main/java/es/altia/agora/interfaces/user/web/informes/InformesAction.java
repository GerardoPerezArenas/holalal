...
public class InformesAction extends ActionSession{
   ...
   protected static Logger log =
           LogManager.getLogger(InformesAction.class.getName());
...
//           }
   ...
   String crearParametroTodos(HttpServletRequest request){
    ...
    String verVol = "";
	  ...
	  if(filtroReq != null && !filtroReq.equals("")) {
	  	todos = filtroReq + ConstantesDatos.SEPARADOR;
	  } else {
	  	todos = "%" + ConstantesDatos.SEPARADOR;
	  }
	  if(grupoReq != null && !grupoReq.equals("")) {
	  	todos += grupoReq + ConstantesDatos.SEPARADOR;
	  } else {
	  	todos += "%" + ConstantesDatos.SEPARADOR;
	  }
	  if(tipoSalida != null && !tipoSalida.equals("")) {
	  	todos += tipoSalida + ConstantesDatos.SEPARADOR;
	  } else {
	  	todos += "%" + ConstantesDatos.SEPARADOR;
	  }
	  if(tiempo != null && !tiempo.equals("")) {
	  	todos += tiempo + ConstantesDatos.SEPARADOR;
	  } else {
	  	todos += "%" + ConstantesDatos.SEPARADOR;
	  }
	  if(verPend != null && !verPend.equals("")) {
	  	todos += verPend + ConstantesDatos.SEPARADOR;
	  } else {
	  	todos += "%" + ConstantesDatos.SEPARADOR;
	  }
	  if(verFin != null && !verFin.equals("")) {
	  	todos += verFin + ConstantesDatos.SEPARADOR;
	  } else {
	  	todos += "%" + ConstantesDatos.SEPARADOR;
	  }
	  if(verVol != null && !verVol.equals("")) {
	  	todos += verVol + ConstantesDatos.SEPARADOR;
	  } else {
	  	todos += "%" + ConstantesDatos.SEPARADOR;
	  }
  ...
  GeneralValueObject recuperaParametrosDesdeTodos(String todos){
    ...
    StringTokenizer valores = null;
	  if (todos != null) {
	    valores = new StringTokenizer(todos,ConstantesDatos.SEPARADOR,false);
