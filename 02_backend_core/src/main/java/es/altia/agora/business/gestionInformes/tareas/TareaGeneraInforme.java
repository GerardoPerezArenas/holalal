...
public class TareaGeneraInforme implements Job {
    ...
    private static Logger _log = LogManager.getLogger(TareaGeneraInforme.class);
    ...
    public void execute(JobExecutionContext arg0) throws JobExecutionException {
        ...
        try {
            ...
            ServletContext servletContext = (ServletContext) jd.getJobDataMap().get("servletContext");
            ...
            if (datosInforme.size()!=0) {
                ...
                if(iTipoFichero==0){
                    ...
                    // Si el informe es en HTML o CSV, se genera con JFreeReport ya que con Pentaho hay problemas al generar el informe en estos formatos con JAVA 5
                    ...
                    _log.debug("Generar informe EXCEL");                    
                     fichero = ManejadorInformesExcel.getInstance().generarInformeExcel(columnasInforme, datosInforme, servletContext, host,context,usuario,Integer.parseInt(tipoFichero),listaTiposCamposInforme,informeVO,informeTableModel,"generar",protocolo);
