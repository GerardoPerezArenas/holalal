...
public class InformesManager {
   ...
   protected static Logger m_Log =
           LogManager.getLogger(InformesManager.class.getName());
...
/* Obtine los códigos de todas las unidades*/
 ...
 /**
     ...
     */
    public Vector getListadoInformeRegistro(String consulta, Vector<String> filtrosFecha, String[] params){
        Vector<RegistroValueObject> listado = new Vector<RegistroValueObject>();
        InformesDAO infDAO = InformesDAO.getInstance();
 
        try{
             listado=infDAO.getListadoInformeRegistro(consulta, filtrosFecha, params);
        }catch(Exception ce){
             m_Log.error("JDBC Technical problem "+ ce.getMessage());
        } finally{
