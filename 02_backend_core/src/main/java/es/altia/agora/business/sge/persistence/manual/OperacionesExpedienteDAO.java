...
public class OperacionesExpedienteDAO {
      ...
      protected static Logger m_Log =  LogManager.getLogger(OperacionesExpedienteDAO.class.getName());
    ...
    public ArrayList <OperacionExpedienteVO> recuperarOperacionesExpediente(int codOrganizacion, 
        ...
        try {
            ...
            sql.append("SELECT oe.ID_OPERACION, oe.COD_MUNICIPIO, oe.EJERCICIO, ")
                    .append("oe.NUM_EXPEDIENTE, oe.TIPO_OPERACION, oe.FECHA_OPERACION, oe.COD_USUARIO, ")
                    .append("oe.DESCRIPCION_OPERACION, ")
                    .append("au.USU_LOG, au.USU_NOM ")
               .append("FROM ");
            if (isExpHistorico) {
                sql.append("HIST_OPERACIONES_EXPEDIENTE oe ");
            } else {
                sql.append("OPERACIONES_EXPEDIENTE oe ");
            }     
            
            sql.append("INNER JOIN FLBGEN.A_USU au ON au.USU_COD = oe.COD_USUARIO ")
                    .append("WHERE oe.COD_MUNICIPIO = ? AND oe.NUM_EXPEDIENTE = ? ")
                    .append("ORDER BY oe.FECHA_OPERACION DESC");
            ...
            while (rs.next()) {
                ...
                operacionExpedienteVO.setNombreUsuario(rs.getString("USU_NOM"));
                operacionExpedienteVO.setDescripcionOperacion(rs.getString("DESCRIPCION_OPERACION"));
                operacionExpedienteVO.setLoginUsuario(rs.getString("USU_LOG"));
