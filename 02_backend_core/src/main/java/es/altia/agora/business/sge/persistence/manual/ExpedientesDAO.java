...
public class ExpedientesDAO {
  ...
  protected static Config m_ConfigTechnical;
    protected static Logger m_Log =
            LogManager.getLogger(ExpedientesDAO.class.getName());
  ...
  public static ExpedientesDAO getInstance() {
    ...
    public int insertarExpediente(AdaptadorSQL oad, Connection con, GeneralValueObject gVO) 
        ...
        if (desdeAsiento != null && !desdeAsiento.equals("")) {
            resultado1 = insertarInteresadoDesdeAsiento(gVO, con);
        
         // Inicio de expediente desde servicio web o sin asiento (codigo original)
        } else {              
            if (m_Log.isDebugEnabled()) m_Log.debug("Inicio desde WSTramitacionFlexia o sin ser desde asiento.");
            GeneralValueObject interesadoPrincipal = null;
            boolean insInteresadoPrincipal = false;
            int resultadoAux = 0;
            Object lista = gVO.getAtributo("listaInteresados");
            if(lista != null) {
                // Hay una lista de interesados: viene del batch de KONTU
                List<GeneralValueObject> listaInteresados = (List<GeneralValueObject>) lista;
                if(!listaInteresados.isEmpty()) {
                    //Obtenemos los datos del primer interesado de la lista
                    interesadoPrincipal = listaInteresados.get(0);
                    insInteresadoPrincipal = true;

                    //Insertamos los otros interesados de la lista
                    for(int i=1; i<listaInteresados.size(); i++){
                        GeneralValueObject interesado = listaInteresados.get(i);
                        gVO.setAtributo("codigoTercero", interesado.getAtributo("codTercero"));
                        gVO.setAtributo("domicilio", interesado.getAtributo("codDomicilio"));
                        gVO.setAtributo("versionTercero", interesado.getAtributo("version"));
                        gVO.setAtributo("rol", interesado.getAtributo("codRol"));

                        m_Log.debug(String.format("Se inserta el interesado de rol %s", interesado.getAtributo("codRol")));
                        boolean exito = InteresadosDAO.getInstance().altaInteresado(gVO, con);
                        if(exito) {
                            resultadoAux++;
                        }
                    }
                }
            } else if (codTercero != null && !"".equals(codTercero)) {
                interesadoPrincipal = gVO;
                insInteresadoPrincipal = true;
            }

            if(insInteresadoPrincipal){
                gVO.setAtributo("codigoTercero", interesadoPrincipal.getAtributo("codTercero"));
                gVO.setAtributo("domicilio", interesadoPrincipal.getAtributo("codDomicilio"));
                gVO.setAtributo("versionTercero", interesadoPrincipal.getAtributo("version"));
                gVO.setAtributo("rol", interesadoPrincipal.getAtributo("codRol"));
                gVO.setAtributo("mostrarInt", "1");

                m_Log.debug(String.format("Se inserta el interesado de rol %s", interesadoPrincipal.getAtributo("codRol")));
                boolean exito = InteresadosDAO.getInstance().altaInteresado(gVO, con);
                if((resultadoAux == 0 && exito) // Si hay un unico interesado y se inserta con exito
                        || (resultadoAux == ((List<GeneralValueObject>)lista).size()-1 && exito)) { // o hay una lista de interesados y todos se insertaron con exito
                    resultado1 = 1;
                } else {
                    resultado1 = 0;
                }
         ...
         try{
            //Registra el alta de interesado en el historico de movimientos de un expediente
            TercerosValueObject tercero = null;
             ...
             if(lista != null) {
                 // Hay una lista de interesados: viene del batch de KONTU
                 listaInteresados = (List<GeneralValueObject>) lista;

             } else if (codTercero != null && !"".equals(codTercero)) {
                 listaInteresados = new ArrayList<>();
                 listaInteresados.add(gVO);
             }
             ...
             if(listaInteresados!=null && !listaInteresados.isEmpty()) {
                 for(int i=0; i<listaInteresados.size(); i++){
                     ...
                     tercero.setIdentificador((String) interesado.getAtributo("codTercero"));
                     tercero.setVersion((String) interesado.getAtributo("version"));
                     tercero.setIdDomicilio((String) interesado.getAtributo("codDomicilio"));
                     ...
                     tercero.setCodRol(Integer.valueOf((String) interesado.getAtributo("codRol")));
  ...
  private GeneralValueObject tramitacionExpedientesVO (TramitacionExpedientesValueObject teVO) {
    ...
    private int insertarInteresadoDesdeAsiento(GeneralValueObject gVO, Connection con) throws SQLException, TechnicalException {
        String codMunicipio = (String) gVO.getAtributo("codMunicipio");
        String codProcedimiento = (String) gVO.getAtributo("codProcedimiento");
        String ejercicio = (String) gVO.getAtributo("ejercicio");
        String numero = (String) gVO.getAtributo("numero");
        ...
        try{
            ...
            m_Log.error("Ha ocurrido un error al insertar interesados cuando el expediente se inicia desde asiento.");
            throw e;
