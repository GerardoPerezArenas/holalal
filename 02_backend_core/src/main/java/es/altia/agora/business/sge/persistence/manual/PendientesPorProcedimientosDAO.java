...
public class PendientesPorProcedimientosDAO {

    private static PendientesPorProcedimientosDAO instance = null;
    protected static Config m_ConfigTechnical; // Para el fichero de configuracion técnico
    protected static Config m_ConfigError; // Para los mensajes de error localizados
    protected static Logger m_Log =
            LogManager.getLogger(PendientesPorProcedimientosDAO.class.getName());
    ...
    protected PendientesPorProcedimientosDAO() {
        super();
    ...
    public static PendientesPorProcedimientosDAO getInstance() {
        // si no hay ninguna instancia de esta clase tenemos que crear una
        if (instance == null) {
            // Necesitamos sincronizacion para serializar (no multithread)
            // Las invocaciones de este metodo
            synchronized(PendientesPorProcedimientosDAO.class) {
                if (instance == null) {
                    instance = new PendientesPorProcedimientosDAO();
                }
            }
        }
        return instance;
    ...
    public Vector consultarP(PendientesPorProcedimientosValueObject pendProcVO,String[] params)
        ...
        String codProcedimiento = pendProcVO.getCodProcedimiento();
        String codMunicipio = pendProcVO.getCodMunicipio();
        ...
        try{
            ...
            from = exp_mun + ","+ exp_pro + "," + exp_eje + "," + exp_num + "," +
                    ...
                    exp_fef + "," + exp_ter + "," + exp_tnv + "," + exp_est + "," + ter_nom + "," +
                    ter_ap1 + "," + ter_pa1 + "," + ter_ap2 + "," + ter_pa2 + "," +
                    exr_ejr + "," + exr_nur;
            where = exp_mun + "=" + codMunicipio + " AND " + exp_pro + "='" + codProcedimiento + "'";
            ...
            join[4] = "LEFT";
            join[5] = "e_exr";
            join[6] = "e_exr." + exr_mun + "=" + exp_mun + " AND e_exr." +
                    exr_pro + "=" + exp_pro + " AND e_exr." + exr_eje + "=" + exp_eje +
                    " AND e_exr." + exr_num + "=" + exp_num;
            join[7] = "false";
            ...
            if(m_Log.isDebugEnabled()) m_Log.debug("PendientesPorProcedimientosDAO, consultar: Sentencia SQL:" + sql);
            rs = st.executeQuery(sql);
            while(rs.next()){
                PendientesPorProcedimientosValueObject pPVO = new PendientesPorProcedimientosValueObject();
                String municipio = rs.getString(exp_mun);
                pPVO.setCodMunicipio(municipio);
                String procedimiento = rs.getString(exp_pro);
                pPVO.setCodProcedimiento(procedimiento);
                String ano = rs.getString(exp_eje);
                pPVO.setAno(ano);
                String numero = rs.getString(exp_num);
                pPVO.setNumero(numero);
                String fechaInicio = rs.getString(exp_fei);
                pPVO.setFechaInicio(fechaInicio);
                String fechaFin = rs.getString(exp_fef);
                pPVO.setFechaFin(fechaFin);
                String tercero = rs.getString(exp_ter);
                pPVO.setTercero(tercero);
                String version = rs.getString(exp_tnv);
                pPVO.setVersion(version);
                String estado = rs.getString(exp_est);
                pPVO.setEstado(estado);
                String nombre = rs.getString(ter_nom);
                String apellido1 = rs.getString(ter_ap1);
                String part1 = rs.getString(ter_pa1);
                String apellido2 = rs.getString(ter_ap2);
                String part2 = rs.getString(ter_pa2);
                ...
                pPVO.setNombreCompleto(nombreCompleto);
                ...
                pPVO.setEntrada(entrada);
                if(fechaFin == null) {
                    listaP.addElement(pPVO);
                }
            }
            m_Log.debug("PendientesPorProcedimientosDAO, consultar: Lista de consulta cargada");
            if(m_Log.isDebugEnabled()) m_Log.debug("PendientesPorProcedimientosDAO, consultar: Tamaño listaP:" + listaP.size());
        }catch (Exception e) {
            listaP = null;
            e.printStackTrace();
            if(m_Log.isErrorEnabled()) m_Log.error(e.getMessage());
            throw new AnotacionRegistroException(m_ConfigError.getString("Error.PendientesPorProcedimientosDAO.consultar"), e);
        }finally {
            try{
                if (st!=null) st.close();
                if (rs!=null) rs.close();
    ...
    public Vector consultarF(PendientesPorProcedimientosValueObject pendProcVO,String[] params)
        ...
        try{
            ...
            while(rs.next()){
                ...
                pPVO.setEntrada(entrada);
                if(fechaFin != null) {
                    listaF.addElement(pPVO);
                }
            }
            m_Log.debug("PendientesPorProcedimientosDAO, consultar: Lista de consulta cargada");
            if(m_Log.isDebugEnabled()) m_Log.debug("PendientesPorProcedimientosDAO, consultar: Tamaño listaF:" + listaF.size());
        }catch (Exception e) {
            listaF = null;
            e.printStackTrace();
    ...
    public Vector getListaProcedimientos(PendientesPorProcedimientosValueObject pPPVO,String[] params)
            throws AnotacionRegistroException, TechnicalException{
        ...
        String municipio = pPPVO.getCodMunicipio();
        int orden = 0;
        ...
        try{
            ...
            con = oad.getConnection();
            st = con.createStatement();
            sql = "SELECT " + pro_cod + ","+ pro_nom + " FROM  E_PRO WHERE " +
                    pro_mun + "=" + municipio;
            String parametros[] = {"2","2"};
            sql += oad.orderUnion(parametros);
            if(m_Log.isDebugEnabled()) m_Log.debug("PendientesPorProcedimientoDAO, getListaProcedimientos: Sentencia SQL:" + sql);
            rs = st.executeQuery(sql);
            while(rs.next()){
                elemListVO = new ElementoListaValueObject(rs.getString(pro_cod),rs.getString(pro_nom),orden++);
                list.addElement(elemListVO);
            }
            m_Log.debug("PendientesPorProcedimientoDAO, getListaProcedimientos: Lista tipos departamentos cargada");
            if(m_Log.isDebugEnabled()) m_Log.debug("PendientesPorProcedimientoDAO, getListaProcedimientos: Tamaño lista:" + list.size());
        }catch (Exception e) {
            list = null;
            e.printStackTrace();
            m_Log.error(e.getMessage());
            throw new AnotacionRegistroException(m_ConfigError.getString("Error.PendientesPorProcedimientoDAO.getListaProcedimientos"), e);
        }finally {
            try{
                if (st!=null) st.close();
                if (rs!=null) rs.close();                
