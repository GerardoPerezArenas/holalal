...
public class TercerosDAO {
    ...
    protected static Logger m_Log =
            LogManager.getLogger(TercerosDAO.class.getName());
...
m_Log.debug("El tiempo de demora es :" + totalTiempo + " miliseg");
    ...
    public int updateDatosContactoTercero(TercerosValueObject terVO, AdaptadorSQLBD bd, Connection con) throws TechnicalException {
        String sql = "";
        int ver = 0;
        int id = 0;
        ...
        try{
            ...
            id = Integer.parseInt(terVO.getIdentificador());
            ver = Integer.parseInt(terVO.getVersion());
            sql = "UPDATE T_TER SET " + campos.getAtributo("telefonoTER") + "= ?," + campos.getAtributo("emailTER") + "= ?";
            sql += " WHERE " + campos.getAtributo("idTerceroTER") + "= ?";

            if(m_Log.isDebugEnabled()) {
                m_Log.debug("Query para actualizar datos de contacto de tercero en T_TER: " + sql);
                m_Log.debug(String.format("Parámetros de la query: %s-%s-%d", terVO.getTelefono(), terVO.getEmail(), id));
            }

            ps = con.prepareStatement(sql);
            int contbd = 1;
            ps.setString(contbd++, terVO.getTelefono());
            ps.setString(contbd++, terVO.getEmail());
            ps.setInt(contbd++, id);
            ps.executeUpdate();
            cerrarStatement(ps);

            sql = "UPDATE T_HTE SET " + campos.getAtributo("telefonoHTE") + "= ?," + campos.getAtributo("emailHTE") + "= ?";
            sql += " WHERE " + campos.getAtributo("idTerceroHTE") + "= ? AND " + campos.getAtributo("versionHTE") + " = ?";
            if(m_Log.isDebugEnabled()) {
                m_Log.debug("Query para actualizar datos de contacto de tercero en T_HTE: " + sql);
                m_Log.debug(String.format("Parámetros de la query: %s-%s-%d-%d", terVO.getTelefono(), terVO.getEmail(), id, ver));
            }
            ...
            ps.setString(contbd++, terVO.getEmail());
            ps.setInt(contbd++, id);
            ps.setInt(contbd++, ver);
            ps.executeUpdate();
            cerrarStatement(ps);
            commitTransaction(bd, con);
        }catch (Exception e) {
            m_Log.error("Ha ocurrido un error al actualizar los datos de contacto del tercero: " + e.getMessage());
            rollBackTransaction(bd, con, e);
            id=0;
            throw new TechnicalException("No se han podido actualizar el teléfono y el email del tercero");
        }finally{
            try {
                cerrarStatement(ps);
                devolverConexion(bd, con);
  ...
  public int setDomicilioTerceroRapido(TercerosValueObject terVO,String[] params)throws TechnicalException{
     ...
     * @param expedientes: Array de String que contiene el número de los expedientes que se van a relacionar con el expediente a tratar
     * @param params: Parámetros de conexión a la base de datos
     * @return int
     * @throws es.altia.common.exception.TechnicalException
     */
    ...
    {        
        ...
        try{
            ...
            if(!(StringUtils.isEmpty(domVO.getIdPaisVia())))
                sql += campos.getAtributo("codPaisViaDNN")+" = " + domVO.getIdPaisVia() + " and ";
            else sql += campos.getAtributo("codPaisViaDNN")+" = " + campos.getAtributo("codigoPais") + " and ";
            if(!(StringUtils.isEmpty(domVO.getIdProvinciaVia())))
                sql += campos.getAtributo("codProvViaDNN")+" = " +domVO.getIdProvinciaVia() + " and ";
            else sql += campos.getAtributo("codProvViaDNN")+" = " +domVO.getIdProvincia() + " and ";
            if(!(StringUtils.isEmpty(domVO.getIdMunicipioVia())))
                sql += campos.getAtributo("codMunViaDNN")+" = "+ domVO.getIdMunicipioVia() + " and ";
            else sql += campos.getAtributo("codMunViaDNN")+" = "+ domVO.getIdMunicipio() + " and ";
            ...
            if(!(StringUtils.isEmpty(domVO.getNumHasta())) && !domVO.getNumHasta().equals("0"))
                sql += campos.getAtributo("numHastaDNN")+" = "+ domVO.getNumHasta() + " and ";
            else sql += campos.getAtributo("numHastaDNN")+" IS NULL and  ";
            if(!(StringUtils.isEmpty(domVO.getLetraHasta())))
                sql += campos.getAtributo("letraHastaDNN")+" = "+ bd.addString(domVO.getLetraHasta()) + " and ";
            else sql += campos.getAtributo("letraHastaDNN")+" IS NULL and  ";
        ...
        try{            
            ...
            if(Normalizado==2){
                ...
                sql = "INSERT INTO T_DNN("+
                  ...
                  if (domVO.getIdPaisVia()  == null ||domVO.getIdPaisVia() .equals("")) {
                    sql += campos.getAtributo("codigoPais") + ",";
                } else {
                    sql += domVO.getIdPaisVia() + ",";
                }

                 if (domVO.getIdProvinciaVia()  == null ||domVO.getIdProvinciaVia() .equals("")) {
                    sql += domVO.getIdProvincia()  + ",";
                } else {
                    sql += domVO.getIdProvinciaVia()  + ",";
                }

                if (domVO.getIdMunicipioVia() == null ||domVO.getIdMunicipioVia().equals("")) {
                    sql +=domVO.getIdMunicipio() + ",";
                } else {
                    sql +=domVO.getIdMunicipioVia() + ",";
                }
                if (domVO.getIdVia() == null || domVO.getIdVia().equals("")) {
                ...
                sql += bd.addString(domVO.getLetraDesde());
                if (domVO.getNumHasta() == null || domVO.getNumHasta().equals("") || domVO.getNumHasta().equals("0")) {
                    sql += ",null,";
                } else {
                    sql +=","+domVO.getNumHasta() + ",";
                }
            ...
            if("no".equals(existe)) {
                sql = "INSERT INTO T_DOT("+
                        ...
                        bd.funcionFecha(bd.FUNCIONFECHA_SYSDATE, null) + "," + terVO.getUsuarioAlta() +
                        ",";
                sql += (domVO.getEnPadron() != null && !domVO.getEnPadron().equals("")) ? domVO.getEnPadron() : '0';
                sql += ")";
                if(m_Log.isDebugEnabled()) m_Log.debug(sql);
                state.executeUpdate(sql);
            }
            ...
            if (domVO.getEsDomPrincipal().equals("true")) {
                ...
                if(m_Log.isDebugEnabled()) m_Log.debug(sql);
                state.executeUpdate(sql);
            }else if (terVO.getDomPrincipal() != null && !terVO.getDomPrincipal().equals("")){
                sql = "UPDATE T_TER SET TER_DOM = " + terVO.getDomPrincipal() +
                     " WHERE TER_COD = " + terVO.getIdentificador();
                if(m_Log.isDebugEnabled()) m_Log.debug(sql);
                state.executeUpdate(sql);
