...
public class MantSubseccionDAO {

    //Para el fichero de configuracion tecnico.
    private static Config m_ConfigTechnical;
    //Para informacion de logs.
    private static Logger m_Log = LogManager.getLogger(MantSubseccionDAO.class.getName());


    private static MantSubseccionDAO instance = null;
    ...
    protected MantSubseccionDAO() {
        //Queremos usar el fichero de configuracion techserver
        m_ConfigTechnical = ConfigServiceHelper.getConfig("techserver");
        //Queremos tener acceso a los mensajes de error localizados

        nombreTabla = m_ConfigTechnical.getString("SQL.U_SSE.Tabla");
        tipoRegistro = m_ConfigTechnical.getString("SQL.U_SSE.TipoRegistro");
        codigo = m_ConfigTechnical.getString("SQL.U_SSE.Codigo");
        descripcion = m_ConfigTechnical.getString("SQL.U_SSE.Descripcion");

    }

    public static MantSubseccionDAO getInstance() {
        //si no hay ninguna instancia de esta clase tenemos que crear una.
        if (instance == null) {
            // Necesitamos sincronizacion para serializar (no multithread) las invocaciones de este metodo.
            synchronized(MantSubseccionDAO.class){
                if (instance == null)
                    instance = new MantSubseccionDAO();
            }
        }
        return instance;
    }
    ...
    public void create(MantSubseccionValueObject mantSubseccionVO, String[] params)
        ...
        try{
            ...
            m_Log.info("MantSubseccionDAO.create");
...
*/
            sql = "INSERT INTO " + nombreTabla + "(" + tipoRegistro + "," + codigo + "," + descripcion +
                    ") VALUES (?, ?, ?)";

            PreparedStatement ps = conexion.prepareStatement(sql);
            int i = 1;
            ps.setString(i++, mantSubseccionVO.getTipoRegistro().toString());
//            ps.setString(i++, (new Integer(valor+1)).toString());
            ps.setString(i++, mantSubseccionVO.getCodigo());
            ps.setString(i++, mantSubseccionVO.getDescripcion());
    ...
    public void delete(MantSubseccionValueObject mantSubseccionVO, String[] params)
        ...
        try{
            m_Log.info("MantSubseccionDAO.delete");
            //m_Log.info("A por el OAD");
            abd = new AdaptadorSQLBD(params);
            ...
            sql = "DELETE FROM " + nombreTabla + " WHERE " + tipoRegistro + "=? AND " + codigo + "=?";
    ...
    public void modify(MantSubseccionValueObject mantSubseccionVO, String[] params)
        ...
        try{
            ...
            m_Log.info("MantSubseccionDAO.modify");
            //m_Log.info("A por el OAD");
            abd = new AdaptadorSQLBD(params);
            //m_Log.info("A por la conexion");
            conexion = abd.getConnection();
            ...
            sql = "UPDATE " + nombreTabla + " SET " + descripcion + "=? WHERE " + tipoRegistro + "=? AND " + codigo +
                    "=?";
    ...
    public Vector findAll(String[] params) throws TechnicalException {
        ...
        try{
            ...
            sql = "SELECT " + tipoRegistro + ", " + codigo + ", " + descripcion + " FROM " + nombreTabla  + " ORDER BY " + tipoRegistro + "," + codigo;
            if(m_Log.isDebugEnabled()) m_Log.debug(sql);
            ...
            while(rs.next()){
                ...
                subsecciones.add(new MantSubseccionValueObject(tipo, cod, desc));
            }
    ...
    public Vector findByTipoRegistro(Character tipoReg, String[] params) throws TechnicalException {
        ...
        try{
            ...
            sql = "SELECT " + tipoRegistro + ", " + codigo + ", " + descripcion + " FROM " + nombreTabla  +
                    " WHERE " + tipoRegistro + "=? ORDER BY " + tipoRegistro + "," + codigo;
