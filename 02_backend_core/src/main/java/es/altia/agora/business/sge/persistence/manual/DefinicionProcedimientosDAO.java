...
public class DefinicionProcedimientosDAO {
    ...
    protected static Logger m_Log = LogManager.getLogger(DefinicionProcedimientosDAO.class.getName());
        ...
        try{
            ...
            if("no".equals(yaExiste)) {
                ...
                ***/
                ...
                sql = "INSERT INTO E_PRO (" + pro_mun + "," + pro_cod + "," + pro_flh + "," + pro_fld +
                        "," + pro_are + "," + pro_plz + "," + pro_und + "," + pro_sil + "," +
                        pro_tip + "," + pro_ini + "," + pro_est + "," + pro_din + "," + pro_tin + "," + pro_loc + "," +
                        pro_tri + ", " + pro_des + ",PRO_PORCENTAJE,PRO_RESTRINGIDO,PRO_LIBRERIA,PLUGIN_ANULACION_NOMBRE,PLUGIN_ANULACION_IMPLCLASS,PRO_INTOBL,PRO_SOLOWS,PLUGIN_RELAC_HISTORICO"  +
                        ",PRO_INIAUTOEXP) VALUES(" + defProcVO.getCodMunicipio() + ",'" +
                        defProcVO.getTxtCodigo();
          ...
          ***/
          ...
          if(defProcVO.getBiblioteca()!=null && ("0".equals(defProcVO.getBiblioteca()) || "1".equals(defProcVO.getBiblioteca()))){
           ...
           if(defProcVO.getAutomatico()!=null && ("0".equals(defProcVO.getAutomatico()) || "1".equals(defProcVO.getAutomatico()))){
               sql += "," + defProcVO.getAutomatico() + ")" ;
           }else{
               sql+=",0)";
           }
   ...
   Statement st = null;
    ...
    public DefinicionProcedimientosValueObject buscar(DefinicionProcedimientosValueObject defProcVO, String[] params)
        ...
        try{
            ...
            from = " PLUGIN_ANULACION_NOMBRE,PLUGIN_ANULACION_IMPLCLASS, PRO_RESTRINGIDO, PRO_INIAUTOEXP,PRO_PORCENTAJE,"+pro_mun + "," + pro_cod + ",pml1." + pml_valor + " AS nombreProc,"
            ...
            while ( rs.next() ) {
                ...
                defProcVO.setAutomatico(rs.getString("PRO_INIAUTOEXP"));
    ...
    Statement stmt = null;
        ...
        try{
            ...
            /** SE INDICA SI EL PROCEDIMIENTO TIENE INICIO AUTOMATICO O NO **/
            if(defProcVO.getAutomatico()!=null && ("0".equals(defProcVO.getAutomatico()) ||  "1".equals(defProcVO.getAutomatico())))
                sql += ", PRO_INIAUTOEXP=" + defProcVO.getAutomatico();
            else
                sql += ", PRO_INIAUTOEXP=0";
 ...
 /**
    ...
    public int obtenerCodigoTramiteInicio(int codOrganizacion, String codProcedimiento, Connection con) throws SQLException {
        String sql = null;
        PreparedStatement st = null;
        ResultSet rs = null;
        int codTramite = -1;
        ...
        try {
            sql = " SELECT PRO_TRI, TRA_UTR FROM E_PRO JOIN E_TRA ON (PRO_MUN = TRA_MUN AND PRO_COD = TRA_PRO AND PRO_TRI = TRA_COD) "
                    +" WHERE PRO_MUN = ? AND  PRO_COD = ?";
            if(m_Log.isDebugEnabled()) {
                m_Log.debug("Query para recuperar el código del trámite de inicio del procedimiento: " + sql);
                m_Log.debug("Parámetros de la query: " + codOrganizacion + "-" + codProcedimiento);
            }
            ...
            rs = st.executeQuery();
            if (rs.next() ) {
                codTramite = rs.getInt("PRO_TRI");
            }
            rs.close();
            st.close();
        } catch (SQLException e){
            m_Log.error("Ha ocurrido un error al recuperar el trámite de inicio del procedimiento " + codProcedimiento);
            throw e;
        } finally {
            try{
                if(rs != null){
