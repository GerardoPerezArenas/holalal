...
public class MantTransporteDAO {
  ...
  //Para informacion de logs.
    protected static Logger m_Log =
          LogManager.getLogger(MantTransporteDAO.class.getName());

    private static MantTransporteDAO instance = null;
  ...
  protected static String res_ttr;
  protected static String res_fec;

  protected MantTransporteDAO() {
    super();
    ...
    tem_ide = m_ConfigTechnical.getString("SQL.R_TTR.identificador");
    tem_cod = m_ConfigTechnical.getString("SQL.R_TTR.codigo");
    tem_des = m_ConfigTechnical.getString("SQL.R_TTR.descripcion");
    tem_act = m_ConfigTechnical.getString("SQL.R_TTR.activo");

    res_ttr = m_ConfigTechnical.getString("SQL.R_RES.tipoTransporte");
    res_fec =  m_ConfigTechnical.getString("SQL.R_RES.fechaAnotacion");

  }

  public static MantTransporteDAO getInstance() {
        //si no hay ninguna instancia de esta clase tenemos que crear una.
        if (instance == null) {
            // Necesitamos sincronizacion para serializar (no multithread) las invocaciones de este metodo.
            synchronized(MantTransporteDAO.class){
                if (instance == null)
                    instance = new MantTransporteDAO();
                }
        }
        return instance;
  }
  ...
  public Vector loadTemas(String[] params) throws TechnicalException, BDException {
    ...
    try{
      ...
      String SQL_BUSCA_TEMAS = "SELECT " + tem_ide + ", " + tem_cod + ", " + tem_des + ", " + tem_act + " FROM R_TTR ORDER BY " + tem_des;
  ...
  return temas;
    ...
    public Vector loadTemasUtilizados(String[] params) throws TechnicalException {
                   ...
                   try{
                     ...
                     // Creamos la select con los parametros adecuados.
                     String SQL_BUSCA_TEMAS_UTIL = "SELECT DISTINCT " + res_ttr + " FROM R_RES WHERE " + res_ttr + " IS NOT NULL";
                     if(m_Log.isDebugEnabled()) m_Log.debug(SQL_BUSCA_TEMAS_UTIL);
                     rs = st.executeQuery(SQL_BUSCA_TEMAS_UTIL);
                     while(rs.next()){
                       m_Log.debug("ENTRA EN EL WHILE");
                       temasUtilizados.addElement(rs.getString(res_ttr));
                       //m_Log.debug("el tamaño de temasUtilizados es : " + temasUtilizados.size());
                     }
                     ...
                     //m_Log.debug("el tamaño de temasUtilizados antes de enviarlo es : " + temasUtilizados.size());
                   }
            ...
            public Vector loadTemasUtilizadosCerrados(String fechaCerradoE,String fechaCerradoS,String[] params) throws TechnicalException {
                   ...
                   try{
                     ...
                     if(!fechaCerradoE.equals("")) {
                       SQL_BUSCA_TEMAS_UTIL = "SELECT DISTINCT " + res_ttr + " FROM R_RES WHERE (" + res_fec + " <= " +
                               oad.convertir("'" + fechaCerradoE + "'", AdaptadorSQL.CONVERTIR_COLUMNA_FECHA, "DD/MM/YYYY")
                               + " AND " + res_ttr + " IS NOT NULL)";
                       if(m_Log.isDebugEnabled()) m_Log.debug(SQL_BUSCA_TEMAS_UTIL);
                       rs = st.executeQuery(SQL_BUSCA_TEMAS_UTIL);
                       while(rs.next()){
                         temasUtilizadosCerrados.addElement(rs.getString(res_ttr));
                       }
                       rs.close();
                     }
  ...
  public Vector eliminarTema(MantTemasValueObject temasVO, String[] params) throws TechnicalException,BDException {
    ...
    try{
      ...
      sql = "DELETE FROM R_TTR WHERE " + tem_ide + " =" + identificador ;
  ...
  public Vector modificarTema(MantTemasValueObject temasVO, String[] params) throws TechnicalException,BDException {
    ...
    try{
      ...
      sql = "UPDATE R_TTR SET " + tem_cod + " = '" + codigo + "', " + tem_des + " = '" + descripcion + "', " + tem_act + " = '" + activo + "' WHERE " + tem_ide + " = " + identificador;
      if(m_Log.isDebugEnabled()) m_Log.debug(sql);
  ...
  public void modificarTemaActivo(String idModificar, String[] params) throws TechnicalException,BDException {
	    ...
	    try{
	      ...
	      sql = "UPDATE R_TTR SET " + tem_act + " = 'NO' WHERE " + tem_ide + " = " + idModificar;
	      if(m_Log.isDebugEnabled()) m_Log.debug(sql);
  ...
  public Vector altaTema(MantTemasValueObject temasVO, String[] params) throws TechnicalException,BDException {
    ...
    try{
      ...
      int ideMax = 0;
      sql = "SELECT " + abd.funcionMatematica(AdaptadorSQL.FUNCIONMATEMATICA_MAX, new String[]{tem_ide}) + " FROM R_TTR";
      ...
      sql = "INSERT INTO R_TTR VALUES( " + (ideMax + 1) + ", '" + codigo + "', '" + descripcion + "', '" + activo + "')";
      if(m_Log.isDebugEnabled()) m_Log.debug(sql);
