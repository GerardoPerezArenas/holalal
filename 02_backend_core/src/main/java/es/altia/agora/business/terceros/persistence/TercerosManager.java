...
public class TercerosManager {
  ...
  protected static Config m_ConfigError; // Para los mensajes de error localizados
    protected static Logger m_Log = LogManager.getLogger(TercerosManager.class.getName());
      ...
      try{
        ...
        if (terVO.getIdentificador()==null || terVO.getIdentificador().equals("0") || terVO.getIdentificador().equals("")) {
                int idTercero;
                    ...
                    if (tercerosReps.size() > 0) {
                        // Se toma el primero de los terceros internos que aparecen
                      TercerosValueObject terceroEncontrado = (TercerosValueObject) tercerosReps.elementAt(0);
                        idTercero = Integer.parseInt(terceroEncontrado.getIdentificador());
                        terVO.setVersion(terceroEncontrado.getVersion());
                        // El tercero existe, los datos de email y telefono que vienen son distintos a los de bd? Entonces actualizar
                      boolean actualizarTercero = false;
                      if (terVO.getEmail() != null && !terVO.getEmail().isEmpty()){
                        if (terceroEncontrado.getEmail() != null && !terceroEncontrado.getEmail().equals(terVO.getEmail())){
                          actualizarTercero = true;
                        }
                      }
                      if (terVO.getTelefono() != null && !terVO.getTelefono().isEmpty()){
                        if (terceroEncontrado.getTelefono() != null && !terceroEncontrado.getTelefono().equals(terVO.getTelefono())){
                          actualizarTercero = true;
                        }
                      }
                      if(actualizarTercero) {
                        int terceroActualizado = terDAO.updateDatosContactoTercero(terceroEncontrado,adapt, con);
                        if(terceroActualizado == 0){
                          //TODO error
                        }
                      }
                    } else {
        ...
        if(max>0){
            ...
            if(domicilios!=null && domicilios.size()==1){
                ...
                //Comprobación de via
                if((domicilio.getIdVia() == null || domicilio.getIdVia().equals("")) && domicilio.getDescVia() != null && !domicilio.getDescVia().equals("")){
                    GeneralValueObject viaGVO = new GeneralValueObject();
                    viaGVO.setAtributo("codPais", ConstantesDatos.CODIGO_PAIS_ESPAÑA);
                    viaGVO.setAtributo("codProvincia", domicilio.getIdProvincia());
                    viaGVO.setAtributo("codMunicipio", domicilio.getIdMunicipio());
                    viaGVO.setAtributo("descVia", domicilio.getDescVia());
                    viaGVO.setAtributo("codTipoVia", domicilio.getIdTipoVia());
                    viaGVO.setAtributo("usuario", terVO.getUsuarioAlta());

                    // En la request que viene de WSTramitacion no se informa el nombre corto de la vía.
                    //Si el nombre largo es mayor que 25 (valor de tamaño de la BBDD para la columna nombre corto) daría un fallo. Se hace substring
                    String nombreCortoVia = domicilio.getDescVia();
                    if (nombreCortoVia.length()<=25){
                        viaGVO.setAtributo("nombreCorto", nombreCortoVia);
                    } else {
                        viaGVO.setAtributo("nombreCorto", nombreCortoVia.substring(0, 24));
                    }
                    m_Log.debug("Se da de alta la via para el domicilio del tercero");
                    int idVia = ViasDAO.getInstance().altaViaNoRepetido(viaGVO, adapt, con);
                    m_Log.debug("La via tiene el codigo "+ idVia);
                    domicilio.setIdVia(Integer.toString(idVia));
                }
