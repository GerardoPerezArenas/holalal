...
public class EstadosDAO  {
  ...
  public Vector eliminarEstado(String codigo, String[] params){
    ...
    try{
      ...
      sql = "DELETE FROM F_EST_FORM WHERE " +
        campos.getString("SQL.F_EST_FORM.codigo")+"='"+codigo+"'";
  ...
  public Vector getListaEstados(String[] params){
    ...
    try{
      ...
      // Creamos la select con los parametros adecuados.
        sql = "SELECT * FROM F_EST_FORM";
        String[] orden = {campos.getString("SQL.F_EST_FORM.codigo"),"1"};
        ...
        while(rs.next()){
          ...
          gVO.setAtributo("codigo",rs.getString(campos.getString("SQL.F_EST_FORM.codigo")));
          gVO.setAtributo("descripcion",rs.getString(campos.getString("SQL.F_EST_FORM.descripcion")));
          resultado.add(gVO);
        }
    }catch (Exception e){
        try{
  ...
  public Vector modificarEstado(GeneralValueObject gVO, String[] params){
    ...
    try{
      ...
      conexion = abd.getConnection();
      abd.inicioTransaccion(conexion);
      sql = "UPDATE F_EST_FORM SET " +
        campos.getString("SQL.F_EST_FORM.codigo")+"='"+gVO.getAtributo("codigo")+"',"+
        campos.getString("SQL.F_EST_FORM.descripcion")+"='"+gVO.getAtributo("descripcion")+"'"+
        " WHERE " +
        campos.getString("SQL.F_EST_FORM.codigo")+"='"+gVO.getAtributo("codigoAntiguo")+"'";
      if(m_Log.isDebugEnabled()) m_Log.debug(sql);
      stmt = conexion.createStatement();
  ...
  public Vector altaEstado(GeneralValueObject gVO, String[] params){
    ...
    try{
      ...
      conexion = abd.getConnection();
      abd.inicioTransaccion(conexion);
      sql = "INSERT INTO F_EST_FORM("+
       campos.getString("SQL.F_EST_FORM.codigo")+","+
       campos.getString("SQL.F_EST_FORM.descripcion")+
       ") VALUES ('" +
