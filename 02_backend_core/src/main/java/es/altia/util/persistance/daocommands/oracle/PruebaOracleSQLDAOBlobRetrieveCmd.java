...
/**
  ...
  *
  **/
public class PruebaOracleSQLDAOBlobRetrieveCmd implements SQLDAOBlobRetrieveCommand {
    private static final Logger _log =
            LogManager.getLogger(PruebaOracleSQLDAOBlobRetrieveCmd.class.getName());
    private static final String CLSNAME = "OracleSQLDAOBlobRetrieveCmd";

	private SQLDAOBlobCommandAdapter retrieveAdapter = null;
	private String sqlQuery = null;


	PruebaOracleSQLDAOBlobRetrieveCmd( SQLDAOBlobCommandAdapter concreteAdapter ) {
	...
	public byte[] retrieve(Connection connection, PrimaryKey primaryKey)
		...
		try {
            ...
            final byte[] result = OracleOperations.toByteArray(((OracleResultSet) resultSet).getBLOB(1));
            if (_log.isInfoEnabled()) _log.info(CLSNAME+" END retrieve()");
            return result;
