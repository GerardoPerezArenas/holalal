...
/**
  ...
  *
  **/
public class StdSQLDAOBlobRetrieveCmd implements SQLDAOBlobRetrieveCommand {


	private SQLDAOBlobCommandAdapter retrieveAdapter = null;
	private String sqlQuery = null;
    private static final Logger _log =
            LogManager.getLogger(StdSQLDAOBlobRetrieveCmd.class.getName());


    StdSQLDAOBlobRetrieveCmd( SQLDAOBlobCommandAdapter concreteAdapter ) {
	...
	public byte[] retrieve(Connection connection, PrimaryKey primaryKey)
		...
		try {
			if (_log.isInfoEnabled()) _log.info("StdSQLDAOBlobRetrieveCmd: BEGIN retrieve("+primaryKey+")");

			/* Create "preparedStatement". */
			//if (_log.isDebugEnabled()) _log.debug("StdSQLDAOBlobRetrieveCmd: QUERY= " + sqlQuery);
			...
			if (!resultSet.next()) {
				if (_log.isErrorEnabled()) _log.error("StdSQLDAOBlobRetrieveCmd: InstanceNotFoundException retrieving PersistentObject");
            ...
            if (_log.isInfoEnabled()) _log.info("StdSQLDAOBlobRetrieveCmd: END retrieve()");
            return result;
