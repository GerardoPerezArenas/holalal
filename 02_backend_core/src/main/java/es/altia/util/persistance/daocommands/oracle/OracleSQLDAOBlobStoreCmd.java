...
/**
  ...
  *
  **/
public class OracleSQLDAOBlobStoreCmd implements SQLDAOBlobStoreCommand {
    private static final Logger _log =
            LogManager.getLogger(OracleSQLDAOBlobStoreCmd.class.getName());
    ...
    OracleSQLDAOBlobStoreCmd( SQLDAOBlobCommandAdapter concreteAdapter ) {
	...
	public void store(Connection connection, PrimaryKey pk, byte[] blob)
        ...
        if ( existsCmd.exists(connection, pk) ) {
            ...
            /*Insert*/
            if (_log.isInfoEnabled()) _log.info(CLSNAME+" not exists... trying insert");
    ...
    private void update(Connection connection, PrimaryKey pk, byte[] blob)
        ...
        try {
            ...
            int i = 1;
            oracleBlob = OracleOperations.newOracleBlob(blob,OracleOperations.toOracleConnection(connection));
            preparedStatement.setBLOB(i++,oracleBlob);
            ...
            OracleOperations.freeOracleBlobSilently(oracleBlob);
        }//try-catch
    }//update
