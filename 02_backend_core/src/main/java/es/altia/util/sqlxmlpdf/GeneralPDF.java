...
public class GeneralPDF {
  ...
  //protected static Config confReg = ConfigServiceHelper.getConfig("Registro");
   ...
   public GeneralPDF(GeneralValueObject parametrosPdfVO){
       ...
       if (m_Log.isDebugEnabled()) m_Log.debug("baseDir: " + BASE_DIR);
       if (m_Log.isDebugEnabled()) m_Log.debug("aplPathReal: " + APL_PATH_REAL);
       if (m_Log.isDebugEnabled()) m_Log.debug("usuDir: " + USU_DIR);
       if (m_Log.isDebugEnabled()) m_Log.debug("estilo: " + estilo);
       if (m_Log.isDebugEnabled()) m_Log.debug("style: " + style);
       if (m_Log.isDebugEnabled()) m_Log.debug("pdfFile: " + pdf);
       if (m_Log.isDebugEnabled()) m_Log.debug("pagina: " + pagina);
       if (m_Log.isDebugEnabled()) m_Log.debug("cabecera: " + cabecera);
       if (m_Log.isDebugEnabled()) m_Log.debug("tamCabecera: " + tamCabecera);
       if (m_Log.isDebugEnabled()) m_Log.debug("pie: " + pie);
       if (m_Log.isDebugEnabled()) m_Log.debug("tamPie: " + tamPie);
   ...
   public GeneralPDF(String[] params, GeneralValueObject parametrosPdfVO){
    ...
    public String getPdf() {
        ...
        try {
            ...
            e.printStackTrace();
            m_Log.error("Error en el método principal de GeneralPDF: " + e.getMessage(), e);
        }
...
/**
   ...
   /**
    * Sobrecarga del método construyeTabla que recibe 1 parámetro y delega en la sobrecarga de 2 parámetros.
    * Al no pasar una hoja xsl determinada se aplicará la hoja por defecto.
   ...
   /**
    * Sobrecarga del método construyeTabla que recibe 2 parámetros y delega en la sobrecarga de 3 parámetros.
    * En este caso la hoja xsl aplicada es una especificada por el usuario.
    * @param sql La sentencia sql a partir de la cual se generará la tabla
    ...
    */
    ...
    /**
     * Sobrecarga del método construyeTabla que recibe 3 parámetros y delega en la sobrecarga de 4 parámetros.
     *
     * @param sql La sentencia sql a partir de la cual se generará la tabla
     * @param xsl La hoja xsl a aplicar
    ...
    public File construyeTabla(String sql, String xsl, List<AnotacionRegistroVO> listaAnotacionesImpresas) throws TechnicalException {
        return construyeTabla(sql, xsl, listaAnotacionesImpresas, null);
    }

    /**
     * Sobrecarga del método construyeTabla que recibe 4 parámetros y que contiene la lógica del método, que se encarga de:
     * construir una tabla en formato xml aplicando la una hoja xsl determinada
     *
     * @param sql La sentencia sql a partir de la cual se generará la tabla
     * @param xsl La hoja xsl a aplicar
     * @param listaAnotacionesImpresas La lista de numero de anotaciones obtenidas a imprimir
     * @param filtrosFecha Vector con los valores de los filtros de fecha que sustituirán a los interrogantes del PreparedStatement antes del executeQuery
     * @return Un File con el descriptor del fichero de la tabla
     */
    public File construyeTabla(String sql, String xsl, List<AnotacionRegistroVO> listaAnotacionesImpresas, Vector<String> filtrosFecha) throws TechnicalException {

        m_Log.debug("GeneralPDF.construyeTabla() - xsl:   " + xsl);
        File f = null;
        String S_LINEA = " ";
        ...
        try {
            ...
            if(filtrosFecha != null){
                int intPos=1;
                for (String fecha : filtrosFecha)  {
                    //m_Log.debug(" ==============> Valor de paso "+(intPos)+": "+fecha);
                    state.setString(intPos++, fecha);
                }
            }
            ...
            if (rs.next()) {//Hay datos
                ...
                do {
                    ...
                    for (IC = 1; IC < IC_MAX + 1; IC++) {
                        ...
                        //m_Log.debug("----> Iteracion "+ IC + " - nombre columna: " + S_COLUMN_NAME);
                        String aux = "";
                        ...
                        S_COLUMN_VALUE = aux;
                        //m_Log.debug("----> Iteracion "+ IC + " - valor columna: " + S_COLUMN_VALUE);
   ...
   private void construyeDocumento(){
	...
	try{
	   ...
	   salida.println(skel);
	   if(estilo != null){
               String flexiaBaseDir = APL_PATH_REAL + File.separator;
                skel = "<meta name=\"base\" value=\"file:" + flexiaBaseDir + "temp\"/>";
                salida.println(skel);
