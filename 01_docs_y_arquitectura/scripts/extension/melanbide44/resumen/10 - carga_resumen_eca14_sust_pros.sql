create or replace
FUNCTION CARGA_RESUMEN_ECA14_sust_PROS (EXPEDIENTE VARCHAR2 ) RETURN VARCHAR2
IS
...
-----------------------------------------------------------------------------
    ...
    CURSOR PROSPECTORES_JUST IS
      select
        ...
        END visitas,
        A_PAGAR
      from eca14_res_PROSPECTORES a
      inner join eca14_jus_PROSPECTORES b on a.ECA_JUS_PROSPECTORES_COD=b.ECA_JUS_PROSPECTORES_COD
      where
...
BEGIN
dbms_output.put_line('INICIO FUNCION CARGA_RESUMEN_ECA14_sust_PROS: '||to_char(SYSDATE,'yyyymmdd hh24:mi:ss'));
--CAMBIO:=0;
...
dbms_output.put_line('FIN PROCESO CARGA_RESUMEN_ECA14_sust_PROS: '||to_char(SYSDATE,'yyyymmdd hh24:mi:ss'));
    RETURN CODIGO;
...
END CARGA_RESUMEN_ECA14_sust_PROS;