create or replace
FUNCTION CARGA_RESUMEN_ECA14_sust_PREP (EXPEDIENTE VARCHAR2 ) RETURN VARCHAR2
IS
...
-----------------------------------------------------------------------------
    ...
    CURSOR PREPARADORES_JUST IS
      select
        ...
        A_PAGAR
      from eca14_res_preparadores a
      inner join eca14_jus_preparadores b on a.ECA_JUS_PREPARADORES_COD=b.ECA_JUS_PREPARADORES_COD
      where
...
BEGIN
dbms_output.put_line('INICIO FUNCION CARGA_RESUMEN_ECA14_sust_PREP: '||to_char(SYSDATE,'yyyymmdd hh24:mi:ss'));
--CAMBIO:=0;
...
dbms_output.put_line('FIN PROCESO CARGA_RESUMEN_ECA14_sust_PREP: '||to_char(SYSDATE,'yyyymmdd hh24:mi:ss'));
    RETURN CODIGO;
...
END CARGA_RESUMEN_ECA14_sust_PREP;