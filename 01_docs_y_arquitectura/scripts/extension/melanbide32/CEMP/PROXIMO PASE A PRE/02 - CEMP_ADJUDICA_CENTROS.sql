create or replace 
PROCEDURE        CEMP_adjudica_centros (ano in number, mensaje out varchar2, exp_estado_incorrecto out varchar2) iS
...
-----------------------------------------------------------------------------
    ...
    CURSOR CENTROS1 (AMB IN NUMBER) IS 
        ...
        -- ,B.ORI_ENT_COD,B.ORI_ENT_ADMLOCAL_VAL, ORI_ENT_SUPRAMUN_VAL, ORI_EXP_COD_VAL, ORI_ENT_OTROS_VAL, ORI_ENT_ACOLOCACION_VAL, ORI_ENT_NUMTRAB_VAL, ORI_ENT_NUMTRAB_DISC_VAL,ORI_NUM_PERS_ATENDIDAS
            ...
            ORI_ENT_ACOLOCACION_VAL desc, ORI_ENT_NUMTRAB_DISC_VAL/case when ORI_ENT_NUMTRAB_VAL is null or ORI_ENT_NUMTRAB_VAL =0 then 1 else ORI_ENT_NUMTRAB_VAL end DESC,
    ...
    CURSOR CENTROS2 (AMB IN NUMBER) IS
        ...
        ORI_ENT_ACOLOCACION_VAL desc, ORI_ENT_NUMTRAB_DISC_VAL/case when ORI_ENT_NUMTRAB_VAL is null or ORI_ENT_NUMTRAB_VAL =0 then 1 else ORI_ENT_NUMTRAB_VAL end DESC ,d.tray desc, ORI_NUM_PERS_ATENDIDAS DESC;          
    ...
    CURSOR CENTROS1CE (AMB IN NUMBER) IS 
         ...
         ---,B.ORI_ENT_COD,B.ORI_ENT_ADMLOCAL_VAL, ORI_ENT_SUPRAMUN_VAL, ORI_EXP_COD_VAL, ORI_ENT_OTROS_VAL, ORI_ENT_ACOLOCACION_VAL, ORI_ENT_NUMTRAB_VAL, ORI_ENT_NUMTRAB_DISC_VAL,ORI_NUM_PERS_ATENDIDAS
            ...
            AND (ORI_ENT_ADMLOCAL_VAL ='S' or ORI_ENT_SUPRAMUN_VAL='S') AND ORI_AMB_COD=AMB
            and b.ext_num like '%/CEMP/%'
            ORDER BY ORI_ENT_SUPRAMUN_VAL desc,d.tray desc, ORI_ENT_ACOLOCACION_VAL desc, ORI_ENT_NUMTRAB_DISC_VAL/case when ORI_ENT_NUMTRAB_VAL is null or ORI_ENT_NUMTRAB_VAL =0 then 1 else ORI_ENT_NUMTRAB_VAL end DESC, ORI_NUM_PERS_ATENDIDAS DESC;       
...
--CURSORES DE AMBITOS DISTRIBUIDOS
    ...
    CURSOR CENTROS2_AD (AMB IN NUMBER) IS
        ...
        ORDER BY 
        case when ORI_EXP_COD_VAL in (1,2) then 1 else ORI_EXP_COD_VAL end,
        ORI_ENT_ACOLOCACION_VAL desc, ORI_ENT_NUMTRAB_DISC_VAL/case when ORI_ENT_NUMTRAB_VAL is null or ORI_ENT_NUMTRAB_VAL =0 then 1 else ORI_ENT_NUMTRAB_VAL end DESC ,tray desc, ORI_NUM_PERS_ATENDIDAS DESC;          
    ...
    SELECT UBICACION FROM (select 
        ROW_NUMBER() OVER (PARTITION BY ORI_AMB_COD,B.ORI_ENT_COD ORDER BY ORI_AMB_COD,B.ORI_ENT_COD,A.ORI_CE_UBIC_COD) ORDEN,
         ...
         D.TRAY
            ...
            ORDER BY ORI_ENT_SUPRAMUN_VAL desc,tray desc, ORI_ENT_ACOLOCACION_VAL desc, ORI_ENT_NUMTRAB_DISC_VAL/case when ORI_ENT_NUMTRAB_VAL is null or ORI_ENT_NUMTRAB_VAL =0 then 1 else ORI_ENT_NUMTRAB_VAL end DESC, ORI_NUM_PERS_ATENDIDAS DESC;       
...
Begin
    ...
    FOR REG_AMBITO IN AMBITOS LOOP
        ...
        --ambito NO DISTRIBUIDO
          ...
          Dbms_Output.Put_Line('AMBITO DISTRIBUIDO: '||AMBITO||' ---- NUM_CENTROS: '|| NUM_CENTROS);
